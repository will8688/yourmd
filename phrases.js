module.exports = [
     'depressive disorder',
     'depression',
     'depressive episode',
     'depressive illness',
     'depressed',
     'depressed mood',
     'dysthymia',
     'depressive neurosis',
     'neurasthenic neurosis',
     'neurasthenia',
     'neurotic depression',
     'nervous breakdown',
     'neurasthaenic neurosis',
     'neurasthaenia',
     'nervous debility',
     'depreshun',
     'not happy',
     'depresion',
     'depressions',
     'morose',
     'miserable',
     'gastroesophageal reflux disease',
     'GERd',
     'gastro-esophageal reflux disease',
     'gastro-esophageal reflux',
     'GORd',
     'GOr',
     'gastro-oesophageal reflux disease',
     'gastro-oesophageal reflux',
     'acid reflux',
     'gastritus',
     'adult health examination',
     'type 2 diabetes mellitus',
     'diabetes mellitus type 2',
     'type ii diabetes mellitus',
     'diabetes mellitus type ii',
     'noninsulin dependent diabetes',
     'essential hypertension',
     'idiopathic hypertension',
     'systemic primary arterial hypertension',
     'primary hypertension',
     'obese',
     'obesity',
     'adiposis',
     'adiposity',
     'diabetes mellitus',
     'Dm',
     'diabetes',
     'dibetis',
     'diebetis',
     'diabeties',
     'daibetes',
     'diabetis',
     'diabities',
     'diebetes',
     'diabetus',
     'asthma',
     'hyperreactive airway disease',
     'airway hyperreactivity',
     'asthmatic',
     'bronchial asthma',
     'bronchial hyperreactivity',
     'BHr',
     'bronchial hyperresponsiveness',
     'bronchial hypersensitivity',
     'athsma',
     'asthmatics',
     'disorder of coronary artery',
     'coronary arteriosclerosis',
     'arteriosclerotic heart disease',
     'CAd',
     'coronary artery disease',
     'coronary sclerosis',
     'CHd',
     'coronary heart disease',
     'allergic rhinitis',
     'Ar',
     'atopic rhinitis',
     'allergic rhinitis due to allergen',
     'hypothyroidism',
     'hypothyroid',
     'well child visit',
     'upper respiratory infection',
     'urti - infection of the upper respiratory tract',
     'upper respiratory tract infection',
     'URi',
     'hypercholesterolaemia',
     'high cholesterol',
     'hypercholesterolemia',
     'backache',
     'back ache',
     'pain in back',
     'back pain',
     'coxcix',
     'cocsix',
     'backpain',
     'backaches',
     'abdominal pain',
     'Ap',
     'low back pain',
     'LBp',
     'low back syndrome',
     'lumbalgia',
     'lumbago',
     'lumbar pain',
     'nonspecific pain in the lumbar region',
     'anaemia',
     'absolute anemia',
     'absolute anaemia',
     'aneamia',
     'low hemoglobin',
     'anemia',
     'amenia',
     'anxiety',
     'anxiousness',
     'feeling anxious',
     'anxiety reaction',
     'anxieties',
     'anxious',
     'urinary tract infectious disease',
     'urinary tract infection',
     'UTi',
     'chronic obstructive lung disease',
     'chronic airway obstruction',
     'chronic irreversible airway obstruction',
     'chronic airflow obstruction',
     'CAFl',
     'chronic airflow limitation',
     'COAd',
     'chronic obstructive airways disease',
     'CAo',
     'CAl',
     'COPd',
     'chronic obstructive pulmonary disease',
     'chronic obstructive airway disease',
     'chronic airway disease',
     'COLd',
     'emphisema',
     'emfisema',
     'emphasima',
     'emphasema',
     'emphasemia',
     'atrial fibrillation',
     'Af',
     'atrial fibrilation',
     'pneumonia',
     'pnumonia',
     'pnemonia',
     'pneumonias',
     'pneumoniae',
     'chest pain',
     'congestive heart failure',
     'congestive cardiac failure',
     'CCf',
     'CHf',
     'congestive heart disease',
     'headache',
     'Ha',
     'pain in head',
     'head pain',
     'cephalalgia',
     'cephalgia',
     'cephalodynia',
     'head ache',
     'headaches',
     'migraine',
     'migrane',
     'migraines',
     'osteoporosis',
     'Op',
     'knee pain',
     'arthralgia of knee',
     'knee joint pain',
     'gonalgia',
     'polyp of colon',
     'colonic polyp',
     'polyp colon',
     'otitis media',
     'Om',
     'sinusitis',
     'sinisitus',
     'sinusitus',
     'coughing',
     'c/o - cough',
     'complaining of cough',
     'cough',
     'observation of cough',
     'coughed',
     'coughs',
     'sleep apnoea',
     'SAs',
     'sleep apnea syndrome',
     'sleep hypopnea',
     'sleep apnoea syndrome',
     'sleep hypopnoea',
     'insomnia',
     'sleeplessness',
     'gout',
     'tobacco dependence syndrome',
     'compulsive tobacco user syndrome',
     'tabagism',
     'tobacco abuse',
     'tobacco dependence',
     'malignant tumor of prostate',
     'malignant prostatic tumour',
     'malignant prostatic tumor',
     'Ca',
     'cancer of prostate',
     'prostrate cancer',
     'perceived constipation',
     'constipation',
     'costiveness',
     'difficulty defaecating',
     'difficulty defecating',
     'difficulty passing stool',
     'difficulty opening bowels',
     'constipated',
     'Cn',
     'difficult passing motion',
     'constapation',
     'constipations',
     'decreased hearing',
     'hearing loss',
     'impaired hearing',
     'HOh',
     'hard of hearing',
     'hearing impairment',
     'difficulty hearing',
     'hypoacusis',
     'Hl',
     'Hi',
     'hearing impaired',
     'acute bronchitis',
     'acute tracheobronchitis',
     'kidney stone',
     'renal stone',
     'nephrolith',
     'renal calculus',
     'calculus of kidney',
     'nephrolithiasis',
     'kidney calculus',
     'lack of energy',
     'lacking in energy',
     'loss of energy',
     'fatigue',
     'tiredness',
     'weariness',
     'tierdness',
     'tirdness',
     'fatigued',
     'fatigues',
     'fatiguing',
     'obstructive sleep apnoea syndrome',
     'OSa',
     'obstructive sleep apnoea',
     'obstructive sleep apnea',
     'malignant tumour of breast',
     'breast cancer',
     'ca - breast cancer',
     'brest cancer',
     'delivery normal',
     'FTNd',
     'full term normal delivery',
     'SVd',
     'spontaneous vaginal delivery',
     'normal delivery',
     'impotence',
     'failure of erection',
     'sexual impotence',
     'erectile dysfunction',
     'impotency',
     'impotencies',
     'irritable bowel syndrome',
     'colon spasm',
     'functional bowel disease',
     'nervous colitis',
     'IBs',
     'irritable bowel',
     'spastic colitis',
     'irritable colon syndrome',
     'Ic',
     'irritable colon',
     'adaptive colitis',
     'membranous colitis',
     'spastic colon',
     'mucous colitis',
     'irratable bowel syndrome',
     'gynaecologic examination',
     'examination of female genitals',
     'female genital examination',
     'tobacco user',
     'tobacco use',
     'neck pain',
     'nonspecific pain in the neck region',
     'cervical pain',
     'neck ache',
     'painful neck',
     'cervicalgia',
     'cerebrovascular accident',
     'CVa',
     'stroke',
     'stroked',
     'strokes',
     'stroking',
     'apoplexy',
     'asthenia',
     'general weakness',
     'debility',
     'lassitude',
     'weakness',
     'weakness - general',
     'feeling weak',
     'weaknesses',
     'asthenic',
     'shoulder pain',
     'shoulder region pain',
     'difficulty breathing',
     'respiration difficult',
     'DIb',
     'difficulty in breathing',
     'dyspnoea',
     'breathless',
     'breathlessness',
     'SOb',
     'shortness of breath',
     'dysnea',
     'dyspnea',
     'median nerve entrapment',
     'carpal tunnel syndrome',
     'distal median nerve compression',
     'distal median nerve entrapment',
     'carpal tunnel',
     'carpal canal',
     'CTs',
     'bronchitis',
     'bronchitus',
     'anticoagulant therapy',
     'diverticular disease of colon',
     'diverticulosis of colon',
     'diverticulosis of the colon',
     'throat irritation',
     'pharyngitis',
     'irritation of the throat',
     'arthritis',
     'joint inflammation',
     'inflammatory arthritis',
     'arthritus',
     'arthiritis',
     'arthritides',
     'diarrhoea symptom',
     'diarrhoea',
     'observation of diarrhoea',
     'observation of diarrhea',
     'd',
     'diarrhea',
     'diahorea',
     'diorrhea',
     'diareare',
     'diahorrea',
     'diorea',
     'diorrea',
     'dioreah',
     'diarohea',
     'diarreah',
     'diarrheas',
     'dizziness and giddiness',
     'dizziness - giddy',
     'dizzy',
     'dizziness',
     'dizzieness',
     'giddyness',
     'virtigo',
     'dizzyness',
     'diziness',
     'alcohol abuse',
     'ethanol abuse',
     'Aa',
     'dyslipidaemia',
     'high blood cholesterol/triglycerides',
     'dyslipidemia',
     'dementia',
     'organic dementia',
     'dementias',
     'benign prostatic hypertrophy without outflow obstruction',
     'syncope',
     'fainting',
     'faint',
     'syncope attack',
     'syncope symptom',
     'syncope and collapse',
     'syncopes',
     'fainted',
     'acute sinusitis',
     'acute inflammation of sinus',
     'acute infection of sinus',
     'acute inflammation of nasal sinus',
     'allergic rhinitis due to pollen',
     'pollinosis',
     'allergic rhinitis due to pollens',
     'hay fever',
     'hayfever',
     'allergy to pollen',
     'seasonal allergic rhinitis',
     'spasmodic rhinorrhea',
     'spasmodic rhinorrhoea',
     'seasonal allergy',
     'pollenosis',
     'hysterectomy',
     'excision procedure on uterus',
     'uterus excision',
     'hysterectomy procedures',
     'hystorectomy',
     'hysterectomies',
     'gastritis',
     'gastric catarrh',
     'cataract',
     'lens opacity',
     'cat. - cataract',
     'lenticular opacity',
     'cataract form',
     'end stage renal disease',
     'ESRf',
     'end stage renal failure',
     'ESCRf',
     'end stage chronic renal failure',
     'ESRd',
     'end stage kidney disease',
     'chronic renal failure',
     'chronic renal failure syndrome',
     'chronic uraemia',
     'chronic uremia',
     'CRf',
     'blood in urine',
     'hematuria',
     'haematuria',
     'hematuria syndrome',
     'blood in urine - haematuria',
     'blood in urine - hematuria',
     'peripheral nerve disease',
     'peripheral neuropathy',
     'disorder of the peripheral nervous system',
     'postpartum',
     'puerperal',
     'puerperium',
     'puerperium, function',
     'postpartum state',
     'postpartum period',
     'hepatitis non-a non-b',
     'viral hepatitis c',
     'viral hepatitis type c',
     'type c viral hepatitis',
     'hepatitis c',
     'palpitations',
     'arrythmia',
     'arythmia',
     'palpatations',
     'palpertations',
     'arhythmia',
     'cutaneous eruption',
     'exanthem',
     'exanthemata',
     'exanthema',
     'rash',
     'eruption',
     'eruption of skin',
     'skin rash',
     'skin eruption',
     'breaking out - eruption',
     'exanthems',
     'rashes',
     'eruptions',
     'rashs',
     'mantoux: positive',
     'positive ppd',
     'renewal of prescription',
     'prescription renewal',
     'cholecystectomy',
     'removal of gallbladder',
     'excision of gallbladder',
     'gallbladder excision',
     'cholecystectomies',
     'type 1 diabetes mellitus',
     'diabetes mellitus type 1',
     'type i diabetes mellitus',
     'diabetes mellitus type i',
     'IDDm',
     'renal failure syndrome',
     'Rf',
     'renal failure',
     'renal insufficiency',
     'renal insufficiency syndrome',
     'appendicectomy',
     'excision of appendix',
     'excision procedures on the appendix',
     'routine antenatal care',
     'peripheral arterial occlusive disease',
     'PAOd',
     'peripheral arterial disease',
     'peripheral angiopathy',
     'PVd',
     'peripheral vascular disease',
     'peripheral artery occlusive disease',
     'pvd-peripheral vascular disease',
     'hyperglycaemic disorder',
     'hyperglycaemia',
     'hyperglycemia',
     'history and physical examination, annual for health maintenance',
     'annual health maintenance examination',
     'seizure disorder',
     'generalized seizure',
     'generalized convulsion',
     'generalised convulsion',
     'Ef',
     'epileptic fit',
     'generalised fit',
     'generalized fit',
     'epileptic seizure',
     'epilepsy',
     'epileptic fits',
     'epileptic seizures',
     'epileptic attack',
     'epileptic disorder',
     'epileptic convulsions',
     'epileptic',
     'Ep',
     'epileptics',
     'fever',
     'pyrexial',
     'pyrexia',
     'febrile',
     'body temperature above reference range',
     'has a temperature',
     'temperature raised',
     'increased body temperature',
     'temperature elevated',
     'high body temperature',
     'hyperthermia',
     'tempreture',
     'temperture',
     'tempature',
     'fevered',
     'fevers',
     'feverish',
     'term infant',
     'baby full term maturity',
     'osteoarthritis of knee',
     'degenerative joint disease of knee',
     'knee djd',
     'gonarthrosis',
     'Oa',
     'senile keratoma',
     'actinic keratosis',
     'atrophic keratosis',
     'Ak',
     'Sk',
     'solar keratosis',
     'hyperplastic keratosis',
     'disseminated superficial actinic porokeratosis',
     'actinic porokeratosis',
     'DSAp',
     'senile keratosis',
     'senile hyperkeratosis',
     'keratinocytic intraepidermal neoplasia',
     'urinary incontinence',
     'unable to prevent bladder emptying',
     'urine incontinence',
     'leaking of urine',
     'incontinence of urine',
     'unable to control bladder',
     'unable to hold urine',
     'lack of bladder control',
     'involuntary urination',
     'unable to hold fluids',
     'Ui',
     'absence of bladder continence',
     'bladder incontinence',
     'loss of bladder control',
     'weak bladder',
     'bladder: incontinent',
     'haemorrhoids',
     'piles',
     'haemorrhoid',
     'piles - haemorrhoids',
     'hemorrhoid',
     'piles - hemorrhoids',
     'haemmorrhoids',
     'haemoroids',
     'hemmoroids',
     'hemorroids',
     'hemeroids',
     'hemroids',
     'hemaroids',
     'seizure',
     'fit',
     'fit - convulsion',
     'convulsion',
     'fitting',
     'seizures',
     'fits',
     'fitted',
     'glaucoma',
     'glucoma',
     'body mass index 30+ - obesity',
     'bmi 30+ - obesity',
     'breast lump',
     'mass in breast',
     'lumpy breasts',
     'breast irregular nodularity',
     'breast mass',
     'lumpy breast',
     'viral disease',
     'disease due to virus',
     'viral infection',
     'disease caused by virus',
     'viral infectious disease',
     'viral illness',
     'abnormal cervical smear',
     'abnormal cervical papanicolaou smear',
     'cellulitis',
     'cellulites',
     'influenza vaccination',
     'influenza immunization',
     'influenza immunisation',
     'rheumatoid arthritis',
     'atrophic arthritis',
     'chronic rheumatic arthritis',
     'rheumatic gout',
     'Ra',
     'rheumatoid disease',
     'rha - rheumatoid arthritis',
     'proliferative arthritis',
     'rhumatoid arthritis',
     'seborrhoeic keratosis',
     'basal cell papilloma',
     'basosquamous papilloma',
     'seborrheic verruca',
     'keratosis seborrheica',
     'verruca seborrheica',
     'seborrheic wart',
     'seborrhoeic verruca',
     'seborrhoeic wart',
     'keratosis seborrhoeica',
     'verruca seborrhoeica',
     'seborrheic keratosis',
     'pigmented basal cell papilloma',
     'senile wart',
     'BCp',
     'keratosis senilis',
     'seborrhoeic dermatitis',
     'SBd',
     'seborrheic dermatitis',
     'seborrheic eczema',
     'seborrhoeic eczema',
     'seborrhoea',
     'hypersteatosis',
     'sebaceous gland overactivity',
     'pain in lower limb',
     'anxiety disorder',
     'subjective vertigo',
     'rotation of self',
     'feel im spinning round',
     'vertigo',
     'vertigo - giddiness',
     'rotary vertigo',
     'rotatory vertigo',
     'vertigo (spinning sensation)',
     'giddiness',
     'chronic low back pain',
     'CLBp',
     'acne vulgaris',
     'common acne',
     'difficulty swallowing',
     'dysphagia',
     'cant get food down',
     'swallowing difficult',
     'cannot get food down',
     'difficulty in swallowing',
     'fall on same level from slipping, tripping or stumbling',
     'osteopenia',
     'hydrops',
     'dropsy',
     'edema',
     'interstitial edema',
     'interstitial oedema',
     'oedema',
     'oedema - lesion',
     'oedematous',
     'edematous',
     'edema - lesion',
     'odema',
     'edemas',
     'muscular rheumatism',
     'fibromyositis',
     'MPDs',
     'myofacial pain dysfunction syndrome',
     'myofascial pain syndrome',
     'fibromyalgia',
     'diffuse myofascial pain syndrome',
     'fibrositis',
     'fybromyalgia',
     'fibromyalga',
     'malignant tumor of colon',
     'cancer of colon',
     'hip pain',
     'arthralgia of hip',
     'hip joint pain',
     'posttraumatic stress disorder',
     'traumatic neurosis',
     'PTSd',
     'post-traumatic stress disorder',
     'post-traumatic stress syndrome',
     'inflammatory dermatosis',
     'dermatitis',
     'psoriasis',
     'psorisis',
     'soriasis',
     'myopia',
     'near sighted',
     'nearsightedness',
     'short-sighted',
     'short-sightedness',
     'shortsightedness',
     'benign essential hypertension',
     'liveborn born in hospital',
     'vaccination required',
     'atypical chest pain',
     'age-related cataract',
     'senile cataract',
     'murmur',
     'heart murmur',
     'finding of heart murmur',
     'observation of heart murmur',
     'murmurs',
     'liver function tests abnormal',
     'abnormal results of liver function studies',
     'angina pectoris',
     'angina',
     'cardiac angina',
     'stenocardia',
     'anginal syndrome',
     'ischemic heart disease - angina',
     'ischaemic heart disease - angina',
     'ischaemic chest pain',
     'impaired fasting glycaemia',
     'impaired fasting glucose',
     'chronic ischaemic heart disease',
     'chronic myocardial ischaemia',
     'chronic coronary insufficiency',
     'chronic sinusitis',
     'chronic rhinosinusitis',
     'chronic infection of sinus',
     'menopause present',
     'menopause',
     'toothache',
     'pain in tooth',
     'dentagra',
     'dentalgia',
     'odontalgia',
     'tooth ache',
     'toothaches',
     'basal cell carcinoma',
     'BCc',
     'basal cell epithelioma',
     'pigmented basal cell carcinoma',
     'rodent ulcer',
     'basiloma',
     'basal cell carcinoma of skin',
     'cancer of skin, basal cell',
     'basalioma',
     'RU',
     'epithelioma basal cell',
     'conjunctivitis',
     'inflammation of conjunctiva',
     'conjuntivitis',
     'conjuctivitis',
     'conjunctivitus',
     'screening procedure',
     'health screening',
     'screening procedures',
     'raised prostate specific antigen',
     'raised psa',
     'elevated psa',
     'malabsorption of glucose',
     'impaired glucose tolerance',
     'prediabetic nonclinical diabetes',
     'latent diabetes',
     'chemical diabetes',
     'IGt',
     'prediabetes',
     'smoker',
     'current smoker',
     'smokers',
     'plantar fasciitis',
     'policemans heel',
     'plantar fascitis',
     'hypertriglyceridemia',
     'irregular periods',
     'irregular menstrual cycle',
     'irregular menstrual bleeding',
     'irregular menstruation',
     'menstrual periods irregular',
     'variable menstrual cycle',
     'irregular uterine bleeding',
     'herpes zoster',
     'zoster',
     'herpes zoster infection',
     'shingles',
     'zona',
     'sensorineural hearing loss',
     'sensorineural deafness',
     'neurosensory deafness',
     'perceptive hearing loss',
     'perceptive deafness',
     'SNd',
     'sensory-neural deafness',
     'sensory-neural hearing loss',
     'SNHl',
     'Pd',
     'rectal hemorrhage',
     'rectorrhagia',
     'proctorrhagia',
     'rectal bleeding',
     'bleeding per rectum',
     'pr - bleeding per rectum',
     'blood per rectum',
     'pr - blood per rectum',
     'Rb',
     'prb - rectal bleeding',
     'h/o: surgery',
     'h/o: operation',
     'history of - surgery',
     'history of surgery',
     'peptic ulcer',
     'peptic ulcer disease',
     'peptic ulceration',
     'PUd',
     'PU',
     'gastroduodenal ulcer',
     'medical examinations/reports',
     'noises in ear',
     'observation of tinnitus',
     'noises in head',
     'ringing in ear',
     'tinnitus',
     'noise in ears',
     'ringing in ears',
     'tinitis',
     'tinitus',
     'tinnatus',
     'myofibroma',
     'leiomyoma',
     'plexiform leiomyoma',
     'lipoleiomyoma',
     'leiomyoma, no icd-o subtype',
     'leiomyoma, no international classification of diseases for oncology subtype',
     'fibroid uterus',
     'fibromyoma',
     'fibroleiomyoma',
     'uterine fibroids',
     'uterine leiomyoma',
     'leiomyoma of uterus',
     'uterine fibroid',
     'uterine leiomyoma - fibroids',
     'fibroids',
     'leiomyoma of body of uterus',
     'leiomyomas',
     'leiomyomata',
     'bipolar disorder',
     'manic-depressive psychosis',
     'manic-depressive illness',
     'MDi',
     'bipolar affective disorder',
     'bipola',
     'bi-polar',
     'cyclophrenia',
     'BPAd',
     'vitamin d deficiency',
     'avitaminosis d',
     'transient cerebral ischaemia',
     'TIa',
     'temporary cerebral vascular dysfunction',
     'transient ischaemic attack',
     'transient ischemic attack',
     'streptococcal sore throat',
     'strept throat',
     'strep throat',
     'streptococcal pharyngitis',
     'septic sore throat',
     'streptococcal angina',
     'onychomycosis due to dermatophyte',
     'tinea of nail',
     'tinea unguium',
     'dermatophytosis of nail',
     'onychomycosis',
     'fungal infection of nail',
     'ringworm of nail',
     'deep venous thrombosis',
     'DVt',
     'deep vein thrombosis',
     'presbyopia',
     'pre-surgery evaluation',
     'pre-operative evaluation',
     'pre-op evaluation',
     'palliative care',
     'neonatal jaundice',
     'icterus neonatorum',
     'bacterial vaginosis',
     'Av',
     'anaerobic vaginosis',
     'Bv',
     'bacterial vaginitis',
     'NSv',
     'nonspecific vaginitis',
     'pain in throat',
     'throat soreness',
     'sore throat',
     'pharyngeal pain',
     'throat pain',
     'throat discomfort',
     'pain in the pharynx',
     'sore throat symptom',
     'soar throat',
     'impacted cerumen',
     'inspissated cerumen',
     'impacted wax',
     'foot pain',
     'sciatica',
     'sciatica neuralgia',
     'neuralgia-neuritis of sciatic nerve',
     'sciatic neuralgia',
     'cocix pain',
     'syatica',
     'ciatica',
     'cocsix pain',
     'siatica',
     'peripheral oedema',
     'vomiting symptom',
     'finding of vomiting',
     'observation of vomiting',
     'vomiting',
     'emesis',
     'vomitting',
     'vomitings',
     'acne',
     'black heads',
     'white heads',
     'acnes',
     'history of polyp of colon',
     'history of colonic polyp',
     'benign prostatic hypertrophy with outflow obstruction',
     'type 2 diabetes mellitus without complication',
     'type ii diabetes mellitus without complication',
     'biliary calculus',
     'cholelithiasis',
     'Cl',
     'stone - biliary',
     'calculus - biliary',
     'calculus in biliary tract',
     'epigastric pain',
     'late effects of cerebrovascular disease',
     'sequelae of cerebrovascular disease',
     'pulmonary embolism',
     'Pe',
     'inguinal hernia',
     'Ih',
     'verruca',
     'verrucous skin lesion',
     'wart',
     'warts',
     'verruca vulgaris',
     'viral warts',
     'viral wart',
     'viral warts due to papilloma virus',
     'mosaic wart',
     'human papilloma virus infection of skin',
     'infectious warts',
     'verruca simplex',
     'common wart',
     'veruca',
     'verrucas',
     'verucca',
     'weight decreasing',
     'losing weight',
     'progressive weight loss',
     'weight decreased',
     'weight loss',
     'reduced weight',
     'weight reduction',
     'sepsis',
     'systemic infection',
     'sepses',
     'diabetic renal disease',
     'diabetic nephropathy',
     'renal disorder associated with diabetes mellitus',
     'history of coronary artery bypass grafting',
     'past history of coronary artery bypass grafting',
     'screening for malignant neoplasm of colon',
     'screening for colon cancer',
     'colon cancer screening',
     'colon neoplasm screening',
     'nausea and vomiting',
     'n&v - nausea and vomiting',
     'n+v - nausea and vomiting',
     'hyperthyroidism',
     'abscess',
     'abscess morphology',
     'abcess',
     'absess',
     'abscessed',
     'abscesses',
     'dental caries',
     'tooth caries',
     'saprodontia',
     'dental cavity',
     'tooth decay',
     'carious lesion',
     'tooth decayed',
     'dental decay',
     'teeth decayed',
     'gastrointestinal haemorrhage',
     'GIh',
     'GIt',
     'gastrointestinal tract haemorrhage',
     'gastrointestinal hemorrhage',
     'gastrointestinal tract hemorrhage',
     'gastrointestinal bleed',
     'gi bleeding',
     'gi hemorrhage',
     'gastrointestinal bleeding',
     'gi haemorrhage',
     'rosacea',
     'acne rosacea',
     'acne erythematosa',
     'parkinson,s disease',
     'idiopathic parkinson,s disease',
     'parkinson disease',
     'parkinsons disease',
     'primary parkinsonism',
     'idiopathic parkinsonism',
     'paralysis agitans',
     'shaking palsy',
     'type ii diabetes mellitus uncontrolled',
     'type ii diabetes mellitus poorly controlled',
     'malignant tumour of lung',
     'ca - lung cancer',
     'joint pain',
     'articular pain',
     'arthralgia',
     'painful joint',
     'arthralgias',
     'menorrhagia',
     'hypermenorrhoea',
     'hypermenorrhea',
     'flooding during periods',
     'heavy period',
     'profuse menstrual flow',
     'heavy periods',
     'morbid obesity',
     'reactive airways dysfunction syndrome',
     'generalized aches and pains',
     'generalized body aches',
     'generalized pain',
     'generalised pain',
     'generalised body aches',
     'hiatal hernia',
     'oesophageal hiatus hernia',
     'esophageal hiatus hernia',
     'Hh',
     'hiatus hernia',
     'esophageal hiatal hernia',
     'oesophageal hiatal hernia',
     'para-esophageal hernia',
     'restless legs',
     'ekbom syndrome',
     'restless legs syndrome',
     'thrombocytopenic disorder',
     'thrombocytopenia',
     'platelet count below reference range',
     'old myocardial infarction',
     'healed coronary',
     'healed myocardial infarction',
     'neuropathy',
     'neuropathy (nerve damage)',
     'neuropathies',
     'myocardial disease',
     'disorder of myocardium',
     'disorder of heart muscle',
     'cardiomyopathy',
     'myocardiopathy',
     'cardiomyopathies',
     'no abnormality detected',
     'NAd',
     'rotator cuff syndrome',
     'rotator cuff rupture',
     'rotator cuff tear',
     'rupture of rotator cuff of shoulder',
     'impingement syndrome',
     'atopic neurodermatitis',
     'atopic dermatitis',
     'Ad',
     'atopic eczema',
     'allergic eczema',
     'allergic dermatitis',
     'disseminated neurodermatitis',
     'keratosis polaris',
     'pain in pelvis',
     'contact dermatitis',
     'Cd',
     'contact eczema',
     'dermatitis venenata',
     'indigestion',
     'dyspepsia',
     'indegestion',
     'nicotine dependence',
     'sprain of ankle',
     'sprain of ankle joint',
     'ankle sprain',
     'sprain of ligament of ankle joint',
     'tonsillectomy',
     'excision of tonsil',
     'ts - tonsillectomy',
     'tonsilectomy',
     'degenerative disorder of macula',
     'carcinoma of breast',
     'exacerbation of asthma',
     'persistent alcohol abuse',
     'chronic alcohol abuse',
     'alcohol dependence',
     'chronic alcoholism',
     'alcohol dependence syndrome',
     'alcoholism',
     'dipsomania',
     'alcohol problem drinking',
     'low serum potassium level - finding',
     'low serum potassium level',
     'hypokalemia',
     'hypopotassaemia',
     'hypopotassemia',
     'hypopotassaemia syndrome',
     'potassium depletion',
     'k deficiency',
     'hypopotassemia syndrome',
     'hypokalemic syndrome',
     'hypokalaemic syndrome',
     'potassium deficiency',
     'tonsillectomy and adenoidectomy',
     'ts and as - tonsillectomy and adenoidectomy',
     'tonsillectomy with adenoidectomy',
     'adenotonsillectomy',
     'mitral valve regurgitation',
     'Mi',
     'mitral incompetence',
     'Mr',
     'mitral regurgitation',
     'mitral insufficiency',
     'mitral valve incompetence',
     'mitral valve insufficiency',
     'telephone encounter',
     'contraception education',
     'health education - contraception',
     'sodium deficiency',
     'salt deficiency',
     'hyponatraemia',
     'na deficiency',
     'hyponatremia',
     'dysfunctional uterine bleeding',
     'DUb',
     'dysfunctional uterine haemorrhage',
     'DUh',
     'dysfunctional uterine hemorrhage',
     'bleeding between periods',
     'intermenstrual bleeding',
     'IMb',
     'spotting between periods',
     'intermenstrual bleeding - irregular',
     'metrorrhagia',
     'bleeding unrelated to menstrual cycle',
     'irregular intermenstrual bleeding',
     'abdominal aortic aneurysm',
     'AAa',
     'skin lesion',
     'rehabilitation therapy management',
     'rehabilitation case management',
     'manage rehabilitation care',
     'cyst of ovary',
     'ovarian cyst',
     'ovarian cystic mass',
     'increased frequency of urination',
     'increased frequency of micturition',
     'pollakiuria',
     'passes water too often',
     'pollakisuria',
     'urinary frequency',
     'finding of frequency of urination',
     'observation of frequency of urination',
     'frequency of urination',
     'FOm',
     'frequency of micturition',
     'periodicity of micturition',
     'ex-smoker',
     'cessation of smoking',
     'past tobacco smoker',
     'threatened miscarriage',
     'threatened abortion (antepartum)',
     'threatened abortion',
     'scoliosis deformity of spine',
     'scoliosis',
     'thyroid nodule',
     'spinal stenosis',
     'Ss',
     'dysmenorrhoea',
     'menorrhalgia',
     'period pain',
     'menstrual cramps',
     'period pain present',
     'painful menstruation',
     'menstrual cramp',
     'dysmenorrea',
     'mammography abnormal',
     'acute otitis media',
     'AOm',
     'alzheimer,s disease',
     'alzheimer disease',
     'alzheimers disease',
     'diabetic neuropathy',
     'diabetes mellitus with neuropathy',
     'acute pharyngitis',
     'degeneration of intervertebral disc',
     'intervertebral disc degeneration',
     'degenerative disc disease',
     'attention deficit hyperactivity disorder, predominantly inattentive type',
     'ADd',
     'attention deficit disorder',
     'attention deficit disorder without hyperactivity',
     'attention deficit without hyperactivity',
     'unplanned pregnancy',
     'accidental pregnancy',
     'impotence of organic origin',
     'organic erectile dysfunction',
     'spinal stenosis of lumbar region',
     'lumbar spinal stenosis',
     'accident',
     'accidental physical contact',
     'accidents',
     'proteinuria',
     'abnormal presence of protein in urine',
     'postpartum examination and care of mother',
     'urticaria',
     'weal',
     'wheal',
     'welt',
     'nettle rash',
     'urticarial rash',
     'hives',
     'hive',
     'wheals',
     'weals',
     'welts',
     'genital herpes simplex',
     'herpes genitalis',
     'malignant neoplasm of female breast',
     'female breast cancer',
     'cancer of female breast',
     'nausea',
     'observation of nausea',
     'nauseous',
     'nauseated',
     'nauseas',
     'queasy',
     'queasiness',
     'chronic rhinitis',
     'generalised multiple sclerosis',
     'multiple sclerosis',
     'disseminated sclerosis',
     'Ms',
     'Ds',
     'multiple scrosis',
     'multiple sclrosis',
     'chronic kidney disease stage 3',
     'ckd stage 3',
     'panic disorder',
     'episodic paroxysmal anxiety disorder',
     'attention deficit hyperactivity disorder',
     'ADHd',
     'attention deficit disorder with hyperactivity',
     'hyperkinetic disorder',
     'hyperkinetic syndrome',
     'minimal brain dysfunction',
     'overactive child syndrome',
     'attention deficit hyperkinetic disorder',
     'MBd',
     'attention deficit hyperactivity disorder, predominantly hyperactive impulsive type',
     'attention deficit hyperactivity disorder, hyperactive impulsive type',
     'adhd',
     'needs influenza immunization',
     'cobalamin deficiency',
     'deficiency of vitamin b>12<',
     'vitamin b12 deficiency',
     'familial combined hyperlipidaemia',
     'familial hyperlipoproteinemia type iib',
     'FCHl',
     'familial combined hyperlipidemia',
     'fredrickson type iib hyperlipoproteinemia',
     'hyperapobetalipoproteinemia',
     'familial hyperlipoproteinaemia type iib',
     'fredrickson type iib hyperlipoproteinaemia',
     'hyperapobetalipoproteinaemia',
     'familial defective apolipoprotein b-100',
     'FDb',
     'familial multiple lipoprotein-type hyperlipidaemia',
     'memory impairment',
     'memory deficit',
     'memory dysfunction',
     'impaired memory',
     'dysmnesia',
     'memory problem',
     'poor memory',
     'bad memory',
     'disturbance of memory',
     'amnesia',
     'LOm',
     'loss of memory',
     'memory gone',
     'memory loss - amnesia',
     'memory loss',
     'colonoscopy',
     'endoscopy of colon',
     'endoscopic examination of colon',
     'endoscopy procedures on the rectum',
     'colonoscopies',
     'otalgia',
     'earache',
     'pain in ear',
     'ear ache',
     'ear pain',
     'earache symptoms',
     'earache symptom',
     'earaches',
     'tremor',
     'involuntary trembling',
     'involuntary quiver',
     'shaking',
     'shakes',
     'trembling',
     'quivering',
     'the shakes',
     'shaking all over',
     'has a tremor',
     'trembles',
     'shakings',
     'tremblings',
     'tremoring',
     'tremors',
     'rubella non-immune',
     'bladder retention of urine',
     'urinary retention',
     'retention of urine',
     'not passing urine',
     'unable to pass urine',
     'cannot pass urine - retention',
     'unable to empty bladder',
     'dysuria',
     'pain passing urine',
     'painful micturition',
     'pain passing water',
     'urination painful',
     'pain on micturition',
     'pain on voiding',
     'passing water hurts',
     'pain emptying bladder',
     'generalised anxiety disorder',
     'GAd',
     'generalized anxiety disorder',
     'arthralgia of the ankle and/or foot',
     'ankle and/or foot joint pain',
     'arthralgia of the ankle and foot',
     'ligation of fallopian tube',
     'tubal ligation',
     'Tl',
     'paroxysmal atrial fibrillation',
     'af - paroxysmal atrial fibrillation',
     'PAf',
     'intermittent atrial fibrillation',
     'rehabilitation therapy',
     'rehabilitation care',
     'rehabilitation treatments and procedures',
     'rehabilitation',
     'rehabilitations',
     'peripheral venous insufficiency',
     'chronic venous insufficiency',
     'nonproliferative diabetic retinopathy',
     'non proliferative diabetic retinopathy',
     'background diabetic retinopathy',
     'NPDr',
     'BDr',
     'shoulder joint pain',
     'arthralgia of the shoulder region',
     'arthralgia of shoulder',
     'moderate recurrent major depression',
     'diverticulitis',
     'diverticulitus',
     'solitary nodule of lung',
     'hyperkalaemia',
     'potassium excess',
     'hyperkalemic syndrome',
     'k overload',
     'k excess',
     'hyperpotassemia',
     'hyperkalaemic syndrome',
     'hyperpotassaemia',
     'raised serum potassium level',
     'potassium overload',
     'hyperkalemia',
     'recurrent major depressive episodes',
     'recurrent major depression',
     'recurrent major depressive disorder',
     'antenatal screening',
     'multiple myeloma',
     'multiple myeloma (clinical)',
     'plasmacytic myeloma',
     'kahlers disease',
     'myelomatosis',
     'myeloma',
     'multiple myeloma, no icd-o subtype',
     'multiple myeloma, no international classification of diseases for oncology subtype',
     'plasma cell myeloma',
     'myloma',
     'regular astigmatism',
     'secondary malignant neoplasm of liver',
     'metastatic malignant neoplasm to liver',
     'cancer metastatic to liver',
     'liver secondary cancer',
     'metastasis to liver',
     'liver secondaries',
     'secondary malignancy of liver',
     'hepatic metastasis',
     'ulcerative colitis',
     'colitis gravis',
     'Uc',
     'idiopathic proctocolitis',
     'ulcerative colitus',
     'ulcerative collitis',
     'vaginitis',
     'inflammation of vagina',
     'acute renal failure syndrome',
     'ARf',
     'acute renal failure',
     'normal pregnancy',
     'amenorrhoea',
     'absence of menstruation',
     'amenorrhea',
     'tendinitis',
     'inflammatory disorder of tendon',
     'tendonitis',
     'tendonitus',
     'rhinitis',
     'irritation of nose',
     'mixed anxiety and depressive disorder',
     'anxiety depression',
     'pain in upper limb',
     'pain of upper limb',
     'epistaxis',
     'nosebleed',
     'nose bleed',
     'nasal hemorrhage',
     'nasal haemorrhage',
     'bleeding from nose',
     'observation of bleeding of nose',
     'finding of bleeding of nose',
     'epistaxes',
     'nosebleeds',
     'crohns disease',
     'crohn disease',
     'crohns disease',
     'Re',
     'regional enteritis',
     'crohns regional enteritis',
     'granulomatous enteritis',
     'crones disease',
     'chromes disease',
     'chrones',
     'chrons disease',
     'chrohns disease',
     'nuclear senile cataract',
     'muscle pain',
     'myalgia',
     'muscle ache',
     'myodynia',
     'myosalgia',
     'myoneuralgia',
     'aching muscles',
     'myalgias',
     'contraception care education',
     'teach contraception care',
     'contraception counselling',
     'general contraceptive advice',
     'contraception counseling',
     'pilar tumor',
     'proliferating tricholemmal cyst',
     'proliferating trichilemmal tumor',
     'proliferating trichilemmal cyst',
     'proliferating trichilemmal tumour',
     'trichilemmal cyst',
     'follicular isthmus cyst',
     'pilar cyst',
     'isthmus catagen cyst',
     'proliferating pilar cyst',
     'keratinizing cyst',
     'cystic epidermoid choristoma',
     'epithelial inclusion cyst',
     'sebaceous cyst',
     'steatoma',
     'wen',
     'keratinising cyst of skin',
     'keratinising cyst',
     'keratin cyst',
     'epidermal inclusion cyst',
     'inclusion cyst',
     'epidermoid cyst of skin',
     'epithelial cyst',
     'keratinous cyst',
     'epidermoid cyst',
     'epidermal cyst',
     'EIc',
     'impaired cognition',
     'cognitive impairment',
     'cognitive disturbance',
     'cognitive dysfunction',
     'cognitive decline',
     'acute exacerbation of chronic obstructive airways disease',
     'acute exacerbation of copd',
     'acute exacerbation of chronic obstructive pulmonary disease',
     'pre-anaesthetic assessment',
     'dysfunction of eustachian tube',
     'ETd',
     'eustachian tube dysfunction',
     'eustachian tube disorder',
     'disorder of eustachian tube',
     'Et',
     'auditory tube disorder',
     'disease of eustachian tube',
     'internal haemorrhoids',
     'internal haemorrhoid',
     'internal hemorrhoid',
     'substance abuse',
     'nondependent abuse of drugs',
     'nondependent abuse of substance',
     'harmful substance use',
     'pain',
     'pain observations',
     'dolor',
     'painful',
     'part hurts',
     'paining',
     'pains',
     'barretts esophagus',
     'barrett esophagus',
     'barretts esophagus',
     'columnar-lined esophagus',
     'gastric metaplasia of esophagus',
     'barretts syndrome',
     'columnar-lined oesophagus',
     'gastric metaplasia of oesophagus',
     'CELLo',
     'columnar epithelial-lined lower oesophagus',
     'Bo',
     'barretts oesophagus',
     'CLe',
     'columnar epithelial-lined lower esophagus',
     'barratts oesophagus',
     'vaginal discharge',
     'observation of vaginal discharge',
     'virginal discharge',
     'cerebrovascular disease',
     'cerebrovascular lesion',
     'CVd',
     'well woman health examination',
     'malignant melanoma',
     'Mm',
     'malignant melanoma, no icd-o subtype',
     'malignant melanoma, no international classification of diseases for oncology subtype',
     'melanoma',
     'melanosarcoma',
     'melanomas',
     'melanocarcinoma',
     'folliculitis',
     'fh: diabetes mellitus',
     'family history: diabetes mellitus',
     'mitral valve prolapse',
     'MVp',
     'j.b. barlows syndrome',
     'mitral valve prolapse syndrome',
     'floppy valve syndrome',
     'click-murmur syndrome',
     'barlows syndrome',
     'click syndrome',
     'asynchronous valve closure syndrome',
     'j.b. barlow syndrome',
     'mitral valve posterior leaflet prolapse',
     'mitral valve prolapse, posterior leaflet',
     'chronic hepatitis c',
     'chronic type c viral hepatitis',
     'cervical smear biopsy taken',
     'cervical smear taken',
     'papanicolaou smear taken',
     'smear - cervical:taken',
     'hypermetropia',
     'longsightedness',
     'longsighted',
     'hyperopia',
     'farsightedness',
     'long-sighted',
     'long-sightedness',
     'far-sighted',
     'far-sightedness',
     'endometriosis (clinical)',
     'endometriosis',
     'endemetriosis',
     'cva - cerebrovascular accident due to cerebral artery occlusion',
     'cerebrovascular accident due to cerebral artery occlusion',
     'gestational diabetes mellitus',
     'gestational diabetes',
     'maternal gestational diabetes mellitus',
     'diabetes mellitus arising in pregnancy',
     'GDm',
     'cirrhosis of liver',
     'hepatic cirrhosis',
     'cirrosis',
     'cirrhosis',
     'cirhosis',
     'cirosis',
     'injury of head',
     'injury of head region',
     'head injury',
     'dehydration',
     'pure water depletion syndrome',
     'dehydrations',
     'dehydrated',
     'herpes simplex',
     'herpes simplex complex',
     'herpes simplex infection',
     'herpes simplex viral infection',
     'fracture of bone',
     'fracture',
     'broken bone',
     'fractured',
     'fractures',
     'fracturing',
     'overweight',
     'patient overweight',
     'adjustment disorder',
     'adaptation reaction',
     'adjustment reaction',
     'acute situational disturbance',
     'transient situational disturbance',
     'brief situational non-psychotic disorder',
     'tinea pedis',
     'epidermophytosis pedis',
     'athletes foot',
     'dermatophytosis of foot',
     'Tp',
     'ringworm of foot',
     'aortic valve stenosis',
     'stenosed aortic valve',
     'aortic stenosis',
     'As',
     'type b viral hepatitis',
     'serum hepatitis',
     'hepatitis b',
     'Sh',
     'hepatitis b infection',
     'viral hepatitis type b',
     'ingrowing nail',
     'unguis incarnatus',
     'onyxis',
     'onychocryptosis',
     'unguis aduncus',
     'Oc',
     'ingrown nail',
     'ingrowing toenail',
     'ingrown toenail',
     'embedded toenail',
     'IGTn',
     'postmenopausal bleeding',
     'PMb',
     'bleeding after menopause',
     'follow-up visit',
     'goitre',
     'thyroid goiter',
     'thyroid goitre',
     'swelling of thyroid gland',
     'thyroid enlargement',
     'struma - goitre',
     'struma - goiter',
     'enlargement of thyroid',
     'struma of thyroid',
     'thyromegaly',
     'goiter',
     'secondary malignant neoplasm of bone',
     'metastatic tumour of bone',
     'metastatic tumor of bone',
     'secondary cancer of bone',
     'secondary malignant neoplasm of bone and bone marrow',
     'osseous metastasis',
     'bony metastasis',
     'metastatic malignant neoplasm to bone',
     'cancer metastatic to bone',
     'tumour metastatic to bone',
     'tumor metastatic to bone',
     'pulmonary emphysema',
     'emphysema of lung',
     'snoring symptoms',
     'snoring symptom',
     'snoring',
     'observation of snoring',
     'snores',
     'finding of snoring',
     'myocardial infarction',
     'myocardial infarct',
     'infarction of heart',
     'cardiac infarction',
     'heart attack',
     'chest discomfort',
     'venereal disease screening',
     'vd- venereal disease screening',
     'polycystic ovaries',
     'stein-leventhal syndrome',
     'polycystic ovarian disease',
     'cystic disease of ovaries',
     'polycystic ovarian syndrome',
     'PCo',
     'multicystic ovaries',
     'PCOs',
     'sclerocystic ovary syndrome',
     'polycystic ovary syndrome',
     'PCOd',
     'caesarean section',
     'Cs',
     'cesarean section',
     'cesarean delivery only',
     'microscopic hematuria',
     'wrist joint pain',
     'arthralgia of wrist',
     'pain in wrist',
     'wrist pain',
     'personal history of primary malignant neoplasm of breast',
     'umbilical hernia',
     'exomphalos',
     'exumbilication',
     'congenital exomphalos',
     'pleural effusion',
     'coronary artery bypass graft',
     'CABg',
     'CBg',
     'coronary bypass graft',
     'coronary artery bypass grafting',
     'coronary artery bypass graft operations',
     'CAg',
     'coronary artery graft',
     'aortocoronary artery bypass graft',
     'aortocoronary bypass for heart revascularization',
     'acb graft',
     'vascular aortocoronary bypass',
     'cardiac revascularization with aortocoronary bypass anastomosis',
     'aortocoronary bypass for heart revascularisation',
     'cardiac revascularisation with aortocoronary bypass anastomosis',
     'cardiac revascularisation with bypass anastomosis',
     'acute retention of urine',
     'ARU',
     'acute urinary retention',
     'false labour',
     'spurious labor',
     'spurious labour',
     'vaginal bleeding',
     'observations of vaginal bleeding',
     'bleeding from vagina',
     'shadow of lung',
     'acquired hypothyroidism',
     'primary hypothyroidism',
     'preterm rupture of membranes',
     'premature rupture of membranes',
     'PROm',
     'rupture of membranes prior to onset of labor',
     'rupture of amniotic sac under 24 hours before onset of labour',
     'rupture of amniotic sac under 24 hours before onset of labor',
     'proms',
     'human immunodeficiency virus infection',
     'HIv',
     'hiv infection',
     'contusion',
     'bruising',
     'bruise',
     'ecchymosis',
     'contusion - lesion',
     'bruise - lesion',
     'ecchymoses',
     'brusing',
     'contusions',
     'bruised',
     'bruises',
     'secondary malignant neoplasm of lung',
     'secondary malignant tumour of lung',
     'secondary malignant tumor of lung',
     'pulmonary metastasis',
     'secondary cancer of lung',
     'metastatic malignant neoplasm to lung',
     'cancer metastatic to lung',
     'problem situation relating to social and personal history',
     'raised serum calcium level',
     'hypercalcaemia',
     'hypercalcemia syndrome',
     'hypercalcaemia syndrome',
     'hypercalcinaemia',
     'arthroplasty of knee',
     'repair of knee joint',
     'arthroplasty of the knee',
     'knee arthroplasty',
     'knee replacement',
     'total knee replacement',
     'total knee arthroplasty',
     'arthroplasty, knee, tibial plateau',
     'carotid artery occlusion',
     'carotid artery obstruction',
     'prolonged second stage of labour',
     'lipoma',
     'lipoma, no icd-o subtype',
     'lipoma, no international classification of diseases for oncology subtype',
     'lipoma (clinical)',
     'lipomas',
     'poisoning by drug and/or medicinal substance',
     'therapeutic agent toxicity',
     'drug toxicity',
     'drug poisoning',
     'poisoning by drug or medicinal substance',
     'medicament poisoning',
     'intoxication by drug',
     'medicinal poisoning',
     'paranoid schizophrenia',
     'paraphrenic schizophrenia',
     'postoperative visit',
     'candidal vulvovaginitis',
     'monilial vulvovaginitis',
     'vulvovaginal thrush',
     'h/o: cesarean section',
     'history of cesarean section',
     'allergic reaction to substance',
     'hypersensitivity reaction',
     'hypersensitivity',
     'fetal or neonatal effect of maternal premature rupture of membrane',
     'foetal or neonatal effect of maternal premature rupture of membrane',
     'noncompliance with treatment',
     'does not comply with treatment',
     'noncompliance',
     'falls',
     'fall',
     'fallen',
     'falling',
     'fell',
     'muscle strain',
     'pulled muscle',
     'strain',
     'strained',
     'strains',
     'schizophrenic disorders',
     'schizophrenia',
     'left bundle branch block',
     'LBBb',
     'elevated blood-pressure reading without diagnosis of hypertension',
     'elevated blood pressure reading without diagnosis of hypertension',
     'neurologic disorder associated with type 2 diabetes mellitus',
     'neurologic disorder associated with type ii diabetes mellitus',
     'neurologic complication of adult-onset type diabetes mellitus',
     'diabetes type 2 with neurological disorder',
     'unilateral inguinal hernia',
     'conduction disorder of the heart',
     'disorder of heart conduction',
     'lateral epicondylitis',
     'tennis elbow',
     'lateral epicondylitis of elbow',
     'burn',
     'burn injury',
     'burn - lesion',
     'sun burn',
     'sunburn',
     'burned',
     'burns',
     'burnt',
     'brittle diabetes mellitus',
     'labile diabetes',
     'brittle diabetes',
     'unstable diabetes mellitus',
     'pyelonephritis',
     'varicose veins of lower extremity',
     'phlebectasia of lower extremity',
     'varix of lower extremity',
     'Vv',
     'varicose veins of leg',
     'varicose veins of lower limb',
     'hemiplegia',
     'hemiplegia (paralysis on one side)',
     'removal of suture',
     'removal of sutures',
     'ROs',
     'chronic back pain',
     'chronic pain',
     'bronchiolitis',
     'mild persistent asthma',
     'secondary malignant neoplasm of lymph node',
     'secondary lymph node cancer',
     'metastatic malignant neoplasm to lymph node',
     'cancer metastatic to lymph nodes',
     'mixed hyperlipidemia',
     'multiple-type hyperlipidaemia',
     'multiple-type hyperlipidemia',
     'female stress incontinence',
     'female urinary stress incontinence',
     'localized, primary osteoarthritis of the ankle and/or foot',
     'primary localized osteoarthrosis of ankle and/or foot',
     'primary localised osteoarthrosis of ankle and/or foot',
     'localized, primary osteoarthritis of the ankle and foot',
     'localised, primary osteoarthritis of the ankle and foot',
     'hypesthesia',
     'reduced sensation',
     'limited sensation',
     'impaired sensation',
     'hypoesthesia',
     'hypoaesthesia (reduced sensation)',
     'hypoesthesia (reduced sensation)',
     'reduced sensation of skin',
     'tactile hypesthesia',
     'hypesthesias',
     'hand pain',
     'painful hand',
     'psychotic disorder',
     'psychosis',
     'psychoses',
     'menopausal syndrome',
     'acute myocardial infarction of anterior wall',
     'acute anterior myocardial infarction',
     'hormone replacement therapy',
     'HRt',
     'bronchiectasis',
     'bronchiectasia',
     'lumbar radiculopathy',
     'osteoarthritis of hip',
     'degenerative joint disease of hip',
     'acute coronary syndrome',
     'ACs',
     'deviated nasal septum',
     'dyspnea on exertion',
     'exertional dyspnea',
     'breathlessness on exertion',
     'dyspnea on effort',
     'dyspnoea on effort',
     'short of breath on exertion',
     'SOBOe',
     'shortness of breath on exertion',
     'exertional dyspnoea',
     'paraesthesia',
     'paraesthesia (numbness/tingling)',
     'paresthesia (numbness/tingling)',
     'paraesthesias',
     'paresthesia',
     'hypoglycaemic disorder',
     'hypoglycemic syndrome',
     'hypoglycemia',
     'deep venous thrombosis of lower extremity',
     'deep vein thrombosis of lower limb',
     'deep venous thrombosis of leg',
     'deep venous thrombosis of lower limb',
     'complication of surgical procedure',
     'misadventure of surgical procedure',
     'mild intermittent asthma',
     'general examination of patient',
     'abdominal pain in pregnancy',
     'systemic lupus erythematosus',
     'SLe',
     'disseminated lupus erythematosus',
     'generalised ischaemic myocardial dysfunction',
     'ischemic cardiomyopathy',
     'ischaemic cardiomyopathy',
     'fresh blood passed per rectum',
     'hematochezia',
     'bloody stool',
     'bright red blood in stool',
     'bright red blood per rectum',
     'passage of bloody stools',
     'BRBPr',
     'feces: blood',
     'blood in feces',
     'faeces: blood',
     'blood in faeces',
     'blood in stool',
     'lumbar spondylosis',
     'osteoarthritis of lumbar spine',
     'acute tonsillitis',
     'infective tonsillitis',
     'oesophagitis',
     'aortic valve regurgitation',
     'aortic valve insufficiency',
     'aortic valve incompetence',
     'aortic regurgitation',
     'aortic incompetence',
     'Ai',
     'aortic insufficiency',
     'malignant neoplasm of skin',
     'skin cancer',
     'cancer of skin',
     'mental health impairment',
     'abnormal mental state',
     'altered mental status',
     'atrial flutter',
     'loss of hair',
     'falling hair',
     'thinning hair',
     'alopecia',
     'bald',
     'hair loss disorder',
     'baldness',
     'spot baldness',
     'losing my hair',
     'hairloss',
     'thin hair',
     'alopecias',
     'high risk pregnancy',
     'HRp',
     'liver enzymes abnormal',
     'missed miscarriage',
     'Ma',
     'missed abortion',
     'silent miscarriage',
     'fetal death before 22 weeks with retention of dead fetus',
     'foetal death before 22 weeks with retention of dead foetus',
     'malignant tumor of urinary bladder',
     'bladder cancer',
     'malignant neoplasm of urinary bladder',
     'ca - bladder cancer',
     'bladder tumor',
     'pulmonary hypertension',
     'PHt',
     'nasal congestion',
     'stuffed-up nose',
     'congested nose',
     'flank pain',
     'haemoptysis',
     'coughing up blood',
     'blood streaked sputum',
     'viral gastroenteritis',
     'viral diarrhea',
     'viral vomiting',
     'viral diarrhoea',
     'enteritis due to specified virus',
     'right upper quadrant pain',
     'primary open angle glaucoma',
     'chronic simple glaucoma',
     'primary open-angle glaucoma',
     'POAg',
     'CSg',
     'COAg',
     'chronic open-angle glaucoma',
     'malignant tumor of ovary',
     'cancer of ovary',
     'ovarian cancer',
     'incomplete emptying of bladder',
     'incomplete bladder emptying',
     'pruritus of skin',
     'pruritic disorder',
     'pruritus - disorder',
     'pruritic dermatitis',
     'itching',
     'itch',
     'itchy',
     'itching of skin',
     'pruritus',
     'itch of skin',
     'pruritis',
     'prurutis',
     'itches',
     'fibrocystic disease of breast',
     'schimmelbuschs disease',
     'fibrocystic breast changes',
     'fibrocystic mastopathy',
     'fibrocystic change of breast',
     'cystic disease of breast',
     'chronic cystic mastitis',
     'diffuse cystic mastopathy',
     'gross cystic disease of breast',
     'surgical follow-up',
     'gastroenteritis presumed infectious',
     'gastroenteritis - presumed infectious origin',
     'joints gout affected',
     'gouty arthropathy',
     'articular gout',
     'gouty arthritis',
     'acute pancreatitis',
     'acute myocardial infarction',
     'AMi',
     'feeling suicidal',
     'suicidal thoughts',
     'cerebral haemorrhage',
     'intracerebral haemorrhage (ich)',
     'intracerebral hemorrhage (ich)',
     'urge incontinence of urine',
     'urge urinary incontinence',
     'must urinate at once with urge',
     'lack of bladder control if desire resisted',
     'lack of bladder control if desire urgent',
     'megaloblastic anemia due to vitamin b>12< deficiency',
     'vitamin b12-deficient megaloblastic anaemia',
     'vitamin b12-deficient megaloblastic anemia',
     'megaloblastic anaemia due to cobalamin deficiency',
     'vitamin b>12< deficiency anaemia',
     'megaloblastic anemia due to cobalamin deficiency',
     'vitamin b>12< deficiency anemia',
     'b-cell chronic lymphocytic leukaemia',
     'b-cll - b-cell chronic lymphocytic leukemia',
     'b-cll - b-cell chronic lymphocytic leukaemia',
     'chronic lymphocytic leukemia',
     'chronic lymphocytic leukemia, b-cell type',
     'b-cell chronic lymphocytic leukemia/small lymphocytic lymphoma',
     'b-cell chronic lymphocytic leukaemia/small lymphocytic lymphoma',
     'chronic lymphatic leukaemia',
     'chronic lymphocytic leukaemia, b-cell type',
     'chronic lymphoid leukaemia',
     'chronic lymphoid leukemia',
     'chronic lymphatic leukemia',
     'chronic lymphoid leukemia, disease',
     'CLl',
     'chronic lymphocytic leukaemia',
     'BCLl',
     'disorder of lymphoid system',
     'disorders of lymph node and lymphatics',
     'disorder of lymphatic vessel',
     'disorder of lymphatics',
     'lymphangiopathy',
     'disorder of lymphatic system',
     'chlamydial infection',
     'tonsillitis',
     'tonsillitus',
     'tonsilitus',
     'tonsilitis',
     'abnormal gait',
     'gait abnormality',
     'gait problem',
     'gait difficulty',
     'dermatophytosis of the body',
     'tinea corporis',
     'dermatophytosis of the trunk',
     'tinea circinatus',
     'tinea circinata',
     'herpes circinatus',
     'body tinea',
     'assault',
     'assaulted',
     'assaulting',
     'assaults',
     'accidental fall',
     'fall - accidental',
     'transplant of kidney',
     'kidney transplantation',
     'renal transplant',
     'tx - renal transplantation',
     'kidney transplant',
     'tx - kidney transplantation',
     'transplantation of kidney',
     'renal graft',
     'renal transplantation procedures',
     'weight increasing',
     'weight increased',
     'weight gain',
     'orthostatic hypotension',
     'postural hypotension',
     'pancreatitis',
     'inflammation of pancreas',
     'closed fracture of distal end of radius',
     'closed fracture of lower end of radius',
     'first degree perineal laceration',
     'laceration of superficial layers of perineal structures',
     'obstetrical laceration, first degree',
     'not for resuscitation',
     'DNr',
     'do not resuscitate',
     'DNAr',
     'do not attempt resuscitation',
     'do not resuscitate status',
     'intracranial injury',
     'benign hypertension',
     'history of total hysterectomy',
     'bells palsy',
     'bell palsy',
     'bells palsy',
     'idiopathic acute facial nerve palsy',
     'bradycardia',
     'heart rate slow',
     'slow heart beat',
     'decreased heart rate',
     'bradycardias',
     'polymyalgia rheumatica',
     'forestier-certonciny syndrome',
     'rhizomelic pseudopolyarthritis',
     'senile arthritis',
     'PMr',
     'dental abscess',
     'dental sepsis',
     'tooth abscess',
     'dysplasia of cervix',
     'cervical dysplasia',
     'dysplasia of cervix uteri',
     'serous otitis media',
     'total abdominal hysterectomy',
     'TAh',
     'follow-up orthopedic assessment',
     'giant urticaria',
     'angioedema',
     'urticaria-angio-oedema',
     'angio-oedema-urticaria',
     'angioedema-urticaria',
     'urticaria-angioedema',
     'quinckes disease',
     'angioneurotic oedema',
     'quinckes oedema',
     'quinckes edema',
     'angioneurotic edema',
     'angio-edema',
     'angio-oedema',
     'venous varices',
     'varices',
     'varicosities',
     'varicose veins',
     'varix',
     'varicose vein',
     'phlebectasia',
     'uneven venous ectasia',
     'venous ectasia',
     'varicosity',
     'total replacement of hip',
     'total hip replacement',
     'THr',
     'prosthetic total arthroplasty of the hip',
     'total reconstruction of hip with prosthesis',
     'administration of antineoplastic agent',
     'chemotherapy',
     'chemotherapies',
     'spasm',
     'spasms',
     'lactose intolerance',
     'Lm',
     'lactose malabsorption',
     'cows milk enteropathy',
     'milk intolerance',
     'sleep disorder',
     'pain of breast',
     'pain in the breast',
     'breast painful',
     'mastodynia',
     'painful breasts',
     'mastalgia',
     'breast pain',
     'tachycardia',
     'heart rate fast',
     'rapid heart beat',
     'increased heart rate',
     'pulse fast',
     'tachycardia - pulse',
     'rapid pulse',
     'tachycardias',
     'acute conjunctivitis',
     'diffuse non-hodgkins lymphoma, lymphoblastic (clinical)',
     'diffuse non-hodgkins lymphoma, lymphoblastic',
     'diffuse non-hodgkin lymphoma, lymphoblastic',
     'lymphoblastic lymphoma, diffuse',
     'lymphoblastoma',
     'lymphoblastic lymphoma',
     'malignant lymphoma - lymphoblastic',
     'b-cell lymphoma (clinical)',
     'b-cell lymphoma',
     'malignant lymphoma - category',
     'malignant lymphoma (clinical)',
     'lymphoma (clinical)',
     'malignant lymphoma',
     'lymphoma',
     'non-hodgkins lymphoma (clinical)',
     'malignant lymphoma, non-hodgkins type',
     'non-hodgkins lymphoma - disorder',
     'NHl',
     'non-hodgkins lymphoma',
     'non-hodgkin lymphoma',
     'precursor cell lymphoblastic lymphoma',
     'malignant lymphoma, lymphoblastic',
     'malignant lymphoma, convoluted cell [obs]',
     'lymphoblastoma [obs]',
     'non-hodgkin lymphoma (category)',
     'non-hodgkin lymphoma - category',
     'lymphosarcoma',
     'malignant lymphoma - small cleaved cell',
     'diffuse non-hodgkins small cleaved cell (diffuse) lymphoma',
     'lymphoblastic lymphosarcoma',
     'malignant lymphoma, lymphocytic - poorly differentiated',
     'malignant lymphoma - mixed small and large cell',
     'reticulolymphosarcoma, diffuse',
     'diffuse non-hodgkins mixed small and large cell (diffuse) lymphoma',
     'reticulolymphosarcoma',
     'malignant lymphoma - mixed lymphocytic-histiocytic',
     'malignant lymphoma, non-hodgkin',
     'b-cell lymphoma, no icd-o subtype',
     'reticulum cell sarcoma, diffuse [obs]',
     'reticulum cell sarcoma [obs]',
     'reticulosarcoma, diffuse [obs]',
     'reticulosarcoma [obs]',
     'malignant lymphoma, undifferentiated cell, non-burkitt [obs]',
     'malignant lymphoma, undifferentiated cell type [obs]',
     'malignant lymphoma, small cleaved cell, diffuse [obs]',
     'malignant lymphoma, small cleaved cell [obs]',
     'malignant lymphoma, small cell, noncleaved, diffuse [obs]',
     'malignant lymphoma, lymphocytic, poorly differentiated, diffuse [obs]',
     'malignant lymphoma, lymphocytic, intermediate differentiation, nodular [obs]',
     'malignant lymphoma, diffuse',
     'malignant lymphoma, cleaved cell [obs]',
     'lymphosarcoma, diffuse [obs]',
     'lymphosarcoma [obs]',
     'non-hodgkin lymphoma, no icd-o subtype',
     'non-hodgkin lymphoma, no international classification of diseases for oncology subtype',
     'malignant lymphoma, non-hodgkins',
     'microglioma [obs]',
     'malignant lymphoma, no icd-o subtype',
     'malignant lymphoma, no international classification of diseases for oncology subtype',
     'primary central nervous system lymphoma',
     'microglioma',
     'primary cns lymphoma',
     'PCNSl',
     'b-cell neoplasm',
     'mantle cell lymphoma',
     'mantle zone lymphoma [obs]',
     'malignant lymphomatous polyposis',
     'malignant lymphoma, lymphocytic, intermediate differentiation, diffuse [obs]',
     'malignant lymphoma, centrocytic [obs]',
     'lymphomas',
     'supraventricular tachycardia',
     'SVt',
     'hyperparathyroidism',
     'HPTh',
     'periodontitis',
     'history of malignant neoplasm of prostate',
     'h/o: pacemaker in situ',
     'history of pacemaker in situ',
     'intermittent hypertension',
     'family history of ischaemic heart disease',
     'fh: ischaemic heart disease',
     'fh: ischemic heart disease',
     'renal disorder due to type 2 diabetes mellitus',
     'renal disorder associated with type ii diabetes mellitus',
     'kidney disorder associated with type 2 diabetes mellitus',
     'diabetes type 2 with nephropathy',
     'nonexudative age-related macular degeneration',
     'atrophic age-related macular degeneration',
     'atrophic senile macular retinal degeneration',
     'dry senile macular degeneration',
     'dry senile macular retinal degeneration',
     'geographic atrophy of the macula',
     'nonexudative senile macular degeneration',
     'nonexudative senile macular retinal degeneration',
     'nonneovascular age-related macular degeneration',
     'vaginal delivery',
     'history of malignant neoplasm of skin',
     'h/o: malignant neoplasm of skin',
     'personal history of primary malignant neoplasm of skin',
     'h/o: primary malignant neoplasm of skin',
     'tension-type headache',
     'tension headache',
     'psychogenic headache',
     'stress headache',
     'cellulitis and abscess of lower leg',
     'chronic pain syndrome',
     'abnormal weight loss',
     'abnormal decrease in weight',
     'hyperlipidaemia, group a',
     'pure hypercholesterolemia',
     'acute upper respiratory infection',
     'AURTi',
     'acute upper respiratory tract infection',
     'acute uri',
     'electrocardiogram abnormal',
     'abnormal ekg finding',
     'abnormal ecg',
     'abnormal behaviour',
     'strange behavior',
     'abnormal conduct',
     'disturbance of conation',
     'strange behaviour',
     'problem behavior',
     'behavioral problem',
     'behavioural problem',
     'AIDs',
     'acquired immune deficiency syndrome',
     'acquired immunodeficiency syndrome',
     'acquired immune deficiency syndrome (aids)',
     'immunodeficiency due to human immunodeficiency virus infection',
     'cerebral thrombosis',
     'thrombosis of cerebral arteries',
     'cerebral arterial thrombosis',
     'inflammatory disease of liver',
     'inflammatory disorder of liver',
     'hepatitis',
     'inflammatory liver disease',
     'hepititus',
     'post percutaneous transluminal coronary angioplasty',
     'post ptca',
     'colitis',
     'colon inflammation',
     'post-term pregnancy',
     'post-dates',
     'prolonged pregnancy',
     'post term pregnancy',
     'prolonged gestation',
     'single major depressive episode',
     'major depression, single episode',
     'major depressive disorder, single episode',
     'abnormal liver function',
     'abnormal hepatic function',
     'croup',
     'croup syndrome',
     'family history of malignant neoplasm of breast',
     'skin tag',
     'achrochordon',
     'soft papilloma',
     'SKt',
     'soft fibroma',
     'fibroepithelial polyp',
     'fibroepithelial papilloma',
     'fibrous polyp',
     'acrochordon',
     'cutaneous tag',
     'cutaneous polyp',
     'tag',
     'tagged',
     'tagging',
     'tags',
     'metabolic syndrome x',
     'insulin resistance syndrome',
     'metabolic syndrome',
     'dysmetabolic syndrome x',
     'reavens syndrome',
     'diverticulitis of colon',
     'artificial lens present',
     'artificial lens in situ',
     'preoperative cardiovascular examination',
     'diabetic retinopathy',
     'retinal abnormality - diabetes-related',
     'Dr',
     'female infertility',
     'advanced open-angle glaucoma',
     'glaucoma simplex',
     'donders glaucoma',
     'pigmentary glaucoma',
     'secondary open-angle glaucoma with pigment dispersion',
     'open-angle glaucoma',
     'open cleft glaucoma',
     'wide-angle glaucoma',
     'open angle glaucoma',
     'OAg',
     'tietzes disease',
     'tietze disease',
     'cyriaxs syndrome',
     'slipping rib syndrome',
     'costochondral junction syndrome',
     'peristernal perichondritis',
     'tietzes syndrome',
     'costal chondritis',
     'costochondritis',
     'hiv positive',
     'htlv-3 antibody positive',
     'human immunodeficiency virus (hiv) positive',
     'human immunodeficiency virus positive',
     'postpartum care',
     'postpartal care',
     'postnatal care',
     'postpartum care only (separate procedure)',
     'gallstone',
     'cholecystolithiasis',
     'gallstones',
     'gallbladder stones',
     'gallbladder calculus',
     'Gs',
     'gaul stones',
     'gall stones',
     'borderline blood pressure',
     'condyloma acuminatum',
     'venereal wart',
     'condylomata acuminatum',
     'venereal warts',
     'condylomata acuminata',
     'anogenital warts',
     'anogenital wart',
     'AGw',
     'verruca acuminata',
     'genital warts',
     'genital wart',
     'sarcoidosis',
     'boecks sarcoidosis',
     'boecks sarcoid',
     'besnier-boeck-schaumann syndrome',
     'miliary lupoid of boeck',
     'lupus pernio of besnier',
     'benign lymphogranulomatosis of schaumann',
     'darier-roussy sarcoid',
     'sarcoid',
     'neurogenic dysfunction of the urinary bladder',
     'neurogenic bladder',
     'neuropathic bladder',
     'tobacco dependence in remission',
     'tobacco dependence, in remission',
     'wheezing',
     'wheezy',
     'asthmatic breath sounds',
     'asthmatic breathing',
     'wheeze',
     'wheezings',
     'wheezed',
     'wheezes',
     'elevated levels of transaminase & lactic acid dehydrogenase',
     'elevated levels of transaminase and lactic acid dehydrogenase',
     'helicobacter pylori gastrointestinal tract infection',
     'ascites',
     'peritoneal dropsy',
     'abdominal dropsy',
     'hydrops abdominis',
     'hydroperitonia',
     'hydroperitoneum',
     'low blood pressure',
     'hypotension',
     'hypopiesis',
     'arterial hypotension',
     'hypotensions',
     'bursitis',
     'inflammation of bursa',
     'miscarriage',
     'vaginal expulsion of product of conception',
     'vaginal expulsion of fetus',
     'spontaneous abortion',
     'vaginal expulsion of foetus',
     'misscarage',
     'miscarage',
     'miscarrage',
     'miscarriages',
     'peptic reflux disease',
     'peptic oesophagitis',
     'peptic reflux oesophagitis',
     'peptic esophagitis',
     'peptic reflux esophagitis',
     'methicillin resistant staphylococcus aureus carrier',
     'paroxysmal supraventricular tachycardia',
     'urethritis',
     'inflammation of urethra',
     'urarthritis',
     'noninfectious gastroenteritis',
     'malignant tumour of thyroid gland',
     'thyroid cancer',
     'thyroid ca',
     'pressure ulcer',
     'pressure sore',
     'contact ulcer',
     'bedsore',
     'bedsores',
     'hoarse',
     'hoarseness - throat symptom',
     'hoarseness symptom',
     'husky voice',
     'croaky voice',
     'voice hoarseness',
     'hoarseness',
     'prostatitis',
     'inflammation of prostate',
     'verruca plantaris',
     'plantar wart',
     'Vp',
     'verrucae pedis',
     'verruca pedis',
     'laboratory test result abnormal',
     'laboratory finding abnormal',
     'anemia of chronic disease',
     'anemia of chronic disorder',
     'anemia of systemic disease',
     'anaemia of chronic disorder',
     'anaemia of systemic disease',
     'secondary anemia',
     'delivery and/or maternal condition affecting management',
     'dysplastic nevus of skin',
     'atypical naevus of skin',
     'atypical nevus of skin',
     'dysplastic naevus',
     'Dn',
     'atypical naevus',
     'atypical nevus',
     'dysplastic nevus',
     'degeneration of lumbar intervertebral disc',
     'right lower quadrant pain',
     'sterility',
     'infertile',
     'difficulty conceiving',
     'cannot achieve a pregnancy',
     'hernia of abdominal wall',
     'external abdominal hernia',
     'benign paroxysmal positional vertigo',
     'benign paroxysmal positional nystagmus',
     'BPPv',
     'benign recurrent vertigo',
     'pityriasis versicolor',
     'Pv',
     'Tv',
     'tinea versicolor',
     'tinea flava',
     'tinea versicolor due to pityrosporum furfur',
     'tinea versicolor due to malassezia furfur',
     'hernia of anterior abdominal wall',
     'ventral hernia',
     'injury of lower leg',
     'dry skin',
     'anhydrotic skin',
     'drug reaction',
     'adverse reaction to drug',
     'adverse drug effect',
     'adverse drug reaction',
     'ADr',
     'ulcer of foot',
     'foot ulcer',
     'impacted tooth',
     'impacted tooth - disorder',
     'impacted teeth',
     'acquired trigger finger',
     'nodular tendinous disease of finger',
     'triggering of finger',
     'trigger finger',
     'chronic osteoarthritis',
     'arthritis sicca',
     'arthroxerosis',
     'acute stress disorder',
     'gross stress reaction',
     'acute crisis reaction',
     'acute stress reaction',
     'psychic shock',
     'acute reaction to stress',
     'history of malignant neoplasm of bladder',
     'h/o: malignant neoplasm of bladder',
     'personal history of primary malignant neoplasm of urinary bladder',
     'peripheral circulatory disorder associated with type 2 diabetes mellitus',
     'peripheral circulatory disorder associated with type ii diabetes mellitus',
     'abdominal hysterectomy',
     'respiratory failure',
     'allergic disposition',
     'allergy',
     'allergies',
     'active advance directive (copy within chart)',
     'advance directive signed (copy in notes)',
     'active advanced healthcare directive (copy within chart)',
     'stress',
     'stress - value',
     'state of stress',
     'psychological stress',
     'stressed',
     'stresses',
     'stressing',
     'impaired glucose tolerance in pregnancy',
     'pregnancy and impaired glucose tolerance',
     'chest wall pain',
     'recurrent urinary tract infection',
     'recurrent uti - urinary tract infection',
     'degeneration of lumbosacral intervertebral disc',
     'astigmatism',
     'ganglion cyst',
     'myxoid cyst',
     'ganglion',
     'benign cystic mucinous tumor',
     'benign cystic mucinous tumour',
     'ganglion of joint',
     'mucous cyst of skin',
     'ganglion cyst of joint',
     'sick sinus syndrome',
     'transplantation of liver',
     'tx - liver transplantation',
     'hepatic transplantation',
     'ltx - liver transplant',
     'liver transplantation procedures',
     'cocaine abuse',
     'intermittent asthma',
     'intermittent asthma (asthma)',
     'cervical radiculopathy',
     'cervical root neuropathy',
     'cervical root lesion',
     'diabetes mellitus screening',
     'coronary atherosclerosis',
     'atherosclerosis of coronary artery',
     'methicillin resistant staphylococcus aureus infection',
     'infection due to methicillin resistant staphylococcus aureus',
     'mrsa (methicillin resistant staphylococcus aureus) infection',
     'biopsy of breast',
     'inflammatory disease of female pelvic organs and/or tissues',
     'female pelvic inflammation',
     'pid - pelvic inflammatory disease',
     'female pelvic inflammatory disease',
     'pelvic inflammatory disease',
     'pain in eye',
     'ocular pain',
     'eye pain',
     'exercise-induced asthma',
     'EIa',
     'exercise induced asthma',
     'prematurity of fetus',
     'prematurity of foetus',
     'preterm infant',
     'prematurity',
     'postoperative care',
     'postmature infancy',
     'postterm infancy',
     'postmature foetus',
     'postmature fetus',
     'postmature baby',
     'post-term infant',
     'postmaturity of baby',
     'temporomandibular joint-pain-dysfunction syndrome',
     'tmj syndrome',
     'costens syndrome',
     'mandibular dysfunction',
     'costens complex',
     'temporomandibular joint pain dysfunction syndrome',
     'snapping jaw',
     'TMJPDs',
     'myofascial pain - dysfunction syndrome of tmj',
     'temporomandibular joint disorder',
     'TMj',
     'temporomandibular joint syndrome',
     'fetal distress affecting management of mother',
     'abnormal fetal heart rate and/or rhythm affecting management of mother',
     'foetal distress affecting management of mother',
     'abnormal foetal heart rate and/or rhythm affecting management of mother',
     'nocturia',
     'adjustment disorder with depressed mood',
     'infestation by sarcoptes scabiei var hominis',
     'scabies',
     'sarcoptic itch',
     'scabbies',
     'visual impairment',
     'visual difficulty',
     'impaired vision',
     'abnormal vision',
     'sight impaired',
     'difficulty seeing',
     'partial sight',
     'problem sight',
     'problem seeing',
     'reduced ability to see',
     'poor vision',
     'ulcer of lower extremity',
     'ulcer of lower limb',
     'leg ulcer',
     'primary testicular failure',
     'primary failure of the testes',
     'male hypogonadism',
     'eunuchoidism, hypogonadism',
     'eunuchoidism hypogonadism',
     'primary gonadal failure',
     'testicular hypogonadism',
     'eunuchoidism',
     'testicular failure',
     'primary male hypogonadism',
     'closed intertrochanteric fracture',
     'carcinoma of bladder',
     'pain in limb',
     'limb pain',
     'family history of cancer of colon',
     'complication occurring during pregnancy',
     'history of malignant neoplasm of colon',
     'fracture of ankle',
     'fracture of distal end of tibia and fibula',
     'ankle fracture',
     'injury of hand',
     'hand injury',
     'postoperative wound infection',
     'infected surgical wound',
     'closed transcervical fracture of femur',
     'primary dilated cardiomyopathy',
     'dilated cardiomyopathy',
     'congestive cardiomyopathy',
     'CCm',
     'COCm',
     'DCm',
     'congestive dilated cardiomyopathy',
     'proliferative diabetic retinopathy',
     'PDr',
     'cardiac catheterization',
     'CCa',
     'cardiac catheterisation',
     'cardiac catheterisation procedure',
     'cardiac catheterization procedure',
     'insertion of catheter into heart chamber',
     'cardiac catheterization procedures',
     'major depressive disorder',
     'major depression',
     'fall on same level from slipping',
     'chronic bronchitis',
     'advanced maternal age gravida',
     'ama - advanced maternal age',
     'advanced maternal age patient',
     'recurrent major depression in complete remission',
     'pregnancy-induced hypertension',
     'Gh',
     'gestational hypertension',
     'hypertension induced by pregnancy',
     'hypertension complicating pregnancy',
     'PIh',
     'epididymitis',
     'impetigo',
     'impetigo contagiosa',
     'impitigo',
     'impertigo',
     'schizoaffective schizophrenia',
     'cyclic schizophrenia',
     'schizoaffective disorder',
     'history of cerebrovascular accident',
     'h/o: stroke',
     'h/o: cva',
     'mild recurrent major depression',
     'cerebral palsy',
     'cerebral palsy (cp)',
     'congenital cerebral palsy',
     'infantile cerebral palsy',
     'Cp',
     'implantation of cardiac pacemaker',
     'implantation of heart pacemaker',
     'carotid artery stenosis',
     'carotid artery narrowing',
     'squamous cell carcinoma of skin',
     'scc - cutaneous squamous cell carcinoma',
     'cutaneous squamous cell carcinoma',
     'SCc',
     'spinous cell carcinoma',
     'hypogonadism',
     'trochanteric bursitis',
     'whirlbone lameness',
     'graves disease',
     'exophthalmic goiter',
     'exophthalmic goitre',
     'basedows disease',
     'basedow disease',
     'graves disease',
     'toxic diffuse goiter with exophthalmos',
     'graves disease with exophthalmos',
     'malignant neoplastic disease',
     'cancer',
     'malignant neoplasm',
     'malignant tumour',
     'malignant tumor',
     'cancers',
     'moderate persistent asthma',
     'foetal distress, in liveborn infant',
     'fetal intrauterine distress, not clear if noted before or after onset of labor in liveborn infant',
     'foetal intrauterine distress, not clear if noted before or after onset of labour in liveborn infant',
     'fetal intrauterine distress, not clear if noted before or after onset of labour in liveborn infant',
     'obsessive-compulsive disorder',
     'obsessive compulsive disorder',
     'obsessive compulsive neurosis',
     'anancastic neurosis',
     'anankastic neurosis',
     'OCd',
     'pain in female pelvis',
     'mood disorder',
     'affective disorder',
     'disorder of affect',
     'degeneration of cervical intervertebral disc',
     'corneal abrasion',
     'scratch of cornea',
     'scratch to cornea',
     'ulcer of anus',
     'solitary ulcer of anus',
     'anal ulcer',
     'anal fissure',
     'fissure in ano',
     'nontraumatic tear of anus',
     'heart failure',
     'Hf',
     'myocardial failure',
     'weak heart',
     'cardiac failure',
     'cardiac insufficiency',
     'adjustment disorder with mixed emotional features',
     'adjustment reaction with mixed disturbance of emotion',
     'febrile convulsion',
     'pyrexial convulsion',
     'febrile fit',
     'febrile seizure (from fever)',
     'febrile seizure',
     'fever seizure',
     'degenerative joint disease of hand',
     'osteoarthritis - hand joint',
     'osteoarthrosis of hand',
     'chronic type b viral hepatitis',
     'chronic viral hepatitis b',
     'meconium in amniotic fluid affecting management of mother',
     'intermittent claudication',
     'charcots syndrome',
     'myasthenia angiosclerotica',
     'blepharitis',
     'eyelid inflammation',
     'inflammation of lid margin',
     'inflammation of eyelid',
     'stie',
     'cyst',
     'cists',
     'cysts',
     'heartburn',
     'pyrosis',
     'heartburn symptom',
     'burning reflux',
     'heart burn',
     'heartburns',
     'h/o malignant melanoma',
     'history of malignant melanoma',
     'mental retardation',
     'exudative age-related macular degeneration',
     'disciform macular degeneration',
     'disciform senile macular retinal degeneration',
     'exudative senile macular retinal degeneration',
     'junius-kuhnt degeneration',
     'kuhnt junius degeneration',
     'neovascular age-related macular degeneration',
     'wet senile macular degeneration',
     'subretinal neovascularization of macula',
     'subretinal neovascularisation of macula',
     'EMd',
     'exudative macular degeneration',
     'family history of malignant neoplasm of gastrointestinal tract',
     'family history of malignant neoplasm of digestive organ',
     'hypoxia',
     'decreased oxygen supply',
     'hypoxic',
     'influenza',
     'flu',
     'grippe',
     'influenzae',
     'stable angina',
     'cataract extraction and insertion of intraocular lens',
     'iol - cataract extraction and insertion of intraocular lens',
     'CEIOl',
     'bunion',
     'bunion of great toe',
     'swelling of first metatarsophalangeal joint of hallux',
     'bunions',
     'genuine stress incontinence',
     'stress urinary incontinence',
     'stress bladder incontinence',
     'incontinence when straining',
     'Si',
     'stress incontinence',
     'urinary stress incontinence',
     'GSi',
     'opioid dependence',
     'narcotism',
     'antepartum hemorrhage',
     'APh',
     'antepartum haemorrhage',
     'pneumothorax',
     'kidney disease',
     'disorder of kidney',
     'renal disorder',
     'nephropathy',
     'renal disease',
     'nephrosis',
     'disease of kidney',
     'atopic conjunctivitis',
     'allergic conjunctivitis',
     'sprain of knee',
     'sprain of knee joint',
     'sprain of ligament of knee joint',
     'knee sprain',
     'influenza due to influenza a virus',
     'cystitis',
     'bladder infection',
     'cistitis',
     'cystisis',
     'cystitus',
     'stricture of oesophagus',
     'esophagostenosis',
     'oesophagostenosis',
     'stenosis of esophagus',
     'stenosis of oesophagus',
     'oesophageal stricture',
     'esophageal stricture',
     'moderate major depression, single episode',
     'single major depressive episode, moderate',
     'screening for malignant neoplasm of breast',
     'breast cancer screening',
     'screening for breast cancer',
     'breast neoplasm screening',
     'cannabis abuse',
     'conductive hearing loss',
     'CHl',
     'conductive deafness',
     'neutropenic disorder',
     'injury of finger',
     'finger injury',
     'lower urinary tract symptoms',
     'LUTs',
     'unconscious',
     'mental status, unconsciousness',
     'unconsciousness',
     'loss of consciousness',
     'concusion',
     'intestinal obstruction',
     'intestinal occlusion',
     'bowel obstruction',
     'obstruction of intestine',
     'Io',
     'lymphadenopathy',
     'La',
     'Ln',
     'swelling of lymph node',
     'swollen lymph glands',
     'swollen lymph nodes',
     'lymphadenopathy - swelling',
     'enlargement of lymph nodes',
     'swelling of lymph nodes',
     'lymphadenopathies',
     'adenopathy',
     'candidiasis of vagina',
     'vaginal candidiasis',
     'vaginal thrush',
     'vaginal candidosis',
     'vaginal candida',
     'monilial infection of vagina',
     'candida of vagina',
     'pelvic mass',
     'pelvic lump',
     'mass of pelvic structure',
     'endometrial carcinoma',
     'ankle joint pain',
     'arthralgia of ankle',
     'ankle pain',
     'jaundice',
     'icteric',
     'jaundiced',
     'icterus',
     'severe recurrent major depression without psychotic features',
     'inflammation of cervix',
     'trachelitis',
     'cervicitis',
     'inflammatory disease of the cervix',
     'sickle cell trait',
     'as - sickle cell trait',
     'heterozygous for hb s',
     'drepanocytosis',
     'rbcs - sickle cells present',
     'sickle cells present',
     'hemoglobin s-a disorder',
     'hemoglobin a-s genotype',
     'hemoglobin s trait',
     'heterozygous hemoglobin s',
     'haemoglobin a-s genotype',
     'haemoglobin s-a disorder',
     'haemoglobin s trait',
     'heterozygous haemoglobin s',
     'squamous cell carcinoma',
     'squamous cell carcinoma, no icd-o subtype',
     'squamous cell carcinoma, no international classification of diseases for oncology subtype',
     'squamous carcinoma',
     'squamous cell epithelioma',
     'epidermoid carcinoma',
     'arthralgia of the lower leg',
     'small bowel obstruction',
     'SBo',
     'dermatophytosis',
     'tinea',
     'ringworm',
     'microsporic tinea',
     'compression fracture',
     'menopausal flushing',
     'hot flush',
     'hot flashes',
     'hot flushes',
     'menopausal hot flashes',
     'menopausal hot flushes',
     'climacteric flushing',
     'developmental delay',
     'pernicious anaemia',
     'addisonian pernicious anemia',
     'megaloblastic anemia due to impaired absorption of cobalamin',
     'addisons anemia',
     'biermers anemia',
     'addisonian pernicious anaemia',
     'biermers anaemia',
     'megaloblastic anaemia due to impaired absorption of cobalamin',
     'Pa',
     'drug abuse',
     'medication abuse',
     'left lower quadrant pain',
     'claudication',
     'claudications',
     'external hordeolum',
     'stye',
     'boil of eyelid',
     'sty',
     'furuncle of eyelid',
     'hordeolum, external',
     'hordeolum externum',
     'hordeolum',
     'styes',
     'sties',
     'furuncle',
     'boil',
     'furunculus',
     'pus lump',
     'furuncles',
     'boils',
     'haemopoietic stem cell transplant',
     'transfusion of stem cells',
     'disorder of lung',
     'lung disorder',
     'pulmonary disease',
     'migraine without aura',
     'common migraine',
     'atypical migraine',
     'disorder of refraction',
     'ametropia',
     'refractive error',
     'acute appendicitis',
     'oedema of foot',
     'pedal edema',
     'pedal oedema',
     'foot oedema',
     'foot edema',
     'foot swelling',
     'swollen foot',
     'swollen feet',
     'pulled back muscle',
     'strain of back muscle',
     'amyloidosis',
     'amyloidoses',
     'hernia repair',
     'herniorrhaphy',
     'herniorrhaphies',
     'hodgkins disease (clinical)',
     'Hd',
     'hodgkins disease',
     'malignant hodgkins lymphoma',
     'hodgkin disease',
     'lymphoma, hodgkins',
     'hodgkins granuloma (clinical)',
     'hodgkins granuloma',
     'hodgkin granuloma',
     'hodgkin lymphoma, nodular lymphocyte predominance (clinical)',
     'hodgkins paragranuloma (clinical)',
     'hodgkin lymphoma, nodular lymphocyte predominance',
     'hodgkins sarcoma (clinical)',
     'hodgkins sarcoma',
     'hodgkin sarcoma',
     'hodgkin lymphoma',
     'malignant lymphoma, hodgkins',
     'hodgkin lymphoma, no icd-o subtype',
     'hodgkin lymphoma, no international classification of diseases for oncology subtype',
     'hodgkin sarcoma [obs]',
     'hodgkins disease, lymphocytic predominance, nodular',
     'hodgkin paragranuloma, nodular [obs]',
     'hodgkin paragranuloma [obs]',
     'hodgkin lymphoma, lymphocyte predominance, nodular',
     'hodgkin granuloma [obs]',
     'primary malignant neoplasm of liver',
     'ca liver - primary',
     'hydrocele of testis',
     'hydrocele',
     'hydrocoele',
     'hydroceles',
     'bilateral cataracts',
     'cataracts',
     'pancytopenia',
     'bone marrow failure',
     'pain in elbow',
     'elbow pain',
     'eating disorder',
     'cervical radiculitis',
     'radicular syndrome of upper limbs',
     'brachial (cervical) neuritis',
     'cervical neuritis',
     'brachial radiculitis',
     'pneumococcal vaccination',
     'carcinoma of prostate',
     'prostate cancer',
     'prostate carcinoma',
     'glaucoma suspect',
     'chronic cough',
     'open-angle glaucoma - borderline',
     'secondary malignant neoplasm of brain',
     'secondary cancer of brain',
     'metastasis to brain',
     'metastatic malignant neoplasm to brain',
     'cancer metastatic to brain',
     'extraction of cataract',
     'Ce',
     'cataract extraction',
     'excision of cataract',
     'late effects of respiratory tuberculosis',
     'cosmetic surgery',
     'alcohol intoxication',
     'acute alcoholism',
     'drunkenness',
     'acute alcohol intoxication',
     'drunk',
     'abrasion',
     'abrasions',
     'positions of breech presentation',
     'malposition of fetus in breech presentation',
     'breech presentation',
     'premature labour',
     'premature onset of labour',
     'preterm labour',
     'preterm labor',
     'premature onset of labor',
     'fh: cardiovascular disease',
     'family history: cardiovascular disease',
     'fh: cvs disorder',
     'family history of cardiovascular disease',
     'raynauds phenomenon',
     'raynaud phenomenon',
     'paroxysmal digital cyanosis',
     'raynards phenomenon',
     'history of fall',
     'posterior rhinorrhoea',
     'postnasal drip',
     'discharge from back of nose',
     'PNd',
     'postnasal catarrh',
     'discharge from nasopharynx',
     'acute gastritis',
     'nasal polyp',
     'polyp in anterior nares',
     'nasal polyp - anterior',
     'nasal polyposis',
     'polyp of nasal cavity',
     'essential tremor',
     'history of myocardial infarction',
     'past history of myocardial infarction',
     'replacement of total knee joint',
     'candidiasis',
     'infection by candida species',
     'moniliasis',
     'candidosis',
     'monilia infection',
     'candida infection',
     'thrush',
     'candidiasis of mouth',
     'mycotic stomatitis',
     'oral candidiasis',
     'candida of mouth',
     'oral thrush',
     'oral moniliasis',
     'oral candidosis',
     'candidosis of mouth',
     'candida infection of mouth',
     'moniliasis of mouth',
     'esophageal candidiasis',
     'frush',
     'ocular hypertension',
     'Oh',
     'OHt',
     'aortic valve disorder',
     'aortic valve disease',
     'AVd',
     'oedema of lower extremity',
     'oedema of lower limb',
     'edema of lower limb',
     'internal hemorrhoids without complication',
     'urine induced contact dermatitis',
     'nappy rash',
     'diaper dermatitis',
     'diaper erythema',
     'ammonia dermatitis',
     'jacquets dermatitis',
     'jacquets erythema',
     'urine-induced contact dermatitis',
     'ammoniacal napkin dermatitis',
     'nappy rash - irritant',
     'irritant napkin dermatitis',
     'cholangitis',
     'pregnancy-related examination',
     'persistent microalbuminuria associated with type 2 diabetes mellitus',
     'persistent microalbuminuria associated with type ii diabetes mellitus',
     'primary malignant neoplasm of lung',
     'subacromial impingement',
     'impingement syndrome of shoulder region',
     'impingement syndrome of shoulder',
     'shoulder impingement syndrome',
     'idiopathic urticaria',
     'idiopathic angio-oedema-urticaria',
     'idiopathic angioedema-urticaria',
     'screening for malignant neoplasm of prostate',
     'arthritis of knee',
     'ultrasound scan for fetal anomaly',
     'ultrasound scan for foetal anomaly',
     'common duodenal ulcer',
     'duodenal ulcer disease',
     'duodenal ulcer',
     'Du',
     'DUd',
     'surgery',
     'surgical procedure',
     'operation',
     'surgical procedures',
     'operative procedure',
     'surgical',
     'surgeries',
     'operations',
     'diabetic peripheral neuropathy',
     'hypervolaemia',
     'volume excess',
     'fluid volume excess',
     'fluid overload',
     'fluid excess',
     'cholecystitis',
     'inflamed gallbladder',
     'gynaecomastia',
     'gynaecomazia',
     'gynecomazia',
     'hypertrophy of male breast',
     'gynecomastia',
     'paronychia',
     'perionychia',
     'acute pyelonephritis',
     'acute pn - pyelonephritis',
     'APn',
     'acute kidney infection',
     'musculoskeletal chest pain',
     'gunshot wound',
     'gsw (gun shot wound)',
     'gun shot wound',
     'carcinoma of upper lobe, bronchus or lung',
     'melaena',
     'altered blood passed per rectum',
     'altered blood in stool',
     'black, tarry stool',
     'tarry stools',
     'lipoma of skin',
     'history of percutaneous transluminal coronary angioplasty',
     'history of percutaneous transluminal angioplasty for coronary artery disease',
     'occipitoanterior position',
     'menometrorrhagia',
     'metromenorrhagia',
     'intermenstrual heavy bleeding',
     'coin lesion of lung',
     'oedema of lower leg',
     'persistent proteinuria associated with type 2 diabetes mellitus',
     'persistent proteinuria associated with type ii diabetes mellitus',
     'dog bite',
     'cigarette smoker',
     'cocaine type drug dependence',
     'cocaine dependence',
     'vasovagal attack',
     'vagal attack',
     'gowers syndrome',
     'vasovagal syncope',
     'vaso vagal episode',
     'vasodepressor syncope',
     'abdominal aortic aneurysm without rupture',
     'right bundle branch block',
     'right fascicular block',
     'RBBb',
     'alcoholic cirrhosis',
     'alcoholic liver cirrhosis',
     'alcoholic cirrhosis of liver',
     'fibrosis of lung',
     'pulmonary fibrosis',
     'cirrhosis of lung',
     'neoplasm of uncertain behaviour of skin',
     'malignant melanoma of skin',
     'cutaneous malignant melanoma',
     'melanoma of skin',
     'urgent desire to urinate',
     'urinary precipitancy',
     'urgency of micturition',
     'urgency - urination',
     'precipitancy of micturition',
     'must hurry to pass urine',
     'precipitancy of urine',
     'urgency to micturate',
     'urging to urinate',
     'urgency to pass urine',
     'bursitis of hip',
     'chronic alcoholism in remission',
     'fiberoptic esophagogastroduodenoscopy',
     'oesophagogastroduodenoscopy',
     'upper gastrointestinal endoscopy',
     'upper gi endoscopy',
     'esophagogastroduodenoscopy procedures',
     'esophagogastroduodenoscopy',
     'recurrent pancreatitis',
     'relapsing pancreatitis',
     'chronic pancreatitis',
     'gastric atonia',
     'gastroparesis syndrome',
     'gastric atony',
     'gastric stasis',
     'gastroparesis',
     'ectopic pregnancy',
     'eptopic pregnancy',
     'etopic pregnancy',
     'eccyesis',
     'muscle weakness',
     'muscle strength reduced',
     'decreased muscle strength',
     'herpes labialis',
     'cold sores',
     'fever blister',
     'herpes simplex labialis',
     'cold sore',
     'coldsore',
     'hospice care',
     'hashimoto thyroiditis',
     'autoimmune lymphocytic chronic thyroiditis',
     'lymphocytic thyroiditis',
     'struma lymphomatosa',
     'hashimotos disease',
     'struma lymphomatosis',
     'chronic lymphocytic thyroiditis',
     'hashimotos thyroiditis',
     'tight foreskin',
     'tight prepuce',
     'phimosis',
     'pain in toe',
     'toe pain',
     'pulmonary tuberculosis',
     'PTb',
     'tb - pulmonary tuberculosis',
     'respiratory tuberculosis',
     'tuberculosis of respiratory system',
     'vaginal show',
     'celiac disease',
     'coeliac disease',
     'coeliac sprue',
     'idiopathic steatorrhoea',
     'coeliac syndrome',
     'idiopathic steatorrhea',
     'celiac sprue',
     'celiac syndrome',
     'wheat-sensitive enteropathy',
     'gluten enteropathy',
     'gluten-responsive sprue',
     'GSe',
     'gluten-sensitive enteropathy',
     'non-tropical sprue',
     'gluten-induced enteropathy syndrome',
     'nontropical sprue',
     'cramp in lower leg',
     'h/o: alcoholism',
     'history of alcoholism',
     'secondary malignant neoplasm of pleura',
     'secondary malignant tumour of pleura',
     'secondary malignant tumor of pleura',
     'pleural seedling',
     'metastasis to pleura',
     'metastatic malignant neoplasm to pleura',
     'cancer metastatic to pleura',
     'fracture of hand',
     'fracture of hand bone',
     'fracture of bone of hand',
     'cyst of breast',
     'nephrotic syndrome',
     'Ns',
     'drug overdose',
     'Od',
     'overdose of drug',
     'overdose',
     'overdosed',
     'overdoses',
     'overdosing',
     'cirrhosis - non-alcoholic',
     'cirrhosis, nonalcoholic',
     'cirrhosis of liver not due to alcohol',
     'chondromalacia of patella',
     'chondromalacia patella',
     'degeneration of articular cartilage of patella',
     'chondromalacia patellae',
     'spinal stenosis in cervical region',
     'cervical spinal stenosis',
     'current drinker',
     'alcohol user',
     'current drinker of alcohol',
     'drinks alcohol',
     'strain of neck muscle',
     'cervical strain',
     'carcinoma of colon',
     'vitiligo',
     'cystocele',
     'vaginal cystocele',
     'female cystocele',
     'bladder cystocele',
     'cystoceles',
     'dysphasia',
     'chronic abdominal pain',
     'recurrent major depression in partial remission',
     'disease due to trichomonadidae',
     'infection by trichomonas',
     'trichomonas infection',
     'trichomoniasis',
     'trichomonosis',
     'osteomyelitis',
     'OSTm',
     'pyogenic inflammation of bone',
     'mass of neck',
     'lump on neck',
     'fall from one level to another',
     'arterial, arteriole and capillary disease',
     'history and physical examination, school',
     'school child health examination',
     'complete miscarriage',
     'complete spontaneous abortion',
     'vasectomy',
     'male surgical sterilisation procedure',
     'male surgical sterilization procedure',
     'contraceptive method vasectomy',
     'vasectomy for contraception',
     'vasectomy, unilateral or bilateral (separate procedure), including postoperative semen examination(s)',
     'gastric ulcer',
     'stomach ulcer',
     'Gu',
     'gastric ulceration',
     'numbness',
     'loss of sensation',
     'deadness - numbness',
     'fetal or neonatal effect of breech delivery and extraction',
     'foetal or neonatal effect of breech delivery and extraction',
     'cyst - pilonidal',
     'pilonidal sinus',
     'coccygeal fistula',
     'coccygeal sinus',
     'pilonidal fistula',
     'piliferous cyst',
     'transplantation of heart',
     'heart transplant',
     'ctx - cardiac transplant',
     'httx - heart transplant',
     'htx - heart transplant',
     'heart transplantation',
     'pneumonitis due to inhalation of food or vomitus',
     'varicose veins of the leg with eczema',
     'varicose eczema leg',
     'stasis dermatitis',
     'gravitational eczema',
     'stasis eczema',
     'varicose eczema',
     'varicose dermatitis',
     'venous eczema',
     'hypoxemia',
     'arterial hypoxaemia',
     'arterial hypoxemia',
     'paraplegia',
     'paralysis of both lower limbs',
     'lower paraplegia',
     'paraplegia (complete or partial paralysis of legs)',
     'paraplegias',
     'tear of tympanic membrane',
     'perforation of tympanic membrane',
     'perforation of ear drum',
     'perforation of eardrum',
     'perforated eardrum',
     'perforation tympanic membrane',
     'ventricular tachycardia',
     'Vt',
     'ventricular tachyarrhythmia',
     'mixed urinary incontinence',
     'mixed incontinence',
     'hypertensive renal disease with renal failure',
     'diabetic oculopathy associated with type 2 diabetes mellitus',
     'diabetic oculopathy associated with type ii diabetes mellitus',
     'trigeminal neuralgia',
     'Tn',
     'tic douloureux',
     'trifacial neuralgia',
     'fothergills neuralgia',
     'retinal detachment',
     'separation of retinal layers',
     'sensory retinal detachment',
     'Rd',
     'detached retina',
     'leucopenia',
     'leukocytopenia',
     'leucocytopenia',
     'leukopenia',
     'vitreous hemorrhage',
     'intragel vitreous haemorrhage',
     'intragel vitreous hemorrhage',
     'Vh',
     'vitreous haemorrhage',
     'fetal growth restriction',
     'IUGr',
     'intrauterine growth retardation',
     'poor fetal growth',
     'FGr',
     'fetal growth retardation',
     'poor foetal growth state',
     'poor foetal growth',
     'foetal growth retardation',
     'poor fetal growth state',
     'microsomic baby',
     'microsomia',
     'muscular headache',
     'muscle contraction headache',
     'history of cerebrovascular accident without residual deficits',
     'intramural leiomyoma of uterus',
     'interstitial uterine fibroid',
     'intramural uterine leiomyoma',
     'intramural uterine fibroid',
     'viral hepatitis, type a',
     'infectious hepatitis',
     'hepatitis a',
     'retinal disorder',
     'retinal disease',
     'retinopathy',
     'retinopathies',
     'tinea capitis',
     'ringworm of the scalp',
     'black dot ringworm',
     'dermatophytosis of scalp',
     'tinea of scalp',
     'asymptomatic coronary heart disease',
     'ménières disease',
     'aural vertigo',
     'auditory vertigo',
     'ménières syndrome',
     'ménières vertigo',
     'menieres disease',
     'otogenic vertigo',
     'meniere syndrome',
     'meniere disease',
     'meniere disorder',
     'fracture of phalanx of finger',
     'fracture of finger',
     'finger fracture',
     'fracture of phalanx of hand',
     'oophorectomy',
     'excision of ovary',
     'oophorectomy, partial or total, unilateral or bilateral; for ovarian, tubal or primary peritoneal malignancy, with para-aortic and pelvic lymph node biopsies, peritoneal washings, peritoneal biopsies, diaphragmatic assessments, with or without salpingectomy(s), with or without omentectomy',
     'oophorectomies',
     'muscle atrophy',
     'muscle wasting disorder',
     'muscle thinning',
     'amyotrophia',
     'amyotrophy',
     'muscular atrophy',
     'muscle wasting',
     'incontinence of feces',
     'bowel incontinence',
     'involuntary stool',
     'bowels: incontinent',
     'incontinent of feces',
     'incontinent of faeces',
     'mitral valve disorder',
     'mitral valve disease',
     'MVd',
     'anaemia due to chronic blood loss',
     'chronic blood loss anemia',
     'chronic hemorrhagic anemia',
     'iron deficiency anemia due to chronic blood loss',
     'chronic blood loss anaemia',
     'atherosclerosis of arteries of the extremities',
     'insulin treated type 2 diabetes mellitus',
     'insulin treated type ii diabetes mellitus',
     'diabetes type 2 on insulin',
     'disorder of joint of spine',
     'spondyloarthropathy',
     'spondylosis without myelopathy',
     'spinal osteoarthritis',
     'spondylosis',
     'spondylarthrosis',
     'spondarthropathy',
     'osteoarthritis of spine',
     'osteoarthritis of the spine',
     'tuberculosis of lung, confirmed by sputum microscopy with or without culture',
     'pterygium',
     'web eye',
     'h/o: peptic ulcer',
     'history of peptic ulcer',
     'ulnar neuropathy',
     'lung mass',
     'trichomonal vaginitis',
     'vaginal trichomoniasis',
     'trichomonal leukorrhoea vaginalis',
     'trichomonal leukorrhea vaginalis',
     'trichomonal fluor vaginalis',
     'foreign body in respiratory tract',
     'bypass gastroenterostomy',
     'gastroenterostomy',
     'patellofemoral stress syndrome',
     'patellofemoral syndrome',
     'chronic kidney disease stage 4',
     'ckd stage 4',
     'ischaemic heart disease',
     'IHd',
     'ischemic heart disease',
     'myocardial ischaemia',
     'cardiac ischemia',
     'multiple nodules of lung',
     'multiple pulmonary nodules',
     'non-toxic multinodular goiter',
     'multinodular non-toxic goitre',
     'multinodular non-toxic goiter',
     'pain in finger',
     'finger pain',
     'cervical spondylosis without myelopathy',
     'cellulitis of lower leg',
     'body mass index 25-29 - overweight',
     'bmi 25-29 - overweight',
     'clouded consciousness',
     'confused',
     'muddled',
     'dullness of senses',
     'dazed',
     'confusion',
     'bewilderment',
     'dazed state',
     'confusions',
     'mixed conductive and sensorineural hearing loss',
     'mixed conductive and sensorineural deafness',
     'mixed hearing loss',
     'MHl',
     'mixed deafness',
     'mixed type deafness',
     'poly-drug misuser',
     'poly-drug abuser',
     'multi-drug misuser',
     'hyperuricemia',
     'uricacidaemia',
     'uricacidemia',
     'closed fracture of neck of femur',
     'blurring of visual image',
     'hazy vision',
     'foggy vision',
     'mist over eyes',
     'cloudy vision',
     'misty vision',
     'implantation of automatic cardioverter/defibrillator, total system (aicd)',
     'implantation of automatic cardioverter/defibrillator, total system',
     'bipolar ii disorder',
     'bipolar 2 disorder',
     'disturbance in sleep behavior',
     'implantation of aortic valve prosthesis or synthetic device',
     'sprain of wrist',
     'sprain of wrist joint',
     'wrist sprain',
     'dupuytrens disease of palm',
     'palmar fibromatosis',
     'palmar fascial fibromatosis',
     'dupuytren disease of palm',
     'dupuytrens disease of palm, with contracture',
     'dupuytren disease of palm, with contracture',
     'dupuytrens contracture',
     'dupuytren contracture',
     'dupuytrens contracture',
     'dupuytrens disease',
     'dupuytren disease',
     'contracture of palmar fascia',
     'personality disorder',
     'multiple joint pain',
     'arthralgia of multiple joints',
     'polyarthralgia',
     'external hemorrhoids',
     'external haemorrhoid',
     'external hemorrhoid',
     'panic attack',
     'mechanical low back pain',
     'patent foramen ovale',
     'PFo',
     'hyperprolactinaemia',
     'bronze diabetes',
     'bronzed diabetes',
     'haemochromatosis',
     'iron storage disease',
     'hemachromatosis',
     'hematochromatosis',
     'lumbosacral radiculopathy',
     'lumbosacral root lesion',
     'heart valve disorder',
     'valvular heart disease',
     'heart valve disease',
     'disorder of heart valve',
     'asystole',
     'cardiac standstill',
     'cardiac arrest',
     'molluscum contagiosum infection',
     'Mc',
     'molluscum contagiosum',
     'molluscum verrucosum',
     'chronic kidney disease stage 2',
     'ckd stage 2',
     'ischemic leg ulcer',
     'arterial leg ulcer',
     'secondary malignant neoplasm of peritoneum',
     'metastasis to peritoneum',
     'peritoneal seedling',
     'secondary malignant peritoneal deposit',
     'cancer metastatic to peritoneum',
     'metastatic malignant neoplasm to peritoneum',
     'thoracic back pain',
     'narcotic drug user',
     'allergy to penicillin',
     'penicillin allergy',
     'removal of staples',
     'diplopia',
     'seeing double',
     'double vision',
     'gammaherpesviral mononucleosis',
     'infectious mononucleosis',
     'pfeiffers disease',
     'infective mononucleosis',
     'glandular fever',
     'Gf',
     'monocytic angina',
     'Im',
     'gland fever',
     'ureteric stone',
     'calculus of ureter',
     'ureteral calculus',
     'ureteral stone',
     'ureteric calculus',
     'ureterolithiasis',
     'bronchospasm',
     'bronchial spasm',
     'bronchospasms',
     'postviral fatigue syndrome',
     'chronic fatigue syndrome',
     'myalgic encephalomyelitis syndrome',
     'Me',
     'myalgic encephalomyelitis',
     'CFs',
     'iceland disease',
     'benign myalgic encephalomyelitis',
     'paronychia of toe',
     'perionychia of toe',
     'medical assessment',
     'premature delivery',
     'premature pregnancy delivered',
     'premature childbirth',
     'premature birth of newborn',
     'neck sprain',
     'cervical sprain',
     'multinodular goiter',
     'thyroid goiter multinodular',
     'thyroid goitre multinodular',
     'multiple thyroid nodules',
     'ptosis of eyelid',
     'droopy eyelid',
     'blepharoptosis',
     'drooping eyelid',
     'ptosis',
     'ptosis eyelid',
     'environmental allergy',
     'failure to thrive in infant',
     'failure to thrive',
     'FTt',
     'wry neck/torticollis',
     'torticollis',
     'wry neck',
     'contracture of neck',
     'insertion of intrauterine contraceptive device',
     'insertion of intrauterine device',
     'insertion of iud',
     'fitting of coil',
     'iucd - fitting of intrauterine contraceptive device',
     'iud - fitting of intrauterine device',
     'introduction of intrauterine contraceptive device',
     'fitting of intrauterine device',
     'insertion of intrauterine device (iud)',
     'tuberculosis screening',
     'Tb',
     'history of malignant basal cell neoplasm of skin',
     'history of malignant basal cell tumor of skin',
     'history of malignant basal cell tumour of skin',
     'history of cataract extraction',
     'acute bronchiolitis',
     'capillary pneumonia',
     'acute capillary bronchiolitis',
     'common bile duct calculus',
     'calculus of common bile duct',
     'common bile duct stone',
     'viral disease characterized by exanthem',
     'nonspecific exanthematous viral infection',
     'nonspecific viral rash',
     'nonspecific viral exanthem',
     'viral exanthem',
     'viral rash',
     'viral exanthemata',
     'pes planus',
     'flat foot',
     'low medial arch of foot',
     'fallen arches',
     'fallen arch',
     'idiopathic peripheral neuropathy',
     'foreign body in pharynx',
     'pharyngeal fb - foreign body',
     'foreign body in throat',
     'keloid scar',
     'cheloid',
     'keloid cicatrix',
     'keloid',
     'cheloid of skin',
     'keloid of skin',
     'keloids',
     'thyroidectomy',
     'thyroid gland excision',
     'excision of thyroid gland',
     'thyroidectomy, removal of all remaining thyroid tissue following previous removal of a portion of thyroid',
     'thyroidectomies',
     'old anterior cruciate ligament disruption',
     'old disruption of anterior cruciate ligament',
     'contact dermatitis due to poison ivy',
     'allergic dermatitis due to rhus toxicodendron',
     'contact dermatitis due to rhus toxicodendron',
     'allergic dermatitis due to poison ivy',
     'contact dermatitis due to poison-ivy',
     'screening mammography',
     'mammography - screening',
     'mammogram - screening',
     'jaw pain',
     'pain of jaw',
     'renal impairment',
     'renal dysfunction',
     'impaired renal function',
     'ataxia',
     'ataxia (loss of muscle coordination)',
     'muscular incoordination',
     'dyssynergia',
     'ataxias',
     'insertion of intraocular lens',
     'insertion of prosthetic intraocular lens',
     'insertion of pseudophakos',
     'IOl',
     'ins intraocular lens',
     'intraocular lens implantation',
     'lens implantation',
     'implantation of intraocular lens',
     'insertion of lens prosthesis',
     'insertion of intraocular lens prosthesis (secondary implant), not associated with concurrent cataract removal',
     'age related macular degeneration',
     'AAMd',
     'age-related macular degeneration',
     'ARMd',
     'senile macular retinal degeneration',
     'SMd',
     'senile macular degeneration',
     'AMd',
     'multiple benign melanocytic nevi',
     'moles multiple benign',
     'uterine prolapse',
     'descensus uteri',
     'uterine hernia',
     'prolapse of uterus',
     'prolapsed uterus',
     'descens uteri',
     'renal mass',
     'kidney mass',
     'pneumonitis',
     'incoordination',
     'lack of coordination',
     'coordination problem',
     'abnormal coordination',
     'blindness - both eyes',
     'both eyes total visual impairment',
     'primary hyperparathyroidism',
     'musculoskeletal pain',
     'rheumatic pain',
     'mycosis',
     'mycotic disease',
     'mycoses',
     'fungal infection',
     'disease caused by fungus',
     'fungal infectious disease',
     'fungus infection',
     'primigravida',
     'primip',
     'first pregnancy',
     'previous pregnancy 1',
     'total abdominal hysterectomy with bilateral salpingo-oophorectomy',
     'tah bso - total abdominal hysterectomy and bilateral salpingo-oophorectomy',
     'bso - total abdominal hysterectomy and bilateral salpingo-oophorectomy',
     'total abdominal hysterectomy and bilateral salpingo-oophorectomy',
     'urethral stenosis',
     'urethral stricture',
     'leucocytosis',
     'leukocytosis',
     'ventricular premature beats',
     'ventricular ectopic beats',
     'ventricular premature contractions',
     'ventricular premature systoles',
     'ventricular extrasystoles',
     'ventricular premature complex',
     'premature ventricular complex',
     'ventricular ectopic complex',
     'ventricular premature depolarisation',
     'ventricular premature depolarization',
     'diabetic foot ulcer',
     'chronic headache disorder',
     'haemangioma',
     'haemangioma - morphology',
     'hemangioma - morphology',
     'hemangioma, no icd-o subtype',
     'hemangioma, no international classification of diseases for oncology subtype',
     'chorioangioma',
     'angioma',
     'chorangioma',
     'benign haemangioma',
     'hemangioma',
     'angiomas',
     'angiomata',
     'hemangiomas',
     'hemangiomata',
     'lymphedema',
     'lymphatic oedema',
     'lymphatic edema',
     'acquired lymphoedema',
     'lymphoedema',
     'acquired lymphedema',
     'postmenopausal state',
     'postmenopausal',
     'postmenopause',
     'chronic ulcer of skin',
     'chronic skin ulcer',
     'arthroscopic knee operation',
     'arthroscopic knee procedure',
     'arthroscopic surgical procedure on knee',
     'arthroscopy of knee',
     'arthroscopy of knee joint',
     'left heart failure',
     'LVf',
     'left ventricular failure',
     'left-sided heart failure',
     'excessive and frequent menstruation',
     'polymenorrhagia',
     'frequent heavy periods',
     'epimenorrhagia',
     'hyperpolymenorrhea',
     'hyperpolymenorrhoea',
     'housing lack',
     'homeless',
     'living on the street',
     'platelet count above reference range',
     'thrombocytosis',
     'increased platelets',
     'thrombocythaemia',
     'thrombocythemia',
     'disease of liver',
     'hepatopathy',
     'liver disease',
     'disorder of liver',
     'Ld',
     'disorder of carotid artery',
     'carotid artery disease',
     'disease of carotid artery',
     'left ventricular hypertrophy',
     'lv hypertrophy',
     'lv+ - left ventricular hypertrophy',
     'LVh',
     'altered bowel function',
     'altered bowel habits',
     'change in bowel habit',
     'altered bowel habit',
     'change in bowel pattern',
     'cellulitis and abscess of foot excluding toe',
     'nondependent alcohol abuse in remission',
     'at risk for falls',
     'at risk of falls',
     'fall risk',
     'streptococcus group b infection of the infant',
     'infantile group b strep infection',
     'concussion injury of brain',
     'brain concussion',
     'commotio cerebri',
     'MTBi',
     'mild traumatic brain injury',
     'concussions',
     'feeding problems in newborn',
     'feeding difficulties in newborn',
     'bipolar i disorder',
     'bipolar 1 disorder',
     'viral pharyngitis',
     'tooth eruption disorder',
     'tooth eruption disturbance',
     'anomaly of tooth eruption',
     'abnormal tooth eruption',
     'lower respiratory tract infection',
     'LRTi',
     'lower respiratory infection',
     'chest cold',
     'hydronephrosis',
     'hydronephroses',
     'borderline personality disorder',
     'oesophageal varices',
     'Ov',
     'oesophageal varix',
     'esophageal varix',
     'ov - esophageal varices',
     'cardiac pacemaker procedure',
     'generalized-onset seizures',
     'generalised convulsive epilepsy',
     'hypersomnia',
     'excessive sleep',
     'sleeps too much',
     'excessive sleepiness',
     'hypersomnia (excessive sleeping)',
     'sensorineural hearing loss, bilateral',
     'partial deafness',
     'scrotal varices',
     'scrotal varices - varicocele',
     'varicocele',
     'varicoceles',
     'subarachnoid haemorrhage',
     'SAh',
     'subarachnoid hemorrhage',
     'subarachnoid intracranial haemorrhage',
     'subarachnoid intracranial hemorrhage',
     'incisional hernia',
     'postoperative incisional hernia',
     'varicella',
     'varicella infection',
     'chicken pox',
     'chicken poxs',
     'chicken pocks',
     'chicken pocs',
     'malignant tumour of head and neck',
     'malignant tumour of head and/or neck',
     'malignant tumor of head and/or neck',
     'pain in testicle',
     'orchialgia',
     'orchidalgia',
     'orchidodynia',
     'pain of testes',
     'pain in testis',
     'transplant follow-up',
     'tinea cruris',
     'pain in female genitalia on intercourse',
     'female coitalgia',
     'laryngitis',
     'larnygitis',
     'hypertrophy of nail',
     'nail overgrowth',
     'Ox',
     'onychauxic',
     'onychauxis',
     'nail hypertrophy',
     'accidentally caught in or between objects',
     'accident - crushed by object',
     'crush accident',
     'crushed in between objects',
     'caught, crushed, jammed or pinched in or between objects',
     'amblyopia',
     'amblyopic',
     'splenectomy',
     'excision of spleen tissue',
     'polyp of cervix',
     'cervical polyp',
     'cystic disease of kidney',
     'cystic kidney disease',
     'hepatic encephalopathy',
     'hepatocerebral encephalopathy',
     'He',
     'portal systemic encephalopathy',
     'gaustads syndrome',
     'transient hepatargy syndrome',
     'disorder of skin',
     'dermatosis',
     'skin disorder',
     'dermatopathy',
     'dermatological disease',
     'dermatological disorder',
     'skin disease',
     'dermatoses',
     'blood glucose abnormal',
     'child attention deficit disorder',
     'serum cholesterol borderline high',
     'postherpetic neuralgia',
     'PHn',
     'post-herpetic neuralgia',
     'post-zoster neuralgia',
     'frank haematuria',
     'gross hematuria',
     'macroscopic hematuria',
     'macroscopic haematuria',
     'cramp',
     'muscle cramps',
     'cramp in muscle',
     'muscle cramp',
     'cramps',
     'interstitial lung disease',
     'diffuse parenchymal lung disease',
     'ILd',
     'replacement of aortic valve',
     'AVr',
     'aortic valve replacement',
     'excision and replacement of aortic valve',
     'replacement, aortic valve',
     'complete atrioventricular block',
     'third degree atrioventricular block',
     'third degree heart block',
     'complete heart block',
     'CHb',
     'malignant tumor of kidney',
     'renal malignant tumour',
     'renal malignant tumor',
     'ca - renal cancer',
     'cancer of kidney',
     'renal cancer',
     'skin nodule',
     'nodule of skin',
     'cutaneous nodule',
     'otitis',
     'inflammatory disorder of ear',
     'septic shock',
     'septicaemic shock',
     'septicemic shock',
     'sepsis-associated hypotension',
     'septicemia',
     'toxemia',
     'disorder of thyroid gland',
     'thyroid disorder',
     'thyroid disease',
     'hypertrophic cardiomyopathy',
     'HCm',
     'hypertrophic obstructive cardiomyopathy',
     'obstructive cardiomyopathy',
     'HOCm',
     'change of dressing',
     'dressing change',
     'reapplying a fresh dressing',
     'renewing dressing',
     'redressing wound',
     'repeating dressing',
     'legal blindness usa',
     'legal blindness (united states of america)',
     'infectious gastroenteritis',
     'septic gastroenteritis',
     'gastric flu',
     'h/o: hysterectomy',
     'history of hysterectomy',
     'occlusive disease of artery of lower extremity',
     'subdural hemorrhage',
     'subdural intracranial hemorrhage',
     'subdural intracranial haemorrhage',
     'subdural haematoma',
     'SDh',
     'subdural hematoma',
     'subdural intracranial haematoma',
     'subdural intracranial hematoma',
     'hepatitis b carrier',
     'speech problem',
     'manic bipolar i disorder',
     'bipolar i disorder, most recent episode manic',
     'h/o: deep vein thrombosis',
     'history of deep vein thrombosis',
     'secondary pulmonary hypertension',
     'h/o: thrombosis',
     'history of thrombosis',
     'vesicular eczema of hands and/or feet',
     'cheiropodopompholyx',
     'pompholyx',
     'pompholyx eczema',
     'vesicular eczema of hands and feet',
     'dyshidrotic dermatitis',
     'dyshidrotic eczema',
     'dyshidria',
     'gonorrhoea',
     'Gc',
     'gonococcus infection',
     'clap',
     'GCi',
     'gonococcal infection',
     'infection due to neisseria gonorrhoeae',
     'gonorhoea',
     'clapping',
     'gonorrhea',
     'gonococcal',
     'gonorrhoeal',
     'derangement of knee',
     'internal derangement of knee',
     'internal derangement of knee joint',
     'IDk',
     'appendicitis',
     'inflammed appendics',
     'apendicitis',
     'appendicitus',
     'diabetic polyneuropathy',
     'operative procedure on spinal structure',
     'operative procedure on spinal structures',
     'spinal operation',
     'operative procedure on vertebral column',
     'operative procedure on spine',
     'neonatal hypoglycemia',
     'prolonged rupture of membranes',
     'rupture of amniotic sac 24 or more hours before labor',
     'rupture of amniotic sac 24 or more hours before labour',
     'vasomotor rhinitis',
     'VMr',
     'monoclonal gammopathy (clinical)',
     'monoclonal gammopathy',
     'monoclonal gammopathy of uncertain significance',
     'monoclonal gammopathy of undetermined significance',
     'MGUs',
     'paraproteinaemia',
     'paraproteinemia',
     'benign monoclonal gammopathy',
     'asymptomatic monoclonal gammopathy',
     'renal disorder associated with type 1 diabetes mellitus',
     'renal disorder associated with type i diabetes mellitus',
     'kidney disorder associated with type 1 diabetes mellitus',
     'tuberculous',
     'infection due to mycobacterium tuberculosis',
     'tuberculosis',
     'tuberuclosis',
     'feeding problem',
     'chronic tonsillitis',
     'h/o: tuberculosis',
     'history of tuberculosis',
     'acute duodenal ulcer with haemorrhage',
     'regional lymph node metastasis present',
     'regional lymph node involvement present',
     'faecal occult blood: positive',
     'occult blood in stools',
     'occult blood in stool',
     'guaiac-positive stools',
     'hammer toe',
     'carcinoma of cervix',
     'malignant tumor of cervix',
     'cancer of cervix',
     'malignant neoplasm of cervix',
     'malignant neoplasm of cervix uteri',
     'cancer of the uterine cervix',
     'cervical cancer',
     'prolapsed lumbar intervertebral disc',
     'blood in vomit - symptom',
     'haematemesis',
     'vomiting of blood',
     'vomiting blood',
     'cholangiocarcinoma of biliary tract',
     'cholangiocarcinoma',
     'bile duct carcinoma',
     'bile duct adenocarcinoma',
     'cholangiocellular carcinoma',
     'perianal abscess',
     'alpha trait thalassaemia',
     'alpha thalassaemia trait',
     'alpha thalassemia trait',
     'bite of nonvenomous arthropod',
     'insect bites - nonvenomous',
     'superficial nonvenomous insect bite',
     'nonvenomous insect bite',
     'adult attention deficit hyperactivity disorder',
     'adult attention deficit disorder',
     'spondylolisthesis',
     'SPl',
     'malignant tumor of esophagus',
     'cancer of oesophagus',
     'oesophageal cancer',
     'malignant neoplasm of oesophagus',
     'cancer of esophagus',
     'esophageal cancer',
     'malignant neoplasm of esophagus',
     'esophageal tumour',
     'esophageal carcinoma',
     'esophageal neoplasm',
     'aphtha',
     'aphthae',
     'recurrent aphthous ulcer',
     'recurrent ulcerative stomatitis',
     'canker sores',
     'recurrent aphthous stomatitis',
     'recurrent aphthous ulceration',
     'aphthous ulcer of mouth',
     'oral aphthae',
     'aphthous stomatitis',
     'canker sore',
     'spasticity',
     'muscle spasticity',
     'muscle spasm - tone',
     'muscular spasticity',
     'adenocarcinoma of lung',
     'type 1 diabetes mellitus uncontrolled',
     'type i diabetes mellitus uncontrolled',
     'type i diabetes mellitus poorly controlled',
     'paresis of lower extremity',
     'muscle weakness of lower extremity',
     'incomplete miscarriage',
     'incomplete spontaneous abortion',
     'retained products after miscarriage',
     'retained tissue after pregnancy loss',
     'retained products after spontaneous abortion',
     'ventricular septal abnormality',
     'ventricular septal defect',
     'VSd',
     'interventricular septal defect',
     'neuropathic pain',
     'paroxysmal ventricular tachycardia',
     'fracture of distal end of radius and ulna',
     'fracture of lower end of both ulna and radius',
     'relationship problems',
     'relational problem',
     'oropharyngeal dysphagia',
     'transfer dysphagia',
     'acute gastric ulcer',
     'acute ulcer of stomach',
     'ankylosing spondylitis',
     'marie-strumpell spondylitis',
     'idiopathic ankylosing spondylitis',
     'marie strümpell spondylitis',
     'rheumatoid spondylitis',
     'spondylosis deformans',
     'bekhterevs disease',
     'rheumatoid arthritis of spine',
     'spondilitis',
     'injury of knee',
     'knee injury',
     'traumatic brain injury',
     'brain damage - traumatic',
     'cholesterol screening',
     'osteoarthritis of glenohumeral joint',
     'degenerative joint disease of glenohumeral joint',
     'degenerative joint disease of shoulder region',
     'osteoarthritis of shoulder',
     'osteoarthritis of shoulder joint',
     'osteoarthritis of shoulder region',
     'fetal or neonatal effect of maternal medical problem',
     'foetal or neonatal effect of maternal medical problem',
     'loin pain',
     'suicide by self-administered drug',
     'portal hypertension',
     'duodenitis',
     'non-toxic uninodular goitre',
     'non-toxic single thyroid nodule',
     'nontoxic uninodular thyroid goiter',
     'nontoxic uninodular thyroid goitre',
     'acute cholecystitis',
     'ureteric colic',
     'renal colic',
     'atrial arrhythmia',
     'mass of body structure',
     'observation of a mass',
     'lump',
     'mass',
     'localised mass',
     'localized mass',
     'lumping',
     'lumps',
     'masses',
     'nuclear cataract',
     'nuclear sclerosis',
     'cannabis dependence',
     'drug-induced hypoglycaemia',
     'chronic effect of ultraviolet radiation on normal skin (photo-aging)',
     'stab wound',
     'disorder of brain',
     'encephalopathy',
     'encephalopathies',
     'chronic urticaria',
     'recurrent periodic urticaria',
     'cystoid macular oedema',
     'cme - cystoid macular edema',
     'cmo - cystoid macular oedema',
     'corns and callus',
     'corns and callosities',
     'bony callus',
     'callus of bone',
     'asthma without status asthmaticus',
     'malocclusion of teeth',
     'malocclusion',
     'malocclusions',
     'chronic rheumatic heart disease',
     'chronic rheumatic carditis',
     'aortic aneurysm',
     'aneurysm of aorta',
     'anurism',
     'chalazion',
     'meibomian gland cyst',
     'tarsal cyst',
     'meibomian cyst',
     'allergic reaction to drug',
     'allergic drug reaction',
     'drug allergy',
     'allergy to drug',
     'medication allergy',
     'medicine allergy',
     'malignant tumor of nasopharynx',
     'malignant tumor of epipharynx',
     'malignant tumor of postnasal space',
     'cancer of nasopharynx',
     'malignant neoplasm of nasopharynx',
     'malignant tumour of epipharynx',
     'malignant tumour of postnasal space',
     'iud contraception',
     'contraceptive iud in situ',
     'iud in situ',
     'intrauterine device (iud) contraception',
     'intrauterine device contraception',
     'coil contraception',
     'h/o: musculoskeletal disease',
     'history of musculoskeletal disease',
     'history of musculoskeletal disorder',
     'respiratory tract infection',
     'RTi',
     'hemiplegia as late effect of cerebrovascular disease',
     'single liveborn born in hospital by cesarean section',
     'lower abdominal pain',
     'acute cardiac pulmonary edema',
     'acute cardiogenic pulmonary oedema',
     'acute cardiogenic pulmonary edema',
     'cardiogenic pulmonary oedema',
     'cardiogenic pulmonary edema',
     'chronic maxillary sinusitis',
     'chronic antritis',
     'cervical lymphadenopathy',
     'emphysematous bronchitis',
     'chronic obstructive bronchitis',
     'chronic bronchitis with emphysema',
     'bronchitis with airway obstruction',
     'COb',
     'obstructive chronic bronchitis',
     'bursitis of knee',
     'bursitis of knee region',
     'popliteal bursitis',
     'semimembranosus bursitis',
     'common cold',
     'acute coryza',
     'acute nasal catarrh',
     'acute rhinitis',
     'infective rhinitis',
     'acute nasopharyngitis',
     'infective nasopharyngitis',
     'head cold',
     'acute infective rhinitis',
     'cold',
     'colds',
     'administrative procedure',
     'administrative procedures',
     'psychosexual dysfunction',
     'lightheadedness',
     'light-headedness',
     'feels light headed',
     'lightheaded',
     'dizziness - light-headed',
     'adenomatous polyp of colon',
     'hypertensive renal disease',
     'hypertensive nephropathy',
     'diabetic ketoacidosis',
     'ketoacidosis in diabetes mellitus',
     'diabetes mellitus with ketoacidosis',
     'diabetic acidosis',
     'history of abnormal cervical papanicolaou smear',
     'polyp of corpus uteri',
     'intrauterine polyp',
     'endometrial polyp',
     'uterine polyp',
     'mild cognitive disorder',
     'mild cognitive impairment',
     'subjective tinnitus',
     'tinnitus aurium',
     'nondependent alcohol abuse, continuous',
     'foreign body accident - orifice',
     'carcinoma of lower lobe, bronchus or lung',
     'acute confusion',
     'delirium',
     'organic brain syndrome',
     'acute confusional state',
     'OBs',
     'acute psycho-organic syndrome',
     'acute organic reaction',
     'acute encephalopathy',
     'ABs',
     'acute brain syndrome',
     'delirious',
     'deliriums',
     'pityriasis rosea',
     'Pr',
     'pityriasis circinata et maculata',
     'cutis laxa',
     'systematised elastorrhexis',
     'systematized elastorrhexis',
     'systemic elastorrhexis',
     'generalised dermatochalasis',
     'generalised elastorrhexis',
     'generalised elastolysis',
     'generalized dermatochalasis',
     'generalized elastolysis',
     'generalized elastorrhexis',
     'primary elastolysis',
     'chalazodermia',
     'dermatomegaly',
     'dermatolysis',
     'inguinal pain',
     'groin pain',
     'arthralgia of the pelvic region and thigh',
     'oral contraception',
     'inhibition of ovulation',
     'oral contraceptive',
     'pill - oral contraception',
     'cephalopelvic disproportion',
     'disproportion of mixed maternal and foetal origin with normally formed foetus',
     'disproportion of mixed maternal and fetal origin with normally formed fetus',
     'CPd',
     'disproportion between fetal head and pelvis',
     'disproportion between fetus and pelvis',
     'disproportion between foetus and pelvis',
     'fetopelvic disproportion',
     'foetopelvic disproportion',
     'thyroiditis',
     'hemodialysis',
     'hemodialysis procedure',
     'haemodialysis',
     'haemodialysis procedure',
     'extracorporeal hemodialysis',
     'extracorporeal haemodialysis',
     'hemodialysis procedures',
     'hemodialyses',
     'fracture of rib',
     'rib fracture',
     'chronic serous otitis media',
     'simple chronic serous otitis media',
     'glue ear - serous',
     'otitis media with effusion - serous',
     'chronic secretory otitis media, serous',
     'chronic non-suppurative otitis media with effusion - serous',
     'sarcoma',
     'malignant mesenchymal tumour',
     'malignant mesenchymal tumor',
     'soft tissue tumour, malignant',
     'sarcoma, no icd-o subtype',
     'sarcoma, no international classification of diseases for oncology subtype',
     'mesenchymal tumour, malignant',
     'soft tissue tumor, malignant',
     'mesenchymal tumor, malignant',
     'soft tissue sarcoma',
     'malignant neoplasm of soft tissue',
     'malignant tumour of soft tissue',
     'malignant tumor of soft tissue',
     'sarcoma of soft tissue',
     'active rheumatic fever',
     'acute rheumatic fever',
     'acute rheumatic attack',
     'rheumatic fever',
     'rhf - rheumatic fever',
     'acute rheumatic arthritis',
     'acute articular rheumatism',
     'impacted third molar tooth',
     'impacted 8',
     'impacted wisdom tooth',
     'complication of implant',
     'interauricular septal defect',
     'congenital atrial septal defect',
     'atrial septal defect',
     'interatrial septal defect',
     'ASd',
     'disorder of muscle',
     'myopathic disease',
     'myopathy',
     'disorder of skeletal and/or smooth muscle',
     'plagiocephaly',
     'lateral curvatures of skull unequal',
     'asymmetric head',
     'contusion of knee',
     'contusion, knee',
     'superficial bruising of knee',
     'clostridium difficile infection',
     'clostridium difficile gastrointestinal tract infection',
     'open wound of finger with tendon involvement',
     'examination - action',
     'clinical examination - action',
     'physical examination -action',
     'examination',
     'physical examination procedure',
     'clinical examination',
     'physical examination',
     'examination of patient',
     'physical assessment',
     'physical examination',
     'examinations',
     'history and physical examination, sports participation',
     'child examination',
     'disorder of lipid metabolism',
     'lipid metabolism disorder',
     'tricuspid valve regurgitation',
     'Ti',
     'tricuspid incompetence',
     'tricuspid insufficiency',
     'Tr',
     'tricuspid regurgitation',
     'tricuspid valve incompetence',
     'tricuspid valve insufficiency',
     'prostatism',
     'intertrigo',
     'erythema intertrigo',
     'eczema intertrigo',
     'acute myeloid leukaemia',
     'acute non-lymphocytic leukemia',
     'acute myeloid leukemia, no icd-o subtype',
     'acute myeloid leukaemia, no icd-o subtype',
     'acute myeloid leukemia, no international classification of diseases for oncology subtype',
     'acute granulocytic leukemia',
     'acute myelogenous leukemia',
     'acute myeloblastic leukemia',
     'acute myelocytic leukemia',
     'acute granulocytic leukaemia',
     'acute myelocytic leukaemia',
     'acute non-lymphocytic leukaemia',
     'acute myelogenous leukaemia',
     'acute myeloblastic leukaemia',
     'acute myeloid leukemia - category',
     'acute myeloid leukaemia - category',
     'acute myeloid leukemia, disease',
     'acute myeloid leukemia',
     'AMl',
     'upper gastrointestinal hemorrhage',
     'upper gi - gastrointestinal haemorrhage',
     'upper gi - gastrointestinal hemorrhage',
     'upper gi haemorrhage',
     'upper gastrointestinal bleeding',
     'upper gi hemorrhage',
     'upper gi bleeding',
     'breathing-related sleep disorder',
     'prosthetic arthroplasty of the hip',
     'hip replacement',
     'insertion of hip prosthesis',
     'insertion of prosthesis or prosthetic device of hip',
     'intestinal adhesions with obstruction',
     'raynauds disease',
     'raynaud disease',
     'raynauds disease',
     'pituitary adenoma',
     'pituitary adenoma, no icd-o subtype',
     'pituitary adenoma, no international classification of diseases for oncology subtype',
     'adenoma of pituitary',
     'nondependent alcohol abuse, episodic',
     'paralytic syndrome of dominant side as late effect of stroke',
     'viral syndrome',
     'nonspecific syndrome suggestive of viral illness',
     'acute gastric ulcer with haemorrhage',
     'bleeding acute gastric ulcer',
     'acute gastric ulcer with bleeding',
     'esotropia',
     'convergent squint',
     'convergent strabismus',
     'cross-eye',
     'closed fracture of metacarpal bone',
     'syphilis',
     'lues',
     'infection by treponema pallidum',
     'treponemal infection',
     'treponemal disease',
     'luetic disease',
     'syphillis',
     'rectal pain',
     'pain in rectum',
     'proctalgia',
     'rectalgia',
     'polysubstance dependence',
     'sensory neuropathy',
     'sensory peripheral neuropathy',
     'cut - accidental',
     'constipation by outlet obstruction',
     'psoriasis with arthropathy',
     'psoriatic arthropathy',
     'amyotrophic lateral sclerosis',
     'bulbar motor neuron disease',
     'lou gehrigs disease',
     'ALs',
     'twin pregnancy',
     'closed fracture of pelvis',
     'enteritis due to rotavirus',
     'transient hypertension of pregnancy',
     'seen by paediatrician',
     'unemployed',
     'without employment',
     'out of work',
     'Ue',
     'u/e - unemployed',
     'unemployed',
     'closed fracture of metatarsal bone',
     'closed fracture metatarsal',
     'generalised abdominal pain',
     'general abdominal pain-symptom',
     'vaginal hysterectomy',
     'colpohysterectomy',
     'vaginal excision of uterus',
     'congestion of nasal sinus',
     'sinus congestion',
     'carcinoma of sigmoid colon',
     'pneumonia due to pseudomonas',
     'pseudomonal pneumonia',
     'symptomatic sinus bradycardia',
     'hydrocephalus',
     'hydrocephaly',
     'family history of cancer',
     'fh: cancer - *',
     'fh: malignant disease',
     'closed fracture of patella',
     'rectal polyp',
     'polyp of rectum',
     'large prostate',
     'prostatomegaly',
     'enlarged prostate',
     'swollen legs',
     'swelling of lower limb',
     'animal bite wound',
     'injury caused by animal bite',
     'wellness health education, guidance, and counseling',
     'wellness health teaching, guidance, and counselling',
     'wellness health teaching, guidance, and counseling',
     'acute cholangitis',
     'disorder of intestine',
     'enteropathy',
     'intestinal disease',
     'enteropathies',
     'pleurisy',
     'pleuritis',
     'plurisy',
     'plurasy',
     'pluracy',
     'family history of prostate cancer',
     'cerebral atherosclerosis',
     'fracture of radius',
     'placement of stent',
     'stent placement',
     'dilation and curettage of uterus',
     'dilatation and curettage',
     'd&c - dilatation and curettage',
     'dilation and curettage',
     'acute duodenal ulcer',
     'ADu',
     'fetal intrauterine distress first noted during labour and/or delivery in liveborn infant',
     'foetal intrauterine distress first noted during labour and/or delivery in liveborn infant',
     'hypomagnesemia',
     'magnesium deficiency syndrome',
     'hyperbilirubinaemia',
     'bilirubinemia',
     'bilirubinaemia',
     'serum bilirubin raised',
     'hyperbilirubinemia',
     'microalbuminuria',
     'complete trisomy 21 syndrome',
     't21 - trisomy 21',
     'downs syndrome',
     'down syndrome',
     'mongoloid',
     'mongolism',
     'laparoscopic cholecystectomy',
     'endoscopic cholecystectomy',
     'laparoscopy, surgical; cholecystectomy',
     'cervical spondylosis with myelopathy',
     'spondylogenic compression of cervical spinal cord',
     'cervical spinal cord compression',
     'cervical spondylitic cord compression',
     'pressure ulcer of buttock',
     'pressure sore of buttock',
     'cervical spondylosis',
     'cervical osteoarthritis',
     'osteoarthritis of cervical spine',
     'cervical spondylarthritis',
     'cervical spine degeneration',
     'cervical arthritis',
     'cervical spondylitis',
     'spondalosis',
     'spondilosis',
     'radiology result abnormal',
     'allogeneic bone marrow transplantation',
     'allogeneic bone marrow transplant',
     'allogeneic bone marrow graft',
     'hematopoietic progenitor cell (hpc); allogeneic transplantation per donor',
     'drainage of abscess',
     'auditory testing',
     'audiogram',
     'hearing examination',
     'hearing test',
     'clinical test of hearing',
     'individual hearing examination',
     'auditory acuity evaluation',
     'disorder of transplanted kidney',
     'complication of transplanted kidney',
     'lumbar post-laminectomy syndrome',
     'acute bronchiolitis due to respiratory syncytial virus',
     'pleuritic pain',
     'painful breathing -pleurodynia',
     'pleurodynia',
     'pleuralgia',
     'pleural pain',
     'abnormal findings on diagnostic imaging of lung',
     'paralytic syndrome of nondominant side as late effect of stroke',
     'paralytic syndrome of non-dominant side as late effect of stroke',
     'abdomen endoscopy',
     'laparoscopy',
     'endoscopy of abdomen',
     'celioscopy',
     'peritoneoscopy',
     'coelioscopy',
     'laproscopy',
     'laparoscopies',
     'LAp',
     'benign neoplasm of colon',
     'benign tumour of colon',
     'benign tumor of colon',
     'submucous leiomyoma of uterus',
     'submucous uterine leiomyoma',
     'submucous uterine fibroid',
     'special educational needs',
     'SEn',
     'atonic postpartum haemorrhage',
     'immediate postpartum hemorrhage',
     'hemorrhage within 24 hours following delivery of placenta',
     'haemorrhage within 24 hours following delivery of placenta',
     'immediate postpartum haemorrhage',
     'complication of renal dialysis',
     'failed induction of labor',
     'unsuccessful attempt to induce labour',
     'unsuccessfully induced labour',
     'unsuccessfully induced labor',
     'unsuccessful attempt to induce labor',
     'herniation of nucleus pulposus',
     'myasthenia gravis',
     'Mg',
     'erb-goldflam disease',
     'neoplasm of brain',
     'intracranial tumor',
     'intracranial neoplasm',
     'ICt',
     'intracranial tumour',
     'Bt',
     'brain tumour',
     'brain tumor',
     'short stature disorder',
     'short stature',
     'small stature',
     'primordial dwarfism',
     'pure dwarf',
     'pure dwarfism',
     'true dwarf',
     'normal dwarf',
     'normal dwarfism',
     'true dwarfism',
     'nanosomia',
     'constitutional short stature',
     'physiologic dwarfism',
     'constitutional dwarfism',
     'hypertrophy of nasal turbinates',
     'lymphadenitis',
     'adenitis',
     'inflammation of lymph node',
     'lymphadenosis',
     'excessive cerumen in ear canal',
     'radiologic infiltrate of lung',
     'cognitive disorder',
     'polyuria',
     'passes too much urine',
     'urine output high',
     'polyuric state',
     'counseling - action',
     'counseling',
     'counselling service',
     'counselling',
     'hallucinations',
     'hallucination',
     'undernourished',
     'malnourished',
     'underfed',
     'nutritional disorder',
     'disorder of nutrition',
     'malnutrition',
     'nutritional disease',
     'nutritional deficiency',
     'undernutrition',
     'undernutrition syndrome',
     'nutritional deficiency disorder',
     'nutritional deficiencies',
     'dietary deficiency',
     'deficiencies',
     'deficits',
     'epicondylitis',
     'impaired glucose tolerance test',
     'abnormal glucose tolerance test',
     'gangrene',
     'gangrenous',
     'gangrenous disorder',
     'iritis',
     'asthmatic bronchitis',
     'wheezy bronchitis',
     'disorder due to type 2 diabetes mellitus',
     'disorder due to type ii diabetes mellitus',
     'cortical senile cataract',
     'disorder of rotator cuff',
     'exanthema subitum',
     'three day fever',
     'exanthem subitum',
     'pseudorubella',
     'sixth disease',
     'roseola infantum',
     'roseola',
     'enthesopathy of ankle and/or tarsus',
     'intrauterine pregnancy',
     'subserous leiomyoma of uterus',
     'subserous uterine fibroid',
     'subserous uterine leiomyoma',
     'carcinoma of pancreas',
     'abdominal mass',
     'abdominal lump',
     'hirsutism',
     'hirsuties',
     'pilosis',
     'hirsutes',
     'hirsuitism',
     'hirsutisms',
     'acute maxillary sinusitis',
     'acute antritis',
     'localization-related epilepsy',
     'focal epilepsy',
     'local epilepsy',
     'partial epilepsy',
     'human papilloma virus infection',
     'hand joint inflamed',
     'hand joint active',
     'hypertrophy of breast',
     'cervical intraepithelial neoplasia',
     'CIn',
     'blood pressure taking',
     'Bp',
     'collapse of vertebra',
     'collapsed vertebra',
     'spondylomalacia',
     'bacteraemia',
     'bacteremias',
     'metatarsalgia',
     'pain in ball of foot',
     'cardiomegaly - hypertensive',
     'hypertensive heart disease',
     'hypertensive cardiopathy',
     'hypertensive cardiomegaly',
     'hypertensive cardiovascular disease',
     'HHd',
     'strain of rotator cuff capsule',
     'strain of rotator cuff of shoulder',
     'mixed bipolar i disorder',
     'bipolar i disorder, most recent episode mixed',
     'malaise and fatigue',
     'failure to gain weight',
     'poor weight gain',
     'not putting on weight',
     'not gaining weight',
     'unable to gain weight',
     'insect bite',
     'insect bite - wound',
     'vascular sclerosis',
     'arteriosclerosis',
     'arteriosclerotic vascular disease',
     'arteriosclerotic',
     'transurethral prostatectomy',
     'loop prostatectomy',
     'tur of prostate',
     'endoscopic resection of prostate',
     'transurethral resection of prostate',
     'TURp',
     'endoscopic prostatectomy',
     'infective hepatitis immunisation',
     'infectious hepatitis vaccinat',
     'thyrotoxicosis with or without goitre',
     'thyrotoxicosis',
     'renal artery stenosis of unknown cause',
     'renal artery stenosis',
     'RAs',
     'aspiration pneumonia',
     'inhalation pneumonia',
     'carcinoma in situ of breast',
     'non-invasive carcinoma of breast',
     'beta thalassemia trait',
     'polyneuropathy associated with type 2 diabetes mellitus',
     'polyneuropathy associated with type ii diabetes mellitus',
     'diabetes type 2 with diabetic polyneuropathy',
     'abdominal pregnancy',
     'arteriosclerosis obliterans',
     'onychogryposis',
     'Og',
     'onychogryphosis',
     'hook nail',
     'fracture of tibia',
     'compression fracture of vertebral column',
     'compression fracture of spine',
     'closed supracondylar fracture of humerus',
     'closed fracture distal humerus, supracondylar',
     'dysarthria',
     'dysarthrias',
     'maternal pyrexia in labour',
     'maternal pyrexia during labor',
     'depot contraception',
     'depot contraceptive',
     'chronic gastric ulcer',
     'chronic ulcer of stomach',
     'synovial cyst of knee',
     'ganglion of knee',
     'bakers cyst of knee',
     'synovial cyst of popliteal space',
     'bakers cyst',
     'popliteal cyst',
     'inflamed seborrhoeic keratosis',
     'fracture of clavicle',
     'collar bone fracture',
     'fracture of collar bone',
     'shoulder tendinitis',
     'shoulder tendonitis',
     'walking disability',
     'difficulty walking',
     'impaired walking',
     'prostate specific antigen abnormal',
     'abdominal abscess',
     'intra-abdominal abscess',
     'hyperemesis gravidarum',
     'hyperemesis of pregnancy',
     'excessive pregnancy vomiting',
     'excessive vomiting in pregnancy',
     'hyperemesis arising during pregnancy',
     'persistent and/or vicious vomiting arising during pregnancy',
     'hyperemesis in pregnancy',
     'postoperative hypothyroidism',
     'postsurgical hypothyroidism',
     'hypothyroidism after surgery',
     'small cell carcinoma of lung',
     'SCLc',
     'small cell lung cancer',
     'oat cell carcinoma of lung',
     'SCCl',
     'food allergy',
     'closed fracture of one or more phalanges of hand',
     'infection due to resistant organism',
     'resistant infection',
     'pyrexia of unknown origin',
     'PUo',
     'fever of unknown origin',
     'f.u.o.',
     'non-toxic nodular goiter',
     'nontoxic nodular thyroid goitre',
     'nontoxic nodular thyroid goiter',
     'fh: coronary thrombosis',
     'family history: coronary thrombosis',
     'preterm premature rupture of membranes',
     'PPROm',
     'prostate mass',
     'reduced libido',
     'low libido',
     'current non-smoker',
     'skin - benign mole and naevus',
     'contagious disease',
     'communicable disease',
     'disorder due to infection',
     'infective disorder',
     'disease due to infection',
     'infectious disease',
     'infection',
     'infections',
     'exercise stress echocardiography',
     'keratosis pilaris',
     'Kp',
     'oligomenorrhea',
     'relative amenorrhoea',
     'relative amenorrhea',
     'infrequent periods',
     'infrequent menstruation',
     'cyst of bartholins gland duct',
     'cyst of bartholin gland duct',
     'bartholins cyst',
     'bartholins gland cyst',
     'cyst of bartholins gland',
     'bartholins gland duct cyst',
     'undifferentiated schizophrenia',
     'atypical schizophrenia',
     'open wound',
     'open wound of body region',
     'screening for malignant neoplasm of cervix',
     'cervical neoplasia screening',
     'cervical cytology screening',
     'alcohol withdrawal syndrome',
     'closed fracture of lower end of radius and ulna',
     'closed fracture radius and ulna, distal',
     'closed fracture of radius and ulna, lower end',
     'high density lipoprotein deficiency',
     'exercise tolerance test abnormal',
     'positive exercise ecg test',
     'exercise ecg abnormal',
     'exercise ecg positive',
     'echocardiography',
     'us scan of heart',
     'cardiac echo',
     'echocardiogram',
     'cardiac us scan',
     'us heart scan',
     'diagnostic ultrasound of heart',
     'ultrasonography of heart',
     'echocardiographic procedure',
     'echocardiography procedures',
     'echocardiograms',
     'respiratory syncytial virus infection',
     'RSv',
     'urinary bladder stone',
     'vesical calculus',
     'bladder stone',
     'vesicolithiasis',
     'bladder calculus',
     'calculus of bladder',
     'fetal or neonatal effect of multiple pregnancy',
     'foetal or neonatal effect of multiple pregnancy',
     'fall from chair or bed',
     'vesicoureteric reflux',
     'vesicoureteral reflux',
     'VUr',
     'ureteric reflux',
     'hyperhidrosis',
     'hyperidrosis',
     'heel pain',
     'talalgia',
     'allergic asthma',
     'atopic asthma',
     'allergic atopic asthma',
     'spontaneous pneumothorax',
     'aphasia',
     'loss of power of expression or comprehension',
     'aphasic disturbance',
     'aphasias',
     'bleeding',
     'hemorrhage',
     'haemorrhage',
     'extravasation of blood',
     'blood loss',
     'bleedings',
     'hemorrhages',
     'hemorrhaging',
     'chronic ulcer of lower extremity',
     'chronic ulcer of lower limb',
     'chronic ulcer of lower leg',
     'neurologic disorder associated with diabetes mellitus',
     'neurologic complication of diabetes mellitus',
     'diabetic neurologic disease',
     'right inguinal hernia',
     'RIh',
     'family history of disorder',
     'subcutaneous nodule',
     'nodule of subcutaneous tissue',
     'injury of upper extremity',
     'injury of upper limb',
     'arm injury',
     'macrocytosis',
     'macrocytosis, red cells',
     'hypothyroidism following radioiodine therapy',
     'hypothyroidism, postradioiodine therapy',
     'radioactive iodine-induced hypothyroidism',
     'rupture of anterior cruciate ligament',
     'ACl',
     'anterior cruciate ligament rupture',
     'tear of anterior cruciate ligament',
     'end stage renal failure on dialysis',
     'single major depressive episode, mild',
     'mild major depression, single episode',
     'parainfluenza',
     'never smoked tobacco',
     'never smoked',
     'inflammatory disorder of penis',
     'inflammation of penis',
     'balanitis',
     'hypertrophy of tonsils',
     'enlargement of tonsils',
     'tonsillar hypertrophy',
     'tonsillar enlargement',
     'flexible fiberoptic sigmoidoscopy',
     'FOs',
     'fibreoptic sigmoidoscopy',
     'fiberoptic sigmoidoscopy',
     'Fs',
     'flexible sigmoidoscopy',
     'sigmoidoscopy, flexible',
     'standard chest x-ray abnormal',
     'lentigo',
     'lentiginosis',
     'fracture of humerus',
     'fracture of upper arm',
     'papilloma of skin',
     'mild non-proliferative diabetic retinopathy',
     'mild nonproliferative retinopathy',
     'mild non proliferative retinopathy',
     'mild nonproliferative diabetic retinopathy',
     'disorder of cardiovascular system',
     'cvd - cardiovascular disease',
     'cvs disease - cardiovascular system disease',
     'disorder of the circulatory system',
     'cardiovascular system disease',
     'cardiovascular disorder',
     'disorder of circulatory system',
     'cardiovascular disease',
     'dyssomnia',
     'sleep problem',
     'sleep disturbance',
     'dysomnia',
     'transitory tachypnoea of newborn',
     'TTn',
     'transient tachypnoea of newborn',
     'newborn transitory tachypnoea',
     'newborn transitory tachypnea',
     'transient tachypnea of newborn',
     'wet lung syndrome in newborn',
     'tachypnea resolving about 6 hours postnatally',
     'idiopathic tachypnea of newborn',
     'tachypnoea resolving about 6 hours postnatally',
     'idiopathic tachypnoea of newborn',
     'hypocalcemia',
     'hypocalcaemia syndrome',
     'hypocalcemia syndrome',
     'calcium deficiency disease',
     'gastroduodenitis',
     'gastritis and duodenitis',
     'squamous cell carcinoma of lung',
     'epidermoid carcinoma of lung',
     'spinal cord disease',
     'neurologic myelopathy',
     'spinal cord disorder',
     'Mp',
     'myelopathy',
     'colicky pain',
     'abdominal colic',
     'colicky abdominal pain',
     'intestinal colic',
     'colic',
     'spasmodic abdominal pain',
     'colics',
     'alveolar hypoventilation',
     'febrile neutropenia',
     'neutropaenic fever',
     'neutropenic fever',
     'history of amputation of leg through tibia and fibula',
     'history of amputation of leg below knee',
     'c/o - debility - malaise',
     'complaining of debility and malaise',
     'orthopedic device application',
     'endogenous depression',
     'abnormal weight',
     'body weight problem',
     'chronic pericoronitis',
     'hand joint pain',
     'arthralgia of the hand',
     'vasculitis',
     'angiitis',
     'phlebitus',
     'sacroiliac joint inflamed',
     'sacroiliac joint active',
     'inflammation of sacroiliac joint',
     'patent ductus arteriosus',
     'patent ductus botalli',
     'PDa',
     'patent arterial duct',
     'radial styloid tenosynovitis',
     'de quervains disease',
     'current tear of medial cartilage and/or meniscus of knee',
     'closed fracture of lateral malleolus',
     'closed fracture ankle, lateral malleolus',
     'iron deficiency',
     'chronic constipation',
     'noncompliance with medication regimen',
     'noncompliance: medication regimen',
     'fibroadenosis of breast',
     'fibroadenosis breast',
     'endocrine/metabolic screening',
     'hemiplegia as late effect of cerebrovascular accident',
     'history of harvesting of stem cells for allotransplant',
     'history of stem cell donation',
     'bilateral tubal ligation',
     'severe recurrent major depression with psychotic features',
     'simple renal cyst',
     'cyst of kidney',
     'renal cyst',
     'simple cyst of kidney',
     'single renal cyst',
     'achilles tendinitis',
     'achilles tendonitis',
     'gilberts syndrome',
     'gilberts syndrome',
     'gilbert-lereboullet syndrome',
     'familial nonhemolytic bilirubinemia',
     'chronic intermittent juvenile jaundice',
     'constitutional hepatic dysfunction',
     'cholemia familiaris simplex',
     'familial nonhemolytic jaundice',
     'gilberts disease',
     'hereditary nonhemolytic jaundice',
     'benign unconjugated bilirubinemia syndrome',
     'meulengracht syndrome',
     'low-grade chronic hyperbilirubinemia syndrome',
     'congenital familial cholemia',
     'benign unconjugated bilirubinaemia syndrome',
     'congenital familial cholaemia',
     'cholaemia familiaris simplex',
     'gilbert syndrome',
     'familial nonhaemolytic bilirubinaemia',
     'hereditary nonhaemolytic jaundice',
     'familial nonhaemolytic jaundice',
     'low-grade chronic hyperbilirubinaemia syndrome',
     'knee joint effusion',
     'effusion of knee',
     'left upper quadrant pain',
     'neurological disorder associated with type 1 diabetes mellitus',
     'neurological disorder associated with type i diabetes mellitus',
     'diabetes type 1 with neurological disorder',
     'panic disorder without agoraphobia',
     'adenocarcinoma of large intestine',
     'epilepsy characterized by intractable complex partial seizures',
     'feeding difficulties and mismanagement',
     'large liver',
     'hepatomegaly',
     'hyperparathyroidism due to renal insufficiency',
     'secondary hyperparathyroidism of renal origin',
     'macrocytic anaemia',
     'chronic cor pulmonale',
     'COr',
     'chronic pulmonary heart disease',
     'chronic cardiopulmonary disease',
     'hypovolaemia',
     'sodium and water depletion',
     'fluid depletion',
     'fluid volume deficit',
     'volume depletion',
     'fluid volume depletion',
     'depletion of volume of plasma and/or extracellular fluid',
     'suicide attempt',
     'computed tomography result abnormal',
     'history of renal transplant',
     'h/o: kidney recipient',
     'history of - kidney recipient',
     'rib pain',
     'puncture',
     'puncture wound',
     'prick',
     'puncture wound - injury',
     'punctured',
     'punctures',
     'puncturing',
     'pricked',
     'chronic post-traumatic stress disorder',
     'acute otitis externa',
     'autistic disorder',
     'kanners syndrome',
     'autism',
     'autism disorder',
     'infantile autism',
     'autistic disorder of childhood onset',
     'autism, childhood onset',
     'childhood autism',
     'aneurysm of thoracic aorta',
     'bilateral hearing loss',
     'ultrasound scan abnormal',
     'fracture of phalanx of foot',
     'toe fracture',
     'fracture of phalanx of toe',
     'fracture of toe',
     'excessive fetal growth affecting management of mother',
     'large for dates affecting management of mother',
     'fracture of mandible',
     'fracture of lower jaw',
     'autologous bone marrow transplant',
     'ABMt',
     'autologous bone marrow graft',
     'hematopoietic progenitor cell (hpc); autologous transplantation',
     'disorder of lower extremity',
     'heart disease screening',
     'cellulitis and abscess of buttock',
     'radiation therapy complication',
     'malignant tumor of pancreas',
     'cancer of pancreas',
     'pancreatic cancer',
     'ca - pancreatic cancer',
     'cancer of pancrees',
     'deep histiocytoma',
     'reticulohistiocytoma',
     'juvenile histiocytoma',
     'histiocytoma',
     'deep benign fibrous histiocytoma',
     'sclerosing angioma',
     'sclerosing haemangioma',
     'sclerosing hemangioma',
     'reticulohistiocytic granuloma',
     'giant cell histiocytoma',
     'reticulohistiocytoma cutis',
     'fibrous histiocytoma of skin',
     'benign fibrous histiocytoma of skin',
     'fibrous xanthoma of skin',
     'dermatofibroma',
     'sclerosing angioma of skin',
     'solitary reticulohistiocytoma',
     'cutaneous histiocytoma',
     'subepidermal nodular fibrosis',
     'pleomorphic fibroma',
     'dermatofibroma lenticulare',
     'dermatofibroma, no icd-o subtype',
     'dermatofibroma, no international classification of diseases for oncology subtype',
     'stasis ulcer',
     'venous ulcer',
     'venous stasis ulcer',
     'venous ulcer of leg',
     'stasis ulcer of leg',
     'gravitational ulcer',
     'stasis leg ulcer',
     'varicose ulcer',
     'exotropia',
     'divergent strabismus',
     'external strabismus',
     'divergent squint',
     'xt - exotropia',
     'divergent concomitant strabismus',
     'temporal arteritis',
     'cranial arteritis',
     'hortons disease',
     'Ta',
     'temporal giant cell arteritis',
     'giant cell arteritis',
     'fractured nasal bones',
     'broken nose',
     'fractured nose',
     'fracture of nose',
     'fracture of nasal complex',
     'fracture of nasal bones',
     'drowsy',
     'mental status, drowsy',
     'somnolence (sleepiness)',
     'sleepy',
     'somnolence',
     'drowsiness',
     'sleepiness',
     'drowsiness, function',
     'nonunion of fracture',
     'fracture, ununited',
     'fracture, nonunion',
     'residual cognitive deficit as late effect of cerebrovascular accident',
     'residual cognitive deficit, late effect of stroke',
     'asbestosis',
     'pulmonary asbestosis',
     'amianthosis',
     'asbestos pneumoconiosis',
     'reticulosarcoma morphology',
     'reticulum cell sarcoma morphology',
     'reticulosarcoma',
     'reticulum cell sarcoma',
     'diabetic autonomic neuropathy associated with type 2 diabetes mellitus',
     'contraception',
     'family planning',
     'birth control',
     'contraception care',
     'family planning treatments and procedures',
     'family planning: contraception',
     'family planning care: contraception',
     'family planning education',
     'Fp',
     'pretransplant evaluation of donor',
     'splenomegaly',
     'enlargement of spleen',
     'large spleen',
     'closed fracture of shaft of femur',
     'closed fracture shaft of femur',
     'closed fracture of clavicle',
     'posttraumatic osteoporosis',
     'sudecks atrophy',
     'posttraumatic atrophy of bone',
     'shoulder-hand syndrome',
     'reflex sympathetic dystrophy of upper extremity',
     'steinbrockers syndrome',
     'complex regional pain syndrome, type i',
     'sudeks atrophy',
     'algoneurodystrophy',
     'RSd',
     'reflex sympathetic dystrophy',
     'CRPs',
     'complex regional pain syndrome type i',
     'algodystrophy',
     'open wound of finger',
     'open wound, finger',
     'chronic prostatitis',
     'malignant tumour of stomach',
     'cancer of stomach',
     'gastric cancer',
     'patellar tendonitis',
     'anemia of chronic renal failure',
     'anemia of chronic renal insufficiency',
     'anaemia of chronic renal insufficiency',
     'neoplastic disease',
     'neoplasia',
     'Ng',
     'neoplastic growth',
     'new growth',
     'neoplasm',
     'chronic nonalcoholic liver disease',
     'fatty metamorphosis of liver',
     'hematoma',
     'hematomas',
     'syndrome of inappropriate vasopressin secretion',
     'syndrome of inappropriate secretion of antidiuretic hormone',
     'SIAd',
     'syndrome of inappropriate antidiuresis',
     'SIADh',
     'syndrome of inappropriate antidiuretic hormone secretion',
     'syndrome of inappropriate adh production',
     'schwartz-bartter syndrome',
     'administrative reason for encounter',
     'gastric erosion',
     'chronic mixed headache syndrome',
     'viral gastroenteritis due to norwalk-like agents',
     'small round structured virus gastroenteritis',
     'norwalk virus food poisoning',
     'norwalk virus gastroenteritis',
     'norwalk-like virus gastroenteritis',
     'dialysis procedure',
     'renal dialysis',
     'kidney dialysis',
     'artificial kidney dialysis',
     'dialysis services and procedures',
     'dialysis',
     'disorder of stoma',
     'implantation of mitral valve prosthesis or synthetic device',
     'cardiomegaly',
     'enlarged heart',
     'mastitis',
     'cellulitis of foot',
     'non-infective non-allergic rhinitis',
     'NINa',
     'fracture of fibula',
     'vaginal irritation',
     'chronic apical abscess',
     'chronic dentoalveolar abscess',
     'chronic periapical abscess',
     'suppurative apical periodontitis',
     'chronic alveolar abscess',
     'chronic periradicular abscess',
     'periapical abscess',
     'dentoalveolar abscess',
     'alveolar abscess',
     'apical abscess',
     'motor vehicle on road in collision with pedestrian',
     'intracranial meningioma',
     'meningioma of brain',
     'cranial meningioma',
     'gallbladder pain',
     'biliary colic',
     'hepatic colic',
     'biliary colic symptom',
     'pseudophakia',
     'tetanus vaccination',
     'tetanus immunization',
     'tetanus immunisation',
     'chronic duodenal ulcer',
     'retinal oedema',
     'wolff-parkinson-white pattern',
     'ventricular pre-excitation with arrhythmia',
     'wolff parkinson white syndrome',
     'WPw',
     'wolff-parkinson-white syndrome',
     'atrial paroxysmal tachycardia',
     'PAt',
     'paroxysmal atrial tachycardia',
     'kyphosis deformity of spine',
     'gibbosity',
     'gibbus',
     'humpback',
     'hunchback',
     'kyphosis',
     'nummular eczema',
     'nummular dermatitis',
     'nummular eczematous dermatitis',
     'exudative neurodermatitis',
     'carotid bruit',
     'adjustment disorder with anxious mood',
     'adjustment reaction with anxious mood',
     'adjustment disorder with anxiety',
     'bipolar affective disorder, current episode depression',
     'manic-depressive - now depressed',
     'toxaemia of pregnancy',
     'pre-eclampsia',
     'eph - oedema, proteinuria and hypertension of pregnancy',
     'pre-eclamptic toxaemia',
     'PEt',
     'EPh',
     'edema, proteinuria and hypertension of pregnancy',
     'pre-eclamptic toxemia',
     'proteinuric hypertension of pregnancy',
     'preeclampsia',
     'liver cell carcinoma',
     'hepatocarcinoma',
     'hepatocellular carcinoma',
     'malignant hepatoma',
     'HCc',
     'LCc',
     'liver carcinoma',
     'primary carcinoma of liver',
     'hepatoma, malignant',
     'hepatoma',
     'history of squamous cell carcinoma of skin',
     'history of laser assisted in situ keratomileusis',
     'history of lasik - laser assisted in situ keratomileusis',
     'degenerative joint disease of ankle and/or foot',
     'osteoarthritis - ankle and/or foot',
     'osteoarthritis - ankle/foot',
     'osteoarthrosis of ankle and/or foot',
     'mitral valve stenosis',
     'mitral stenosis',
     'induratio penis plastica',
     'peyronies disease',
     'peyronies disease',
     'penile fibromatosis',
     'fibrous cavernositis',
     'plastic induration of penis',
     'malignant tumor of testis',
     'chronic otitis media',
     'retinopathy of prematurity',
     'RLf',
     'retrolental fibroplasia',
     'ROp',
     'terrys syndrome',
     'foreign body in auditory canal',
     'fb - foreign body in eac - external auditory canal',
     'foreign body in external auditory canal',
     'foreign body in ear',
     'teething syndrome',
     'teething',
     'amphetamine dependence',
     'amfetamine dependence',
     'allergic reaction to bee sting',
     'sprained finger/thumb',
     'atypical squamous cells of undetermined significance on cervical papanicolaou smear',
     'low tension glaucoma',
     'normal pressure glaucoma',
     'LTg',
     'normal tension glaucoma',
     'diabetic complication',
     'disorder associated with diabetes mellitus',
     'neoplastic pleural effusion',
     'malignant effusion',
     'malignant pleural effusion',
     'pleural effusion due to malignancy',
     'anemia of renal disease',
     'cough variant asthma',
     'fracture of foot',
     'total, mature senile cataract',
     'fetal or neonatal effect of disproportion during labour and delivery',
     'foetal or neonatal effect of disproportion during labour and delivery',
     'fetal or neonatal effect of cephalopelvic disproportion during labor and delivery',
     'foetal or neonatal effect of cephalopelvic disproportion during labour and delivery',
     'pain control',
     'controlling pain',
     'relieving pain',
     'giving pain relief',
     'pain management',
     'managing pain',
     'haemophilus influenzae pneumonia',
     'benign neoplasm of female breast',
     'delusional disorder',
     'retinal lattice degeneration',
     'lattice retinal degeneration',
     'lattice degeneration',
     'palisade degeneration of retina',
     'pericardial effusion',
     'chronic kidney disease stage 5',
     'ckd stage 5',
     'bicuspid aortic valve',
     'diabetic oculopathy',
     'ophthalmic manifestations of diabetes',
     'diabetic eye disease',
     'biliary cirrhosis',
     'cholangitic cirrhosis',
     'cholestatic cirrhosis',
     'chronic nonsuppurative destructive cholangitis',
     'primary biliary cirrhosis',
     'hanots cirrhosis',
     'pbc- primary biliary cirrhosis',
     'traumatic dislocation of shoulder region',
     'dislocation of shoulder joint',
     'dislocation of glenohumeral joint',
     'shoulder dislocation',
     'dislocation of shoulder',
     'haemangioma of skin',
     'basal cell carcinoma of face',
     'cancer of skin of face, basal cell',
     'cellulitis and abscess of upper limb',
     'mild memory disturbance',
     'disease due to adenovirus',
     'adenovirus infection',
     'hormone replacement therapy requested',
     'exposure to communicable disease',
     'subendocardial ischemia',
     'incision and drainage',
     'i and d',
     'incision and evacuation',
     'poisoning by anticoagulant',
     'anticoagulant poisoning',
     'enlargement of tonsil or adenoid',
     'tonsil and/or adenoid hypertrophy',
     'hypertrophy of tonsils and adenoids',
     'enlargement of tonsils and adenoids',
     'hypertrophy of both palatine and pharyngeal tonsils',
     'tonsillar and adenoid hypertrophy',
     'near syncope',
     'presyncope',
     'hidradenitis',
     'precursor cell lymphoblastic leukaemia',
     'precursor cell lymphoblastic leukemia, not phenotyped',
     'lymphoblastic leukemia, l2 type',
     'lymphoblastic leukemia',
     'fab l2',
     'acute lymphoid leukemia, l2 type',
     'acute lymphocytic leukemia, l2 type',
     'acute lymphocytic leukemia',
     'acute lymphoblastic leukemia-lymphoma',
     'acute lymphoblastic leukemia, precursor-cell type',
     'acute lymphoblastic leukemia, l2 type',
     'acute lymphoblastic leukemia',
     'acute lymphatic leukemia, l2 type',
     'acute lymphatic leukemia',
     'lymphoblastic leukaemia',
     'lymphoblastic leukaemia, l2 type',
     'precursor cell lymphoblastic leukaemia, not phenotyped',
     'acute lymphatic leukaemia',
     'acute lymphoid leukaemia, l2 type',
     'acute lymphoblastic leukaemia, l2 type',
     'acute lymphoblastic leukaemia, precursor-cell type',
     'acute lymphoblastic leukaemia-lymphoma',
     'acute lymphocytic leukaemia, l2 type',
     'acute lymphocytic leukaemia',
     'acute lymphoblastic leukaemia',
     'acute lymphatic leukaemia, l2 type',
     'acute lymphoid leukaemia',
     'ALl',
     'acute lymphoid leukaemia, disease',
     'acute lymphoid leukemia, disease',
     'not up-to-date with immunisations',
     'postablative hypothyroidism',
     'labyrinthitis',
     'labirynthitis',
     'labyrynthitis',
     'labrynthitis',
     'diabetic peripheral angiopathy',
     'diabetic peripheral vascular disease',
     'diabetes with peripheral circulatory disorders',
     'chronic tension-type headache',
     'haemorrhoidectomy',
     'excision of haemorrhoid(s)',
     'excision of hemorrhoid(s)',
     'excision of pile',
     'hemorrhoidectomy',
     'posterior vitreous detachment',
     'procedure on heart',
     'heart procedure',
     'peritoneal dialysis-associated peritonitis',
     'peritonitis secondary to peritoneal dialysis',
     'cardioverter defibrillator procedure',
     'fetal or neonatal effect of maternal anesthesia and/or analgesia',
     'fetal or neonatal reaction or intoxication from maternal opiate and/or tranquilliser during labour and/or delivery',
     'fetal or neonatal reaction or intoxication from maternal opiate and/or tranquilizer during labor and/or delivery',
     'foetal or neonatal reaction or intoxication from maternal opiate and/or tranquilliser during labour and/or delivery',
     'foetal or neonatal effect of maternal anaesthesia and/or analgesia',
     'closed fracture of ankle',
     'narcolepsy',
     'paroxysmal sleep',
     'gelineaus syndrome',
     'narcoleptic syndrome',
     'multicystic kidney',
     'multicystic renal dysplasia',
     'MCKd',
     'multicystic kidney disease',
     'fibrocystic kidney disease',
     'multiple congenital cysts of kidney',
     'congenital cystic kidney disease',
     'congenital fibrocystic kidney',
     'congenital polycystic kidney disease',
     'sponge kidney',
     'multicystic dysplastic kidney',
     'PKd',
     'polycystic kidney disease',
     'congenital cystic disease of kidney',
     'congenital cyst of kidney',
     'PCk',
     'fibrocystic kidney',
     'non-alcoholic fatty liver',
     'NAFLd',
     'nonalcoholic fatty liver disease',
     'adenocarcinoma of prostate',
     'olecranon bursitis',
     'miners, elbow',
     'capped elbow',
     'students elbow',
     'inflammation of bursa of olecranon',
     'miners elbow',
     'dermatitis medicamentosa',
     'eruption due to drug',
     'drug-induced rash',
     'drug rash',
     'drug eruption',
     'drug-induced dermatosis',
     'chronic paranoid schizophrenia',
     'metabolic acidosis',
     'mallory-weiss tear',
     'mallory-weiss syndrome',
     'gastro-oesophageal laceration-haemorrhage syndrome',
     'gastro-esophageal laceration-hemorrhage syndrome',
     'gastrooesophageal laceration-hemorrhage syndrome',
     'gastroesophageal laceration-hemorrhage syndrome',
     'partial hysterectomy',
     'partial or subtotal hysterectomy',
     'subtotal hysterectomy',
     'diabetic foot examination',
     'complex partial seizure with impairment of consciousness',
     'complex partial seizure',
     'psychomotor fit',
     'complex partial seizures with consciousness impaired',
     'lichenified eczema',
     'lichenified eczematous dermatitis',
     'lichen simplex chronicus',
     'lichen simplex',
     'LSc',
     'circumscribed neurodermatitis',
     'neurodermatitis circumscripta',
     'local neurodermatitis',
     'alopecia areata',
     'alopecia circumscripta',
     'patchy loss of hair',
     'circumscribed alopecia',
     'hypersomnia with sleep apnoea',
     'trichiasis',
     'orchitis and epididymitis',
     'epididymo-orchitis',
     'perimenopausal disorder',
     'injury of eye region',
     'periocular injury',
     'shoulder joint inflamed',
     'shoulder joint active',
     'percutaneous transluminal coronary angioplasty',
     'percutaneous transluminal coronary balloon angioplasty',
     'PTCa',
     'percutaneous angioplasty of coronary artery',
     'balloon angioplasty of coronary artery',
     'percutaneous transluminal coronary angioplasty; single major coronary artery or branch',
     'graft versus host disease',
     'GVHd',
     'graft-versus-host disease',
     'haemorrhage in early pregnancy',
     'hemorrhage before 22 weeks gestation',
     'bleeding in early pregnancy',
     'haemorrhage before 22 weeks gestation',
     'mantoux: negative',
     'negative ppd',
     'laminectomy',
     'spinal laminectomy',
     'laminectomies',
     'swallowed foreign body',
     'large for gestation age fetus',
     'accelerated foetal growth',
     'foetal growth acceleration',
     'large for gestation age foetus',
     'lfd - foetus large for dates',
     'LGa',
     'large for gestational age foetus',
     'large-for-dates foetus',
     'large-for-dates fetus',
     'accelerated fetal growth',
     'large for gestational age fetus',
     'fetal growth acceleration',
     'lfd - fetus large for dates',
     'large for gestational dates',
     'carcinomatosis',
     'disseminated carcinomatosis',
     'carcinomatoses',
     'kidney excision',
     'excision procedures on the kidney',
     'hallux valgus',
     'HAv',
     'hallux abductovalgus',
     'Hv',
     'dystrophia unguium',
     'onychodystrophy',
     'nail dystrophy',
     'dystrophic nail',
     'face ache',
     'pain in face',
     'facial pain',
     'pain of face',
     'senile dementia',
     'Sd',
     'type 2 diabetes mellitus with ulcer',
     'type ii diabetes mellitus with ulcer',
     'polysubstance abuse',
     'keratitis',
     'plantar fascial fibromatosis',
     'plantar fibromatosis',
     'ledderhoses disease',
     'dupuytren disease of foot',
     'family history of asthma',
     'fh: asthma',
     'family history: asthma',
     'abnormal glucose tolerance test during pregnancy - baby not yet delivered',
     'neurogenic bowel',
     'multigravida',
     'multip',
     'complication of procedure',
     'willing to be donor of kidney',
     'benign localized hyperplasia of prostate',
     'localized bph (benign prostatic hyperplasia)',
     'localised bph (benign prostatic hyperplasia)',
     'cerebral embolism',
     'cerebral arterial embolism',
     'chronic duodenal ulcer with haemorrhage',
     'bleeding chronic duodenal ulcer',
     'lichen planus',
     'Lp',
     'ruber planus',
     'decreased sexual function',
     'impaired sexual function',
     'sexual disorder',
     'abnormal sexual function',
     'sexual dysfunction',
     'meningitis',
     'meningitus',
     'leukaemia',
     'leukaemia morphology',
     'leukemia morphology',
     'subacute leukemia [obs]',
     'chronic leukemia [obs]',
     'aleukemic leukemia [obs]',
     'leukaemia, no icd-o subtype',
     'leukemia, no icd-o subtype',
     'leukemia, no international classification of diseases for oncology subtype',
     'subacute leukaemia [obs]',
     'aleukaemic leukaemia [obs]',
     'chronic leukaemia [obs]',
     'leukemia',
     'leukemia, disease',
     'leukaemia, disease',
     'lukemia',
     'leukemias',
     'bursitis of shoulder',
     'bursitis of shoulder region',
     'restrictive lung disease',
     'painful mouth',
     'oral cavity pain',
     'oral pain',
     'venous retinal branch occlusion',
     'BVo',
     'branch retinal occlusion',
     'venous retinal tributary occlusion',
     'branch retinal vein thrombosis',
     'branch retinal vein occlusion',
     'BRVo',
     'BRVt',
     'abscess of skin and/or subcutaneous tissue',
     'skin abscess',
     'cutaneous abscess',
     'replacement of mitral valve',
     'MVr',
     'mitral valve replacement',
     'excision and replacement of mitral valve',
     'diphtheria + tetanus vaccine',
     'diphtheria + tetanus vaccine',
     'diphtheria+tetanus vaccine',
     'tetanus diphtheria vaccination',
     't/d vaccination',
     'dt vaccination',
     'diphtheria and tetanus double vaccination',
     'Dt',
     'displaced tooth',
     'anomaly of tooth position',
     'malpositioned tooth',
     'bacterial conjunctivitis',
     'purulent conjunctivitis',
     'mucopurulent conjunctivitis',
     'displacement of lumbar intervertebral disc without myelopathy',
     'malignant tumor of ascending colon',
     'ca ascending colon',
     'aftercare',
     'at risk of sexually transmitted infection',
     'malignant melanoma, metastatic',
     'metastatic malignant melanoma',
     'known or suspected fetal abnormality affecting management of mother',
     'known or suspected foetal abnormality affecting management of mother',
     'neuralgia',
     'paroxysmal nerve pain',
     'neuralgias',
     'neurodynia',
     'vulvovaginitis',
     'pulmonary oedema',
     'visual alteration',
     'visual disturbance',
     'unilateral paresis',
     'hemiparesis',
     'weakness of one side of body',
     'hemiparesis (weakness on one side)',
     'osteoarthritis of multiple joints',
     'contusion of hip',
     'superficial bruising of hip',
     'contusion, hip',
     'anxiety state',
     'examination of breast',
     'breast examination',
     'tuberculosis of lung, confirmed by culture only',
     'unwanted fertility',
     'partially edentulous mandible',
     'anorectal fistula',
     'fistula-in-ano',
     'fistula in ano',
     'uveitis',
     'intraocular inflammation',
     'wound dehiscence',
     'wound reopened - observation',
     'burst wound',
     'wound opened up',
     'wound rupture',
     'fracture of pelvis',
     'pelvic fracture',
     'menopausal symptom',
     'continuous chronic alcoholism',
     'cocaine dependence in remission',
     'granulomatosis with polyangiitis',
     'wegener granulomatosis',
     'necrotising respiratory granulomatosis',
     'wegeners granulomatosis',
     'necrotizing respiratory granulomatosis',
     'head and neck injury',
     'severe major depression, single episode, without psychotic features',
     'left inguinal hernia',
     'LIh',
     'anaemia of pregnancy',
     'degenerative polyarthritis',
     'osteoarthritis',
     'degenerative arthritis',
     'osteoarthrosis',
     'hypertrophic arthritis',
     'degenerative joint disease',
     'hypertrophic polyarthritis',
     'degenerative arthropathy',
     'arthropathy',
     'arthrosis',
     'joint disorder',
     'joint disease',
     'disorder of joint',
     'osteo-arthritis',
     'ostearthritis',
     'osteo-arthrosis',
     'arthroses',
     'h/o: pulmonary embolus',
     'history of pulmonary embolus',
     'herpangina',
     'enteroviral vesicular pharyngitis',
     'vesicular pharyngitis',
     'idiopathic scoliosis',
     'undifferentiated inflammatory polyarthritis',
     'polyarthritis',
     'inflammatory polyarthropathy',
     'inflammatory polyarthritis',
     'inflammatory arthritis of multiple joints',
     'mass of pancreas',
     'night sweats',
     'nonruptured cerebral aneurysm',
     'unruptured cerebral aneurysm',
     'cerebral aneurysm, nonruptured',
     'peritonitis',
     'peritonitis of undetermined cause',
     'stomatitis',
     'inflammatory condition of oral mucous membrane',
     'oral mucositis',
     'inflammation of mouth',
     'mucositus',
     'h/o: asbestos exposure',
     'history of asbestos exposure',
     'naevus of choroid',
     'choroidal naevus',
     'choroidal nevus',
     'benign melanoma of choroid',
     'vitreous syneresis',
     'vitreous degeneration',
     'good neonatal condition at birth',
     'healthy newborn',
     'sputum - symptom',
     'infusion care education',
     'teach infusion care',
     'central retinal vein occlusion',
     'CRVt',
     'central retinal vein thrombosis',
     'CRVo',
     'closed fracture carpal bone',
     'closed fracture of wrist',
     'closed fracture of carpal bone',
     'social phobia',
     'social anxiety disorder',
     'atelectasis',
     'pulmonary collapse with atelectasis',
     'pulmonary atelectasis',
     'pulmonary collapse',
     'collapse of lung',
     'atelectases',
     'nipple discharge symptom',
     'discharge from nipple',
     'observation of nipple discharge',
     'breast discharge',
     'biceps tendinitis',
     'bicipital tendinitis',
     'scarlet fever',
     'scarlatina',
     'scarletina',
     'scarlett fever',
     'acute urinary tract infection',
     'urinary tract infection of sudden onset and/or short duration',
     'acute uti (urinary tract infection)',
     'detrusor instability of bladder',
     'detrusor dyssynergia',
     'detrusor instability',
     'Di',
     'nasal discharge present',
     'nose running',
     'nose dripping',
     'snuffles',
     'nasal discharge',
     'rhinorrhoea',
     'rhinorrhea',
     'nasal catarrh',
     'discharge from nose',
     'swelling of limb',
     'discharge from penis',
     'mood disorder due to a general medical condition',
     'concussion with no loss of consciousness',
     'excision of breast tissue',
     'mastectomy',
     'mastectomies',
     'malignant lymphoma of extranodal and/or solid organ site',
     'cocaine dependence, continuous',
     'staphylococcal pneumonia',
     'gastro-oesophageal reflux disease with oesophagitis',
     'gastroesophageal reflux disease with esophagitis',
     'gastroesophageal reflux disease with oesophagitis',
     'reflux esophagitis',
     'esophageal reflux with esophagitis',
     'oesophageal reflux with oesophagitis',
     'reflux oesophagitis',
     'chronic intracranial subdural haematoma',
     'anorectal disorder',
     'sepsis due to escherichia coli',
     'chronic gastric ulcer with hemorrhage',
     'bleeding chronic gastric ulcer',
     'galactorrhea due to non-obstetric cause',
     'galactorrhoea not associated with childbirth',
     'galactorrhoea',
     'galactorrhea',
     'milk from non-pregnant breast',
     'inappropriate lactation',
     'inappropriate production of milk',
     'anaemia in neoplastic disease',
     'lumbar sprain',
     'lumbar back sprain',
     'hip joint inflamed',
     'hip joint active',
     'bilateral salpingectomy with oophorectomy',
     'BSo',
     'bilateral salpingo-oophorectomy',
     'complete salpingo-oophorectomy',
     'sprain of shoulder',
     'sprain of shoulder joint',
     'anisometropia',
     'nissen fundoplication',
     'nissen operation, fundoplication of stomach',
     'fundoplication',
     'total fundoplication',
     'TFp',
     'oesophagogastric fundoplasty',
     'apnoea',
     'apneic',
     'has stopped breathing',
     'not breathing',
     'apnoeic',
     'stops breathing',
     'apneas',
     'tricuspid incompetence, non-rheumatic',
     'tricuspid valve regurgitation, nonrheumatic',
     'tricuspid insufficiency, non-rheumatic',
     'tricuspid regurgitation, non-rheumatic',
     'expressive language disorder',
     'developmental expressive language disorder',
     'chronic diarrhoea',
     'cervical intraepithelial neoplasia grade 1',
     'cervical intraepithelial neoplasia grade i',
     'cin i - cervical intraepithelial neoplasia 1',
     'mild cervical dysplasia',
     'mild dysplasia of cervix',
     'vaccination for diphtheria, pertussis, and tetanus',
     'primary degenerative dementia of the alzheimer type, senile onset, uncomplicated',
     'dementia of the alzheimers type, with late onset, uncomplicated',
     'vitrectomy',
     'excision of vitreous opacity',
     'pregnancy test positive',
     'disorder of kidney and/or ureter',
     'incontinence',
     'closed fracture of surgical neck of humerus',
     'closed fracture of neck of humerus',
     'bacterial enteritis',
     'intestinal infection due to bacteria',
     'closed fracture of femur',
     'closed fracture of thigh',
     'closed fracture of upper leg',
     'meralgia paraesthetica',
     'entrapment of lateral cutaneous nerve of thigh',
     'compression of lateral cutaneous femoral nerve of thigh',
     'lateral cutaneous femoral nerve of thigh syndrome',
     'bernhardt-rot syndrome',
     'bernhardts paresthesia',
     'bernhardts paraesthesia',
     'hypertension screening',
     'keratoplasty',
     'corneal transplant',
     'grafting of cornea',
     'corneal graft',
     'keratoplasty procedures on the cornea',
     'corneal ulcer',
     'foreign body',
     'disorder due to presence of foreign body',
     'Fb',
     'foreign body of body structure',
     'exogenous material',
     'foreign material',
     'serum creatinine raised',
     'total splenectomy',
     'total excision of spleen',
     'splenectomy; total, en bloc for extensive disease, in conjunction with other procedure (list in addition to code for primary procedure)',
     'panic disorder with agoraphobia',
     'mild mental retardation (i.q. 50-70)',
     'mild mental retardation (intelligence quotient 50-70)',
     'mild mental handicap',
     'mild mental retardation, iq in range 50-70',
     'faeces contents abnormal',
     'stool contents abnormal',
     'heart valve replacement - prosthesis',
     'prosthetic replacement of heart valve',
     'stented coronary artery',
     'streptococcal tonsillitis',
     'screening for malignant neoplasm of skin',
     'screening for skin cancer',
     'feeling irritable',
     'irritability',
     'fussiness',
     'irritable',
     'abdominal wall pain',
     'malignant tumor of lower third of esophagus',
     'ca lower third oesophagus',
     'ca lower third esophagus',
     'malignant neoplasm of lower third of oesophagus',
     'malignant neoplasm of lower third of esophagus',
     'malignant neoplasm of upper-outer quadrant of female breast',
     'primary sclerosing cholangitis',
     'PSc',
     'cellulitis and abscess of hand excluding digits',
     'malignant tumor of large intestine',
     'cancer of large intestine',
     'cancer of large bowel',
     'infant care',
     'infant health care',
     'second degree perineal laceration',
     'obstetrical laceration, second degree',
     'laceration of inner and/or muscular layers of perineal structures',
     'thoracic outlet syndrome',
     'TOs',
     'thyroid function tests abnormal',
     'closed fracture of calcaneus',
     'closed fracture of heel bone',
     'closed fracture of os calcis',
     'sexually transmitted infectious disease',
     'venereal disease',
     'statutory venereal disease',
     'Vd',
     'sexually transmissible disease',
     'disease with a predominantly sexual mode of transmission',
     'sexually transmitted disease',
     'STd',
     'stds',
     'injury of lower extremity',
     'injury of lower limb',
     'herpesvirus infection',
     'herpes infection',
     'colectomy',
     'lumbar disc lesion',
     'subacute bacterial endocarditis prophylaxis',
     's.b.e. prophylaxis',
     'sbe prophylaxis',
     'worried well',
     'psyche disturbed with normal general body function',
     'physically well but worried',
     'peritonsillar abscess',
     'quinsy',
     'angina tonsillaris',
     'quincy',
     'rhd negative',
     'rh negative',
     'rhesus negative',
     'rhegmatogenous retinal detachment',
     'retinal detachment with break',
     'conjunctival haemorrhage',
     'ankle oedema',
     'swollen ankle - oedema',
     'swollen ankle - edema',
     'swollen ankle',
     'ankle swelling',
     'central sleep apnea syndrome',
     'CSa',
     'central sleep apnoea',
     'central sleep apnea',
     'delayed milestone',
     'delayed developmental milestone',
     'viral enteritis',
     'retained foreign body in eye',
     'retained intraocular foreign body',
     'IOFb',
     'intraocular foreign body',
     'foreign body in eyeball',
     'foreign body in eye',
     'h/o: heart failure',
     'history of heart failure',
     'chronic anaemia',
     'simple chronic anaemia',
     'chronic simple anaemia',
     'chronic simple anemia',
     'acute and subacute liver necrosis',
     'atypical absence seizure',
     'failure to progress in second stage of labor',
     'no progress with delivery',
     'no progress in second stage of labour',
     'no progress in second stage of labor',
     'normocytic anemia',
     'haemorrhoids without complication',
     'family history of stroke',
     'fh: cva',
     'family history of cva',
     'fh: stroke',
     'malignant tumor of larynx',
     'cancer of larynx',
     'laryngeal cancer',
     'cystic fibrosis',
     'Cf',
     'fibrocystic disease',
     'mucoviscidosis',
     'cistic fibrosis',
     'systic fibrosis',
     'perioral dermatitis',
     'POd',
     'closed fracture of upper end of humerus',
     'closed fracture of the proximal humerus',
     'closed fracture of proximal end of humerus',
     'pus cells in urine',
     'pyuria',
     'fracture of tooth',
     'tooth fracture',
     'broken tooth',
     'broken teeth',
     'chipped tooth',
     'fractured tooth',
     'hyperbetalipoproteinemia',
     'familial hypercholesterolemia',
     'familial hyperbetalipoproteinaemia',
     'essential familial hypercholesterolaemia',
     'essential familial hypercholesterolemia',
     'familial hyperbetalipoproteinemia',
     'ldl receptor disorder',
     'LDl',
     'low density lipoprotein receptor disorder',
     'low density lipoprotein catabolic defect',
     'dysphagia as a late effect of cerebrovascular accident',
     'dysphagia, late effect of stroke',
     'history of calculus of kidney',
     'history of nephrolith',
     'history of kidney stone',
     'history of nephrolithiasis',
     'history of renal calculus',
     'migraine with typical aura',
     'migraine with aura',
     'classical migraine',
     'hyperpigmentation',
     'hyperpigmentation of skin',
     'hyperpigmentation disorder',
     'peripheral vertigo',
     'peripheral vestibular vertigo',
     'vestibular vertigo',
     'benign neoplasm of skin',
     'benign tumour of skin',
     'benign tumor of skin',
     'acute low back pain',
     'acute back pain - lumbar',
     'diverticulosis of sigmoid colon',
     'diverticulosis of sigmoid',
     'normal delivery procedure',
     'Nd',
     'chemotherapy follow-up',
     'neoplasm of uncertain behavior of brain and spinal cord',
     'neoplasm of uncertain behavior of brain and/or spinal cord',
     'cellulitis and abscess of face',
     'accident caused by hot liquid and vapour, including steam',
     'fracture of face bones',
     'fracture of facial bone',
     'fracture of facial bones',
     'external hemorrhoids without complication',
     'history of papillary adenocarcinoma of thyroid',
     'history of papillary thyroid carcinoma',
     'carrier of vancomycin resistant enterococcus',
     'infection by methicillin sensitive staphylococcus aureus',
     'infection by methicillin susceptible staphylococcus aureus',
     'disorder of joint of ankle and/or foot',
     'arthropathy of ankle and/or foot',
     'unilateral inguinal hernia with obstruction but no gangrene',
     'unilateral obstructed inguinal hernia',
     'history and physical examination, complete',
     'urolithiasis',
     'anti-nuclear factor positive',
     'anti-nuclear antibody positive',
     'antinuclear factor, positive',
     'opioid abuse',
     'keratoconus',
     'cornea conical',
     'tenosynovitis',
     'tendinous synovitis',
     'tendovaginitis',
     'tenontolemmitis',
     'tenovaginitis',
     'vaginal synovitis',
     'inflammation of tendon sheath',
     'disorder of breast',
     'breast disease',
     'bilateral inguinal hernia',
     'bleeding oesophageal varices',
     'oesophageal varices with bleeding',
     'BOv',
     'esophageal varices with bleeding',
     'bleeding esophageal varices',
     'esophageal varices with hemorrhage',
     'oesophageal varices with haemorrhage',
     'hyperlipidemia screening',
     'lipid screening',
     'history of malignant neoplasm of bronchus',
     'h/o: malignant neoplasm of bronchus',
     'personal history of primary malignant neoplasm of bronchus',
     'open wound of hand with tendon involvement',
     'endometriosis of pelvis',
     'pelvic endometriosis',
     'marital counselling',
     'benign neoplasm of skin of face',
     'sputum retention',
     'no liquor observed vaginally',
     'no flow of amniotc liquor',
     'moderate depression',
     'legal termination of pregnancy',
     'legally induced abortion - top',
     'legally induced abortion',
     'legal abortion',
     'anorexia nervosa',
     'An',
     'oppositional defiant disorder',
     'closed fracture of upper end of tibia',
     'chloasma',
     'melasma',
     'secondary polycythaemia',
     'secondary erythrocytosis',
     'acquired polycythemia',
     'acquired polycythaemia',
     'amputation of leg through tibia and fibula',
     'trans-tibiofibular amputation of lower leg',
     'BKa',
     'below knee amputation',
     'amputation of leg below knee',
     'amputation below-knee',
     'bk amputation',
     'amputation, leg, through tibia and fibula',
     'primary cardiomyopathy',
     'chronic myeloid leukaemia',
     'chronic granulocytic leukemia',
     'chronic myelogenous leukemia',
     'chronic myelocytic leukemia',
     'chronic myelogenous leukemia, no icd-o subtype',
     'chronic myelogenous leukemia, no international classification of diseases for oncology subtype',
     'chronic granulocytic leukaemia',
     'chronic myelogenous leukaemia',
     'chronic myelocytic leukaemia',
     'CMl',
     'CGl',
     'chronic myeloid leukemia',
     'chronic myeloid leukaemia, disease',
     'chronic myeloid leukemia, disease',
     'dystonia',
     'dystonia disorder',
     'dystonias',
     'avascular necrosis of the head of femur',
     'speech delay',
     'slow to talk',
     'lumbosacral spondylosis without myelopathy',
     'lumbar and/or sacral osteoarthritis',
     'fracture of femur',
     'fracture of thigh',
     'fracture of upper leg',
     'monoparesis - arm',
     'weakness of arm',
     'liver mass',
     'closed fracture of tibia and fibula',
     'abdominal bloating',
     'bloating symptom',
     'abdomen feels bloated',
     'abdominal distension',
     'swollen abdomen',
     'meteorism',
     'bloat',
     'bloating',
     'swelling of abdomen',
     'bloated abdomen',
     'abdominal distention',
     'abdominal swelling',
     'abdomen distended',
     'bloated',
     'gassiness',
     'educational problem',
     'oligohydramnios',
     'deficient liquor',
     'reduced liquor volume',
     'scanty liquor',
     'pediculosis capitis',
     'head lice infestation',
     'lousy hair',
     'nits',
     'pediculus capitis infestation',
     'lice infested hair',
     'nit infested hair',
     'head louse infestation',
     'head lice',
     'malignant tumour of thoracic part of oesophagus',
     'malignant neoplasm of thoracic esophagus',
     'malignant neoplasm of thoracic oesophagus',
     'cephalhematoma due to birth trauma',
     'cephalohematoma due to birth trauma',
     'foetal cephalhaematoma',
     'cephalohematoma',
     'cephalhematoma',
     'fetal cephalhaematoma',
     'cephalhaematoma',
     'cephalohaematoma',
     'coronary angioplasty',
     'repair of coronary artery',
     'impaired mobility',
     'physical mobility impairment',
     'contusion of chest',
     'superficial bruising of chest wall',
     'inflammatory bowel disease',
     'IBd',
     'closed fracture of phalanx of foot',
     'closed fracture of phalanx of toe',
     'closed fracture of toe',
     'vitreous floaters',
     'muscae volitantes',
     'musca volitans',
     'floaters in visual field',
     'showers of floaters',
     'floaters',
     'chronic pelvic pain of female',
     'chronic pain in female pelvis',
     'chronic pelvic pain',
     'abscess of breast',
     'ophthalmic migraine',
     'ocular migraine',
     'metatarsal bone fracture',
     'fracture of metatarsal bone',
     'rheumatic mitral stenosis',
     'rheumatic mitral valve obstruction',
     'opioid withdrawal',
     'narcotic withdrawal',
     'helicobacter pylori-associated gastritis',
     'gastritis caused by helicobacter pylori',
     'helicobacter-associated gastritis',
     'multiple pregnancy',
     'tuberculosis of pleura',
     'tuberculous pleuritis',
     'tuberculous pleurisy',
     'acute viral pharyngitis',
     'meibomian gland dysfunction',
     'posterior blepharitis',
     'mgd-meibomian gland dysfunction',
     'senile angioma',
     'campbell de morgan spot',
     'campbell de morgan angioma',
     'blood spots on skin',
     'senile nevus of skin',
     'senile naevus of skin',
     'senile ectasia',
     'cherry angioma',
     'de morgans spots',
     'vitamin b12 deficiency (non anaemic)',
     'cyanocobalamine deficiency (non anemic)',
     'cyanocobalamine deficiency (non anaemic)',
     'major depression single episode, in partial remission',
     'major depression, single episode, in partial remission',
     'acute type b viral hepatitis',
     'acute hepatitis b',
     'cardiogenic shock',
     'blindness of one eye',
     'blind eye',
     'blindness, unilateral',
     'blind or low vision - one eye only',
     'first degree atrioventricular block',
     'first degree heart block',
     'incomplete atrioventricular block, first degree',
     'osgood schlatter disease',
     'juvenile osteochondrosis of tibial tubercle',
     'osgood-schlatters disease',
     'osgood schlatters disease',
     'tibia vara',
     'blounts disease',
     'non-rachitic bowleg',
     'aseptic necrosis of medial condyle of tibia',
     'osteochondrosis deformans tibiae',
     'tibial tubercle juvenile osteochondritis',
     'closed fracture of nasal bones',
     'closed fracture nasal bone',
     'closed fracture nose',
     'undescended testicle',
     'cryptorchism',
     'cryptorchidism',
     'UDt',
     'undescended testes',
     'undescended testis',
     'maldescent of testis',
     'imperfectly descended testis',
     'IDt',
     'vocal cord palsy',
     'vocal cord paresis',
     'vocal cord paralysis',
     'VCp',
     'paralysis of vocal cords',
     'vocal fold palsy',
     'malignant tumour of oral cavity',
     'malignant tumour of mouth',
     'malignant tumor of mouth',
     'cancer of oral cavity',
     'ca - mouth cancer',
     'mouth cancer',
     'malignant neoplasm of mouth',
     'retinal dialysis',
     'retinal round hole',
     'retinal hole',
     'retinal atrophic hole',
     'hole in retina',
     'retinal defect',
     'retinal break',
     'retinal tear',
     'diffuse interstitial pulmonary fibrosis',
     'usual interstitial pneumonitis',
     'UIp',
     'DIPf',
     'fibrosing alveolitis',
     'acute interstitial pneumonia',
     'hamman rich syndrome',
     'primary atypical interstitial pneumonia',
     'atypical interstitial pneumonia',
     'primary atypical pneumonia',
     'acute interstitial pneumonitis',
     'atypical bronchopneumonia',
     'familial idiopathic pulmonary fibrosis',
     'idiopathic pulmonary fibrosis',
     'usual interstitial pneumonia',
     'idiopathic fibrosing alveolitis',
     'cryptogenic fibrosing alveolitis',
     'placement of stent in coronary artery',
     'insertion of coronary artery stent',
     'bacterial pneumonia',
     'uncomplicated arteriosclerotic dementia',
     'multi-infarct dementia, uncomplicated',
     'vascular dementia, uncomplicated',
     'multi infarct dementia, uncomplicated',
     'perirectal abscess',
     'cervical myelopathy',
     'cervical intraepithelial neoplasia grade 2',
     'moderate cervical dysplasia',
     'cervical intraepithelial neoplasia grade ii',
     'cin 2 - cervical intraepithelial neoplasia 2',
     'moderate dysplasia of cervix',
     'intradermal naevus',
     'dermal nevus',
     'dermal naevus',
     'dermal cellular naevus',
     'cellular nevus',
     'intradermal melanocytic nevus',
     'IDn',
     'intradermal nevus',
     'cellular naevus',
     'intradermal melanocytic naevus',
     'malignant tumor of cecum',
     'cancer of caecum',
     'malignant neoplasm of caecum',
     'malignant neoplasm of cecum',
     'cancer of cecum',
     'red eye',
     'total nephrectomy',
     'simple nephrectomy',
     'total excision of kidney',
     'nephrectomy',
     'nephrectomies',
     'cerebral infarction due to embolism of cerebral arteries',
     'panniculitis',
     'maculopapular eruption',
     'maculopapular exanthema',
     'maculopapular rash',
     'soy protein sensitivity',
     'low back strain',
     'lumbar strain',
     'weakness of limb',
     'loss of power in limb',
     'diabetic glomerulopathy',
     'diabetic intracapillary glomerulosclerosis',
     'kimmelstiel-wilson syndrome',
     'nodular glomerulosclerosis',
     'diabetic glomerulosclerosis',
     'magnetic resonance imaging of brain abnormal',
     'infiltrating duct carcinoma of breast',
     'invasive duct carcinoma of breast',
     'infiltrating ductal carcinoma of breast',
     'invasive ductal carcinoma of breast',
     'infiltrating ductular carcinoma',
     'autologous peripheral blood stem cell transplant',
     'affective psychosis',
     'hypertensive heart disease with congestive heart failure',
     'psychogenic impotence',
     'aphakia',
     'absence of lens',
     'medial epicondylitis',
     'medial epicondylitis of elbow joint',
     'medial epicondylitis of elbow',
     'golfers elbow',
     'contusion of back',
     'superficial bruising of back',
     'open wound of hand',
     'diastolic dysfunction',
     'history of malignant neoplasm of kidney',
     'h/o: malignant neoplasm of kidney',
     'personal history of primary malignant neoplasm of kidney',
     'asteatosis cutis',
     'xerosis cutis',
     'secondary malignant neoplasm of skin',
     'metastatic malignant neoplasm to skin',
     'cancer metastatic to skin',
     'dermal metastasis',
     'cutaneous metastasis',
     'malignant infiltration of skin',
     'skin secondaries',
     'secondary cancer of skin',
     'metastasis to skin',
     'high risk sexual behaviour',
     'unsafe sexual practices',
     'h/o: lower limb amputation',
     'history of lower limb amputation',
     'chest injury',
     'shoulder strain',
     'diabetic macular oedema',
     'cerebral infarction',
     'sinus headache',
     'sinus pain',
     'candidiasis of skin',
     'candidosis of skin',
     'candida of skin',
     'closed fracture of base of neck of femur',
     'closed fracture of cervicotrochanteric section of femur',
     'monoparesis - leg',
     'weakness of leg',
     'toxic diffuse goitre',
     'toxic diffuse thyroid goitre',
     'toxic diffuse thyroid goiter',
     'thyrotoxicosis with diffuse goitre',
     'diffuse toxic goitre',
     'toxic primary thyroid hyperplasia',
     'thyrotoxicosis with diffuse goiter',
     'diffuse toxic goiter',
     'thyroid disease in pregnancy',
     'acute appendicitis with generalised peritonitis',
     'rheumatic mitral regurgitation',
     'mitral incompetence - rheumatic',
     'rheumatic mitral incompetence',
     'rheumatic mitral insufficiency',
     'chronic neurogenic ulcer of lower limb',
     'trophic leg ulcer',
     'neuropathic leg ulcer',
     'neurogenic leg ulcer',
     'chronic neurogenic ulcer of leg',
     'occlusion of lower limb artery',
     'education, guidance and counselling',
     'teaching, guidance and counselling',
     'teaching, guidance and counseling',
     'non-traumatic intracerebral ventricular haemorrhage',
     'home care of patient',
     'congenital pneumonia',
     'discoid lupus erythematosus',
     'DLe',
     'le - discoid lupus erythematosus',
     'chronic discoid lupus erythematosus',
     'CDLe',
     'tendency to bleed',
     'disorder of hemostatic system',
     'blood coagulation disorder',
     'coagulation disorder',
     'blood clotting disorder',
     'coagulopathy',
     'bleeding diathesis',
     'bleeding tendency',
     'clotting disorder',
     'bleeding disorder',
     'disorder of hemostasis',
     'disorder of haemostasis',
     'neurofibromatosis syndrome',
     'Nf',
     'neurofibromatosis',
     'clinical neurofibromatosis',
     'multiple neurofibromatosis',
     'von recklinghausens disease (except of bone)',
     'recklinghausens disease (except of bone)',
     'heterotropia',
     'strabismus',
     'squint',
     'ocular dissociation',
     'disorder of binocular eye movements',
     'squinting',
     'squints',
     'heterotropy',
     'tetraplegia',
     'quadriplegia',
     'quadriplegias',
     'infection of skin',
     'infection of skin and/or subcutaneous tissue',
     'infective dermatological disorders',
     'skin infection',
     'skin and subcutaneous tissue infection',
     'cannabis dependence in remission',
     'cellulitis of face',
     'postsurgical menopause',
     'surgical menopause',
     'carcinoma of esophagus',
     'equinovarus deformity of foot',
     'talipes equinovarus',
     'congenital talipes equinovarus',
     'clubfoot',
     'TEv',
     'clubfoot - congenital',
     'congenital clubfoot',
     'clubfeet',
     'benign neoplastic disease',
     'neoplasm, benign',
     'benign tumour morphology',
     'benign tumor morphology',
     'unclassified tumour, benign',
     'benign neoplasm',
     'tumor, benign',
     'unclassified tumor, benign',
     'tumour, benign',
     'benign neoplasm, site unspecified',
     'rhabdomyolysis',
     'placenta previa',
     'Pp',
     'placenta praevia',
     'plastic repair of mitral valve',
     'repair of mitral valve',
     'mitral valve repair',
     'mitral valvuloplasty',
     'mitral valvoplasty',
     'valvuloplasty of mitral valve',
     'angioplasty of blood vessel',
     'angioplasty',
     'angioplasties',
     'abdominal discomfort',
     'atherosclerosis of artery',
     'atherosclerosis artery',
     'azotemia',
     'transplantation of pancreas',
     'pancreatic transplant',
     'pancreas transplantation procedures',
     'secondary malignant neoplasm of female breast',
     'metastatic malignant neoplasm to female breast',
     'secondary malignant neoplasm of breast',
     'secondary malignant deposit to breast',
     'metastasis to breast',
     'pneumonia due to klebsiella pneumoniae',
     'poisoning by sedative and/or hypnotic',
     'poisoning by sleeping tablet',
     'poisoning by sleeping drug',
     'poisoning by sleeping pill',
     'essential thrombocythemia',
     'idiopathic thrombocythemia',
     'essential thrombocytosis',
     'idiopathic thrombocythaemia',
     'essential thrombocythemia (clinical disorder)',
     'essential thrombocythaemia',
     'idiopathic hemorrhagic thrombocythemia',
     'essential hemorrhagic thrombocythemia',
     'primary thrombocythaemia',
     'primary thrombocythemia',
     'essential haemorrhagic thrombocythaemia',
     'idiopathic haemorrhagic thrombocythaemia',
     'heart block',
     'Hb',
     'screening for osteoporosis',
     'microcytic anaemia',
     'dental consultation and report',
     'dental examination',
     'severe persistent asthma',
     'hyperplasia of prostate',
     'skin ulcer',
     'cutaneous ulcer',
     'sinus node dysfunction',
     'coronary sinus rhythm disorder',
     'fracture of radius and ulna',
     'reactive depression (situational)',
     'reactive depression',
     'testicular hypofunction',
     'nasal obstruction',
     'nasal obstruction present',
     'nasal airway obstruction',
     'No',
     'coronary artery bypass grafts x 3',
     'triple coronary bypass',
     'cabg x 3 - coronary artery bypass grafts x 3',
     'myringotomy and insertion of tympanic ventilation tube',
     'tympanotomy with intubation',
     'myringotomy and insertion of grommet',
     'benign neoplasm of skin of trunk',
     'atrial fibrillation and flutter',
     'cellulitis and abscess of trunk',
     'influenza due to influenza virus, type b',
     'respiratory distress',
     'distressed breathing',
     'anticoagulant adverse reaction',
     'adverse reaction to anticoagulants',
     'assessment of nutritional status',
     'physical/nutritional assessment',
     'nutritional assessment',
     'assesses nutritional habits and patterns',
     'ostomy care education',
     'teach ostomy care',
     'lead screening',
     'nodular lymphoma of lymph nodes of multiple sites',
     'disorder of gallbladder',
     'gallbladder disease',
     'gallbladder disorder',
     'iga nephropathy',
     'iga glomerulonephritis',
     'immunoglobulin a nephropathy',
     'igan - iga nephropathy',
     'rheumatic heart disease',
     'rheumatic pancarditis',
     'rheumatic carditis',
     'colostomy',
     'colostomy formed',
     'construction of colostomy',
     'colostomy or skin level cecostomy',
     'colostomies',
     'history of malignant neoplasm of cervix',
     'history of cancer of cervix',
     'foot-drop',
     'footdrop',
     'dropfoot',
     'Fd',
     'recurrent hepatitis',
     'chronic hepatitis',
     'Ch',
     'blood chemistry abnormal',
     'creation of ileostomy',
     'ileostomy formed',
     'creation of continent ileostomy',
     'continent ileostomy',
     'ileostomy operation',
     'enterostomy of ileum, brooke-dragstedt type',
     'ileostomy procedure',
     'ileostomy',
     'creation of loop ileostomy',
     'loop ileostomy',
     'ileostomy or jejunostomy, non-tube',
     'ileostomies',
     'generally unwell',
     'general feeling of illness',
     'malaise',
     'feels unwell',
     'feels off-colour',
     'feels off-color',
     'feels poorly',
     'not feeling great',
     'vague bodily discomfort',
     'not feeling well',
     'ill-defined experience',
     'does not feel right',
     'nonulcer dyspepsia',
     'functional dyspepsia',
     'non-ulcer dyspepsia',
     'gastric irritation',
     'non ulcer dyspepsia',
     'viral conjunctivitis',
     'calcaneal spur',
     'cholestatic jaundice syndrome',
     'cholestatic jaundice',
     'obstructive hyperbilirubinemia',
     'obstructive jaundice',
     'lupus erythematosus',
     'Le',
     'lupus',
     'laceration of finger',
     'moderate nonproliferative diabetic retinopathy',
     'moderate nonproliferative retinopathy',
     'malignant tumour of head of pancreas',
     'ca head of pancreas',
     'senile osteopenia',
     'osteopenia of the elderly',
     'chronic active hepatitis',
     'CAh',
     'autoimmune chronic active hepatitis',
     'chronic aggressive hepatitis',
     'autoimmune hepatitis',
     'chronic kidney disease stage 1',
     'ckd stage 1',
     'male urinary stress incontinence',
     'rubella status not known',
     'papular eruption',
     'papular rash',
     'papule of skin',
     'low birth weight infant',
     'LBw',
     'low birth weight',
     'case management',
     'neoplasm of uncertain behaviour of soft tissues',
     'neoplasm of uncertain behavior of connective and other soft tissues',
     'neoplasm of uncertain behaviour of connective and other soft tissues',
     'esophageal dysmotility',
     'diffuse spasm of oesophagus',
     'spasm of oesophagus',
     'DOs',
     'diffuse esophageal spasm',
     'curling of esophagus',
     'rosary bead esophagus',
     'pseudo-obstruction of the esophagus',
     'esophageal spasm',
     'Es',
     'es - oesophageal spasm',
     'barsony-teschendorf syndrome',
     'corkscrew esophagus',
     'dyskinesia of esophagus',
     'esophageal motility disorder',
     'esophageal motor disorder',
     'esophagism',
     'esophagospasm',
     'curling esophagus',
     'spasm of esophagus',
     'oesophageal motility disorder',
     'diffuse oesophageal spasm',
     'dyskinesia of oesophagus',
     'Os',
     'oesophageal spasm',
     'corkscrew oesophagus',
     'curling of oesophagus',
     'curling oesophagus',
     'rosary bead oesophagus',
     'oesophagospasm',
     'oesophageal motor disorder',
     'pseudo-obstruction of the oesophagus',
     'oesophagism',
     'factor viii deficiency',
     'hereditary factor viii deficiency disease',
     'congenital factor viii deficiency',
     'hemophilia a',
     'classical hemophilia',
     'ahg deficiency disease',
     'sex-linked factor viii deficiency',
     'congenital factor viii deficiency disease',
     'haemophilia a',
     'classical haemophilia',
     'haemaphilia',
     'haemafilia',
     'liver cyst',
     'hepatic cyst',
     'cyst of liver',
     'adrenal hypofunction',
     'adrenal failure',
     'hypoadrenalism',
     'adrenal cortical hypofunction',
     'adrenocortical hypofunction',
     'hypoadrenocorticism',
     'adrenal insufficiency',
     'corticoadrenal insufficiency',
     'disorder of peripheral autonomic nervous system',
     'prolactinoma',
     'prolactin-secreting pituitary adenoma',
     'malignant myopia',
     'degenerative progressive high myopia',
     'degenerative myopia',
     'solar degeneration',
     'sun damaged skin',
     'dermatoheliosis',
     'actinic degeneration of skin',
     'photoageing',
     'actinic ageing',
     'actinic aging',
     'photoaging',
     'actinic degeneration',
     'actinic elastosis',
     'sailors skin',
     'farmers skin',
     'solar elastosis',
     'hodgkins disease, nodular sclerosis (clinical)',
     'hodgkins disease, nodular sclerosis',
     'hodgkin disease, nodular sclerosis',
     'hodgkin lymphoma, nodular sclerosis',
     'classical hodgkin lymphoma, nodular sclerosis',
     'fh: hypertension',
     'family history of hypertension',
     'family history: hypertension',
     'single live birth',
     'closed fracture of lumbar vertebra without spinal cord injury',
     'fracture of radial head',
     'high birth weight',
     'high birth weight baby',
     'removal of ear wax',
     'ear wax removal',
     'carotid endarterectomy',
     'carotid thromboendarterectomy',
     'CEa',
     'displacement of intervertebral disc without myelopathy',
     'displacement of intervertebral disc, site unspecified without myelopathy',
     'local infection of wound',
     'infected wound',
     'wound infected',
     'wound infection',
     'septic wound',
     'tubal pregnancy',
     'fallopian tube pregnancy',
     'fallopian pregnancy',
     'disorder of connective tissue',
     'connective tissue disease',
     'fetal or neonatal effect of placenta praevia',
     'foetal or neonatal effect of placenta praevia',
     'malposition and malpresentation of fetus',
     'malposition and malpresentation of foetus',
     'vernal conjunctivitis',
     'spring conjunctivitis',
     'spring ophthalmia',
     'family planning education, guidance, and counselling',
     'family planning teaching, guidance, and counselling',
     'family planning teaching, guidance, and counseling',
     'family planning care teaching, guidance, and counselling',
     'family planning care teaching, guidance, and counseling',
     'acute appendicitis with peritoneal abscess',
     'injury of brachial plexus',
     'traumatic brachial plexus lesion',
     'brachial plexus injury',
     'eosinophil count raised',
     'eosinophilia',
     'hypospadias',
     'aneurysm',
     'aneurysmal dilatation',
     'aneurysms',
     'cellulitis of hand',
     'idiopathic pulmonary arterial hypertension',
     'occlusion of ureter',
     'ureteral obstruction',
     'obstruction of ureter',
     'intrinsic ureteral obstruction',
     'ureteric obstruction',
     'lobar pneumonia',
     'psychomotor epilepsy',
     'complex partial epileptic seizure',
     'chronic respiratory failure',
     'proliferation of myeloid cells',
     'myeloproliferative disorder',
     'subclinical hypothyroidism',
     'crohns disease of small intestine',
     'crohn disease of small intestine',
     'crohns disease, small intestine',
     'regional enteritis of small intestine',
     'regional ileitis of small intestine',
     'segmental ileitis of small intestine',
     'removal of intrauterine device',
     'removal of intrauterine contraceptive device',
     'removal of coil',
     'removing coil',
     'intrauterine contraceptive device removed',
     'iucd -removal of intrauterine contraceptive device',
     'iud - removal of intrauterine device',
     'removal of intrauterine device (iud)',
     'acute lymphoid leukaemia in remission',
     'repair of inguinal hernia',
     'inguinal hernioplasty',
     'inguinal herniorrhaphy',
     'capsular fibrosis',
     'after-cataract',
     'secondary cataract',
     'stopped smoking',
     'partial epilepsy with impairment of consciousness',
     'peripheral circulatory disorder associated with type 1 diabetes mellitus',
     'peripheral circulatory disorder associated with type i diabetes mellitus',
     'hypertensive heart failure',
     'repair of rotator cuff by suture',
     'repair of tendon rotator of cuff by direct suture',
     'suture of tendon of rotator cuff',
     'suture of supraspinatus tendon for rotator cuff repair',
     'immunosuppressive therapy',
     'immunosuppression',
     'immunosuppressions',
     'ischemic colitis',
     'colonic ischaemia',
     'colonic ischemia',
     'cystic acne',
     'acne cystica',
     'cystic acne vulgaris',
     'mortons metatarsalgia',
     'mortons neuroma',
     'interdigital neuralgia',
     'interdigital neuroma',
     'morton metatarsalgia',
     'morton neuroma',
     'mortons neuralgia',
     'mortons neuroma',
     'mortons toe',
     'mortons disease',
     'bladder neck obstruction',
     'BNo',
     'vesicourethral orifice obstruction',
     'bladder outlet obstruction',
     'closed fracture of rib',
     'closed fracture rib',
     'injury of foot',
     'foot injury',
     'female hypogonadism syndrome',
     'female hypogonadism',
     'persistent vomiting',
     'emesis - persistent',
     'uncontrollable vomiting',
     'osteoarthritis of ankle',
     'degenerative joint disease of ankle',
     'ulcer of heel',
     'pain in the coccyx',
     'coccyalgia',
     'coccydynia',
     'coccygalgia',
     'coccyodynia',
     'intervertebral disc disorder',
     'disorder of intervertebral disc',
     'urolith',
     'urinary calculus',
     'urinary stone',
     'dermatomyositis',
     'wagner-unverricht syndrome',
     'polymyositis with skin involvement',
     'periodic limb movement disorder',
     'closed fracture of fibula',
     'ganglion of wrist',
     'ganglion cyst of wrist',
     'recurrent falls',
     'tendinitis of wrist',
     'tendonitis of wrist',
     'gingivitis',
     'sle glomerulonephritis syndrome',
     'lupus nephritis',
     'systemic lupus erythematosus glomerulonephritis syndrome',
     'rheumatic mitral stenosis with regurgitation',
     'rheumatic mitral stenosis with incompetence',
     'rheumatic mitral stenosis with insufficiency',
     'disorder of skin and/or subcutaneous tissue',
     'skin and subcutaneous tissue disease',
     'disorder of the dermis and subcutaneous tissue',
     'disorder of skin and subcutaneous tissue',
     'augmentation mammoplasty',
     'breast augmentation',
     'mammaplasty, augmentation; without prosthetic implant',
     'seasonal allergic conjunctivitis',
     'hay fever conjunctivitis',
     'low maternal weight gain',
     'collapse of cervical vertebra due to osteoporosis',
     'easy bruising',
     'increased tendency to bruise',
     'bruises easily',
     'does bruise easily',
     'drug-induced disorder of liver',
     'erectile dysfunction associated with type 2 diabetes mellitus',
     'parasuicide',
     'suicide attempt by inadequate means',
     'suicide gesture',
     'unsuccessful suicide attempt',
     'bloodstained sputum',
     'expectoration of blood tinged sputum',
     'congenital deviation of nasal septum',
     'deviated nasal septum - congenital',
     'congenital nasal septum deviation',
     'recurrent pyogenic cholangitis',
     'cluster headache',
     'cluster headache syndrome',
     'migrainous neuralgia',
     'hortons headache',
     'vasomotor headache',
     'histamine cephalgia',
     'neuralgic migraine',
     'histamine headache',
     'hortons neuralgia',
     'ciliary neuralgia',
     'chronic cluster headache',
     'closed bimalleolar fracture',
     'ventricular fibrillation',
     'cardiac arrest - ventricular fibrillation',
     'Vf',
     'neoplasm of pituitary gland',
     'pituitary tumour',
     'tumour of pituitary gland',
     'pituitary tumor',
     'tumor of pituitary gland',
     'myoclonic disorder',
     'disorder characterized by myoclonus',
     'myoclonus disorder',
     'disorder characterised by myoclonus',
     'myoclonus',
     'senile osteoporosis',
     'age related osteoporosis',
     'type ii osteoporosis',
     'involutional osteoporosis',
     'pressure ulcer of heel',
     'pressure ulcer on heel',
     'pressure sore on heel',
     'cytomegalovirus infection',
     'CMv',
     'disease due to cytomegalovirus',
     'cytomegalic inclusion disease',
     'salivary gland virus disease',
     'closed fracture of shaft of radius',
     'closed fracture of the radial shaft',
     'addisons disease',
     'addison disease',
     'primary adrenocortical insufficiency',
     'primary hypoadrenalism',
     'adisons disease',
     'hypocorticism',
     'thalassemia',
     'hereditary leptocytosis',
     'thalassaemia syndrome',
     'diastolic heart failure',
     'bullous pemphigoid',
     'pemphigoid',
     'closed fracture of shaft of humerus',
     'closed fracture of humerus, shaft',
     'removal of foreign body',
     'deficiency of macronutrients',
     'protein calorie malnutrition',
     'protein-energy malnutrition',
     'PCm',
     'protein-calorie malnutrition',
     'protein-calorie undernutrition',
     'PEm',
     'astrocytoma of brain',
     'astrocytic tumor of brain',
     'astrocytic tumour of brain',
     'congenital bladder neck stenosis',
     'BNs',
     'bladder neck stenosis',
     'abscess of liver',
     'hepatic abscess',
     'hyperhomocysteinemia',
     'neonatal jaundice associated with preterm delivery',
     'jaundice due to delayed conjugation associated with preterm delivery',
     'hyperbilirubinemia of prematurity',
     'preterm delivery-associated jaundice',
     'neonatal jaundice after preterm delivery',
     'hyperbilirubinaemia of prematurity',
     'crohns disease of colon',
     'Cc',
     'crohns colitis',
     'granulomatous colitis',
     'crohn disease of colon',
     'crohns disease, colon',
     'regional colitis',
     'crohns disease of large bowel',
     'regional enteritis of the large bowel',
     'crohn disease of large bowel',
     'crohns disease, large intestine',
     'gastric polyp',
     'polyp of stomach',
     'gastric polyposis',
     'gastritis polyposa',
     'multiple gastric polyps',
     'secondary hypothyroidism',
     'TSh',
     'thyroid stimulating hormone deficiency',
     'pituitary hypothyroidism',
     'tsh deficiency',
     'hypothyrotropic hypothyroidism',
     'subsequent myocardial infarction',
     'reinfarction of myocardium',
     'generalised epilepsy',
     'open fracture of one or more phalanges of hand',
     'amniotic fluid - bloodstained',
     'bloodstained liquor',
     'bloodstained amniotic fluid',
     'multiparous',
     'active range of joint movement reduced',
     'malignant essential hypertension',
     'accelerated essential hypertension',
     'prostatectomy',
     'prostate excision',
     'excision procedures on the prostate',
     'pericarditis',
     'vascular dementia',
     'multi-infarct dementia',
     'multi infarct dementia',
     'MId',
     'arteriosclerotic dementia',
     'VAd',
     'ischaemic heart disease screening',
     'transplantation of bone marrow',
     'bone marrow graft',
     'BMt',
     'bone marrow transplant',
     'adrenal mass',
     'mass of adrenal gland',
     'shoulder joint unstable',
     'shoulder joint instability',
     'proctitis',
     'bruit',
     'bruits',
     'optic atrophy',
     'optic nerve atrophy',
     'testicular mass',
     'testicular lump',
     'mass of testis',
     'lump in testis',
     'mass of testicle',
     'crushing injury of finger',
     'crush injury of finger',
     'crush injury to finger',
     'neoplasm of liver',
     'tumour of liver',
     'hepatic tumour',
     'hepatic tumor',
     'tumor of liver',
     'burn of hand',
     'short bowel syndrome',
     'SGs',
     'short gut syndrome',
     'acquired short bowel syndrome',
     'short intestine syndrome',
     'post-resection malabsorption',
     'SBs',
     'migraine without aura, not refractory',
     'common migraine, not intractable',
     'migraine without aura, non-intractable',
     'malignant neoplasm of brain',
     'replacement of aortic valve with tissue graft',
     'repair of aortic valve with tissue graft',
     'biological aortic valve replacement',
     'bioprosthetic aortic valve replacement',
     'tissue aortic valve replacement',
     'pes anserinus bursitis',
     'chronic gastritis',
     'Cg',
     'benign neoplasm of prostate',
     'benign tumour of prostate',
     'benign prostatic tumour',
     'benign prostatic tumor',
     'benign tumor of prostate',
     'malignant neoplasm of corpus uteri, excluding isthmus',
     'perinatal jaundice due to galactosemia',
     'upper airway resistance syndrome',
     'malignant neoplasm of gastrointestinal tract',
     'closed fracture of radius and ulna',
     'closed fracture of the radius and ulna',
     'traumatic arthropathy',
     'traumatic arthritis',
     'posttraumatic arthropathy',
     'unilateral recurrent inguinal hernia',
     'current tear of lateral cartilage and/or meniscus of knee',
     'cholesteatoma',
     'epidermoid cholesteatoma',
     'infectious neuronitis',
     'acute infective polyneuritis',
     'post-infectious polyneuritis',
     'guillain-barré syndrome',
     'acute inflammatory demyelinating polyradiculoneuropathy',
     'acute idiopathic polyradiculoneuritis',
     'acute idiopathic polyneuritis',
     'guillain barre syndrome',
     'acute post-infective radiculoneuropathy',
     'acute inflammatory neuropathy',
     'landry-guillain-barré syndrome',
     'ascending paralysis',
     'landry-guillain-barre syndrome',
     'guillain-barre syndrome',
     'systolic hypertension',
     'sprain of medial collateral ligament of knee',
     'seasonal affective disorder',
     'SAd',
     'lichen sclerosus et atrophicus',
     'lichen sclerosus',
     'white spot disease',
     'pain in thoracic spine',
     'screening for cardiovascular system disease',
     'tear of medial meniscus of knee',
     'torn medial meniscus',
     'tear of medial meniscus of knee joint',
     'h/o: atrial fibrillation',
     'history of atrial fibrillation',
     'total shoulder replacement',
     'prosthetic total arthroplasty of the shoulder',
     'TSr',
     'retinal detachment with retinal defect',
     'coronary artery bypass grafts x 4',
     'cabg x 4 - coronary artery bypass grafts x 4',
     'ingrowing nail with infection',
     'ingrowing toenail with infection',
     'skin ulcer of calf',
     'paresthesia of hand',
     'premature infant',
     'acute atopic conjunctivitis',
     'angeluccis syndrome',
     'critical allergic conjunctivitis syndrome',
     'acute allergic conjunctivitis',
     'history and physical examination, occupation',
     'employment medical examination report',
     'occupational health examination',
     'poisoning by aromatic analgesic',
     'major depression, single episode, in complete remission',
     'major depression, single episode, in full remission',
     'anaemia during pregnancy - baby not yet delivered',
     'maternal anaemia in pregnancy, before birth',
     'maternal anemia in pregnancy, before birth',
     'preauricular sinus',
     'congenital preauricular sinus',
     'funny turn',
     'funny spell',
     'bad turn',
     'livebirth',
     'sexual assault',
     'contraceptive use education',
     'adverse effect, due to correct medicinal substance properly administered',
     'urostomy management and care',
     'stoma management and care of urinary tract',
     'spinal cord compression',
     'compression of spinal cord',
     'mammography',
     'radiographic examination of breast',
     'mammogram',
     'mamogram',
     'mammograms',
     'regional ileocolitis',
     'crohns disease of small and large intestines',
     'crohn disease of small and large intestines',
     'crohns disease of small intestine and colon',
     'endometriosis interna',
     'endometriosis of myometrium',
     'endometriosis of uterus',
     'internal endometriosis',
     'adenomyosis',
     'adenomyosis uteri',
     'complete transurethral resection of prostate, including control of postoperative bleeding',
     'inflammatory disorder of muscle',
     'inflammatory myopathy',
     'myositis',
     'muscle inflammation',
     'retinal hemorrhage',
     'retinal haemorrhages',
     'retinal hemorrhages',
     'intraretinal haemorrhage',
     'intraretinal hemorrhage',
     'polyneuropathy',
     'polyneuropathy (multiple nerve disorder)',
     'acquired polyneuropathy',
     'acquired stenosis of nasolacrimal duct',
     'acquired tear duct stenosis',
     'acquired nasolacrimal duct stenosis',
     'acquired dacryostenosis',
     'tooth disorder',
     'disease of teeth',
     'dental disorder',
     'tooth disease',
     'dental disease',
     'sprain of knee and leg',
     'intrauterine device check',
     'malignant neoplasm of uterus',
     'malignant tumour of uterus',
     'malignant tumor of uterus',
     'cancer of uterus',
     'uterine cancer',
     'acanthosis nigricans',
     'acquired acanthosis nigricans',
     'keratosis nigricans',
     'fracture of tibia and fibula',
     'functional heart murmur',
     'functional flow murmur',
     'functional cardiac murmur',
     'endometritis',
     'primary malignant neoplasm of skin of face',
     'paroxysmal tachycardia',
     'Pt',
     'bouveret-hoffmann syndrome',
     'essential paroxysmal tachycardia',
     'polyp of vocal cord or larynx',
     'sacroiliac joint pain',
     'arthralgia of sacroiliac joint',
     'pulmonary sarcoidosis',
     'sarcoidosis of lung',
     'blister',
     'blister of skin and/or mucosa',
     'blistered',
     'congenital dislocation of hip',
     'developmental displacement of the hip',
     'CDh',
     'congenital dislocation of the hip',
     'developmental dislocation of hip',
     'congenital hip dysplasia',
     'developmental dysplasia of the hip',
     'congenital acetabular dysplasia',
     'congenital dysplasia of the hip',
     'dysplasia of acetabulum',
     'cellulitis of toe',
     'secondary malignant neoplasm of soft tissues',
     'metastasis to soft tissue',
     'secondary malignant neoplasm of connective and other soft tissues',
     'metastatic malignant neoplasm to connective and other soft tissues',
     'neonatal jaundice due to delayed conjugation',
     'management of drug regimen',
     'medication actions',
     'medication management',
     'procedure related to management of drug administration',
     'manage medication administration',
     'manage medication treatment',
     'medication administration case management',
     'drug administration',
     'nondependent cocaine abuse in remission',
     'placenta previa with hemorrhage',
     'hemorrhage from placenta previa',
     'haemorrhage from placenta praevia',
     'fetal or neonatal effect of maternal oligohydramnios',
     'foetal or neonatal effect of maternal oligohydramnios',
     'fetal or neonatal effect of oligohydramnios',
     'foetal or neonatal effect of oligohydramnios',
     'instrumental delivery',
     'gallbladder and bile duct calculi',
     'gallbladder and bile duct stones',
     'secondary hypertension',
     'postural orthostatic tachycardia syndrome',
     'pneumonia due to gram negative bacteria',
     'pneumonia due to gram-negative bacteria',
     'gram negative pneumonia',
     'periodic leg movements of sleep',
     'epiphora',
     'tearing',
     'excessive tear production',
     'crocodile tears',
     'haematopoietic aplasia',
     'aplastic anemia',
     'erythroid aplasia',
     'diet education',
     'advice about diet',
     'advice on diet',
     'teach regular diet',
     'diet health education',
     'dietary advice',
     'injury of shoulder region',
     'diabetes insipidus',
     'esotropia with accommodative compensation',
     'accommodative esotropia',
     'accommodative component in esotropia',
     'hepatitis c carrier',
     'pain in penis',
     'penile pain',
     'facial laceration',
     'closed fracture of humerus',
     'closed fracture of upper arm',
     'thrombophlebitis',
     'cerebellar ataxia',
     'cerebellar ataxia (loss of muscle coordination)',
     'hyposecretion of salivary gland',
     'salivary hyposecretion',
     'hyposalivation',
     'hypoactive salivary flow',
     'inadequate salivary flow',
     'aptyalism',
     'xerostomia',
     'absent salivary secretion',
     'aptyalia',
     'asialia',
     'dry mouth',
     'clinical xerostomia',
     'traumatic amputation of finger',
     'health examination of sub-group',
     'secondary malignant neoplasm of brain and spinal cord',
     'benign neoplasm of eyelid including canthus',
     'mass of thyroid gland',
     'thyroid lump',
     'thyroid mass',
     'colostomy management and care',
     'h/o: tubal ligation',
     'history of tubal ligation',
     'lumpectomy of breast',
     'tylectomy of breast',
     'excision of breast lump',
     'breast lumpectomy',
     'tylectomy',
     'lumpectomy',
     'lumpectomies',
     'chronic angle-closure glaucoma',
     'CNAg',
     'chronic narrow angle glaucoma',
     'chronic closed-angle glaucoma',
     'anatomical narrow angle glaucoma',
     'narrow cleft glaucoma',
     'angle-closure glaucoma',
     'ACg',
     'angle closure glaucoma',
     'narrow-angle glaucoma',
     'narrow angle glaucoma',
     'excision of lamina of lumbar vertebra for decompression of spinal cord',
     'decompression laminectomy of lumbar spine',
     'history of cholecystectomy',
     'at risk for infection',
     'potential for infection',
     'child relationship problem',
     'mononucleosis syndrome',
     'up-to-date with immunisations',
     'concussion with less than 1 hour loss of consciousness',
     'cardiac electrophysiology',
     'cardiac electrophysiological study',
     'severe chronic obstructive pulmonary disease',
     'manual pelvic examination',
     'pelvic examination',
     'blunt injury',
     'blunt force injury',
     'blunt trauma',
     'injury whilst engaged in sports activity',
     'endoscopy and biopsy of upper gastrointestinal tract',
     'oesophagogastroduodenoscopy and biopsy',
     'esophagogastroduodenoscopy and biopsy',
     'acute myocardial infarction of lateral wall',
     'acute lateral myocardial infarction',
     'normal menstrual cycle',
     'anterior horn cell disease',
     'thrombosis of arteries of lower extremity',
     'lower limb arterial thrombosis',
     'pruritus ani',
     'perianal itch',
     'anal itch',
     'juvenile chronic arthritis',
     'juvenile arthritis',
     'JCa',
     'juvenile idiopathic arthritis',
     'juvenile seropositive polyarthritis',
     'juvenile rheumatoid arthritis, seropositive',
     'juvenile idiopathic arthritis, polyarthritis, rheumatoid factor positive',
     'juvenile seropositive arthritis',
     'juvenile chronic arthritis, polyarticular seropositive',
     'sodium overload',
     'hypernatremia',
     'na overload',
     'na excess',
     'sodium retention',
     'polysomatizing disorder',
     'briquets disorder',
     'briquets syndrome',
     'briquet syndrome',
     'somatization disorder',
     'fracture of vertebral column',
     'fracture of spinal vertebra',
     'fracture of vertebra',
     'fractured spine',
     'fracture of spine',
     'spinal fracture',
     'gilles de la tourettes syndrome',
     'tourettes syndrome',
     'tourettes disorder',
     'combined vocal and multiple motor tic disorder',
     'gilles de la tourette syndrome',
     'gilles de la tourettes syndrome',
     'postmenopausal osteoporosis',
     'menopausal osteoporosis',
     'type i osteoporosis',
     'type 1 osteoporosis',
     'esr raised',
     'erythrocyte sedimentation rate (esr) raised',
     'erythrocyte sedimentation rate raised',
     'lesion of vulva',
     'post-prandial hypoglycemia',
     'reactive hypoglycaemia',
     'hypoglycemic reaction',
     'hypoglycaemic reaction',
     'onychia of finger',
     'paronychia of finger',
     'perionychia of finger',
     'fingernail infection',
     'postoperative follow-up visit',
     'mycoplasma pneumonia',
     'pneumonia due to mycoplasma pneumoniae',
     'eatons agent pneumonia',
     'infection by pplo',
     'pneumonia due to pplo',
     'mycoplasma pneumoniae pneumonia',
     'mycoplasmal pneumonia',
     'primary atypical pneumonia due to mycoplasma pneumoniae',
     'pap due to mycoplasma pneumoniae',
     'endemic pneumonia',
     'drug resistance to insulin',
     'insulin resistance',
     'disorder of the larynx',
     'disease of larynx',
     'disorder of larynx',
     'vulvitis',
     'adjustment disorder with mixed disturbance of emotions and conduct',
     'adjustment reaction with mixed disturbance of emotion and conduct',
     'hypopituitarism',
     'deficient secretion of one or more pituitary hormones',
     'pituitary insufficiency',
     'pituitary hypofunction',
     'pituitary deficiency',
     'pituitary failure',
     'atherosclerosis of aorta',
     'atherosclerosis aorta',
     'dementia associated with another disease',
     'combined drug dependence, excluding opioids',
     'sprain of ligament of lumbosacral joint',
     'lumbosacral sprain',
     'sprain, lumbosacral ligament',
     'milia',
     'milial cyst',
     'milium',
     'subepidermal keratin cyst',
     'milium cyst',
     'fracture of calcaneus',
     'fracture of os calcis',
     'heel bone fracture',
     'albuminuria',
     'pain in buttock',
     'buttock pain',
     'pygalgia',
     'acute poliomyelitis',
     'polio',
     'poliomyelitis',
     'epidemic acute poliomyelitis',
     'wound',
     'wounds',
     'disorder due to type 1 diabetes mellitus',
     'disorder associated with type i diabetes mellitus',
     'krukenberg tumour',
     'metastatic signet ring cell carcinoma',
     'krukenberg tumor',
     'secondary malignant neoplasm of ovary',
     'metastatic malignant neoplasm to ovary',
     'cancer metastatic to ovary',
     'krukenburg tumor',
     'secondary tumor to ovary',
     'ovarian metastasis',
     'metastasis to ovary',
     'secondary cancer of ovary',
     'secondary tumour to ovary',
     'krukenburg tumour',
     'procedure on endocrine system',
     'unlisted procedure, endocrine system',
     'h/o: vasectomy',
     'history of vasectomy',
     'h/o: head injury',
     'history of head injury',
     'gallbladder calculus with acute cholecystitis and no obstruction',
     'cholelithiasis and acute cholecystitis without obstruction',
     'accidents caused by cutting and piercing instruments or objects',
     'reduction mammoplasty, bilateral',
     'breast reduction, bilateral',
     'bilateral reduction mammoplasty',
     'unsatisfactory ctg tracing',
     'unsatisfactory cardiotochogram tracing',
     'technically poor ctg',
     'administration of steroid',
     'steroid therapy',
     'infected hand',
     'elbow fracture',
     'fracture of elbow',
     'pervasive developmental disorder',
     'pervasive developmental disorder (pdd)',
     'autism spectrum disorder',
     'verbal apraxia',
     'verbal dyspraxia',
     'severe nonproliferative diabetic retinopathy with clinically significant macular edema',
     'severe npdr with csme',
     'refractory migraine without aura',
     'intractable migraine without aura',
     'common migraine without aura, intractable',
     'intractable common migraine',
     'cardiovascular stress test using the dobutamine stress test protocol',
     'finding of first stage of labor',
     'observation of first stage of labour',
     'observation of first stage of labor',
     'first stage of labour',
     'disorder of bone',
     'bone disease',
     'osteopathia',
     'achalasia of esophagus',
     'cardiospasm',
     'achalasia of cardia',
     'lack of reflex relaxation of lower oesophageal sphincter',
     'lack of reflex relaxation of lower esophageal sphincter',
     'sinus tachycardia',
     'venous thrombosis',
     'macrocephaly',
     'macrocephalus',
     'normal pressure hydrocephalus',
     'NPh',
     'low pressure hydrocephalus',
     'hypertensive retinopathy',
     'lupus vulgaris',
     'Lv',
     'tuberculosis lupus exedens',
     'tuberculosis luposa cutis',
     'lupus exedens',
     'tuberculosis cutis luposa',
     'traumatic subdural hemorrhage',
     'traumatic subdural intracranial hemorrhage',
     'traumatic subdural intracranial haemorrhage',
     'subdural haemorrhage following injury',
     'subdural hemorrhage following injury',
     'systolic murmur',
     'Sm',
     'intestinal malabsorption',
     'malabsorption syndrome',
     'malabsorption',
     'demyelinating disease of central nervous system',
     'demyelinating disorders of the central nervous system',
     'demyelinating cns disease',
     'demyelinating disease central nervous system (cns)',
     'optic neuritis',
     'autoimmune thyroiditis',
     'angle-closure glaucoma - borderline',
     'borderline angle closure glaucoma',
     'localized, primary osteoarthritis of the pelvic region and thigh',
     'dysequilibrium syndrome',
     'disequilibrium',
     'dysequilibrium',
     'DEs',
     'macular hole',
     'telangiectasis',
     'telangiectasia',
     'telangiectasia disorder',
     'telangiectases',
     'telangiectasias',
     'post-laminectomy syndrome',
     'failed back syndrome',
     'mass in head or neck',
     'head and neck mass',
     'closed fracture of shaft of tibia',
     'therapeutic drug monitoring assay',
     'disorder of prostate',
     'prostate disease',
     'prostatic disorder',
     'community acquired pneumonia',
     'median neuropathy',
     'median nerve palsy',
     'leg length inequality',
     'lower limb length difference',
     'abscess of bartholins gland',
     'vulvovaginal gland abscess',
     'bartholins abscess',
     'abscess of bartholin gland',
     'pulmonary insufficiency following trauma',
     'adult respiratory distress syndrome',
     'congestive atelectasis',
     'danang lung',
     'vietnam lung',
     'shock lung',
     'ARDs',
     'post-traumatic pulmonary insufficiency',
     'traumatic wet lung',
     'adult hyaline membrane disease',
     'acquired respiratory distress syndrome',
     'pulmonary capillary leak syndrome',
     'atypical facial pain',
     'acute myeloid leukaemia in remission',
     'benign neoplasm of cerebral meninges',
     'red throat',
     'tuberculosis of lung, confirmed histologically',
     'acute gastric mucosal erosion',
     'acute erosion of stomach',
     'acute erosive gastritis',
     'cellulitis and abscess of finger',
     'assault by striking by blunt or thrown object',
     'delivery procedure',
     'birthing procedure',
     'delivery - action',
     'human parturition, function',
     'parturition',
     'childbirth',
     'pregnancy delivered',
     'delivery',
     'unlisted procedure, maternity care and delivery',
     'deliveries',
     'paraesthesia of foot',
     'h/o: disorder',
     'history of disorder',
     'carcinoma of middle lobe, bronchus or lung',
     'uterine size for dates discrepancy',
     'uterine size date discrepancy',
     'gravid uterus size for dates discrepancy',
     'underweight',
     'patient underweight',
     'recurrent dislocation of shoulder region',
     'recurrent dislocation of shoulder',
     'recurrent dislocation of the shoulder joint',
     'ectropion',
     'everted margin',
     'eversion of the eyelid',
     'ectropion of eyelid',
     'eyelashes turned out',
     'eyelid turned out',
     'eyelid everted',
     'injury of ankle',
     'ankle injury',
     'obsessive-compulsive personality trait',
     'obsessive compulsive personality disorder',
     'anancastic personality disorder',
     'anankastic personality disorder',
     'generalised non-convulsive epilepsy',
     'generalized nonconvulsive epilepsy',
     'generalised nonconvulsive epilepsy',
     'acute serous otitis media',
     'acute non-suppurative otitis media - serous',
     'acquired spondylolysis',
     'family problems',
     'communicating hydrocephalus',
     'breast implantation',
     'insertion of prosthesis for breast',
     'insertion of breast prosthesis',
     'disorder of pancreas',
     'precocious sexual development',
     'sexual precocity',
     'precocious puberty',
     'pulmonary aspiration',
     'aspiration',
     'aspirations',
     'aspirated',
     'contusion of elbow',
     'superficial bruising of elbow',
     'contusion, elbow area',
     'cramp in lower leg associated with rest',
     'night cramps',
     'nocturnal lower leg cramp',
     'sleep related lower leg cramp',
     'chronic retention of urine',
     'CRu',
     'facet joint pain',
     'facet joint syndrome',
     'elderly primigravida',
     'elderly primip',
     'lesion of brain',
     'cyst of pancreas',
     'pancreatic cyst',
     'malignant tumour of descending colon',
     'ca descending colon',
     'already on aspirin',
     'caretaking/parenting skills education, guidance, and counselling',
     'caretaking/parenting skills teaching, guidance, and counselling',
     'caretaking/parenting skills teaching, guidance, and counseling',
     'alcoholic fatty liver',
     'alcoholic fatty liver disease',
     'mediastinal lymphadenopathy',
     'paralytic ileus',
     'adynamic ileus',
     'paralysis of intestine',
     'moderate depressed bipolar i disorder',
     'moderate bipolar i disorder, most recent episode depressed',
     'burn of lower limb',
     'leg - burn',
     'leg burns',
     'secondary hyperparathyroidism',
     'melanocytic naevus of face',
     'neoplasm of ovary',
     'tumour of ovary',
     'ovarian tumour',
     'ovarian tumor',
     'tumor of ovary',
     'attention to ileostomy',
     'legal problem',
     'acute pelvic inflammatory disease',
     'acute pid - acute pelvic inflammatory disease',
     'acute pelvic inflammatory disease of the female pelvic organs and/or tissues',
     'pid - acute pelvic inflammatory disease',
     'vaginal tear resulting from childbirth',
     'vaginal tear during delivery',
     'cannabis type drug dependence',
     'adjust hearing aid settings',
     'at risk for deliberate self harm',
     'at risk of dsh - deliberate self harm',
     'acute non-st segment elevation myocardial infarction',
     'NSTEMi',
     'non-st segment elevation mi',
     'arthropathy of spinal facet joint',
     'arthropathy of facet joint',
     'drug-induced mood disorder',
     'drug-induced affective disorder',
     'drug induced mood disorder',
     'drug induced affective syndrome',
     'rheumatic tricuspid valve regurgitation',
     'rheumatic tricuspid valve insufficiency',
     'rheumatic tricuspid valve incompetence',
     'tricuspid incompetence - rheumatic',
     'rheumatic tricuspid regurgitation',
     'rheumatic tricuspid insufficiency',
     'primary localised osteoarthrosis of pelvic region',
     'rheumatic aortic regurgitation',
     'aortic incompetence - rheumatic',
     'rheumatic aortic incompetence',
     'rheumatic aortic insufficiency',
     'pleural effusion associated with pulmonary infection',
     'parapneumonic effusion',
     'secondary malignant neoplasm of colon',
     'metastatic malignant neoplasm to colon',
     'subcapsular cataract',
     'microcephalus',
     'microcephaly',
     'micrencephaly',
     'nanocephaly',
     'erythema nodosum',
     'En',
     'sinus bradycardia',
     'postpartum depression',
     'puerperal depression',
     'renovascular hypertension',
     'hypertension due to renovascular disease',
     'aneurysm of iliac artery',
     'suprapubic pain',
     'hypogastric pain',
     'pain of hypogastrium',
     'chondrocalcinosis due to pyrophosphate crystals',
     'pseudogout',
     'calcium pyrophosphate deposition disease',
     'calcium pyrophosphate arthritis and periarthritis',
     'CPPd',
     'CPDd',
     'chondrocalcinosis',
     'pain in calf',
     'calf pain',
     'encopresis',
     'closed fracture of scaphoid bone of wrist',
     'closed fracture of the scaphoid',
     'closed fracture of navicular bone of wrist',
     'closed fracture of facial bone',
     'closed fracture of olecranon process of ulna',
     'circumcision',
     'prepucectomy',
     'circumscision',
     'antisocial personality disorder',
     'psychopathic personality disorder',
     'dissocial personality disorder',
     'sociopathic personality disorder',
     'psychopathy',
     'psychopathies',
     'open wound of lower limb',
     'polycystic kidney disease, adult type',
     'autosomal dominant adult polycystic kidney disease',
     'autosomal dominant polycystic kidney disease',
     'ADPKd',
     'polycystic kidneys - adult type',
     'factor v leiden mutation',
     'factor 5 leiden mutation',
     'ketoacidosis in type 2 diabetes mellitus',
     'ketoacidosis in type ii diabetes mellitus',
     'diabetes type 2 with ketoacidosis',
     'arteriosclerosis of coronary artery bypass graft',
     'coronary arteriosclerosis of coronary artery bypass graft',
     'peripheral nerve entrapment syndrome',
     'compression neuropathy',
     'nerve entrapment syndrome',
     'trapped nerve',
     'entrapment neuropathy',
     'corneal subepithelial haze',
     'corneal haze',
     'corneal opacity',
     'episodic chronic alcoholism',
     'aortic incompetence, non-rheumatic',
     'aortic valve regurgitation, nonrheumatic',
     'aortic regurgitation, non-rheumatic',
     'aortic insufficiency, non-rheumatic',
     'closed fracture proximal humerus, greater tuberosity',
     'bladder outflow obstruction',
     'BOo',
     'subretinal neovascularisation',
     'srnv-subretinal neovascularization',
     'cnv-choroidal neovascularization',
     'srnv-subretinal neovascularisation',
     'cnv-choroidal neovascularisation',
     'nvm - subretinal neovascular membrane',
     'subretinal neovascular membrane',
     'presumed subretinal neovascular membrane',
     'carcinoma of pyloric antrum',
     'blow out fracture of orbit',
     'blowout fracture of orbit',
     'scalp laceration',
     'congenital anomaly',
     'developmental abnormality',
     'developmental anomaly',
     'developmental malformation',
     'developmental defect',
     'dysgenesis',
     'anomalous formation',
     'abnormal development',
     'malformation',
     'congenital malformation',
     'fetal developmental abnormality',
     'foetal developmental abnormality',
     'congenital abnormality',
     'foetal malformation',
     'fetal malformation',
     'Cm',
     'congenital deformity',
     'malformations',
     'abscess of tonsil',
     'measles-mumps-rubella vaccination',
     'mmr triple vaccination',
     'mmr vaccination',
     'left ventricular diastolic dysfunction',
     'myofascial pain',
     'soft tissue rheumatism',
     'non-articular rheumatism',
     'squamous cell carcinoma of skin of face',
     'squamous cell cancer of skin of face',
     'aneurysm of ascending aorta',
     'anemia due to chemotherapy',
     'calcification of coronary artery',
     'laceration of nail bed of finger',
     'anal polyp',
     'bilateral hydronephrosis',
     'adult failure to thrive syndrome',
     'anaphylaxis',
     'generalized anaphylaxis',
     'generalised anaphylaxis',
     'anaphylactic shock',
     'allergic shock',
     'systemic anaphylaxis',
     'anaphylactic reaction',
     'urinary tract obstruction',
     'obstructive uropathy',
     'amaurosis fugax',
     'momentary blindness',
     'amaurosis fugax (one sided temporary vision loss)',
     'afx - amaurosis fugax',
     'transient monocular blindness',
     'nasal septoplasty',
     'nasal septoplasty without graft',
     'tendinitis and/or tenosynovitis of the elbow region',
     'tendonitis and/or tenosynovitis of the elbow region',
     'lesion of ulnar nerve',
     'congenital pectus excavatum',
     'congenital funnel chest',
     'history of malignant neoplasm of uterine body',
     'history of cancer of uterine body',
     'acute myocardial infarction of inferior wall',
     'acute myocardial infarction of diaphragmatic wall',
     'acute inferior myocardial infarction',
     'acute prostatitis',
     'arthralgia of temporomandibular joint',
     'neurofibromatosis, type 1',
     'neurofibromatosis type 1',
     'neurofibromatosis 1',
     'neurofibromatosis, peripheral type',
     'nf1',
     'von recklinghausen disease',
     'neurofibromatoses',
     'family tension',
     'domestic stress',
     'stress at home',
     'intracranial aneurysm',
     'steroid-induced diabetes',
     'achilles bursitis',
     'capped hock',
     'achillobursitis',
     'alberts syndrome',
     'swediaurs disease',
     'allergic contact dermatitis',
     'ACd',
     'periodontal disease',
     'disease of supporting structures of teeth',
     'human bite - wound',
     'intrinsic asthma',
     'non-allergic asthma',
     'asthma due to internal immunological process',
     'ulcer of esophagus',
     'Ou',
     'oesophageal ulcer',
     'ou - esophageal ulcer',
     'esophageal ulcer',
     'fasciitis',
     'fasciosis',
     'gastroesophageal reflux disease with hiatal hernia',
     'hiatal hernia with gastro-oesophageal reflux disease',
     'hiatal hernia with gastroesophageal reflux disease',
     'disorder of eye',
     'disease of eyeball',
     'disorder of globe',
     'disorder of eyeball',
     'eye disorder',
     'disorder of eye proper',
     'disorder of eye region',
     'ophthalmological disorder',
     'disorder of orbital region',
     'cellulitis of upper arm',
     'traumatic and/or non-traumatic injury',
     'injury',
     'injuries',
     'history of appendicectomy',
     'atherosclerosis of bypass graft of limb',
     'mucous inflammation',
     'catarrhal inflammation',
     'inflammatory disease of mucous membrane',
     'mucous membrane inflammation',
     'mucitis',
     'mucositis',
     'mucosal inflammation',
     'lipids abnormal',
     'oral contraceptive prescribed',
     'complicated migraine',
     'complex migraine',
     'acute peptic ulcer',
     'exchange of intraocular lens',
     'lens exchange',
     'iol - exchange of intraocular lens',
     'peritoneal dialysis catheter exit site infection',
     'peritoneal dialysis cannula exit site infection',
     'residual schizophrenia',
     'restzustand',
     'subtrochanteric fracture of femur',
     'subtrochanteric fracture',
     'hemolytic disease of fetus or newborn due to abo immunization',
     'abo hdn - abo haemolytic disease of the newborn',
     'abo hdn - abo hemolytic disease of the newborn',
     'erythroblastosis foetalis due to abo isoimmunisation',
     'abo haemolytic disease of the newborn',
     'anaemia due to abo incompatibility in the newborn',
     'abo isoimmunisation of the newborn',
     'erythroblastosis fetalis due to abo isoimmunisation',
     'haemolytic disease due to abo isoimmunisation',
     'jaundice due to abo isoimmunisation of the newborn',
     'haemolytic disease of fetus or newborn due to abo immunisation',
     'hemolytic disease due to abo isoimmunization',
     'erythroblastosis fetalis due to abo isoimmunization',
     'anemia due to abo incompatibility in the newborn',
     'jaundice due to abo isoimmunization of the newborn',
     'abo isoimmunization of the newborn',
     'abo hemolytic disease of the newborn',
     'supracervical hysterectomy',
     'supracervical hysterectomy without removal of tube or ovary',
     'acute pulmonary edema',
     'pulmonary oedema - acute',
     'pulmonary edema - acute',
     'acute oedema of lung',
     'acute edema of lung',
     'penetrating keratoplasty',
     'penetrating graft to cornea',
     'Pk',
     'PKp',
     'closed fracture of vertebral column',
     'myeloid metaplasia',
     'screening colonoscopy',
     'disorder of respiratory system',
     'respiratory disease',
     'respiratory system disease',
     'respiratory disorder',
     'senile entropion',
     'age-related entropion',
     'involutional entropion',
     'umbilical hernia without obstruction and without gangrene',
     'primary malignant neoplasm of rectosigmoid junction',
     'closed traumatic dislocation of glenohumeral joint',
     'closed traumatic shoulder dislocation',
     'closed traumatic dislocation of shoulder',
     'closed traumatic dislocation of joint of shoulder region',
     'primary aldosteronism',
     'primary hyperaldosteronism',
     'conns syndrome',
     'conn syndrome',
     'idiopathic hyperaldosteronism',
     'idiopathic aldosteronism',
     'bruxism (teeth grinding)',
     'bruxism',
     'bruxism (grinding teeth)',
     'grinding teeth',
     'teeth gnashing',
     'grinding of teeth',
     'restlessness',
     'motor restlessness',
     'restlessness behaviour',
     'restlessness behavior',
     'feeling agitated',
     'agitation',
     'agitated behaviour',
     'agitated behavior',
     'agitated',
     'unable to keep still',
     'psychomotor agitation',
     'excessive overactivity',
     'increased purposeless goalless activity',
     'agitations',
     'enteroviral vesicular stomatitis with exanthem',
     'hand, foot and mouth disease',
     'hand foot and mouth disease',
     'vesicular stomatitis with xanthem',
     'history of malignant neoplasm of ovary',
     'history of cancer of ovary',
     'atherosclerosis of renal artery',
     'renal artery atheroma',
     'renal artery atherosclerosis',
     'atherosclerosis renal artery',
     'contusion of hand',
     'superficial bruising of hand',
     'hidradenitis suppurativa',
     'suppurative hidradenitis',
     'hidradenitis axillaris',
     'apocrine acne',
     'verneuils disease',
     'lymphocytosis',
     'hypercalciuria',
     'hypercalcuria',
     'contusion of foot',
     'superficial bruising of foot',
     'pseudoexfoliation glaucoma',
     'glaucoma capsulare',
     'secondary open-angle glaucoma with pseudoexfoliation',
     'pseudoexfoliation of lens capsule',
     'PXf',
     'pseudoexfoliation syndrome',
     'PEx',
     'pseudoexfoliation',
     'pseudoexfoliation lens capsule',
     'h/o: chickenpox',
     'history of chickenpox',
     'h/o: kidney donation',
     'history of kidney donation',
     'non-small cell lung cancer',
     'NSCLc',
     'post-traumatic wound infection',
     'needle stick injury',
     'needle prick injury',
     'lesion of liver',
     'injury of tendon of the rotator cuff of shoulder',
     'malignant tumor of transverse colon',
     'ca transverse colon',
     'supernumerary tooth',
     'Sn',
     'supplemental tooth',
     'carcinoma of stomach',
     'gastric carcinoma',
     'sweating symptom',
     'sweating observable',
     'sweating',
     'sweats',
     'diaphoresis',
     'hidropoiesis',
     'hidrosis',
     'perspiration',
     'sweating, function',
     'perspirations',
     'sedative dependence',
     'hemiplegia of nondominant side',
     'multigravida of advanced maternal age',
     'counselling for sexually transmitted disease',
     'bacterial arthritis',
     'pyogenic bacterial arthritis',
     'arthritis due to bacterial infection',
     'septic arthritis',
     'burn of upper limb',
     'viral pneumonia',
     'polyhydramnios',
     'hydramnios',
     'excessive liquor',
     'history and physical examination, pre-employment',
     'endometrial cystic hyperplasia',
     'swiss cheese hyperplasia of the endometrium',
     'assault by cutting and stabbing instruments',
     'peristomal dermatitis',
     'peristomal eczema',
     'intracranial glioma',
     'glioma of brain',
     'diagnostic fibreoptic endoscopic examination of upper gastrointestinal tract',
     'physical child abuse',
     'physical abuse of child',
     'education',
     'educating',
     'education procedure',
     'instruction',
     'teaching',
     'instructions',
     'teachings',
     'disorder of oral soft tissues',
     'oral soft tissue disease',
     'disease of the oral soft tissues',
     'livebirth born before admission to hospital',
     'postpartum hemorrhage',
     'PPh',
     'postpartum haemorrhage',
     'bleeding postpartum',
     'haemorrhage after delivery of foetus',
     'haemorrhage after delivery of fetus',
     'hemorrhage after delivery of fetus',
     'transplant of lung',
     'ltx - lung transplant',
     'lung transplant, single; without cardiopulmonary bypass',
     'von willebrand disorder',
     'vascular haemophilia',
     'angiohemophilia',
     'pseudohemophilia type b',
     'vascular hemophilia',
     'pseudohaemophilia type b',
     'constitutional thrombopathy',
     'factor viii deficiency with vascular defect',
     'von willebrand disease',
     'vwd - von willebrands disease',
     'von willebrand disease, platelet type',
     'von willebrands disease',
     'von willebrand-jurgens disease',
     'angiohaemophilia',
     'pseudo von willebrand disease',
     'platelet type von willebrands disease',
     'platelet type von willebrand disease',
     'platelet type pseudo-von willebrand disease',
     'hereditary progressive muscular dystrophy',
     'muscular dystrophy',
     'Md',
     'progressive muscular dystrophy',
     'PMd',
     'intracranial haemorrhage',
     'osteitis deformans',
     'pagets disease of bone',
     'pagets disease of bone',
     'congenital arteriovenous malformation',
     'av - congenital arteriovenous anomaly',
     'av - congenital arteriovenous malformation',
     'avm - congenital arteriovenous malformation',
     'arteriovenous malformation',
     'entropion',
     'eyelid inverted',
     'eyelashes turned in',
     'eyelid turned in',
     'entropion of eyelid',
     'folded in eyelid',
     'malignant tumor of tongue',
     'cancer of tongue',
     'sprain of foot',
     'FSPr',
     'foot sprain',
     'pulmonary heart disease',
     'cor pulmonale',
     'right heart failure due to disorder of lung',
     'right heart failure due to pulmonary disease',
     'alopecia hereditaria',
     'male pattern alopecia',
     'common baldness',
     'hereditary alopecia',
     'male pattern baldness',
     'androgenic alopecia',
     'pattern baldness',
     'androgenetic alopecia',
     'fracture of thoracic spine',
     'fracture of thoracic vertebra',
     'neoplasm of skin',
     'cutaneous tumour',
     'tumour of skin',
     'skin tumour',
     'cutaneous tumor',
     'skin tumor',
     'tumor of skin',
     'victim of physical abuse',
     'sebaceous hyperplasia',
     'sebaceous gland hyperplasia',
     'benign seborrhoeic hyperplasia',
     'benign seborrheic hyperplasia',
     'absence of breast',
     'fibroadenoma of breast',
     'breast mouse',
     'h/o: bone marrow donation',
     'history of bone marrow donation',
     'mass of parotid gland',
     'parotid lump',
     'slow transit constipation',
     'colonic constipation',
     'constipation by delayed colonic transit',
     'malignant tumour of tonsil',
     'malignant tumour of palatine tonsil',
     'malignant tumour of faucial tonsil',
     'malignant tumor of faucial tonsil',
     'malignant tumor of palatine tonsil',
     'cancer of tonsil',
     'malignant neoplasm of palatine tonsil',
     'malignant neoplasm of faucial tonsil',
     'malignant tumour of hepatic flexure',
     'ca hepatic flexure - colon',
     'iatrogenic cushings disease',
     'drug-induced cushings syndrome',
     'iatrogenic cushings syndrome',
     'iatrogenic cushing disease',
     'iatrogenic syndrome of excess cortisol',
     'ankle ulcer',
     'diabetic oculopathy associated with type 1 diabetes mellitus',
     'diabetic oculopathy associated with type i diabetes mellitus',
     'fracture malunion',
     'malunion of fracture',
     'hemiplegia of dominant side',
     'hepatic failure',
     'liver failure',
     'hepatic insufficiency',
     'liver function failure',
     'liver decompensation',
     'tetralogy of fallot',
     'TOf',
     'fallots tetralogy',
     'subpulmonic stenosis, ventricular septal defect, overriding aorta, and right ventricular hypertrophy',
     'neoplasm of uncertain behaviour of meninges',
     'fracture of cervical spine',
     'fracture of neck',
     'fracture of cervical vertebra',
     'bipolar affective disorder, currently manic, severe, with psychosis',
     'non-rheumatic mitral regurgitation',
     'mitral incompetence, non-rheumatic',
     'pneumonia due to respiratory syncytial virus',
     'mental disorders during pregnancy, childbirth and the puerperium',
     'closed fracture pubis',
     'closed fracture of pubis',
     'open wound of face',
     'craniotomy',
     'incision of bone of skull',
     'craniotomies',
     'evaluation and management of new outpatient in office or other outpatient facility',
     'history and physical examination with management of new patient',
     'nontraumatic rotator cuff tear',
     'complication of urinary catheter',
     'hypertension associated with transplantation',
     'total replacement of right knee joint',
     'posterior subcapsular polar senile cataract',
     'intestinal bypass for morbid obesity',
     'enteroenterostomy, anastomosis of intestine for morbid obesity',
     'female pelvic peritoneal adhesions',
     'specialised medical examination',
     'examination in specific circumstances',
     'special examination',
     'prepatellar bursitis',
     'closed fracture of phalanx of finger',
     'closed fracture of finger',
     'premature atrial contraction',
     'atrial extrasystoles',
     'atrial ectopic',
     'atrial premature systoles',
     'atrial premature contractions',
     'PAc',
     'atrial premature complex',
     'atrial premature depolarisation',
     'atrial premature depolarization',
     'ectopic atrial beats',
     'supraventricular premature beats',
     'supraventricular extrasystoles',
     'supraventricular premature contraction',
     'ophthalmic examination and evaluation',
     'examining eye',
     'ophthalmic examination',
     'examination of eye',
     'eye examination',
     'hypoparathyroidism',
     'deficiency of parathyroid hormone',
     'deficiency of parathyrin',
     'deficiency of pth',
     'cauda equina syndrome with neurogenic bladder',
     'marfans syndrome',
     'marfan syndrome',
     'marfans disease',
     'renal hypertension',
     'central serous chorioretinopathy',
     'central serous retinopathy',
     'csr - central serous retinopathy',
     'central serous choroidopathy',
     'central serous retinopathy',
     'CSc',
     'intervertebral disc disorder of lumbar region with myelopathy',
     'disorder of lumbar intervertebral disc with myelopathy',
     'clostridium difficile colitis',
     'history of malignant lymphoma',
     'history of lymphoma',
     'fecal impaction',
     'faeces - impacted',
     'feces - impacted',
     'fecal impaction in rectum',
     'impacted stool in rectum',
     'impacted feces',
     'impacted faeces',
     'fecal impaction of rectum',
     'faecal impaction of rectum',
     'faecal impaction',
     'diabetic autonomic neuropathy',
     'autonomic neuropathy due to diabetes',
     'testis excision',
     'excision of testis',
     'orchiectomy',
     'total orchidectomy',
     'orchidectomy',
     'excision procedures on the testis',
     'adenocarcinoma of endometrium',
     'endometrial adenocarcinoma',
     'adenocarcinoma of uterus',
     'fracture of lumbar spine',
     'fracture of lumbar vertebra',
     'neoplasm of parotid gland',
     'tumor of parotid gland',
     'tumour of parotid gland',
     'alcohol consumption screening',
     'cannabis dependence, continuous',
     'lyme disease',
     'infection due to borrelia burgdorferi sensu lato',
     'steeres disease',
     'lyme borreliosis',
     'lime disease',
     'lymes disease',
     'venous stasis syndrome',
     'venous stasis',
     'heart irregular',
     'palpitations - irregular',
     'heart beats irregular',
     'irregular heart beat',
     'irregular heart rate',
     'corneal foreign body',
     'abducens nerve palsy',
     'sixth nerve palsy',
     'vi nerve palsy',
     'lateral rectus muscle denervation paresis',
     'abducens nerve paralysis',
     'abducens (sixth) nerve palsy',
     'abducens nerve paresis',
     '6th nerve palsy',
     'sixth cranial nerve palsy',
     'abducens nerve disorder',
     'sixth cranial nerve disease',
     'abducens nerve disease',
     'lateral rectus muscle innervation disorder',
     'sixth cranial nerve disorder',
     'abducens nerve weakness',
     'sixth cranial nerve weakness',
     'carbuncle',
     'carbuncle of skin and/or subcutaneous tissue',
     'pain at rest due to peripheral vascular disease',
     'history of carotid endarterectomy',
     'incipient cataract',
     'water clefts',
     'parathyroidectomy',
     'ptx - parathyroidectomy',
     'excision of parathyroid gland',
     'parathyroidectomy or exploration of parathyroid(s); re-exploration',
     'fracture of ulna',
     'nonspecific ulcerative proctitis',
     'severe major depression with psychotic features',
     'psychotic depression',
     'dermoid cyst of ovary',
     'benign cystic ovarian teratoma',
     'mature cystic teratoma of ovary',
     'congenital anteversion of femur',
     'congenital anteversion of femoral neck',
     'oesophageal varices without bleeding',
     'removal of ureteral stent',
     'combined opioid with other drug dependence',
     'combined opioid with non-opioid drug dependence',
     'fuchs, corneal dystrophy',
     'fuchs corneal dystrophy',
     'fuchs, endothelial dystrophy',
     'fuchs endothelial corneal dystrophy',
     'congenital anomaly of eye',
     'congenital malformation of eye',
     'congenital deformity of eye',
     'congenital abnormality of eye',
     'congenital anomaly of the globe',
     'congenital eye anomalies',
     'abnormal findings on antenatal screening of mother',
     'closed fracture of intracapsular section of femur',
     'open wound of knee and/or leg and/or ankle',
     'open wound of knee, leg and ankle',
     'accident caused by glass edge',
     'accident caused by broken glass',
     'dehiscence of surgical wound',
     'rupture of operation wound',
     'dehiscence of operation wound',
     'operation wound disruption',
     'operation wound dehiscence',
     'wound breakdown',
     'operation wound rupture',
     'postoperative wound breakdown',
     'obstructive hydrocephalus',
     'thyroid colloid nodule',
     'colloid nodule',
     'calculus of kidney and ureter',
     'calculus of kidney with calculus of ureter',
     'macular eruption',
     'macular rash',
     'shock',
     'circulatory collapse',
     'peripheral vascular shutdown',
     'peripheral circulatory failure',
     'peripheral vascular failure',
     'acute circulatory failure',
     'shock - physiological',
     'shocked',
     'shocking',
     'shocks',
     'spastic dysphonia',
     'spasmodic dysphonia',
     'lesion of bladder',
     'colostomy present',
     'primary malignant neoplasm of the peritoneum',
     'laser photocoagulation to retina',
     'retinal laser photocoagulation',
     'laser photocoagulation of retina',
     'well child visit, 2 month',
     'chronic pain in face',
     'chronic facial pain',
     'incomplete tetraplegia due to spinal cord lesion at c1-c4 level',
     'incomplete quadriplegia due to spinal cord lesion at c1-c4 level',
     'incomplete quadriplegia due to spinal cord lesion between first and fourth cervical vertebra',
     'twin liveborn born in hospital',
     'twin live born in hospital',
     'strain of thoracic region',
     'back strain of thoracic region',
     'late effect of intracranial injury without skull fracture',
     'sequelae of intracranial injury',
     'history of malignant neoplasm of thyroid',
     'tic',
     'tics',
     'habit spasm',
     'habit tic',
     'tic disorder',
     'habit disorder',
     'histoplasmosis',
     'attention to gastrostomy',
     'acute meniscal tear, medial',
     'polymyositis',
     'neuromyositis',
     'mumps parotitis',
     'infectious parotitis',
     'epidemic parotitis',
     'mumps',
     'sepsis due to urinary tract infection',
     'urosepsis',
     'moderate mental retardation (i.q. 35-49)',
     'moderate mental handicap',
     'moderate mental retardation (intelligence quotient 35-49)',
     'coarctation of aorta',
     'aortic coarctation',
     'coarctation',
     'dysentery',
     'infectious diarrhoeal disease',
     'infectious diarrhea',
     'infectious diarrhoea',
     'infective diarrhea',
     'excessive thirst',
     'desperate to drink',
     'always thirsty',
     'keen for fluids',
     'exophthalmos',
     'eye displaced forwards',
     'exophthalmia',
     'proptosis',
     'henoch-schönlein purpura',
     'henoch-schonlein purpura',
     'henochs purpura',
     'autoimmune purpura',
     'spring fever',
     'acute vascular purpura',
     'HSp',
     'henoch-schoenlein vasculitis',
     'anaphylactoid purpura',
     'after-cataract with vision obscured',
     'sprain of shoulder and upper arm',
     'sprain shoulder and upper arm',
     'parastomal hernia',
     'hand eczema',
     'dermatitis of hand',
     'hand dermatitis',
     'cellulitis of finger',
     'excessive upper gastrointestinal gas',
     'full of wind',
     'passing flatus',
     'flatulence',
     'wind - flatus',
     'flatus',
     'flatulence/wind',
     'farting',
     'flatulence symptom',
     'wind symptom',
     'flatus, function',
     'intestinal gas excretion',
     'passage of gas by anus',
     'flatulance',
     'empyema',
     'empyemas',
     'primary degenerative dementia of the alzheimer type, senile onset',
     'primary degenerative dementia of the alzheimer type, late onset',
     'dementia of the alzheimers type, late onset',
     'alzheimers disease with late onset',
     'SDAt',
     'senile dementia, alzheimers type',
     'dementia in alzheimers disease with late onset',
     'dementia in alzheimers disease - type 1',
     'developmental disorder',
     'congenital stenosis of pulmonary valve',
     'congenital valvular pulmonic stenosis',
     'pv - congenital pulmonary valve stenosis',
     'sudden hearing loss',
     'stricture of ureter',
     'ureteral stricture',
     'stenosis of ureter',
     'ureteric stenosis',
     'ureteral stenosis',
     'stenosis of precerebral artery',
     'narrowing of precerebral artery',
     'preventive patient evaluation',
     'unlisted preventive medicine service',
     'total cholesterol measurement',
     'serum total cholesterol measurement',
     'serum total cholesterol level',
     'open wound of wrist with tendon involvement',
     'ast/sgot level raised',
     'aspartate aminotransferase/serum glutamic oxaloacetic transaminase level raised',
     'requires polio vaccination',
     'bipolar affective disorder, current episode manic',
     'manic-depressive - now manic',
     'anxiety neurosis',
     'open wound of anterior abdominal wall',
     'fall on steps',
     'fall down stairs',
     'fall down steps',
     'fall on stairs',
     'automatic defibrillator procedure',
     'ICd',
     'internal cardiac defibrillator procedure',
     'automatic cardiac defibrillator procedure',
     'drug-induced hypotension',
     'hypotension due to drugs',
     'allogeneic unrelated bone marrow transplant',
     'allogeneic unrelated bone marrow graft',
     'MUd',
     'matched unrelated donor bone marrow transplant',
     'tight chest',
     'pressure in chest',
     'chest tightness',
     'malignant melanoma of choroid',
     'deficiency anaemias',
     'blind or low vision - both eyes',
     'hearing disorder',
     'disorder of hearing',
     'auditory alteration',
     'hearing problem',
     'disorder of auditory system',
     'auditory dysfunction',
     'acute angle-closure glaucoma',
     'acute closed-angle glaucoma',
     'AACg',
     'acute angle closure glaucoma',
     'hypothermia',
     'body temperature below normal',
     'decreased body temperature',
     'state of hypothermia',
     'temperature subnormal',
     'lower urinary tract infectious disease',
     'lower urinary tract infection',
     'uti - lower urinary tract infection',
     'moderate mixed bipolar i disorder',
     'moderate bipolar i disorder, most recent episode mixed',
     'rickets',
     'insertion of peripherally inserted central catheter',
     'insertion of picc (peripherally inserted central catheter)',
     'repair of reducible inguinal hernia',
     'disruptive behavior disorder',
     'hypertensive heart disease without congestive heart failure',
     'gastric haemorrhage',
     'gastrorrhagia',
     'gastric bleeding',
     'poliomyelitis vaccination',
     'poliomyelitis immunization',
     'poliomyelitis immunisation',
     'exploratory laparotomy',
     'exploration of peritoneal cavity',
     'exploration of peritoneum',
     'exploratory celiotomy',
     'exploration of abdomen',
     'exploratory coeliotomy',
     'exploratory laparotomy, exploratory celiotomy with or without biopsy(s) (separate procedure)',
     'type 2 diabetes mellitus in obese',
     'diabetes mellitus type 2 in obese',
     'biopsy of liver',
     'menstrual problem',
     'problem of menstruation',
     'period problem',
     'disorder of menstruation',
     'menstrual disorder',
     'period disorder',
     'abnormal menstruation',
     'mallet finger',
     'baseball finger',
     'retinal round hole without detachment',
     'round hole of retina without detachment',
     'granuloma pyogenicum',
     'capillary haemangioma of granulation tissue type',
     'capillary hemangioma of granulation tissue type',
     'lobular capillary haemangioma',
     'lobular capillary hemangioma',
     'pyogenic granuloma',
     'granuloma telangiectaticum',
     'pyogenic granuloma of skin',
     'Pg',
     'granuloma telangiectaticum of skin',
     'chronic cholecystitis',
     'tendinitis and/or tenosynovitis of wrist and/or hand',
     'tendinitis and/or tenosynovitis of the wrist and/or hand',
     'keratosis',
     'keratoses',
     'neurodermatitis',
     'laceration of hand',
     'erythema multiforme',
     'Em',
     'target lesion',
     'aseptic necrosis of bone',
     'sialoadenitis',
     'sialadenitis',
     'hypoplasia of maxillary bone',
     'lumbar spinal fusion',
     'fusion of lumbar spine',
     'hibbs operation',
     'impulse control disorder',
     'habit and impulse disorder',
     'cyst of thyroid',
     'articular disc disorder of temporomandibular joint',
     'adenoid excision',
     'as - adenoidectomy',
     'excision of adenoid',
     'ads - adenoidectomy',
     'adenoidectomy',
     'neoplasm of bladder',
     'tumour of urinary bladder',
     'tumor of urinary bladder',
     'NGb',
     'new growth of bladder',
     'h/o: depression',
     'history of depressive disorder',
     'has had depression',
     'history of depression',
     'h/o: tia',
     'history of transient ischemic attack',
     'h/o: tracheostomy',
     'history of tracheostomy',
     'lobectomy of lung',
     'pulmonary lobectomy',
     'removal of lung, other than pneumonectomy; single lobe (lobectomy)',
     'varicose veins of the leg with ulcer',
     'varicose ulcer of lower extremity',
     'varicose ulcer of lower limb',
     'varicose veins of lower extremity with ulcer',
     'varicose veins with ulcer of lower extremity',
     'labyrinthine disorder',
     'labyrinthine vertigo',
     'vestibular disorder',
     'central vestibular vertigo',
     'vertigo of central origin',
     'prolapsed cervical intervertebral disc',
     'shoulder stiff',
     'lymphoproliferative disorder',
     'x-linked lymphoproliferative syndrome',
     'XLPs',
     'ebv - fatal epstein-barr virus syndrome',
     'duncans syndrome',
     'fatal epstein-barr virus syndrome',
     'lymphoproliferative disease',
     'lymphoproliferative disease, no icd-o subtype',
     'lymphoproliferative disease, no international classification of diseases for oncology subtype',
     'osteoarthritis of foot joint',
     'degenerative joint disease of foot',
     'diaphragmatic hernia',
     'acoustic neuroma',
     'acoustic schwannoma',
     'vestibular schwannoma',
     'acoustic neurinoma',
     'acoustic neurilemmoma',
     'cellular schwannoma',
     'plexiform schwannoma',
     'neurilemmoma',
     'schwannoma',
     'neurilemoma',
     'ancient neurilemmoma',
     'ancient schwannoma',
     'plexiform neurilemmoma',
     'cellular neurilemmoma',
     'cellular schwannoma',
     'ancient schwannoma',
     'ancient neurilemoma',
     'psammomatous schwannoma',
     'plexiform schwannoma',
     'degenerated schwannoma',
     'neurinoma',
     'hypertensive left ventricular hypertrophy',
     'mixed rhinitis',
     'periumbilical pain',
     'periumbilical abdominal pain',
     'organic anxiety disorder',
     'anxiety disorder due to a general medical condition',
     'anxiety disorder due to medical disorder',
     'anxiety disorder due to general medical condition',
     'myh-associated polyposis',
     'myh associated polyposis',
     'multiple adenomatous polyps',
     'adenomatous polyposis coli',
     'familial polyposis coli',
     'polyposis coli',
     'familial intestinal polyposis',
     'adenomatosis',
     'familial multiple polyposis syndrome',
     'familial adenomatous polyposis',
     'FPc',
     'FAp',
     'adenomatous polyposis',
     'APc',
     'foreign body in skin',
     'traumatic dislocation of joint of finger',
     'deep venous thrombosis of upper extremity',
     'respiratory condition of fetus or newborn',
     'respiratory condition of foetus or newborn',
     'foetus and newborn respiratory conditions',
     'perinatal respiratory disorders',
     'fetus and newborn respiratory conditions',
     'prosthetic joint dislocation',
     'dislocation of joint prosthesis',
     'hydrosalpinx',
     'macular drusen',
     'drusen of macula',
     'constantly crying baby',
     'sacral dimples',
     'sacral dimple',
     'fracture of distal end of radius',
     'fracture of lower end of radius',
     'scleral buckling',
     'scleral buckle procedure',
     'buckling procedure of sclera',
     'post infarct angina',
     'malignant tumor of sigmoid colon',
     'ca sigmoid colon',
     'superficial laceration of hand',
     'thrombotic stroke',
     'impairment of balance',
     'problem with balance',
     'balance impairment',
     'child abuse',
     'enlarging abdominal aortic aneurysm',
     'sepsis due to gram negative bacteria',
     'retinal neovascularisation',
     'neovascularisation of retina',
     'neovascularization of retina',
     'vasoproliferative retinopathy',
     'retinal vascular proliferation',
     'persistent occipitoposterior position',
     'POp',
     'spasmodic torticollis',
     'superficial ulcer of skin',
     'erosion of skin',
     'retinitis pigmentosa',
     'Rp',
     'heart valve replacement',
     'hereditary motor and sensory neuropathy',
     'hereditary sensory and motor neuropathy',
     'hereditary sensory-motor neuropathy',
     'HSMn',
     'HMSn',
     'hereditary sensorimotor neuropathy',
     'CMt',
     'charcot-marie-tooth disease',
     'cervical disc disorder',
     'cervical disc disease',
     'loose body in knee',
     'joint mice of knee',
     'cleft palate',
     'uranoschisis',
     'palatoschisis',
     'prosthetic joint infection',
     'infected arthroplasty',
     'infected joint prosthesis',
     'closed fracture of acetabulum',
     'closed fracture acetabulum',
     'whiplash injury to neck',
     'acceleration-deceleration injury of neck',
     'whiplash injury',
     'congenital kyphoscoliosis',
     'kyphoscoliosis',
     'subacromial bursitis',
     'postoperative seroma',
     'hyperplasia of mandibular bone',
     'uremia',
     'rectal prolapse',
     'procidentia of rectum',
     'rectal mucosa prolapse',
     'abscess of thorax',
     'abscess of pleural cavity',
     'empyema of pleura',
     'purulent pleurisy',
     'pyothorax',
     'suppurative pleurisy',
     'premenstrual dysphoric disorder',
     'PMDd',
     'haemolytic anaemia',
     'spina bifida',
     'posterior rachischisis',
     'Sb',
     'closed fracture of head of radius',
     'closed fracture radius, head',
     'closed fracture of radius',
     'burn of foot',
     'injury of wrist',
     'wrist injury',
     'axillary lymphadenopathy',
     'inguinal lymphadenopathy',
     'groin lymphadenopathy',
     'viral bronchitis',
     'horseshoe retinal tear without detachment',
     'impaired renal function disorder',
     'biliary stricture',
     'cachexia',
     'cachectic',
     'general health deterioration',
     'general body deterioration',
     'continuous ambulatory peritoneal dialysis',
     'CAPd',
     'comedonal acne',
     'ptosis of eyebrow',
     'brow ptosis',
     'eyebrow ptosis',
     'ptosis brow',
     'calcific tendinitis of shoulder',
     'calcific tendonitis of shoulder',
     'calcifying tendinitis of shoulder',
     'calcifying tendinitis of the shoulder',
     'apatite-associated destructive arthritis of shoulder',
     'milwaukee shoulder',
     'chronic laryngitis',
     'ewings sarcoma of bone',
     'ewing sarcoma of bone',
     'mixed bipolar i disorder in partial remission',
     'bipolar i disorder, most recent episode mixed, in partial remission',
     'simple obesity',
     'exogenous obesity',
     'obesity due to excess calories',
     'alimentary obesity',
     'sustained ventricular tachycardia',
     'bariatric operative procedure',
     'incomplete tetraplegia due to lesion at c5-c7 level',
     'incomplete quadriplegia due to lesion at c5-c7 level',
     'incomplete quadriplegia due to spinal cord lesion between fifth and seventh cervical vertebra',
     'nonsustained ventricular tachycardia',
     'senile dementia of the lewy body type',
     'lewy body dementia',
     'diffuse lewy body disease',
     'SDLt',
     'LBd',
     'lewy body disease',
     'dementia of the lewy body type',
     'DLBd',
     'cortical lewy body disease',
     'CLBd',
     'lewy body variant of alzheimers disease',
     'megaloblastic anaemia due to folate deficiency',
     'folate deficiency anemia',
     'folic acid deficiency anemia',
     'folate deficiency anaemia',
     'folic acid deficiency anaemia',
     'contracture of joint of hand',
     'joint contracture of hand',
     'contracture of hand joint',
     'cryptogenic cirrhosis',
     'atrophy of edentulous alveolar ridge',
     'edentulous alveolar ridge atrophy',
     'retained placenta',
     'disease of mouth',
     'disorder of mouth',
     'mouth disorder',
     'disorder of oral cavity',
     'apnoea in the newborn',
     'apnea neonatorum',
     'apnoea neonatorum',
     'neonatal apnoea',
     'apnoea of newborn',
     'apnea of newborn',
     'h/o: colostomy',
     'history of colostomy',
     'pneumonia and influenza',
     'fracture of clavicle due to birth trauma',
     'functional disorder of bladder',
     'functional voiding disorder',
     'wound hematoma',
     'tooth development and eruption disorder',
     'disorder of tooth development and/or eruption',
     'teeth development and eruption disorder',
     'menopausal and postmenopausal disorders',
     'cellulitis and abscess of neck',
     'unable to balance',
     'loss of balance',
     'alcohol-induced organic mental disorder',
     'alcohol-related disorder',
     'acute non-q wave infarction',
     'numbness of hand',
     'calcification of breast',
     'breast calcification',
     'implantation of intravenous dual chamber permanent pacemaker',
     'work-related counselling',
     'blighted ovum',
     'resorbed foetus',
     'empty gestational sac with ongoing pregnancy',
     'resorbed fetus',
     'anembryonic pregnancy',
     'pathologic ovum',
     'effusion of joint',
     'effusion into joint',
     'joint effusion',
     'hydrarthrosis',
     'rotator cuff tear arthropathy',
     'dream anxiety disorder',
     'nightmare disorder',
     'nightmare',
     'nightmares',
     'smoke inhalation injury',
     'arthropathy of lumbar facet joint',
     'arthropathy of lumbar facet',
     'acute duodenal ulcer with perforation',
     'hepatitis c antibody level',
     'hepatitis c antibody measurement',
     'hcv antibody measurement',
     'anti hcv measurement',
     'hepatitis c antibody',
     'removal of internal fixation device',
     'removal of internal fixation device from bone',
     'hemianopia',
     'hemianopsia',
     'mechanical complication due to urethral indwelling catheter',
     'physical therapy procedure',
     'physiotherapy',
     'physiotherapy procedure',
     'physiotherapies',
     'secondary malignant neoplasm of prostate',
     'metastatic tumour to prostate',
     'metastatic tumor to prostate',
     'metastasis to prostate',
     'metastatic malignant neoplasm to prostate',
     'cancer metastatic to prostate',
     'hypertrophy of adenoids',
     'enlargement of adenoids',
     'adenoidal hypertrophy',
     'adenoidal enlargement',
     'ischaemic optic neuropathy',
     'AIOn',
     'acute ischaemic optic neuropathy',
     'IOn',
     'acute ischemic optic neuropathy',
     'ischemic optic neuropathy',
     'ankle joint inflamed',
     'ankle joint active',
     'mononeuritis',
     'glomerulonephritis',
     'Gn',
     'allergic urticaria',
     'non-healing surgical wound',
     'non healing surgical wound',
     'blepharospasm',
     'spasm of eyelids',
     'essential blepharospasm',
     'benign essential blepharospasm',
     'blepharospasm (spasm of eyelid)',
     'acquired hallux rigidus',
     'hallux rigidus',
     'hallux limitus',
     'rigidity of 1st mtp joint',
     'hallux rigidus - acquired',
     'terminal oesophageal web',
     'terminal esophageal ring',
     'schatzkis ring',
     'acquired schatzkis ring',
     'Sr',
     'schatzki ring',
     'terminal oesophageal ring',
     'lower oesophageal mucosal ring',
     'LOMr',
     'oesophageal ring',
     'lower esophageal mucosal ring',
     'esophageal ring',
     'membranous glomerulonephritis',
     'MGn',
     'chronic nephritic syndrome, diffuse membranous glomerulonephritis',
     'grand mal status',
     'convulsive status epilepticus',
     'depression screening',
     'opioid dependence in remission',
     'fourth nerve palsy',
     'fourth cranial nerve palsy',
     'trochlear nerve palsy',
     'fourth cranial nerve paralysis',
     'trochlear nerve paralysis',
     'superior oblique muscle innervation disorder',
     'trochlear nerve disease',
     'fourth cranial nerve paresis',
     'trochlear nerve weakness',
     'fourth cranial nerve disorder',
     'iv nerve palsy',
     'trochlear nerve disorder',
     '4th nerve palsy',
     'fourth cranial nerve disease',
     'disorder of trochlear nerve',
     'superior oblique palsy',
     'migraine aura without headache',
     'migraine accompaniment',
     'acephalgic migraine',
     'antiphospholipid syndrome',
     'APs',
     'anticardiolipin syndrome',
     'APl',
     'hypnotic or anxiolytic dependence',
     'dependent sedative or hypnotic drug abuse',
     'reduced fetal movement',
     'low foetal movement',
     'low fetal movement',
     'baby moving less',
     'baby kicking less',
     'acute ill-defined cerebrovascular disease',
     'lesion of eyelid',
     'lid lesion',
     'lesion eyelid',
     'vaginal wall prolapse',
     'hypertropia',
     'family history of malignant melanoma',
     'ventricular arrhythmia',
     'fussy infant',
     'unsettled infant',
     'depressed bipolar i disorder in partial remission',
     'bipolar i disorder, most recent episode depressed, in partial remission',
     'presbycusis',
     'age-related hearing loss',
     'senile deafness',
     'presbyacusia',
     'hypophosphatemia',
     'ear discharge present',
     'discharge from ear',
     'discharging ear',
     'ear discharge',
     'observation of ear discharge',
     'finding of ear discharge',
     'otorrhoea',
     'discharge of ear',
     'drainage from external ear canal',
     'legal blindness',
     'hyperventilation',
     'overbreathing',
     'hyperventilating',
     'rheumatic aortic stenosis',
     'rheumatic aortic valve obstruction',
     'acquired thrombocytopenia',
     'secondary thrombocytopenia',
     'generalized metabolic disorder',
     'metabolic disease',
     'metabolic disorders',
     'metabolic disorder',
     'displacement of cervical intervertebral disc without myelopathy',
     'benign neoplasm of meninges',
     'benign tumour of meninges',
     'benign tumor of meninges',
     'benign meningeal neoplasm',
     'acute peptic ulcer with haemorrhage',
     'suspected victim of child abuse',
     'pregnant - planned',
     'planned pregnancy',
     'burn of face and/or head',
     'knee joint operation',
     'operative procedure on knee',
     'malignant neoplasm of cardio-oesophageal junction of stomach',
     'malignant neoplasm of cardioesophageal junction of stomach',
     'malignant neoplasm of gastro-esophageal junction',
     'malignant neoplasm of gastro-oesophageal junction',
     'macroglobulinemia',
     'waldenström macroglobulinemia',
     'waldenstroms macroglobulinemia',
     'waldenstroms macroglobulinaemia',
     'waldenstrom macroglobulinemia',
     'primary macroglobulinaemia',
     'primary macroglobulinemia',
     'amphetamine or psychostimulant dependence in remission',
     'amfetamine or psychostimulant dependence in remission',
     'nondependent cocaine abuse, episodic',
     'recurrent sinusitis',
     'claustrophobia',
     'fear of confined spaces',
     'fear of enclosed spaces',
     'cellulitis and abscess of toe',
     'neonatal jaundice with glucose-6-phosphate dehydrogenase deficiency',
     'remittent-progressive multiple sclerosis',
     'multiple sclerosis (ms) remitting progressive',
     'malignant tumor of islets of langerhans',
     'malignant islet cell tumour',
     'malignant neoplasm of islets of langerhans',
     'malignant islet cell tumor',
     'carcinoma of endocrine pancreas',
     'endocrine pancreatic carcinoma',
     'pancreatic endocrine tumor, malignant',
     'islet cell carcinoma',
     'islet cell adenocarcinoma',
     'hemorrhage of rectum and anus',
     'homozygous beta thalassaemia',
     'cooleys anemia',
     'mediterranean anemia',
     'beta thalassaemia major',
     'cooleys anaemia',
     'thalassaemia major',
     'homozygous thalassemia',
     'homozygous thalassaemia',
     'carcinoma of base of tongue',
     'lung field abnormal',
     'silver bunionectomy',
     'excision of bunion',
     'bunionectomy',
     'excision of exostosis of head of first metatarsal',
     'paracetamol overdose',
     'acute asthma',
     'heterozygous factor v leiden mutation',
     'numbness of lower limb',
     'persistent breast nodularity',
     'persistent nodularity',
     'gestational proteinuria',
     'rotator cuff impingement syndrome',
     'hepatitis c screening',
     'rem sleep behaviour disorder',
     'rem sleep disorder',
     'rapid eye movement sleep behavior disorder',
     'rapid eye movement sleep behaviour disorder',
     'dependence on continuous positive airway pressure ventilation',
     'dependence on continuous positive airway pressure (cpap) ventilation',
     'embolism from thrombosis of vein of lower extremity',
     'perinatal jaundice from hereditary hemolytic anemia',
     'perinatal jaundice from hereditary haemolytic anaemias',
     'perinatal jaundice from hereditary hemolytic anemias',
     'retinal scar',
     'infection due to escherichia coli',
     'colibacillosis',
     'bacterial infection due to e. coli',
     'escherichia coli infection',
     'e. coli infection',
     'thrombophilia',
     'hypercoagulability state',
     'hypercoagulable state',
     'hypercoagulability',
     'onycholysis',
     'detachment of nail',
     'separation of nail plate',
     'dermal mycosis',
     'fungal dermatosis',
     'cutaneous mycosis',
     'fungal dermatitis',
     'dermatomycosis',
     'alkaline phosphatase raised',
     'contusion of shoulder region',
     'shoulder bruise',
     'contusion, shoulder area',
     'reflex sympathetic dystrophy of lower extremity',
     'family history of polyp of colon',
     'cyst of epididymis',
     'epididymal cyst',
     'spermatocele',
     'endocarditis',
     'granuloma annulare',
     'Ga',
     'benign intracranial hypertension',
     'pseudotumor cerebri',
     'nonnes syndrome',
     'otitic hydrocephalus syndrome',
     'noninfective serous meningitis',
     'BIh',
     'IIh',
     'idiopathic intracranial hypertension',
     'pseudotumour cerebri',
     'stridor',
     'stridulous breathing',
     'stridors',
     'postoperative urethral stricture',
     'consultation',
     'consultation with healthcare provider',
     'consulting with',
     'consultation services',
     'consultations',
     'piriformis syndrome',
     'pelvic outlet syndrome',
     'hypertensive heart and renal disease with (congestive) heart failure',
     'hypertensive heart and renal disease with renal failure',
     'closed fracture distal humerus, lateral epicondyle',
     'lethargy',
     'lack of vitality',
     'lethargic',
     'coronary artery bypass grafts x 2',
     'cabg x 2 - coronary artery bypass grafts x 2',
     'osteoarthritis of wrist',
     'degenerative joint disease of wrist',
     'sterilisation education',
     'pathological fracture of vertebra',
     'idiopathic scoliosis and/or kyphoscoliosis',
     'malignant tumor of oropharynx',
     'cancer of oropharynx',
     'malignant tumour of mesopharynx',
     'malignant tumor of mesopharynx',
     'compression fracture of lumbar spine',
     'chronic diastolic heart failure',
     'abnormal cervical papanicolaou smear with positive human papillomavirus deoxyribonucleic acid test',
     'abnormal cervical papanicolaou smear with positive human papillomavirus deoxyribonucleic acid (dna) test',
     'exposure to mycobacterium tuberculosis',
     'delay when starting to pass urine',
     'urinary hesitancy',
     'urinary hesitation',
     'hesitancy of micturition',
     'hesitancy',
     'upper respiratory tract obstruction',
     'abscess of vulva',
     'at risk for impaired skin integrity',
     'potential skin breakdown',
     'skin integrity impairment risk',
     'vulnerable to skin tissue breakdown',
     'skin at risk of breakdown',
     'skin integrity at risk',
     'at risk for skin ulceration',
     'acromegaly',
     'growth hormone hypersecretion syndrome',
     'sth hypersecretion syndrome',
     'marie disease',
     'anterior pituitary adenoma syndrome',
     'acromegalia',
     'mild depressed bipolar i disorder',
     'mild bipolar i disorder, most recent episode depressed',
     'cervical arthrodesis',
     'cervical spinal fusion',
     'benign neoplasm of thyroid gland',
     'benign tumour of thyroid gland',
     'benign tumor of thyroid gland',
     'perimembranous ventricular septal defect',
     'poisoning by opiate and/or related narcotic',
     'opiate and narcotic poisoning',
     'opiate poisoning',
     'opiate agonist poisoning',
     'narcotic poisoning',
     'primary bacterial peritonitis',
     'spontaneous bacterial peritonitis',
     'burn of multiple sites of trunk',
     'multiple burns of trunk',
     'injury of abdomen',
     'abdominal injury',
     'h/o: cornea recipient',
     'history of cornea recipient',
     'malignant neoplasm of central part of female breast',
     'pseudoclaudication syndrome',
     'personality change',
     'change in personality',
     'intersphincteric fistula',
     'superficial anal fistula',
     'submucosal anal fistula',
     'acute hepatic failure',
     'acute liver failure',
     'ALf',
     'fulminant hepatic failure',
     'FHf',
     'fracture of metacarpal bone',
     'fracture of metacarpus',
     'hand fracture - metacarpal bone',
     'open wound of hand, excluding finger(s)',
     'triple vessel disease of the heart',
     'triple vessel coronary artery disease',
     'alpha/beta lipoproteinaemia',
     'infected sebaceous cyst',
     'infected sebaceous cyst of skin',
     'maternal infection',
     'urine cytology abnormal',
     'pneumococcal pneumonia',
     'pneumonia due to streptococcus',
     'streptococcal pneumonia',
     'tooth loss',
     'shedding of tooth',
     'acquired absence of teeth',
     'loss of teeth - acquired',
     'absence of tooth - acquired',
     'missing tooth - acquired',
     'teeth missing',
     'loss of teeth',
     'acquired edentia',
     'degenerative spondylolisthesis',
     'midline cystocele',
     'total replacement of left knee joint',
     'total prosthetic arthroplasty of left knee',
     'partial colectomy with anastomosis',
     'subtotal colectomy with anastomosis',
     'colectomy, partial; with anastomosis',
     'foreign body in esophagus',
     'fb oesophagus',
     'fb esophagus',
     'fetal or neonatal effect of malpresentation, malposition and/or disproportion during labor and/or delivery',
     'bulging',
     'swelling',
     'tumefaction',
     'tumescence',
     'bulge',
     'observation of swelling',
     'part of body swollen',
     'part of body puffy',
     'swellings',
     'bulged',
     'bulges',
     'acquired hallux valgus',
     'varicella vaccination',
     'chickenpox vaccination',
     'vasospasm',
     'blood vessel spasm',
     'vascular spasm',
     'angiospasm',
     'vasospasms',
     'fibreoptic colonoscopy through colostomy',
     'endoscopy of colon through artificial stoma',
     'colonoscopy through artificial stoma',
     'upper abdominal pain',
     'secondary malignant neoplasm of axilla',
     'metastatic malignant neoplasm to axilla',
     'dependence on corticoids',
     'dependence on corticosteroids',
     'corticodependence',
     'closed traumatic dislocation of patellofemoral joint',
     'closed traumatic dislocation of patella',
     'lumbosacral spondylosis',
     'hyperphosphataemia',
     'toxic multinodular goiter',
     'toxic multinodular thyroid goiter',
     'toxic multinodular thyroid goitre',
     'secondary thyroid hyperplasia',
     'thyrotoxicosis due to multinodular goiter',
     'thyrotoxicosis with toxic multinodular goiter',
     'thyrotoxicosis due to multinodular goitre',
     'thyrotoxicosis with toxic multinodular goitre',
     'female infertility of tubal origin',
     'infertility of tubal origin',
     'chronic osteomyelitis',
     'cervico-occipital neuralgia',
     'occipital neuralgia',
     'pinguecula',
     'pinguicula',
     'multiple myeloma in remission',
     'parathyroid adenoma',
     'chronic diarrhea of unknown origin',
     'malignant neoplasm of lateral wall of urinary bladder',
     'glaucoma associated with vascular disorder',
     'dilation of esophagus',
     'dilatation procedure to oesophagus',
     'dilatation procedure to esophagus',
     'excoriation of skin',
     'excoriation',
     'excoriations',
     'urinary symptoms',
     'urinary system symptoms',
     'thumb injury',
     'injury of thumb',
     'closed traumatic dislocation of elbow joint',
     'malignant melanoma of trunk',
     'mass of ovary',
     'ovarian mass',
     'partial seizure',
     'focal seizure',
     'local seizure',
     'local convulsion',
     'partial seizures',
     'ulcer of toe',
     'common peroneal neuropathy',
     'lateral popliteal neuropathy',
     'cutaneous t-cell lymphoma',
     'cutaneous t-cell lymphoma, no icd-o subtype',
     'cutaneous t-cell lymphoma, no international classification of diseases for oncology subtype',
     'cutaneous lymphoma',
     'primary cutaneous t-cell lymphoma',
     'CTCl',
     'cervicovaginal cytology: low grade squamous intraepithelial lesion',
     'cervicovaginal cytology: lgsil',
     'cervicovaginal cytology: lsil',
     'facial nerve disorder',
     'disorders of the seventh nerve',
     'disorders of the viith cranial nerve',
     'disorder of facial nerve',
     'disorder of seventh cranial nerve',
     'facial neuropathy',
     'left ventricular cardiac dysfunction',
     'mild mixed bipolar i disorder',
     'mild bipolar i disorder, most recent episode mixed',
     'pseudoaneurysm',
     'false aneurysm',
     'aneurysmal hematoma',
     'pulsatile hematoma',
     'pulsatile haematoma',
     'aneurysmal haematoma',
     'repair of umbilical hernia',
     'umbilical hernioplasty',
     'umbilical herniorrhaphy',
     'disorder of vocal cord',
     'stenosis of lacrimal canaliculi',
     'stenosis of lacrimal passage',
     'stenosis of lacrimal canaliculus',
     'lactic acidosis',
     'contusion of lower limb',
     'contusion of leg',
     'congenital blocked tear duct',
     'congenital dacryostenosis',
     'congenital obstruction of lacrimal canal',
     'fracture of orbit',
     'fracture of orbital fossa',
     'orbital fracture',
     'increased lipid',
     'polyradiculoneuropathy',
     'malignant neoplasm of upper-inner quadrant of female breast',
     'single major depressive episode, in full remission',
     'pregnancy-related glycosuria',
     'glycosuria during pregnancy',
     'contracture of joint of multiple sites',
     'contracture of multiple joints',
     'burning sensation in eye',
     'radiofrequency ablation operation for arrhythmia',
     'aortoiliac atherosclerosis',
     'aortoiliac disease',
     'carcinoma of cardia',
     'fh: raised blood lipids',
     'family history: raised blood lipids',
     'fh: fats raised',
     'laceration of skin',
     'brain injury with open intracranial wound',
     'varicose veins of lower extremity without ulcer and without inflammation',
     'salmonella food poisoning',
     'salmonella gastroenteritis',
     'enteric paratyphosis',
     'mild depression',
     'screening for disorder',
     'pruritus of vagina',
     'itching of vagina',
     'atypical hyperplasia of breast',
     'carcinoma of exocervix',
     'disorder of esophagus',
     'oesophageal dysphagia',
     'pathological fracture of neck of femur',
     'disorder characterised by fever',
     'febrile disorder',
     'febrile illness',
     'febrile syndrome',
     'febrile illness (fever)',
     'psychophysiologic insomnia',
     'cystic dermoid choristoma',
     'dermoid cyst',
     'dermoid choristoma',
     'dermoid',
     'dermoid tumor',
     'mature cystic teratoma',
     'mental disorder due to drug',
     'liveborn born in hospital by cesarean section',
     'oligodendroglioma',
     'oligodendroglioma, no icd-o subtype',
     'oligodendroglioma, no international classification of diseases for oncology subtype',
     'warfarin overdosage',
     'warfarin overdose',
     'closed fracture of shaft of ulna',
     'closed fracture of the ulnar shaft',
     'disorder of ovary',
     'ovarian disorder',
     'delayed articulatory and language development',
     'developmental language delay',
     'disseminated intravascular coagulation',
     'consumptive coagulopathy',
     'dic syndrome',
     'defibrination syndrome',
     'hemorrhagic fibrinogenolysis',
     'consumptive thrombohemorrhagic disorder',
     'DIc',
     'haemorrhagic fibrinogenolysis',
     'consumptive thrombohaemorrhagic disorder',
     'acute febrile mucocutaneous lymph node syndrome',
     'kawasaki syndrome',
     'MCLs',
     'kawasakis disease',
     'kawasaki disease',
     'mucocutaneous lymph node syndrome',
     'MLNs',
     'kawasakis syndrome',
     'kawasakis mucocutaneous lymph node syndrome',
     'neoplasm of uncertain behaviour of endocrine gland',
     'candidiasis of the esophagus',
     'candidiasis of oesophagus',
     'oesophageal thrush',
     'candidosis of oesophagus',
     'esophageal thrush',
     'candidosis of esophagus',
     'candidiasis of esophagus',
     'candida of esophagus',
     'closed fracture of foot',
     'chronic pharyngitis',
     'chronic sore throat',
     'sore throat - chronic',
     'persistent sore throat',
     'sprain of hip',
     'sprain of hip joint',
     'hip sprain',
     'sprain of hip and thigh',
     'thigh sprain',
     'closed fracture of cervical spine',
     'closed fracture of lower end of humerus',
     'closed fracture of distal end of humerus',
     'closed fracture of the distal humerus',
     'tongue carcinoma',
     'mass of chest wall',
     'loss of sense of smell',
     'no sense of smell',
     'sense of smell lost',
     'sense of smell absent',
     'anosmia (loss of sense of smell)',
     'absent smell',
     'anosmia',
     'loss of the sense of smell',
     'depressed bipolar i disorder',
     'bipolar i disorder, most recent episode depressed',
     'heart disease',
     'disorder of heart',
     'cardiac disorder',
     'morbus cordis',
     'cardiopathy',
     'bullous keratopathy',
     'Bk',
     'hyphaema',
     'blood in anterior chamber',
     'hyphemia',
     'hyphaemia',
     'hyphema',
     'syringomyelia',
     'hydrosyringomyelia',
     'morvans disease',
     'syringomyelia-anesthesia syndrome',
     'myelosyringosis',
     'syringomyelia-anaesthesia syndrome',
     'irradiation cystitis',
     'radiation cystitis',
     'bone pain',
     'osteodynia',
     'cyanotic congenital heart disease',
     'closed fracture of medial malleolus',
     'closed fracture ankle, medial malleolus',
     'family history of osteoporosis',
     'fh: osteoporosis',
     'family history: osteoporosis',
     'portal vein thrombosis',
     'deep vein thrombosis of portal vein',
     'PVt',
     'ganglion of tendon sheath',
     'ganglion cyst of tendon sheath',
     'cirrhosis and chronic liver disease',
     'diverticulum of bladder',
     'bladder diverticulum',
     'unstable knee',
     'instability of knee joint',
     'knee gives way',
     'instability of knee',
     'thrombosed external hemorrhoids',
     'external thrombosed hemorrhoids',
     'thrombosed external hemorrhoid',
     'external thrombosed haemorrhoids',
     'thrombosed external haemorrhoid',
     'thrombosed external pile',
     'acute pain',
     'dysesthesia',
     'dysesthesia (abnormal sensation)',
     'dysaesthesia (abnormal sensation)',
     'dysesthesias',
     'severe mental retardation (i.q. 20-34)',
     'severe mental handicap',
     'severe mental retardation (intelligence quotient 20-34)',
     'chronic hypoxemic respiratory failure',
     'chronic type 1 respiratory failure',
     'chronic type i respiratory failure',
     'leiomyosarcoma',
     'LMs',
     'leiomyosarcoma, no subtype',
     'refractory epilepsy',
     'intractable epilepsy',
     'nephritis',
     'phlebitis',
     'inflammation of vein',
     'intervertebral disc disorder of cervical region with myelopathy',
     'haemoglobinopathy',
     'hemoglobin disease',
     'hemoglobin disorder',
     'globin abnormality',
     'haemoglobin disease',
     'haemoglobin disorder',
     'hemoglobinopathy',
     'amphetamine abuse',
     'drug abuse using speed',
     'amfetamine abuse',
     'premenopausal menorrhagia',
     'excessive bleeding at onset of menopause',
     'climacteric menorrhagia',
     'menopausal menorrhagia',
     'preclimacteric menorrhagia',
     'benign neoplasm of brain',
     'benign brain tumour',
     'benign brain tumor',
     'mediastinal mass',
     'mass of mediastinum',
     'secondary malignant neoplasm of lymph nodes of neck',
     'metastatic malignant neoplasm to lymph nodes of neck',
     'metastasis to cervical lymph nodes',
     'cancer metastatic to neck lymph nodes',
     'corneal scar',
     'disorder of knee',
     'mammographic breast mass',
     'mammographic breast mass finding',
     'diabetic retinopathy screening',
     'screening for cancer',
     'neoplasm screening',
     'examination for population survey',
     'nondependent cannabis abuse, episodic',
     'degeneration of cartilage and/or meniscus of knee',
     'degeneration of cartilage or meniscus of knee',
     'acth deficiency',
     'adrenocorticotropic hormone (acth) deficiency',
     'adrenocorticotropic hormone deficiency',
     'secondary hypoadrenalism',
     'primary gout',
     'acute gouty arthritis',
     'idiopathic gout',
     'acute gouty arthropathy',
     'direct current cardioversion',
     'electrical cardioversion',
     'cardioversion',
     'conversion of cardiac rhythm',
     'cardioversions',
     'defibrillation',
     'fetal or neonatal effect of maternal polyhydramnios',
     'foetal or neonatal effect of maternal polyhydramnios',
     'foetal effect of hydramnios',
     'fetal effect of hydramnios',
     'pustule',
     'pussey spot',
     'pustules',
     'joint swelling',
     'observation of joint swelling',
     'swollen joint',
     'strain of tendon of foot and ankle',
     'disorder of stomach',
     'gastropathy',
     'bacterial endocarditis',
     'Be',
     'closed supracondylar fracture of femur',
     'closed fracture distal femur, supracondylar',
     'neonatal polycythemia',
     'polycythemia neonatorum',
     'plethora of newborn',
     'polycythaemia neonatorum',
     'pedal cycle accident',
     'transurethral cystoscopy',
     'urethrocystoscopy',
     'cystourethroscopy',
     'cystourethroscopy (separate procedure)',
     'disruption of episiotomy wound in the puerperium',
     'episiotomy breakdown',
     'breakdown of perineum',
     'nonsenile cataract',
     'non age related cataract',
     'non age-related cataract',
     'adenocarcinoma of stomach',
     'cancer of stomach, adenocarcinoma',
     'blocked lacrimal canaliculus',
     'obstructed canaliculus',
     'obstruction of lacrimal canaliculus',
     'canalicular obstruction',
     'lacrimal canalicular obstruction',
     'obstruction of lacrimal duct',
     'haemoglobin ss disease with crisis',
     'sickle cell crisis',
     'sickle cell anemia with crisis',
     'sickle cell anaemia with crisis',
     'fistula',
     'fistulous tract',
     'abnormal sinus tract',
     'sinus',
     'fistulae',
     'fistulas',
     'sinuses',
     'oral phase dysphagia',
     'localized infection of skin and/or subcutaneous tissue',
     'twin liveborn born in hospital by caesarean section',
     'twin birth, in hospital, delivered by cesarean section',
     'hypertrophic cardiomegaly',
     'relaxation of pelvic floor',
     'high risk pregnancy due to history of preterm labour',
     'maternal obesity syndrome',
     'excessive weight gain in pregnancy',
     'maternal obesity without hypertension',
     'mechanical complication of breast prosthesis',
     'mechanical complication due to breast prosthesis',
     'mechanical complication due to breast implant',
     'pneumonia due to escherichia coli',
     'escherichia coli pneumonia',
     'pneumonia due to e. coli',
     'intracranial injury, without skull fracture',
     'head injury, without skull fracture',
     'patient discharge',
     'discharge procedure',
     'apraxia',
     'dyspraxia',
     'varicose veins of lower extremity with ulcer and inflammation',
     'operative procedure on shoulder',
     'operative procedure on shoulder region',
     'unlisted procedure, shoulder',
     'subconjunctival haemorrhage',
     'subconjunctival bleed',
     'neonatal jaundice due to delayed conjugation from breast milk inhibitors',
     'breast-feeding inhibitors causing neonatal jaundice',
     'breast milk inhibitor jaundice',
     'breast milk jaundice',
     'increased insulin level',
     'hyperinsulinism',
     'hyperinsulinaemia',
     'hyperinsulinemia',
     'one eye blind, one eye low vision',
     'impairment level: blindness, one eye - low vision other eye',
     'secondary malignant neoplasm of intra-abdominal lymph nodes',
     'metastatic malignant neoplasm to intra-abdominal lymph nodes',
     'cancer metastatic to intra-abdominal lymph nodes',
     'otosclerosis',
     'otospongiosis',
     'disorder of adrenal gland',
     'adrenal disorder',
     'adrenal disease',
     'pancreatic insufficiency',
     'zenkers diverticulum',
     'zenkers hypopharyngeal diverticulum',
     'zenker diverticulum',
     'increased creatine kinase level',
     'raised creatine kinase level',
     'elevated creatine kinase level',
     'encephalitis',
     'charcots arthropathy',
     'charcot arthropathy',
     'charcots joint',
     'arthropathy associated with a neurological disorder',
     'neuropathic arthropathy',
     'haemangioma of liver',
     'hepatic haemangioma',
     'hepatic hemangioma',
     'hepatic angioma',
     'bladder excision',
     'cystectomy',
     'urinary bladder excision',
     'excision procedures on the bladder',
     'cystectomies',
     'derangement of medial meniscus',
     'derangement medial meniscus',
     'hypogammaglobulinaemia',
     'hypogammaglobulinemia',
     'financial problem',
     'c-reactive protein abnormal',
     'traumatic arthropathy-ankle',
     'traumatic arthropathy of ankle',
     'thoracic and lumbosacral neuritis',
     'congenital pes planus',
     'congenital flat foot',
     'hamstring sprain',
     'family history of alcoholism',
     'alcoholism in family',
     'fh - alcoholism',
     'drug overdose - suicide',
     'vascular disorder',
     'disorder of blood vessel',
     'vascular disease',
     'angiopathy',
     'male infertility',
     'complication of transplanted liver',
     'glaucoma associated with ocular inflammation',
     'glaucoma with ocular inflammation',
     'pseudomembranous enterocolitis',
     'pseudomembranous colitis',
     'PMc',
     'autoimmune haemolytic anaemia',
     'ketoacidosis in type 1 diabetes mellitus',
     'ketoacidosis in type i diabetes mellitus',
     'diabetes type 1 with ketoacidosis',
     'history of malignant neoplasm of tongue',
     'history of cancer of the tongue',
     'history of malignant neoplasm of esophagus',
     'history of cancer of esophagus',
     'history of cancer of oesophagus',
     'arteriovenous fistula',
     'arteriovenous shunt',
     'early satiety',
     'early satiety (fullness)',
     'hypoplasia of mandibular bone',
     'gastrostomy operation',
     'gastrostomy',
     'creation of gastrostomy',
     'disorder of eyelid',
     'acute laryngitis',
     'bronchopulmonary dysplasia of newborn',
     'bronchopulmonary dysplasia',
     'chronic lung disease of prematurity',
     'perinatal bronchopulmonary dysplasia',
     'BPd',
     'ventilator lung in newborn',
     'perforation of nasal septum',
     'perforated nasal septum',
     'nasal septal perforation',
     'disorder of teeth and/or supporting structures',
     'herpes simplex without complication',
     'herpes zoster without complication',
     'neoplasm of kidney',
     'renal tumour',
     'tumour of kidney',
     'tumor of kidney',
     'renal tumor',
     'chronic peptic ulcer',
     'coag./bleeding tests abnormal',
     'coagulation/bleeding tests abnormal',
     'diagnostic arthroscopy of knee joint',
     'diagnostic arthroscopy of knee',
     'arthroscopy, knee, diagnostic, with or without synovial biopsy (separate procedure)',
     'infectious colitis, enteritis and gastroenteritis',
     'corn - lesion',
     'corn',
     'clavus',
     'corn of toe',
     'clavi',
     'corns',
     'meconium aspiration syndrome',
     'MAs',
     'neonatal aspiration of meconium',
     'aspiration of meconium',
     'replacing dentures',
     'special observation',
     'special supervision',
     'specialling',
     'sclerosing cholangitis',
     'tophus',
     'urate tophus',
     'gouty tophus',
     'tophi',
     'pregnancy test negative',
     'early onset dysthymia',
     'carcinoma of body of stomach',
     'malignant tumor of lesser curve of stomach',
     'ca lesser curvature - stomach',
     'sputum examination: abnormal',
     'abnormal sputum',
     'chronic acquired lymphoedema',
     'secondary lymphedema',
     'secondary lymphoedema',
     'benzodiazepine overdose',
     'paraumbilical hernia',
     'moderate chronic obstructive pulmonary disease',
     'poor sleep pattern',
     'homonymous hemianopia',
     'homonymous bilateral visual field defects',
     'malignant tumor of urinary tract proper',
     'malignant tumor of urinary tract',
     'malignant tumour of urinary tract',
     'hematometra',
     'uterine haemorrhage',
     'haemometra',
     'haemorrhage in uterus',
     'hemometra',
     'hemorrhage in uterus',
     'uterine hemorrhage',
     'basal cell carcinoma of truncal skin',
     'cancer of skin of trunk, basal cell',
     'disorder of haematopoietic structure',
     'bcg vaccination',
     'bacillus calmette-guerin (bcg) vaccination',
     'bacillus calmette-guerin vaccination',
     'tuberculosis immunisation',
     'bcg immunisation',
     'tuberculosis vaccination',
     'tuberculosis immunization',
     'bcg immunization',
     'allogeneic peripheral blood stem cell transplant',
     'active tuberculosis',
     'partial resection of colon',
     'STc',
     'subtotal colectomy',
     'segmental excision of colon',
     'partial colectomy',
     'segmental colectomy',
     'colectomy, partial',
     'loss of teeth due to extraction',
     'surgical edentia',
     'ulcerative pancolitis',
     'severe pre-eclampsia',
     'pet - severe pre-eclamptic toxaemia',
     'pet - severe pre-eclamptic toxemia',
     'severe proteinuric hypertension of pregnancy',
     'severe pre-eclamptic toxaemia',
     'severe pre-eclamptic toxemia',
     'vitamin b-complex deficiency',
     'vitamin b deficiency',
     'deficiency of vitamin b',
     'disorder of bursa of shoulder region',
     'shoulder bursa disorder',
     'disorder of shoulder bursa',
     'disorder of bursae of shoulder region',
     'known or suspected fetal abnormality',
     'hereditary essential tremor',
     'helicobacter-associated disease',
     'senile lentigo',
     'solar lentigo',
     'liver spot',
     'nevoid lentigo',
     'actinic lentigo',
     'naevoid lentigo',
     'subacute bacterial endocarditis',
     'SBe',
     'endocarditis lenta',
     'thoracic aortic aneurysm without rupture',
     'disorder of tendon of shoulder region',
     'transplantation - action',
     'transplant',
     'tx - transplant',
     'transplantation',
     'transplant procedure',
     'transplantations',
     'transplanted',
     'transplanting',
     'transplants',
     'accident while engaged in household activity',
     'accident at home',
     'household accident',
     'pneumoconiosis due to silica',
     'silicosis',
     'silicatosis',
     'nodular silicosis',
     'discectomy for intervertebral herniated disc, nucleus pulposus',
     'excision of disc for intervertebral herniated disc, nucleus pulposus',
     'vomiting of pregnancy',
     'vomiting as reason for care in pregnancy',
     'primary malignant neoplasm of female genital organ',
     'closed traumatic dislocation acromioclavicular joint',
     'congenital postural scoliosis',
     'congenital scoliosis',
     'corneal oedema',
     'hemospermia',
     'blood in semen',
     'disorder of bladder',
     'disorder of bladder and bladder neck',
     'disorder of urinary bladder',
     'history of amputation of foot',
     'scrotal mass',
     'mass of scrotum',
     'fracture of proximal end of femur',
     'hip fracture',
     'fracture of neck of femur',
     'femoral neck fracture',
     'fracture of hip',
     'nof - fracture of neck of femur',
     'nuclear magnetic resonance abnormal',
     'magnetic resonance imaging scan abnormal',
     'mri scan abnormal',
     'malignant neoplasm of posterior wall of urinary bladder',
     'singers nodes',
     'vocal cord nodule',
     'vocal fold nodule',
     'singers nodule',
     'screamers node',
     'singers node',
     'nodular chorditis',
     'chorditis cantorum',
     'singers, chorditis',
     'bleeding diverticulosis',
     'polyp of gallbladder',
     'non-traumatic rupture of achilles tendon',
     'aspergers disorder',
     'schizoid disorder of childhood',
     'aspergers syndrome',
     'asperger disorder',
     'aspergers disorder',
     'insomnia disorder related to another mental disorder',
     'insomnia due to mental disorder',
     'carcinoma of ampulla of vater',
     'ampullary carcinoma',
     'liposarcoma',
     'liposarcoma morphology',
     'liposarcoma, no icd-o subtype',
     'liposarcoma, no international classification of diseases for oncology subtype',
     'fibroliposarcoma',
     'transitional cell carcinoma of bladder',
     'TCc',
     'focal segmental glomerulosclerosis',
     'FSGs',
     'focal glomerular sclerosis',
     'FGs',
     'focal glomerulosclerosis',
     'ulcer of mouth',
     'oral ulcer',
     'mouth ulceration',
     'ulceration of oral mucosa',
     'mouth ulcer',
     'closed fracture of femur, distal end',
     'closed fracture of lower end of femur',
     'closed fracture of distal end of femur',
     'closed colles, fracture',
     'mass of axilla',
     'lump of axilla',
     'raised tsh level',
     'raised thyroid stimulating hormone level',
     'osteoarthrosis involving multiple sites but not designated as generalized',
     'malignant tumor of rectosigmoid junction',
     'ca rectosigmoid junction',
     'implantation of internal cardiac defibrillator',
     'musculo-aponeurotic fibromatosis',
     'abdominal fibromatosis',
     'abdominal desmoid tumour',
     'abdominal desmoid tumor',
     'abdominal desmoid',
     'mesenteric fibromatosis',
     'retroperitoneal fibromatosis',
     'acute ischaemic heart disease',
     'iliotibial band friction syndrome',
     'iliotibial band syndrome',
     'subclinical hyperthyroidism',
     'stricture of bile duct',
     'preoperative pulmonary examination',
     'adrenocortical hyperfunction',
     'disorder of corticoadrenal overactivity',
     'hypercortisolism',
     'cushings syndrome',
     'cushings syndrome iii',
     'overproduction of cortisol',
     'suprarenogenic syndrome',
     'hypercorticism',
     'itsenko-cushing syndrome',
     'itsenko disease',
     'cusshings syndrome',
     'white blood cell disorder',
     'wbc diseases',
     'spinal arthrodesis',
     'fixation of spine with fusion',
     'spinal fusion',
     'spondylosyndesis',
     'nystagmus',
     'superior vena cava syndrome',
     'acquired unequal leg length',
     'acquired disparity of leg length',
     'contusion of finger',
     'superficial bruising of finger',
     'contusion, finger',
     'peritoneal dialysis',
     'simple partial seizure',
     'simple partial seizure, consciousness not impaired',
     'alcohol withdrawal delirium',
     'delirium tremens',
     'dts - delirium tremens',
     'acute leukaemia',
     'stem cell leukemia',
     'acute leukaemia, morphology, including blast cell or undifferentiated leukaemia',
     'acute leukemia, morphology, including blast cell or undifferentiated leukemia',
     'blast cell leukemia',
     'undifferentiated leukemia',
     'undifferentiated leukaemia',
     'stem cell leukaemia',
     'blast cell leukaemia',
     'acute leukaemia, disease',
     'acute leukemia, disease',
     'haemorrhagic diarrhoea',
     'bloody diarrhea',
     'bloody diarrhoea',
     'tick bite',
     'psychoactive substance-induced organic mental disorder',
     'h/o: respiratory disease',
     'history of respiratory disease',
     'h/o: artificial heart valve',
     'history of artificial heart valve',
     'o/e: oxalate renal calculus',
     'on examination - oxalate renal calculus',
     'amniotic fluid -meconium stain',
     'meconium stained liquor',
     'meconium stained amniotic fluid',
     'meconium staining',
     'a/n care: poor obstetric history',
     'antenatal care: poor obstetric history',
     'pre-pregnancy education',
     'pre-conception education',
     'subtotal abdominal hysterectomy',
     'convergence insufficiency',
     'Ci',
     'aortic stenosis, non-rheumatic',
     'non-neoplastic nevus',
     'drug-induced gastrointestinal disturbance',
     'urinary tract infection following delivery',
     'postpartum urinary tract infection',
     'fetal or neonatal effect of maternal complication of pregnancy',
     'foetal or neonatal effect of maternal complication of pregnancy',
     'closed fracture of middle and/or proximal phalanx of finger',
     'respiratory failure without hypercapnia',
     'hypoxemic respiratory failure',
     'type i respiratory failure',
     'type 1 respiratory failure',
     'acute posterior myocardial infarction',
     'accident due to exposure to weather conditions',
     'exposure',
     'exposures',
     'tenderness',
     'tender pain',
     'sore to touch',
     'soreness',
     'sore pain',
     'arthralgia of the upper arm',
     'carcinoma of breast - upper, outer quadrant',
     'weakness of hand',
     'spondylolisthesis l5/s1 level',
     'relationship counselling',
     'complete denture, including adjustments',
     'fit complete denture',
     'insertion of complete denture',
     'diagnostic amniocentesis',
     'amniocentesis',
     'incision and drainage of amnion',
     'amniocentesis; diagnostic',
     'disorder of endocrine system',
     'endocrinopathy',
     'endocrine disease',
     'endocrine system disease',
     'hormone disturbance',
     'endocrine disturbance',
     'disease of endocrine gland',
     'disorder of endocrine gland',
     'endocrine disorder',
     'endocrinopathies',
     'artificial respiration',
     'mechanical ventilation',
     'artificial ventilation',
     'mechanically assisted breathing',
     'respiratory assist, mechanical',
     'mechanically assisted ventilation',
     'meningococcus vaccination',
     'meningitis vaccination',
     'meningococcus immunisation',
     'meningococcus immunization',
     'infectious eczematoid dermatitis',
     'dermatitis infectiosa eczematoides',
     'infective eczematoid dermatitis',
     'diabetic gastroparesis associated with type 2 diabetes mellitus',
     'diabetes type 2 with gastroparesis',
     'primary focal hyperhidrosis',
     'chronic recurrent sinusitis',
     'fracture of tibial plateau',
     'rheumatoid arthritis of foot',
     'drug seeking behaviour',
     'drug seeking',
     'adenocarcinoma',
     'adenocarcinoma, no subtype',
     'malignant adenomatous neoplasm',
     'adenocarcinomas',
     'malignant carcinoid tumour',
     'decompression of median nerve',
     'carpal tunnel decompression',
     'decompression of median nerve at wrist',
     'carpal tunnel release',
     'CTr',
     'CTd',
     'release of carpal tunnel for nerve decompression',
     'release of carpal tunnel for median nerve decompression',
     'release of transverse carpal ligament for nerve decompression',
     'retinaculotomy of carpal tunnel',
     'hypertensive renal failure',
     'chronic duodenal ulcer with perforation',
     'perforated chronic duodenal ulcer',
     'tonic-clonic seizure',
     'grand mal convulsion',
     'grand mal seizure',
     'tonic-clonic convulsion',
     'tonic-clonic seizures',
     'operation on nasal septum',
     'nasal septal operation',
     'operation on septum of nose',
     'scalding pain on urination',
     'burning pain on urination',
     'tardy ulnar nerve palsy',
     'subdural haemorrhage following injury without open intracranial wound and with moderate loss of consciousness (1-24 hours)',
     'pulmonary congestion',
     'alpha thalassaemia',
     'alpha thalassaemia syndrome',
     'alpha thalassemia syndrome',
     'mechanical ptosis',
     'failed medical induction of labour',
     'bleeding gums',
     'gingival bleeding',
     'gingival crevicular bleeding',
     'bleeding of subgingival space',
     'gingival crevice bleeding',
     'crevicular bleeding of gum',
     'gingival hemorrhage',
     'gingival haemorrhage',
     'mechanical vitrectomy by pars plana approach',
     'pars plana vitrectomy',
     'PPv',
     'vitrectomy, mechanical, pars plana approach; with removal of internal limiting membrane of retina (eg, for repair of macular hole, diabetic macular edema), includes, if performed, intraocular tamponade (ie, air, gas or silicone oil)',
     'globus sensation',
     'benign neoplasm of adrenal gland',
     'benign tumour of adrenal gland',
     'benign tumor of adrenal gland',
     'pseudocyst of pancreas',
     'pancreatic pseudocyst',
     'neoplasm of adrenal gland',
     'adrenal tumour',
     'tumour of adrenal gland',
     'adrenal tumor',
     'tumor of adrenal gland',
     'abnormal involuntary movement',
     'post poliomyelitis syndrome',
     'postpolio syndrome',
     'postpolio muscular atrophy',
     'anoxic encephalopathy',
     'anoxic brain damage',
     'anoxic brain injury',
     'history of malignant neoplasm of lung',
     'h/o: malignant neoplasm of lung',
     'personal history of primary malignant neoplasm of lung',
     'perforation of intestine',
     'intestinal perforation',
     'chondromalacia',
     'respiratory arrest',
     'h/o: poliomyelitis',
     'history of poliomyelitis',
     'congenital nasolacrimal duct obstruction',
     'CNLDo',
     'CNDo',
     'neonatal nasolacrimal duct obstruction',
     'recurrent acute otitis media',
     'acontractile detrusor',
     'atonic bladder',
     'atony of bladder',
     'atonic urinary bladder',
     'congenital malformation of genital organs',
     'congenital deformity of genital organ',
     'congenital anomaly of genital organ',
     'congenital genital organ anomalies',
     'depressed bipolar i disorder in full remission',
     'bipolar i disorder, most recent episode depressed, in full remission',
     'foot callus',
     'callosity on foot',
     'transient global amnesia',
     'TGa',
     'radiation proctitis',
     'radiation induced proctitis',
     'alcoholic hepatitis',
     'Ah',
     'contracture of knee joint',
     'knee joint contracture',
     'partial seizure evolving to secondary generalized seizure',
     'erythema',
     'erythema - observation',
     'erythema of skin',
     'erythematous condition',
     'viremia',
     'viral sepsis',
     'dislocation of patellofemoral joint',
     'dislocated patella',
     'redundant prepuce and phimosis',
     'redundant prepuce with phimosis',
     'petechiae',
     'petechia',
     'petechial haemorrhage',
     'petechial hemorrhage',
     'abrasion of lower limb',
     'graze of leg',
     'carcinoma of anal canal',
     'alpha-1-antitrypsin deficiency',
     'alpha-1-proteinase inhibitor deficiency',
     'mass of lower limb',
     'history of substance abuse',
     'history of drug abuse',
     'h/o: drug abuse',
     'decreased muscle tone',
     'poor muscle tone',
     'low muscle tone',
     'hypotonus',
     'muscle hypotonicity',
     'loose muscle tone',
     'muscle hypotonia',
     'hypotonia',
     'bladder dysfunction',
     'accidental antepartum hemorrhage',
     'placental abruption',
     'ablatio placentae',
     'abruptio placentae',
     'premature detachment of normally implanted placenta',
     'premature detachment of placenta',
     'premature separation of placenta',
     'portal cirrhosis',
     'laennecs cirrhosis',
     'Pc',
     'chronic atrial fibrillation',
     'lid retraction',
     'retraction of eyelid',
     'chondrosarcoma, no icd-o subtype',
     'chondrosarcoma morphology',
     'chondrosarcoma',
     'fibrochondrosarcoma',
     'chondrosarcoma, no international classification of diseases for oncology subtype',
     'exposure to hepatitis b virus',
     'irregular astigmatism',
     'postcoital bleeding',
     'bleeding after intercourse',
     'PCb',
     'contusion of toe',
     'superficial bruising of toe',
     'bruise of toe',
     'fracture of skull',
     'fractured skull',
     'foreign body in nose',
     'fb - nasal foreign body',
     'closed fracture of distal phalanx of finger',
     'closed fracture finger distal phalanx',
     'episcleritis',
     'stenosis of cervix',
     'stricture of cervix',
     'purulent nasal discharge',
     'purulent rhinitis',
     'otitis externa',
     'Oe',
     'swimmers ear',
     'infective otitis externa',
     'infective oe',
     'refractive amblyopia',
     'ametropic amblyopia',
     'meridional amblyopia',
     'obstruction of pelviureteric junction',
     'PUj',
     'pelviureteric obstruction',
     'UPj',
     'ureteropelvic obstruction',
     'abnormal urine',
     'age-related cognitive decline',
     'transoesophageal echocardiography',
     'TOe',
     'transesophageal echocardiography',
     'tee procedure',
     'transesophageal echocardiogram',
     'transoesophageal echocardiogram',
     'echocardiography, transesophageal (tee) for guidance of a transcatheter intracardiac or great vessel(s) structural intervention(s) (eg,tavr, transcathether pulmonary valve replacement, mitral valve repair, paravalvular regurgitation repair, left atrial appendage occlusion/closure, ventricular septal defect closure) (peri-and intra-procedural), real-time image acquisition and documentation, guidance with quantitative measurements, probe manipulation, interpretation, and report, including diagnostic transesophageal echocardiography and, when performed, administration of ultrasound contrast, doppler, color flow, and 3d',
     'bronchoscopy',
     'endoscopy of bronchus',
     'by - bronchoscopy',
     'tracheobronchial endoscopy',
     'bronchial endoscopy',
     'bronchoscopies',
     'acquired scoliosis',
     'kidney transplant evaluation',
     'neoplasm of breast',
     'tumour of breast',
     'breast tumour',
     'breast tumor',
     'tumor of breast',
     'ischaemic ulcer',
     'arterial ulcer',
     'ulcer of artery',
     'gastric ulcer with haemorrhage',
     'bleeding gastric ulcer',
     'fh: breast disease',
     'family history of breast disorder',
     'family history: breast disease',
     'h/o: artificial joint',
     'history of artificial joint',
     'neutropenia',
     'neutrophil count below reference range',
     'syphilis titre test positive',
     'hysterosalpingography',
     'hysterosalpingogram',
     'salp - hysterosalpingogram',
     'HSg',
     'breast fed',
     'infant breastfed',
     'Bf',
     'breastfeeding',
     'breastfed',
     'malignant neoplasm of lower-outer quadrant of female breast',
     'emotionally unstable personality disorder',
     'combined drug dependence, excluding opioid, in remission',
     'nondependent cannabis abuse in remission',
     'chronic obstructive pulmonary disease with acute lower respiratory infection',
     'non-obstructive reflux-associated chronic pyelonephritis',
     'chronic non-obstructive atrophic pyelonephritis',
     'nephropathy associated with vesicoureteral reflux',
     'Rn',
     'reflux nephropathy',
     'glenoid labrum tear',
     'tear of glenoid labrum',
     'neonatal candidiasis of perineum',
     'repair of aneurysm of abdominal aorta with graft',
     'accidents caused by knives, swords and daggers',
     'total parenteral nutrition',
     'total parenteral nutrition administration',
     'suralimentation',
     'superalimentation',
     'parenteral hyperalimentation',
     'central hyperalimentation',
     'hyperalimentation',
     'TPn',
     'tpn procedure',
     'diabetic mononeuropathy',
     'diabetic mononeuropathy simplex',
     'acute transient psychotic disorder',
     'central venous cannula insertion',
     'inserting central line',
     'cvp line insertion',
     'central line insertion',
     'atypical pneumonia',
     'dissection of thoracic aorta',
     'dissecting aneurysm of thoracic aorta',
     'immunodeficiency disorder',
     'immunodeficiency disease',
     'immunodeficiency',
     'immunodeficiencies',
     'perforation of small intestine',
     'small bowel perforation',
     'undifferentiated connective tissue disease',
     'sigmoidoscopy',
     'sigy - sigmoidoscopy',
     'SIGGy',
     'sigmoidoscopies',
     'proctosigmoidoscopy',
     'tibial torsion',
     'bruise of head',
     'superficial bruising of head',
     'simple goitre',
     'non-toxic goiter',
     'non-toxic simple goiter',
     'non-toxic simple goitre',
     'non-toxic goitre',
     'iodine-deficiency-related endemic goiter',
     'iodine-deficiency-related diffuse (endemic) goitre',
     'iodine-deficiency-related diffuse (endemic) goiter',
     'endemic goiter',
     'simple iodine deficiency goitre',
     'simple iodine deficiency goiter',
     'congenital capsular cataract',
     'feeding poor',
     'poor feeding',
     'fracture of greater trochanter',
     'lumbosacral strain',
     'erythema infectiosum',
     'parvovirus b19 infection',
     'slapped cheek syndrome',
     'fifth disease',
     'erythema contagiosum',
     'parvo virus',
     'infection resistant to penicillin',
     'infection due to penicillin-resistant organism',
     'carcinoma of main bronchus',
     'peptic ulcer without hemorrhage, without perforation and without obstruction',
     'endoscopic catheterisation of pancreatic duct and bile duct systems',
     'endoscopic retrograde cholangiopancreatography',
     'ERCp',
     'endoscopic retrograde choledochopancreatography',
     'diagnostic endoscopic retrograde examination of bile duct and pancreatic duct',
     'endoscopic retrograde cholangiopancreatography (ercp)',
     'corpus luteum cyst',
     'lutein cyst',
     'luteal cystic ovary disease',
     'hyperosmolar non-ketotic state in type 2 diabetes mellitus',
     'avascular necrosis of bone',
     'AVn',
     'avascular necrosis',
     'ischemic necrosis',
     'ischaemic necrosis',
     'rheumatoid arteritis',
     'acute st segment elevation myocardial infarction',
     'STEMi',
     'st elevation myocardial infarction',
     'acute urticaria',
     'thrombophlebitis of superficial veins of lower extremity',
     'coronary artery bypass graft occlusion',
     'ventral incisional hernia of anterior abdominal wall without obstruction and without gangrene',
     'incisional hernia of anterior abdominal wall without obstruction and without gangrene',
     'no evidence of recurrence of cancer',
     'gastrointestinal stromal tumour, uncertain malignant potential',
     'gastrointestinal pacemaker cell tumor',
     'gastrointestinal autonomic nerve tumor',
     'GISt',
     'GANt',
     'gastrointestinal stromal tumor',
     'gastrointestinal autonomic nerve tumour',
     'gastrointestinal pacemaker cell tumour',
     'gastrointestinal stromal tumour',
     'gastrointestinal stromal sarcoma',
     'gastrointestinal stromal tumor, malignant',
     'gist, malignant',
     'gastrointestinal stromal tumour, malignant',
     'snapping thumb syndrome',
     'clicking thumb',
     'trigger thumb',
     'triggering of thumb',
     'trigger thumb - acquired',
     'nodular tendinous disease of thumb',
     'major depression in remission',
     'major depression, in remission',
     'major depressive disorder, in remission (mdd)',
     'abnormal posture',
     'abnormal body position',
     'family history of malignant neoplasm of ovary',
     'injury of digital nerve',
     'digital nerve injury',
     'exposure to human immunodeficiency virus',
     'chronic inflammatory demyelinating polyneuritis',
     'arteriosclerotic gangrene',
     'fracture of shaft of femur',
     'open wound of forearm with tendon involvement',
     'scar of skin',
     'cicatrix of skin',
     'abscess of lung',
     'bladder trabeculation',
     'trabeculated bladder',
     'poems syndrome',
     'polyneuropathy organomegaly endocrinopathy monoclonal gammopathy and skin changes',
     'POEMs',
     'polyneuropathy organomegaly endocrinopathy monoclonal and skin changes',
     'polyneuropathy, organomegaly, endocrinopathy, monoclonal gammopathy, and skin changes syndrome',
     'polyneuropathy, organomegaly, endocrinopathy, monoclonal gammopathy, and skin changes (poems) syndrome',
     'pain due to any device, implant and/or graft',
     'chronic mastoiditis',
     'congenital spondylolysis of lumbosacral region',
     'congenital lumbosacral spondylolysis',
     'congenital hydrocoele',
     'congenital hydrocele of tunica vaginalis',
     'congenital hydrocoele of tunica vaginalis',
     'poisoning by antidepressant',
     'poisoning by antidepressant drug',
     'mycobacteriosis',
     'infection due to mycobacteria',
     'mycobacterial disease',
     'closed fracture of thoracic vertebra without spinal cord injury',
     'closed fracture of dorsal vertebra without spinal cord injury',
     'anisocoria',
     'unequal pupils',
     'anisocoria - unequal pupil diameter',
     'unequal pupil diameter',
     'microcytosis',
     'microcytosis, red cells',
     'recurrent erosion of cornea',
     'recurrent erosion syndrome',
     'recurrent corneal erosion',
     'lacunar infarction',
     'lacunar stroke',
     'LACi',
     'Li',
     'lacunar infarct',
     'hypogonadotropic hypogonadism',
     'gonadotrophin deficiency',
     'secondary hypogonadism',
     'contusion of wrist',
     'superficial bruising of wrist',
     'contusion, wrist',
     'cellulitis of periorbital region',
     'periorbital cellulitis',
     'derangement of meniscus',
     'disorder of meniscus of knee',
     'derangement of meniscus of knee joint',
     'transfusion of blood product',
     'blood product administration by intravascular infusion',
     'transfusing blood products',
     'blood transfusion',
     'transfusion, blood or blood components',
     'injury of face',
     'facial injury',
     'face injury',
     'decreased testosterone level',
     'low testosterone',
     'radial neuropathy',
     'radial nerve palsy',
     'orbital cellulitis',
     'postseptal orbital cellulitis',
     'cardiomyopathy associated with another disorder',
     'secondary cardiomyopathy',
     'cardiomyopathy, secondary',
     'closed traumatic dislocation of hip',
     'closed traumatic dislocation of hip joint',
     'open wound of scalp',
     'tachypnoea',
     'tachypnoeic',
     'tachypneic',
     'rapid respiration',
     'rapid breathing',
     'tonsil carcinoma',
     'tendon contracture',
     'contracture of tendon sheath',
     'foreign body - finger',
     'facial palsy',
     'facial nerve paralysis',
     'ectopic rhythm',
     'premature beats',
     'ectopics',
     'extrasystoles',
     'extrasystolic arrhythmia',
     'ectopic beats',
     'ectopic rhythm abnormality',
     'shoulder joint deformity',
     'swallowing painful',
     'odynophagia',
     'painful swallowing',
     'pain on swallowing',
     'trabeculectomy ab externo',
     'ocular trabeculectomy',
     'creation of subscleral fistula to sclera',
     'creation of guarded fistula to sclera',
     'trabeculectomy',
     'cervical lymphadenitis',
     'cervical adenitis',
     'alternating exotropia',
     'chandler syndrome',
     'corneal endothelial dystrophy',
     'dystrophy of corneal endothelium',
     'endothelial corneal dystrophy',
     'stammering',
     'stammer',
     'stutter',
     'dysphemia',
     'non-fluent speech',
     'stuttered',
     'stuttering',
     'stutters',
     'furunculosis of skin and/or subcutaneous tissue',
     'recurring skin boils',
     'recurring furuncles of skin',
     'hb ss disease',
     'hemoglobin s-s disease',
     'drepanocythemia',
     'hereditary hemoglobinopathy disorder homozygous for hemoglobin s',
     'sickle cell anemia',
     'drepanocythaemia',
     'sickle cell anaemia',
     'haemoglobin s disease',
     'haemoglobin s-s disease',
     'hemoglobin s disease',
     'hb s disease',
     'sickling disorder due to haemoglobin s',
     'sickle cell disease',
     'sickle cell syndrome',
     'prolapse of vaginal vault after hysterectomy',
     'post-hysterectomy vaginal vault prolapse',
     'mass of urinary bladder',
     'bladder mass',
     'mass of bladder',
     'family history of malignant neoplasm of lung',
     'family history of lung cancer',
     'cyst of jaw',
     'diarrhoea of presumed infectious origin',
     'instability of pelvic floor',
     'pain in forearm',
     'lumbosacral radiculitis',
     'acquired hemolytic anemia',
     'tetraplegic cerebral palsy',
     'cerebral palsy with spastic tetraplegia',
     'tetraplegic spastic cerebral palsy',
     'quadriplegic spastic cerebral palsy',
     'spastic quadriplegic cerebral palsy',
     'quadriplegic cerebral palsy',
     'tibialis tendinitis',
     'tibialis tendonitis',
     'chronic otitis externa',
     'cellulitis of abdominal wall',
     'laparoscopic appendicectomy',
     'endoscopic appendicectomy',
     'endoscopic appendectomy',
     'laparoscopy, surgical, appendectomy',
     'amputation above-knee',
     'ak amputation',
     'amputation of leg above knee',
     'amputation of leg through femur',
     'amputation supracondylar, above-knee',
     'kirk amputation of thigh',
     'AKa',
     'above knee amputation',
     'amputation of thigh through femur',
     'transfemoral amputation',
     'amputation, thigh, through femur, any level',
     'late effect of spinal cord injury',
     'sequelae of injury of spinal cord',
     'chronic pansinusitis',
     'photosensitivity',
     'photosensitivity of skin',
     'benign neoplasm of soft tissues of upper limb',
     'benign tumour of soft tissue of upper limb',
     'benign tumor of soft tissue of upper limb',
     'hepatitis a virus antibody, total measurement',
     'tuberculous adenitis',
     'tuberculosis of lymph node',
     'tuberculous lymphadenopathy',
     'melanoma in situ of skin (clinical)',
     'melanoma in situ of skin',
     'ISm',
     'in situ melanoma of skin',
     'in situ malignant melanoma of skin',
     'ISMm',
     'melanoma in situ',
     'h/o: liver recipient',
     'history of liver recipient',
     'placenta incomplete',
     'ruptured ectopic pregnancy',
     'aborted ectopic pregnancy',
     'rheumatic aortic stenosis with regurgitation',
     'rheumatic aortic stenosis with insufficiency',
     'rheumatic aortic stenosis with incompetence',
     'hyposensitization to allergens',
     'allergen immunotherapy',
     'desensitization treatment',
     'desensitization to allergens',
     'desensitizing immunotherapy',
     'desensitisation to allergens',
     'desensitising immunotherapy',
     'desensitisation treatment',
     'nondependent mixed drug abuse in remission',
     'non-rheumatic mitral valve stenosis',
     'nephrotic syndrome in amyloidosis',
     'staghorn calculus',
     'pigmented skin lesion',
     'fetal or neonatal effect of maternal infection',
     'foetal or neonatal effect of maternal infection',
     'foetal or neonatal effect of maternal infectious disease',
     'fetal or neonatal effect of maternal infectious disease',
     'neonatal dacryocystitis and conjunctivitis',
     'traumatic rupture of lumbar intervertebral disc',
     'accidentally struck by falling object',
     'late effect of motor vehicle accident',
     'subglottic stenosis',
     'anisometropic amblyopia',
     'double lung transplant',
     'dltx - double lung transplant',
     'malingering',
     'feigning illness',
     'menopause ovarian failure',
     'ureteroileostomy',
     'construction of ileal conduit',
     'ureteroileal conduit with ileal bladder including bowel anastomosis',
     'uretero-ileostomy',
     'ileal ureterostomy',
     'creation of ileal conduit',
     'ileoureterostomy with ileal bladder',
     'construction of standard ileal conduit',
     'ureteroileal conduit (ileal bladder), including intestine anastomosis (bricker operation)',
     'broken skin',
     'skin breakdown',
     'defect in skin surface',
     'impaired skin integrity',
     'skin integrity impairment',
     'mixed receptive-expressive language disorder',
     'subdural haemorrhage following injury without open intracranial wound and with brief loss of consciousness (less than one hour)',
     'noninflammatory disorders of the ovary, fallopian tube and broad ligament',
     'open wound of foot, excluding toe(s)',
     'accident caused by machinery',
     'industrial machinery accident',
     'schizoaffective disorder, manic type',
     'schizophreniform psychosis, manic type',
     'hemangioma of skin and subcutaneous tissue',
     'swelling of scrotum',
     'swollen scrotum',
     'generalized pruritus',
     'generalised itching',
     'generalized itching',
     'hearing aid worn',
     'acute schizophrenia-like psychotic disorder',
     'foot joint pain',
     'arthralgia of foot',
     'abnormal female sexual function',
     'mobitz type ii atrioventricular block',
     'mobitz type 2 second degree atrioventricular block',
     'mobitz type ii incomplete atrioventricular block',
     'second degree mobitz type ii incomplete atrioventricular block',
     'soft tissue injury',
     'STi',
     'antineoplastic adverse reaction',
     'soft tissue swelling',
     'difficulty sleeping',
     'poor sleep',
     'numbness of foot',
     'local recurrence of malignant tumour of rectum',
     'traction detachment of retina',
     'traction retinal detachment',
     'TRd',
     'toxic effect of lead compound',
     'lead - toxic effect',
     'lead compound poisoning',
     'saturnine poisoning',
     'plumbism',
     'lead poisoning',
     'neuroleptic-induced tardive dyskinesia',
     'neuroleptic induced tardive dyskinesia',
     'benign paraproteinemia',
     'burning mouth syndrome',
     'BMs',
     'orodynia',
     'stomatopyrosis',
     'basal cell carcinoma of nose',
     'cancer of nose, basal cell',
     'alt (sgpt) level raised',
     'alanine aminotransferase (alt) (serum glutamic-pyruvic transaminase) (sgpt) level raised',
     'alanine aminotransferase (serum glutamic-pyruvic transaminase) level raised',
     'laryngopharyngeal reflux',
     'harvesting of peripheral blood stem cells',
     'harvest of peripheral blood stem cells',
     'PBSc',
     'peripheral blood stem cell harvest',
     'peripheral blood stem cell collection',
     'refractory migraine with aura',
     'intractable migraine with aura',
     'intractable classical migraine',
     'classical migraine, intractable',
     'chemotherapy-induced neutropenia',
     'passive smoker',
     'involuntary smoker',
     'exposed to second hand tobacco smoke',
     'ETs',
     'exposed to tobacco smoke',
     'exposed to environmental tobacco smoke',
     'permanent atrial fibrillation',
     'gestational age unknown',
     'complete tetraplegia due to lesion at c5-c7 level',
     'complete quadriplegia due to lesion at c5-c7 level',
     'complete quadriplegia due to spinal cord lesion between fifth and seventh cervical vertebra',
     'medulloblastoma, melanotic',
     'melanotic medulloblastoma',
     'medulloblastoma',
     'MDb',
     'medulloblastoma, no icd-o subtype',
     'medulloblastoma, no international classification of diseases for oncology subtype',
     'mixed glioma',
     'oligoastrocytoma',
     'anaplastic oligoastrocytoma',
     'closed fracture of one rib',
     'calculus of bile duct with obstruction',
     'gestational diabetes mellitus, class a>2<',
     'gdm, class a>2<',
     'idiopathic retroperitoneal fibrosis',
     'retroperitoneal fibrosis',
     'RPf',
     'ormonds disease',
     'sclerosing retroperitonitis',
     'primary idiopathic dilated cardiomyopathy',
     'history and physical examination, administrative',
     'tubo-ovarian abscess',
     'tubo ovarian abscess',
     'crst syndrome',
     'calcinosis cutis, raynauds, sclerodactyly and telangiectasia',
     'CRSt',
     'calcinosis, raynauds phenomenon, sclerodactyly, telangiectasia syndrome',
     'calcinosis, raynauds phenomenon, sclerodactyly, and telangiectasia (crst) syndrome',
     'calcinosis, raynaud phenomenon, sclerodactyly, and telangiectasia (crst) syndrome',
     'calcinosis, raynauds phenomenon, sclerodactyly, and telangiectasia syndrome',
     'paralysis of diaphragm',
     'diaphragmatic paralysis',
     'fracture of acetabulum',
     'cerebrovascular system anomalies',
     'congenital anomaly of cerebrovascular system',
     'congenital anomaly of cerebral vessels',
     'acquired deformity of hip',
     'prolapse of female genital organs',
     'genital prolapse',
     'employment problem',
     'atrophic vulvovaginitis',
     'senile vulvovaginitis',
     'frontal sinusitis',
     'insertion of implantable venous access port',
     'intolerant of cold',
     'sensitive to cold',
     'feels the cold',
     'chilly person',
     'cold sensitivity',
     'primary malignant neoplasm of ovary',
     'rupture of tendon of biceps',
     'rupture of biceps tendon',
     'rupture biceps tendon',
     'rupture of achilles tendon',
     'rupture achilles tendon',
     'autoimmune disease',
     'autoimmune disorder',
     'pancreaticoduodenectomy',
     'whipple operation',
     'whipple procedure',
     'whipple operation, proximal pancreatectomy',
     'disorder of shoulder',
     'disorder of shoulder region',
     'exposure keratoconjunctivitis',
     'exposure keratitis',
     'exposure keratopathy',
     'lagophthalmic keratitis',
     'laboratory procedure',
     'laboratory procedures',
     'laboratory test',
     'laboratory procedures -general',
     'subclavian steal syndrome',
     'subclavian artery stenosis',
     'bereavement counselling',
     'counseling for bereavement',
     'counselling for bereavement',
     'migraine variants',
     'partial oculomotor nerve palsy',
     'third nerve palsy - partial',
     'chronic urinary tract infection',
     'food intolerance',
     'malabsorption due to food intolerance',
     'repair of ventral hernia',
     'repair of hernia of anterior abdominal wall',
     'ventral herniorrhaphy',
     'ventral hernioplasty',
     'proximal muscle weakness',
     'chiari malformation type i',
     'arnold chiari type 1',
     'candidal intertrigo',
     'candida infection of flexural skin',
     'candida intertrigo',
     'flexural candidosis',
     'general symptom',
     'visual testing',
     'non-cardiac chest pain',
     'noncardiac chest pain',
     'peripheral blood stem cell graft',
     'peripheral blood stem cell transplantation',
     'peripheral blood stem cell rescue',
     'second stage peripheral stem cell infusion',
     'injury of toe',
     'osteomyelitis of ankle and/or foot',
     'has difficulty with speech',
     'disturbance in speech',
     'speech impairment',
     'speech abnormality',
     'swelling of hand',
     'swollen hand',
     'malignant tumour of ureter',
     'cancer of ureter',
     'ureter ca',
     'history of alcohol abuse',
     'swallowing problem',
     'impaired swallowing',
     'swallowing impairment',
     'respiratory insufficiency',
     'leucoplakia of oral mucosa',
     'aphasia as late effect of cerebrovascular disease',
     'tenosynovitis of wrist',
     'sepsis due to staphylococcus',
     'osteomalacia',
     'closed fracture of orbital floor (blow-out)',
     'closed orbital blow-out fracture',
     'closed blow-out fracture orbit',
     'rheumatic disease of tricuspid valve',
     'rheumatic tricuspid valve disease',
     'acute cystitis',
     'cellulitis of breast',
     'vascular insufficiency of intestine',
     'ischemic bowel disease',
     'vascular insufficiency of the intestine',
     'intestinal ischemia',
     'ischemic disease of gut',
     'intestinal ischaemia',
     'ischaemic bowel disease',
     'ischaemic disease of gut',
     'peripheral neuritis',
     'neuritis',
     'gastric varices',
     'gastric varix',
     'secondary malignant neoplasm of kidney',
     'metastatic tumour in kidney',
     'metastatic tumor in kidney',
     'metastasis to renal parenchyma',
     'secondary renal cancer',
     'renal metastasis',
     'secondary cancer of kidney',
     'metastatic malignant neoplasm to kidney',
     'cancer metastatic to kidney',
     'visual system disorder',
     'disorder of vision',
     'vision disorder',
     'small intestine excision',
     'resection of small intestine',
     'excision small intestine',
     'small bowel resection',
     'candidiasis of vulva',
     'vulval candidiasis',
     'vulval thrush',
     'vulval candidosis',
     'defective dental restoration',
     'defective restoration of teeth',
     'abnormal tooth restoration',
     'late effects of poliomyelitis',
     'sequelae of poliomyelitis',
     'late effects of acute poliomyelitis',
     'inactive tuberculosis',
     'tuberculosis infection latent',
     'LTBi',
     'latent tuberculosis infection',
     'injury of elbow',
     'elbow injury',
     'open wound of wrist',
     'neoplasm of lung',
     'tumour of lung',
     'tumor of lung',
     'neoplasm of prostate',
     'tumour of prostate',
     'tumor of prostate',
     'NGp',
     'new growth of prostate',
     'chronic myelomonocytic leukemia',
     'chronic myelomonocytic leukaemia',
     'chronic myelomonocytic leukemia, type ii',
     'chronic myelomonocytic leukemia, type i',
     'chronic myelomonocytic leukemia in transformation [obs]',
     'chronic myelomonocytic leukaemia in transformation [obs]',
     'chronic myelomonocytic leukaemia, type i',
     'chronic myelomonocytic leukaemia, type ii',
     'lithotripsy',
     'lithotrity',
     'lithotripsies',
     'musculoskeletal system endoscopy',
     'arthroscopy',
     'arthroscopic procedure',
     'unlisted procedure, arthroscopy',
     'arthroscopies',
     'h/o: urinary stone',
     'history of urinary stone',
     'history of urinary tract calculus',
     'h/o: obstetric problem',
     'history of obstetric problem',
     'school admission medical examination',
     'malignant tumour of trigone of urinary bladder',
     'malignant tumour of trigone of bladder',
     'malignant tumor of trigone of bladder',
     'malignant neoplasm of trigone of urinary bladder',
     'benign neoplasm of skin of upper limb and shoulder',
     'benign neoplasm of skin of upper limb',
     'persistent insomnia',
     'lupus anticoagulant disorder',
     'lupus anticoagulant',
     'lupus anticoagulant inhibitor syndrome',
     'sle inhibitor syndrome',
     'non-traumatic subdural hemorrhage',
     'non-traumatic intracranial subdural hemorrhage',
     'non-traumatic intracranial subdural haemorrhage',
     'subdural hemorrhage - nontraumatic',
     'enterocutaneous fistula',
     'enteric cutaneous fistula',
     'postoperative urinary tract infection',
     'enlarged uterus',
     'uterine enlargement',
     'large uterus',
     'cerebral oedema',
     'intracranial swelling',
     'lumbar spina bifida with hydrocephalus',
     'ebsteins anomaly',
     'ebstein anomaly of tricuspid valve',
     'ebsteins anomaly',
     'ebsteins anomaly of tricuspid valve',
     'ebsteins malformation of tricuspid valve',
     'closed fracture distal tibia',
     'inflammatory neuropathy',
     'syndrome of infant of diabetic mother',
     'infant of a diabetic mother syndrome',
     'maternal diabetes syndrome',
     'late effect of accidental injury',
     'personal care impairment',
     'personal condition impairment of self care',
     'difficulty with personal care',
     'gross motor impairment',
     'multiple system atrophy',
     'MSa',
     'heroin dependence',
     'heroin addiction',
     'house dust mite allergy',
     'common variable agammaglobulinaemia',
     'CVAg',
     'common variable immunodeficiency',
     'CVi',
     'late onset immunoglobulin deficiency',
     'CVId',
     'acquired hypogammaglobulinaemia',
     'common variable hypogammaglobulinaemia',
     'acquired hypogammaglobulinemia',
     'common variable hypogammaglobulinemia',
     'needle biopsy of prostate',
     'hypoglycemic event in diabetes',
     'tear of meniscus of knee',
     'tear of cartilage of knee',
     'torn meniscus',
     'sudden death',
     'acute osteomyelitis of the ankle and/or foot',
     'acute osteomyelitis of the ankle and foot',
     'nodular lymphoma',
     'follicular low grade b-cell lymphoma',
     'nodular malignant lymphoma',
     'malignant lymphoma, centroblastic-centrocytic, follicular',
     'germinoblastoma, follicular',
     'follicular non-hodgkins lymphoma',
     'follicular non-hodgkin lymphoma',
     'follicular lymphoma',
     'giant follicular lymphoma',
     'nodular lymphosarcoma',
     'follicular lymphosarcoma',
     'brill - symmers, disease',
     'malignant lymphoma, follicle center, follicular',
     'malignant lymphoma, follicle center',
     'malignant lymphoma, follicular',
     'malignant lymphoma, follicle centre',
     'malignant lymphoma, follicle centre, follicular',
     'malignant lymphoma, nodular',
     'malignant lymphoma, lymphocytic, nodular',
     'localised oedema',
     'local edema',
     'local oedema',
     'ruptured achilles tendon - traumatic',
     'h/o: anemia',
     'history of anemia',
     'history of anaemia',
     'partnership problems',
     'cat bite - wound',
     'history of bladder neoplasm',
     'lesion of penis',
     'lesion penis',
     'transient synovitis of hip',
     'ileostomy present',
     'ileostomy, has currently',
     'rhabdomyosarcoma',
     'rhabdosarcoma',
     'rhabdomyosarcoma, no subtype',
     'esophageal varices in cirrhosis of the liver',
     'bipolar i disorder, most recent episode hypomanic',
     'bipolar affective disorder, current episode hypomanic',
     'local recurrence of malignant tumor of colon',
     'posterior subcapsular cataract',
     'psc - posterior subcapsular cataract',
     'tympanomastoidectomy',
     'closed anterior dislocation of humerus',
     'acute transudative otitis media',
     'acute middle ear effusion',
     'acute mee - acute middle ear effusion',
     'acute secretory otitis media',
     'acute nonsuppurative otitis media',
     'acute non-suppurative otitis media',
     'normal labor',
     'active labour',
     'active labor',
     'eutocia',
     'acute myeloid leukaemia with maturation',
     'acute myeloblastic leukemia with maturation',
     'acute granulocytic leukemia with maturation',
     'acute myelogenous leukemia with maturation',
     'acute myelocytic leukemia with maturation',
     'fab m2',
     'acute granulocytic leukaemia with maturation',
     'acute myeloblastic leukaemia with maturation',
     'acute myelogenous leukaemia with maturation',
     'acute myelocytic leukaemia with maturation',
     'acute myeloid leukaemia with maturation, fab m2',
     'm2 - acute myeloblastic leukemia with maturation',
     'm2 - acute myeloblastic leukaemia with maturation',
     'visual loss after cataract extraction',
     'erythromelalgia',
     'weir mitchells disease',
     'erythralgia',
     'erythermalgia',
     'carcinoma of gallbladder',
     'corneal epithelial dystrophy',
     'corneal epithelial and basement membrane dystrophy',
     'basement membrane corneal dystrophy',
     'anterior membrane corneal dystrophy',
     'epithelial corneal dystrophy',
     'EBMd',
     'epithelial basement membrane dystrophy',
     'connective tissue disease overlap syndrome',
     'mixed collagen vascular disease',
     'sharps syndrome',
     'mixed connective tissue disease',
     'MCTd',
     'hydroureteronephrosis',
     'pneumoconiosis',
     'herpes simplex type 1 infection',
     'herpes simplex 1',
     'herpes simplex 1 infection',
     'hereditary factor ix deficiency disease',
     'factor ix deficiency',
     'haemophilia b',
     'congenital factor ix deficiency',
     'hemophilia b',
     'christmas disease',
     'sex-linked factor ix deficiency disease',
     'ptc deficiency disease',
     'cramp in limb',
     'cramp in extremity',
     'congestive heart failure due to left ventricular systolic dysfunction',
     'ileal pouchitis',
     'diverticulitis of cecum',
     'polyp',
     'polyps',
     'polips',
     'history of chronic urinary tract infection',
     'mass of tongue',
     'glycosuria',
     'glucosuria',
     'neonatal acne',
     'acne neonatorum',
     'infantile acne',
     'repair of musculotendinous cuff of shoulder',
     'repair of rotator cuff of shoulder',
     'rotator cuff repair',
     'moderate smoker (20 or less per day)',
     'broken tooth with complication',
     'abnormal auditory perception',
     'disease due to plasmodiidae',
     'malarial fever',
     'malaria',
     'paludism',
     'plasmodiosis',
     'maleria',
     'has tingling sensation',
     'pins and needles',
     'tingling',
     'pins and needles sensation',
     'tinglings',
     'intermittent exotropia',
     'x(t) - intermittent exotropia',
     'intermittent comitant exotropia',
     'intermittent divergent squint',
     'peptic ulcer with haemorrhage',
     'bleeding peptic ulcer',
     'ulcer on tongue',
     'tongue ulceration',
     'retained dental root',
     'recurrent major depression in remission',
     'closed fracture of vault of skull without intracranial injury',
     'closed fracture of fifth metatarsal bone',
     'feeding disorder of infancy or early childhood',
     'feeding disorder, infancy or early childhood',
     'poisoning by wasp sting',
     'affective personality trait',
     'cycloid personality',
     'unstable personality trait',
     'tends to be unstable in affect',
     'cyclothymic personality',
     'cyclothymia',
     'cyclothymic disorder',
     'affective personality disorder',
     'endocervical polyp',
     'muscle fasciculation',
     'muscular fasciculation',
     'writhing muscles',
     'spontaneous contraction of muscle',
     'flickering muscles',
     'fluttering muscles',
     'fasciculation',
     'laparotomy',
     'opening of peritoneum',
     'opening of abdomen',
     'opening of peritoneal cavity',
     'celiotomy',
     'coeliotomy',
     'laparotomies',
     'disorder of phosphorus metabolism',
     'phosphorus metabolism disorder',
     'herpes zoster with ophthalmic complication',
     'herpes zoster ophthalmicus',
     'zoster ophthalmicus',
     'herpes zoster with ophthalmic complications',
     'zoster ocular disease',
     'keratoconjunctivitis',
     'superficial keratitis with conjunctivitis',
     'shoulder girdle dystocia',
     'impacted shoulders',
     'shoulder dystocia',
     'pulmonic valve regurgitation',
     'pulmonary regurgitation',
     'PVr',
     'pulmonic insufficiency',
     'pulmonic valve insufficiency',
     'pulmonary valve insufficiency',
     'pulmonary valve incompetence',
     'pulmonic regurgitation',
     'pulmonary valve regurgitation',
     'pulmonic valve incompetence',
     'benign neoplasm of ovary',
     'benign ovarian tumour',
     'benign ovarian tumor',
     'malignant lymphoma of lymph nodes of multiple sites',
     'hypoalbuminaemia',
     'serum albumin low',
     'open wound of forearm',
     'open wound of foot',
     'h/o: premature delivery',
     'history of premature delivery',
     'history of preterm delivery',
     'transverse myelopathy syndrome',
     'transverse myelitis',
     'cervical incompetence',
     'abnormal dilatation of cervix before onset of labor',
     'abnormal dilatation of cervix before onset of labour',
     'incompetence of cervix',
     'incompetent cervix',
     'malignant tumour of body of pancreas',
     'ca body of pancreas',
     'malignant neoplasm of body of pancreas',
     'single umbilical artery',
     'congenital absence of umbilical artery',
     'closed fracture lumbar vertebra',
     'secondary parkinsonism',
     'secondary parkinsons disease',
     'secondary parkinson disease',
     'symptomatic parkinsonism',
     'parkinsonism',
     'disorders presenting primarily with parkinsonism',
     'parkinsonisms',
     'atrioventricular block',
     'AVb',
     'av block',
     'acute hepatitis c',
     'organic mood disorder',
     'organic affective disorder',
     'osteoarthritis of elbow',
     'degenerative joint disease of elbow',
     'osteoarthritis of finger joint',
     'degenerative joint disease of finger',
     'osteoarthritis of finger',
     'osteoarthritis of joint of finger',
     'congenital ptosis',
     'congenital ptosis of upper eyelid',
     'congenital prolapse',
     'intra-abdominal and pelvic swelling, mass and lump',
     'growth delay',
     'groin mass',
     'mass of inguinal region',
     'groin lump',
     'local recurrence of malignant tumour of breast',
     'arachnoid cyst',
     'occlusion of bile duct',
     'obstruction of bile duct',
     'cholestasis',
     'bile stasis',
     'diabetic gastroparesis',
     'malignant tumour of vulva',
     'cancer of vulva',
     'malignant tumor of maxillary sinus',
     'malignant tumour of maxillary antrum',
     'malignant tumor of maxillary antrum',
     'pyloric stenosis',
     'Ps',
     'motor neuron disease',
     'motor neurone disease',
     'MNd',
     'obliterative bronchiolitis',
     'Ob',
     'bronchiolitis obliterans',
     'optic disc swelling',
     'optic disc oedema',
     'papilledema',
     'papilloedema',
     'edema of optic disc',
     'optic papillitis',
     'papillitis',
     'tenosynovitis of hand',
     'hyperplasia of maxillary bone',
     'sepsis due to coagulase negative staphylococcus',
     'tongue tie',
     'ankyloglossia',
     'tongue-tie',
     'acute severe exacerbation of asthma',
     'polyp of duodenum',
     'duodenal polyp',
     'mental disorder',
     'mental illness',
     'mental disease',
     'psychiatric illness',
     'psychiatric disorder',
     'mental health disorder',
     'cheilitis',
     'ankle instability',
     'insomnia disorder related to known organic factor',
     'organic insomnia',
     'dilated cardiomyopathy secondary to alcohol',
     'alcoholic cardiomyopathy',
     'secondary physiologic amenorrhoea',
     'secondary amenorrhea',
     'loss of menstrual period',
     'secondary amenorrhoea',
     'idiopathic peripheral autonomic neuropathy',
     'cystic fibrosis of the lung',
     'mucoviscidosis involving the lung',
     'pulmonary cystic fibrosis',
     'asymptomatic human immunodeficiency virus infection',
     'asymptomatic hiv infection',
     'measurement of helicobacter pylori antibody',
     'helicobacter pylori antibody assay',
     'serologic test for helicobacter pylori',
     'antibody; helicobacter pylori',
     'disorder of biliary tract',
     'disease of biliary system',
     'liver excision',
     'hepatectomy',
     'resection of liver',
     'excision procedures on the liver',
     'overlapping malignant neoplasm of bronchus and lung',
     'malignant neoplasm of overlapping lesion of bronchus and lung',
     'intraductal carcinoma in situ of breast',
     'intraductal carcinoma of breast',
     'intraductal carcinoma, noninfiltrating',
     'non-infiltrating intraductal carcinoma',
     'non-infiltrating intraductal adenocarcinoma',
     'intraductal adenocarcinoma, noninfiltrating',
     'intraductal carcinoma',
     'ductal intraepithelial neoplasia 3',
     'ductal carcinoma in situ',
     'din 3',
     'DCIs',
     'intraductal carcinoma, noninfiltrating, no icd-o subtype',
     'intraductal carcinoma, noninfiltrating, no international classification of diseases for oncology subtype',
     'mixed bipolar i disorder in full remission',
     'bipolar i disorder, most recent episode mixed, in full remission',
     'fracture of multiple ribs',
     'multiple fractures of ribs',
     'paraphimosis',
     'fh: neoplasm of ovary',
     'family history: neoplasm of ovary',
     'fh: allergy',
     'family history of allergy',
     'family history: allergy',
     'h/o: malaria',
     'history of malaria',
     'h/o: gastrostomy',
     'history of gastrostomy',
     'dating/booking us scan',
     'booking scan',
     'dating/booking ultrasound scan',
     'thyroid disorder screening',
     'agranulocytosis',
     'granulocytopenic disorder',
     'granulocytopenia',
     'yag laser capsulotomy of lens',
     'yttrium-aluminum-garnet (yag) laser capsulotomy of lens',
     'yttrium-aluminum-garnet laser capsulotomy of lens',
     'breech malpresentation successfully converted to cephalic presentation',
     'primary hyperoxaluria',
     'primary oxalosis',
     'compulsive gambling',
     'pathological gambling disorder',
     'gambling disorder',
     'single major depressive episode, severe, with psychosis',
     'brief depressive adjustment reaction',
     'oesophageal varices associated with another disorder',
     'transverse lie with antenatal problem',
     'cellulitis and abscess of ankle',
     'synovitis and tenosynovitis',
     'unicameral bone cyst',
     'solitary bone cyst',
     'traumatic bone cyst',
     'bone cyst',
     'simple bone cyst',
     'local cyst of bone',
     'derangement of lateral meniscus',
     'lateral meniscus derangement',
     'derangement of lateral meniscus of knee',
     'fall on same level from stumbling',
     'accidentally struck by or against objects or persons',
     'complete obstruction of lacrimal canaliculus',
     'plugged tear duct',
     'blocked tear duct',
     'critical lower limb ischemia',
     'eosinophilic oesophagitis',
     'sliding hiatus hernia',
     'sliding diaphragmatic hernia',
     'sliding oesophageal hiatus hernia',
     'sliding esophageal hiatus hernia',
     'stress fracture',
     'fatigue fracture',
     'march fracture',
     'macular pigment deposit',
     'wrinkled skin',
     'rugosity of skin',
     'wrinkling',
     'fear of becoming fat',
     'fear of obesity',
     'spasm of bladder',
     'bladder spasm',
     'congenital laryngomalacia',
     'embryonic cyst of broad ligament',
     'prostatic intraepithelial neoplasia',
     'PIn',
     'malignant tumor of salivary gland',
     'salivary gland cancer',
     'cancer of salivary gland',
     'fracture of radial neck',
     'inflammatory disorder of jaw',
     'chronic osteomyelitis of ankle and/or foot',
     'malignant melanoma of lower limb',
     'eye disorder screening',
     'screening for eye conditions',
     'current knee cartilage tear',
     'fetal death',
     'foetal death',
     'apnea of prematurity',
     'squamous cell carcinoma of tongue',
     'carcinoma of larynx',
     'laryngeal carcinoma',
     'rheumatoid arthritis - hand joint',
     'rheumatoid arthritis of hand joint',
     'rheumatoid arthritis of hand',
     'scoliosis of lumbar spine',
     'lumbar spine scoliosis',
     'sacroiliac instability',
     'sacroiliac joint unstable',
     'deformity of hand',
     'lesion of nose',
     'metastasis to head and neck lymph node',
     'homozygous factor v leiden mutation',
     'bleeding gastric erosion',
     'non-infective enteritis and colitis',
     'squamous cell carcinoma of mouth',
     'industrial accident',
     'spastic paraparesis',
     'closed fracture of two ribs',
     'mammoplasty',
     'reconstruction of breast',
     'mastoplasty',
     'breast reconstruction',
     'mammaplasty',
     'breast reconstruction with other technique',
     'infantile colic',
     'three month colic',
     'infantile colic - symptom',
     'implantation of cochlear prosthetic device',
     'insertion of cochlear implant',
     'malignant tumour of gallbladder',
     'gallbladder ca',
     'malignant tumor of parotid gland',
     'cancer of parotid gland',
     'malignant tumour of splenic flexure',
     'ca splenic flexure - colon',
     'systolic dysfunction',
     'middle ear reconstruction',
     'repair of middle ear',
     'plastic repair of middle ear',
     'tympanoplasty',
     'middle ear otoplasty',
     'tympanoplasty without mastoidectomy (including canalplasty, atticotomy and/or middle ear surgery), initial or revision; without ossicular chain reconstruction',
     'adjustment reaction of adolescence',
     'adolescent situation reaction',
     'adolescent turmoil',
     'unsteady when walking',
     'unstable when walking',
     'disequilibrium when walking',
     'unsteady gait',
     'instability of gait',
     'genital tear resulting from childbirth',
     'obstetrical laceration',
     'trauma to perineum during delivery',
     'perineal laceration during delivery',
     'perineal tear resulting from childbirth',
     'percutaneous insertion of inferior vena cava filter',
     'insertion of inferior vena caval filter',
     'IVc',
     'insertion of caval filter',
     'insertion of antiembolic filter into vena cava',
     'solar erythema',
     'erythema solare',
     'solar dermatitis',
     'metastatic squamous cell carcinoma',
     'squamous cell carcinoma, metastatic',
     'adult victim of abuse',
     'adult abuse',
     'transitional cell carcinoma of kidney',
     'plasmacytoma',
     'plasma cell tumor',
     'solitary myeloma',
     'solitary plasmacytoma',
     'plasmacytoma of bone',
     'plasma cell tumour',
     'solitary plasmacytoma of bone',
     'plasmacytoma - disorder',
     'nodular goitre',
     'nodular hyperplasia of thyroid',
     'fracture, closed',
     'closed fracture',
     'closed fracture of bone',
     'malignant neoplasm of bone',
     'history of reconstruction of anterior cruciate ligament tear',
     'history of anterior cruciate ligament tear reconstruction',
     'late effect of traumatic injury to brain',
     'late effect of brain injury',
     'body fluid retention',
     'recurrent pulmonary embolism',
     'acquired absence of all teeth',
     'complete edentulism',
     'chronic left-sided ulcerative colitis',
     'retained products of conception, following delivery with hemorrhage',
     'postpartum hemorrhage with retained placenta',
     'postpartum haemorrhage with retained placenta',
     'haemorrhage from retained portion of placenta and/or membranes',
     'hemorrhage from retained portion of placenta and/or membranes',
     'bypass of stomach with short limb roux-en-y gastroenterostomy',
     'endovascular repair of abdominal aortic aneurysm',
     'endovascular repair of abdominal aortic aneurysm procedures',
     'mass of submandibular region',
     'swelling of upper arm',
     'at risk for noncompliance',
     'potential noncompliance',
     'cholecystectomy with exploration of common duct',
     'anemia of prematurity',
     'penis implantation',
     'insertion of penile prosthesis',
     'implantation of penile prosthesis',
     'cerebral herniation',
     'hernia cerebri',
     'replacement of mitral valve with tissue graft',
     'tissue mitral valve replacement',
     'biological mitral valve replacement',
     'bioprosthetic mitral valve replacement',
     'nasopharyngitis',
     'rhinopharyngitis',
     'empyema of mastoid',
     'mastoiditis',
     'mastoid empyema',
     'thrombosed internal hemorrhoids',
     'gastrectomy',
     'vitreous detachment',
     'vitreous separation',
     'ulcer of rectum',
     'solitary ulcer of rectum',
     'solitary rectal ulcer',
     'rectal ulcer',
     'gastric ulcer without hemorrhage, without perforation and without obstruction',
     'movement disorder',
     'haemorrhagic oesophagitis',
     'burn of neck',
     'manic bipolar i disorder in partial remission',
     'bipolar i disorder, most recent episode manic, in partial remission',
     'vertebrobasilar artery syndrome',
     'basilar artery syndrome',
     'vertebrobasilar arterial insufficiency',
     'vertebrobasilar insufficiency',
     'insufficiency - basilar artery',
     'history and physical examination, camp',
     'degeneration of thoracic intervertebral disc',
     'pressure ulcer of dorsum of foot',
     'pressure ulcer of lower back',
     'perinatal intraventricular haemorrhage',
     'intraventricular hemorrhage from any perinatal cause',
     'neonatal intraventricular hemorrhage',
     'intraventricular haemorrhage from any perinatal cause',
     'neonatal intraventricular haemorrhage',
     'injury of trigeminal nerve',
     'injury of fifth cranial nerve',
     'injury to trigeminal nerve',
     'trigeminal (5th) nerve injury',
     'fifth cranial nerve injury',
     'trigeminal nerve injury',
     'polyneuropathy due to drug',
     'drug-related polyneuropathy',
     'severe major depression without psychotic features',
     'polyradiculopathy',
     'hereditary corneal dystrophy',
     'injury of back',
     'back injury',
     'moderate manic bipolar i disorder',
     'moderate bipolar i disorder, most recent episode manic',
     'dependent personality disorder',
     'total hysterectomy with removal of both tubes and ovaries',
     'bilateral hysterosalpingo-ovariectomy',
     'closed fracture of forearm',
     'secondary malignant neoplasm of intrapelvic lymph nodes',
     'metastatic malignant neoplasm to intrapelvic lymph nodes',
     'cancer metastatic to intrapelvic lymph nodes',
     'secondary malignant neoplasm of rectum',
     'metastatic malignant neoplasm to rectum',
     'cancer metastatic to rectum',
     'retinal deposits',
     'closed fracture of multiple ribs',
     'closed fracture multiple ribs',
     'stenosis of trachea',
     'tracheal stricture',
     'tracheal stenosis',
     'developmental speech disorder',
     'DSd',
     'h/o: hypertension',
     'history of hypertension',
     'h/o: pneumonia',
     'history of pneumonia',
     'eye excision',
     'excision of eye',
     'removal of eye',
     'enucleation of eyeball',
     'surgical enucleation of eye',
     'complete excision of eye',
     'ophthalmectomy',
     'removal of eyeball',
     'enucleation of eye; without implant',
     'coxsackie virus disease',
     'traumatic arthropathy of the ankle and/or foot',
     'traumatic arthropathy of the ankle and foot',
     'anterior shin splints',
     'benign tumor of eyelid',
     'benign neoplasm of skin of eyelid',
     'coronary artery bypass grafts x 5',
     'cabg x 5 - coronary artery bypass grafts x 5',
     'ruptured cerebral aneurysm',
     'percutaneous endoscopic gastrostomy',
     'PEg',
     'endometrial hyperplasia',
     'reconstruction of anterior cruciate ligament of knee',
     'surgical repair of cranial cruciate ligament of knee',
     'acl - repair of anterior cruciate ligament',
     'repair of anterior cruciate ligament of knee joint',
     'reconstruction of anterior cruciate ligament of knee joint',
     'acl - reconstruction of anterior cruciate ligament',
     'abdominal bruit',
     'female infertility associated with anovulation',
     'anovulatory',
     'anovulatory cycle',
     'female infertility of anovulatory origin',
     'anovulatory infertility',
     'anovular cycle',
     'wrist joint inflamed',
     'wrist joint active',
     'lesion of tongue',
     'excision of skin tag',
     'attention deficit hyperactivity disorder, combined type',
     'attention deficit hyperactivity disorder combined type',
     'attention deficit hyperactivity disorder combined',
     'preseptal cellulitis',
     'central retinal artery occlusion',
     'CRAo',
     'CRa',
     'primary angle-closure glaucoma',
     'vaginal enterocele',
     'enterocele',
     'posterior vaginal hernia',
     'pouch of douglas hernia',
     'hernia into pouch of douglas',
     'neurofibroma',
     'neurofibroma, no icd-o subtype',
     'neurofibroma, no international classification of diseases for oncology subtype',
     'purpura',
     'peliosis',
     'purpuric disorder',
     'disorder characterised by purpura',
     'disorder characterized by purpura',
     'purpuric',
     'history of reduction of breast',
     'history of breast reduction',
     'history of sustained ventricular tachycardia',
     'fibrous histiocytoma, malignant',
     'malignant fibrous histiocytoma',
     'malignant fibroxanthoma',
     'undifferentiated high grade pleomorphic sarcoma',
     'fibroxanthoma, malignant',
     'neuroma, no icd-o subtype',
     'neuroma, no international classification of diseases for oncology subtype',
     'neuroma',
     'neuroma (excluding acoustic)',
     'neuroma (excl. acoustic)',
     'ventriculoperitoneal shunt',
     'creation of vp shunt',
     'insertion ventriculoperitoneal shunt',
     'creation of ventriculo-peritoneal shunt',
     'ventriculoperitoneostomy',
     'creation of shunt; ventriculo-peritoneal, -pleural, other terminus',
     'marital conflict',
     'marital problems',
     'aspergillosis',
     'infection due to aspergillus',
     'anal pain',
     'carious exposure of pulp',
     'dental caries extending to pulp',
     'malignant hypertension',
     'congenital cystic disease of liver',
     'congenital polycystic disease of liver',
     'fibrocystic disease of liver',
     'congenital cystic liver',
     'congenital hepatic cyst',
     'PLd',
     'polycystic liver disease',
     'congenital polycystic liver disease',
     'fibrocystic liver disease',
     'cystic disease of liver',
     'cardiovascular stress testing',
     'moderate major depression',
     'contusion of thigh',
     'superficial bruising of thigh',
     'contusion, thigh',
     'repair of retina for retinal detachment',
     'reattachment of retina',
     'repair of retinal detachment',
     'burns of multiple sites',
     'oedema of the upper extremity',
     'academic underachievement',
     'disorder associated with menstruation and/or menopause',
     'postoperative haemorrhage',
     'postoperative bleeding',
     'haemorrhage postprocedure',
     'hemorrhage postprocedure',
     'disease related peripheral neuropathy',
     'irritant contact dermatitis',
     'primary irritant dermatitis',
     'non-allergic contact dermatitis',
     'sprain of sacroiliac region',
     'absent renal function',
     'non-functioning kidney',
     'NFk',
     'occupational disorder',
     'benign teratoma of ovary',
     'disorder of uterus',
     'uterine disease',
     'uterine disorder',
     'immunoglobulin g subclass deficiency',
     'immunoglobin g subclass deficiency',
     'igg subclass deficiency',
     'selective immunoglobulin g deficiency',
     'selective igg immunodeficiency',
     'brain damage due to hypoxia',
     'hypoxic brain damage',
     'brain disorder resulting from a period of impaired oxygen delivery to the brain',
     'haemophilus influenzae type b vaccination',
     'haemophilus b vaccination',
     'fh: thyroid disorder',
     'fh: thyroid disease',
     'family history: thyroid disorder',
     'h/o: schizophrenia',
     'history of schizophrenia',
     'serum iron raised',
     'requires a meningitis vaccination',
     'ear disorder screening',
     'malignant neoplasm of lower-inner quadrant of female breast',
     'craniopharyngioma',
     'rathkes pouch tumour',
     'rathkes pouch tumor',
     'short-sleeper',
     'cataplexy and narcolepsy',
     'unilateral total vocal cord paralysis',
     'unilateral complete paralysis of vocal cords',
     'complete paralysis of one vocal cord',
     'complete unilateral paralysis of vocal cords',
     'chronic asthmatic bronchitis',
     'chronic wheezy bronchitis',
     'noninflammatory cervical disorder',
     'non inflammatory disorder of cervix',
     'umbilical granuloma',
     'tenosynovitis of ankle',
     'radial polydactyly',
     'preaxial polydactyly',
     'accessory thumb',
     'infections specific to perinatal period',
     'fracture of mandible, closed',
     'closed fracture of lower jaw bone',
     'closed fracture of mandible',
     'closed fracture of inferior maxilla',
     'fracture of lower jaw, closed',
     'acute periodontitis',
     'acute pericementitis',
     'injury of unknown intent due to fall from height',
     'pressure ulcer of sacral region',
     'sacral pressure sore',
     'pressure ulcer on sacrum',
     'pressure sore on sacrum',
     'post-surgical wound care',
     'post-operative wound care',
     'habitual drinker',
     'deformity of foot',
     'persistent fetal circulation',
     'persistent foetal circulation',
     'persistent pulmonary hypertension of the newborn',
     'PFc',
     'PPHn',
     'persistent pulmonary hypertension in newborn',
     'persistent fetal circulation syndrome',
     'persistent foetal circulation syndrome',
     'benign essential hypertension complicating and/or reason for care during pregnancy',
     'uninodular goitre',
     'breastfeeding problem in the newborn',
     'endoscopy of bladder',
     'endoscopic examination of bladder',
     'endoscopy of urinary bladder',
     'cystoscopy',
     'cystoscopies',
     'self poisoning by carbon monoxide',
     'viral screening',
     'problematic behavior in children',
     'footling breech presentation',
     'foetal leg presenting',
     'foetal foot presenting',
     'fetal foot presenting',
     'fetal leg presenting',
     'malignant mesothelioma of pleura',
     'chronic allergic conjunctivitis',
     'follicular cyst of ovary',
     'graafian follicle cyst',
     'cyst of graafian follicle',
     'follicular cystic ovary disease',
     'endogenous hyperlipidemia',
     'endogenous hyperlipaemia',
     'endogenous hyperlipemia',
     'fredrickson type iv hyperlipoproteinaemia',
     'fredrickson type iv lipidaemia',
     'fredrickson type iv lipidemia',
     'pure hyperglyceridaemia',
     'familial hypertriglyceridemia',
     'monoclonal paraproteinemia',
     'maternal concern',
     'anxious mother',
     'acute wheezy bronchitis',
     'history of sexually transmitted disease',
     'history of venereal disease',
     'history of sexually transmitted disease (std)',
     'autonomic neuropathy',
     'well man health examination',
     'laminotomy for decompression and exploration',
     'laminectomy for decompression and exploration',
     'laceration of lip',
     'puncture wound of skin',
     'dog bite - wound',
     'burn of genitalia',
     'acute epiglottitis',
     'acute epiglottitis and supraglottitis',
     'abnormal vaginal bleeding',
     'myelosclerosis with myeloid metaplasia',
     'myelofibrosis with myeloid metaplasia',
     'myelofibrosis as a result of myeloproliferative disease',
     'megakaryocytic myelosclerosis',
     'chronic idiopathic myelofibrosis',
     'agnogenic myeloid metaplasia',
     'primary myelofibrosis',
     'osteomyelofibrosis',
     'myelofibrosis',
     'Mf',
     'myelosclerosis',
     'closed fracture of distal fibula',
     'manic bipolar i disorder in full remission',
     'bipolar i disorder, most recent episode manic, in full remission',
     'anemia secondary to renal failure',
     'femoral false aneurysm',
     'pseudoaneurysm of femoral artery',
     'mild chronic obstructive pulmonary disease',
     'rapid atrial fibrillation',
     'at risk of coronary heart disease',
     'at risk of chd',
     'hepatitis b core antibody level',
     'haemothorax',
     'pleural haemorrhage',
     'pleural hemorrhage',
     'hemothorax',
     'psychiatric diagnostic interview, examination, history, mental status and disposition',
     'individual psychiatric examination',
     'closed fracture of malar and/or maxillary bones',
     'tonic-clonic epilepsy',
     'grand mal epilepsy',
     'complication of medical care',
     'medical care complication',
     'carcinoid syndrome',
     'hormonal tumour',
     'hormonal tumor',
     'excessive serotonin secretion',
     'hormone secretion by carcinoid tumour',
     'hormone secretion by carcinoid tumor',
     'progressive angina',
     'primary malignant neoplasm of pancreas',
     'primary malignant neoplasm of soft tissues',
     'crohns disease of ileum',
     'crohns ileitis',
     'crohn disease of ileum',
     'unilateral femoral hernia with obstruction but no gangrene',
     'unilateral obstructed femoral hernia',
     'cesarean wound disruption',
     'congenital pigmented melanocytic nevus',
     'congenital pigmented naevus',
     'congenital melanocytic naevus',
     'congenital melanocytic nevus',
     'congenital pigmented nevus',
     'congenital pigmented melanocytic nevus of skin',
     'congenital melanocytic naevus of skin',
     'congenital pigmented naevus of skin',
     'congenital dermal melanocytic naevus',
     'congenital dermal melanocytic nevus',
     'congenital melanocytic nevus of skin',
     'congenital pigmented nevus of skin',
     'spider bite wound',
     'orthoptic training',
     'orthoptic exercises',
     'orthoptic-pleoptic training',
     'synovitis',
     'persistent proteinuria associated with type 1 diabetes mellitus',
     'persistent proteinuria associated with type i diabetes mellitus',
     'tendinitis of foot',
     'tendonitis of foot',
     'hearing assessment',
     'history of amputation of lesser toe',
     'history of amputation of toe, other than great toe',
     'history of radical retropubic prostatectomy',
     'severe major depression, single episode, with psychotic features',
     'major depressive disorder, single episode, severe with psychotic features',
     'mass of head',
     'pathological fracture due to osteoporosis',
     'cutaneous graft-versus-host disease',
     'graft versus host disease of skin',
     'aneurysm of infrarenal abdominal aorta',
     'raised helicobacter pylori antibody',
     'helicobacter pylori antibody above reference range',
     'helicobacter pylori antibody elevation',
     'microcalcifications of the breast',
     'chronic peptic ulcer with hemorrhage',
     'thyrotoxicosis factitia',
     'factitious hyperthyroidism',
     'factitious hyperthyroidism from ingestion of excessive thyroid material',
     'hepatorenal syndrome',
     'HRf',
     'hepatorenal failure',
     'parent/child conflict',
     'parent-child problem',
     'anxiety disorder of childhood',
     'mobitz type i incomplete atrioventricular block',
     'mobitz type 1 second degree atrioventricular block',
     'mobitz type i wenckebach atrioventricular block',
     'wenckebachs phenomenon',
     'wenckebachs incomplete av block',
     'wenckebach',
     'repair of sliding inguinal hernia',
     'repair inguinal hernia, sliding, any age',
     'transplantation of small intestine',
     'small bowel transplant',
     'chronic periodontitis',
     'chronic pericementitis',
     'chronic periodontal disease',
     'adult periodontitis',
     'adult-onset periodontitis',
     'bleeding ulcer of esophagus',
     'bleeding oesophageal ulcer',
     'bleeding esophageal ulcer',
     'haemorrhagic ulcer of oesophagus',
     'hemorrhagic ulcer of esophagus',
     'residual foreign body in soft tissue',
     'bleeding from anus',
     'concussion with loss of consciousness',
     'trigeminal nerve disorder',
     'disorder of the fifth cranial nerve',
     'disorders of the fifth nerve',
     'disorders of the vth cranial nerve',
     'hereditary peripheral neuropathy',
     'acute respiratory failure',
     'hepatitis b surface antibody measurement',
     'hbsab measurement',
     'hepatitis b surface antibody (hbsab)',
     'closed trimalleolar fracture',
     'closed fracture ankle, trimalleolar',
     'contusion of upper limb',
     'contusion of arm',
     'superficial bruising of arm',
     'contusion, upper limb',
     'arm bruise',
     'superficial bruising of upper limb',
     'encopresis with constipation and overflow incontinence',
     'pilonidal cyst without abscess',
     'pilonidal cyst with no abscess',
     'angiography',
     'diagnostic angiogram',
     'diagnostic angiography',
     'angiogram',
     'mr angiography',
     'MRa',
     'angiographies',
     'angiograms',
     'complication of transplanted pancreas',
     'bone density study, dual photon absorptiometry',
     'bone mineral content study, dual photon absorptiometry',
     'bone density (bone mineral content) study, 1 or more sites; dual photon absorptiometry, 1 or more sites',
     'constrictive pericarditis',
     'subarachnoid haemorrhage following injury without open intracranial wound and with brief loss of consciousness (less than one hour)',
     'arthroscopy of knee with medial and lateral meniscectomy',
     'endoscopic total meniscectomy of knee',
     'injury of neck',
     'neck injury',
     'second degree uterine prolapse',
     'malignant melanoma of skin of face',
     'secondary malignant neoplasm of pancreas',
     'metastasis to pancreas',
     'secondary malignant deposit in pancreas',
     'pancreatic metastasis',
     'metastatic malignant neoplasm to pancreas',
     'cancer metastatic to pancreas',
     'haemorrhage of colon',
     'colonic hemorrhage',
     'colonic haemorrhage',
     'secondary glaucoma',
     'glaucoma due to ocular disease',
     'seborrheic dermatitis of scalp',
     'seborrhoeic eczema of scalp',
     'seborrhoea capitis',
     'seborrhea capitis',
     'seborrheic eczema of scalp',
     'pityriasis capitis',
     'closed fracture of acromial end of clavicle',
     'closed fracture clavicle, lateral end',
     'phantom limb',
     'phantom limb syndrome',
     'fls - phantom limb syndrome',
     'PLs',
     'hypertensive heart and renal disease with both (congestive) heart failure and renal failure',
     'anorectal pain',
     'localized, primary osteoarthritis of the shoulder region',
     'primary localised osteoarthrosis of shoulder region',
     'primary localized osteoarthrosis of shoulder region',
     'elbow joint pain',
     'arthralgia of elbow',
     'laceration of eyelid',
     'odontogenic cyst',
     'developmental odontogenic cyst',
     'gastrointestinal anastomotic stricture',
     'osteoarthritis of acromioclavicular joint',
     'degenerative joint disease of acromioclavicular joint',
     'synovial cyst',
     'ganglia',
     'frontal headache',
     'contusion of face, scalp and neck, excluding eye(s)',
     'contusion face, scalp and neck, excluding eyes',
     'aseptic necrosis of head and/or neck of femur',
     'disorder of iron metabolism',
     'infection and/or inflammatory reaction due to internal prosthetic device, implant and/or graft',
     'infection and inflammation reaction due to internal prosthetic device, implant or graft',
     'vaginal dryness',
     'hepatosplenomegaly',
     'pityriasis streptogenes',
     'pityriasis alba',
     'pneumocystosis pneumonia',
     'PCp',
     'pneumocystis pneumonia',
     'pulmonary pneumocystosis',
     'pneumocystosis jirovecii pneumonia',
     'pneumocystosis',
     'spastic neurogenic bladder',
     'spastic neuropathic bladder',
     'partial obstruction of small bowel',
     'partial obstruction of small intestine',
     'partial small bowel obstruction',
     'hemiplegic cerebral palsy',
     'unilateral spastic cerebral palsy',
     'spastic hemiplegic cerebral palsy',
     'congenital hemiplegia',
     'globus hystericus',
     'globus pharyngeus',
     'idiopathic globus',
     'globus syndrome',
     'retinoschisis',
     'schisis of retina',
     'Rs',
     'history of live donor partial hepatectomy',
     'history of liver donation',
     'history of partial hepatectomy for liver donation',
     'perianal fistula',
     'fistula of perianal skin',
     'cerebral abscess',
     'parenchymal intracranial abscess',
     'injury of ulnar nerve',
     'glaucoma of childhood',
     'infantile glaucoma',
     'juvenile glaucoma',
     'developmental glaucoma',
     'lipodystrophy',
     'dystrophy of fatty tissue',
     'pregnancy detection examination',
     'pregnancy test',
     'scleritis',
     'inflammation of white of eye',
     'breath smells unpleasant',
     'smelly breath',
     'bad breath - halitosis',
     'breath smells offensive',
     'foul breath',
     'bad breath',
     'fetor oris',
     'fetor ex ore',
     'stomatodysodia',
     'osteochondritis dissecans',
     'idiopathic avascular necrosis',
     'osteochondrosis dissecans',
     'chronic lymphoid leukaemia in remission',
     'malignant melanoma of skin of trunk',
     'lives alone',
     'home alone',
     'drug-induced purpura',
     'compartment syndrome',
     'atrioseptoplasty',
     'septal atrioplasty of heart',
     'atrioseptopexy',
     'closure of atrial septal defect',
     'correction of atrial septal defect',
     'repair of atrial septal defect',
     'repair of atrial septum of heart',
     'repair of defect of interatrial septum',
     'repair of defect of atrial septum',
     'closure of defect of interatrial septum',
     'asd - repair of atrial septal defect',
     'chorioamnionitis',
     'membranitis',
     'contusion of face',
     'superficial bruising of face',
     'bruise of face',
     'noncompliance with diagnostic testing',
     'noncompliance: diagnostic testing',
     'torsion of ovary',
     'ovarian torsion',
     'fh: diabetes in pregnancy',
     'family history: diabetes in pregnancy',
     'fh: atopy',
     'family history: atopy',
     'h/o: raised blood lipids',
     'history of raised blood lipids',
     'h/o splenectomy',
     'history of splenectomy',
     'vulval pain',
     'pain of vulva',
     'exercise tolerance test',
     'exercise test',
     'review of medication',
     'medication review',
     'sézarys disease of lymph nodes of multiple sites',
     'sézary disease of lymph nodes of multiple sites',
     'sezarys disease of lymph nodes of multiple sites',
     'type 2 diabetes mellitus with hyperosmolar coma',
     'active infantile autism',
     'cocaine dependence, episodic',
     'collagenous colitis',
     'cataract secondary to ocular disease',
     'complicated cataract',
     'cataracta complicata',
     'low vision, both eyes',
     'impairment level: low vision of both eyes',
     'disorder of soft tissue',
     'soft tissue disorder',
     'acute recurrent pancreatitis',
     'acute gastric ulcer with perforation',
     'winter itch',
     'pruritus hiemalis',
     'asteatotic eczema',
     'eczema craquele',
     'eczema craquelé',
     'dry skin dermatitis',
     'acquired xeroderma',
     'senile asteatotic eczema',
     'winter eczema',
     'cervical disc disorder with myelopathy',
     'cervical root syndrome',
     'osteolysis',
     'osteolytic lesion',
     'postphlebitic syndrome',
     'post-phlebitic dermatosis of lower leg',
     'post-phlebitic syndrome',
     'congenital trigger thumb',
     'fetal or neonatal effect of transverse lie during labour and delivery',
     'foetal or neonatal effect of transverse lie during labour and delivery',
     'closed fracture of vault of skull with intracranial injury, with less than 1 hour loss of consciousness',
     'foreign body in hand',
     'acute drug intoxication',
     'acute intoxication',
     'stoned',
     'cardiac arrest with successful resuscitation',
     'oral lichen planus',
     'OLp',
     'perianal crohns disease',
     'perianal crohn disease',
     'chronic pelvic inflammatory disease of the female pelvic organs and/or tissues',
     'chronic pelvic inflammatory disease',
     'chronic pid - chronic pelvic inflammatory disease',
     'contusion of cerebral cortex',
     'cortex contusion',
     'post-inflammatory hyperpigmentation',
     'postinflammatory hyperpigmentation',
     'post-inflammatory hypermelanosis',
     'contracture of elbow joint',
     'elbow joint contracture',
     'napkin candidiasis',
     'candidal nappy rash',
     'monilial nappy rash',
     'diaper candidosis',
     'napkin candidosis',
     'bee sting',
     'accidental exposure to metallic lead',
     'clearing throat - hawking',
     'epiblepharon',
     'medullary carcinoma with amyloid stroma',
     'ultimobranchial thyroid tumour',
     'ultimobranchial thyroid tumor',
     'parafollicular cell carcinoma',
     'c cell carcinoma',
     'medullary thyroid carcinoma',
     'medullary carcinoma of thyroid',
     'MTc',
     'benign neoplasm of respiratory system',
     'adenomatous polyp of cervix',
     'adenomatous polyp of cervix uteri',
     'adenomatous polyp - cervix uteri',
     'calculus of gallbladder with cholecystitis',
     'biliary calculus with cholecystitis',
     'cholelithiasis with cholecystitis',
     'calculus of bile duct with cholecystitis',
     'choledocolithiasis with cholecystitis',
     'strain of tendon of back',
     'leg sprain',
     'fit denture',
     'insertion of denture',
     'fit prosthodontic appliance',
     'congenital quadriplegia',
     'congenital tetraplegia',
     'self inflicted injury',
     'injury - self-inflicted',
     'abscess of scrotum',
     'scrotal abscess',
     'marginal zone b-cell lymphoma',
     'salt lymphoma',
     'nodal marginal zone lymphoma',
     'mucosal-associated lymphoid tissue lymphoma',
     'monocytoid b-cell lymphoma',
     'marginal zone lymphoma',
     'malt lymphoma',
     'bronchial-associated lymphoid tissue lymphoma',
     'balt lymphoma',
     'skin-associated lymphoid tissue lymphoma',
     'mucosa-associated lymphoma',
     'maltoma',
     'nodal marginal zone b-cell lymphoma',
     'extranodal marginal zone b-cell lymphoma of mucosa-associated lymphoid tissue',
     'extranodal marginal zone b-cell lymphoma of mucosa-associated lymphoid tissue (malt-lymphoma)',
     'fusion of posterior lumbar spine',
     'multiple-resistant staphylococcus aureus infection',
     'MRSa',
     'superficial injury of forearm',
     'removal of screws from bone',
     'pathological fracture - upper arm',
     'pathological fracture of humerus',
     'productive cough',
     'producing sputum',
     'loose cough',
     'moist cough',
     'fetal heart rate absent',
     'foetal heart rate absent',
     'vaginal lesion',
     'scoliosis of thoracic spine',
     'scoliosis of dorsal spine',
     'infection of toe',
     'infection toe',
     'left lower zone pneumonia',
     'lobar pneumonia left lower lobe',
     'left basal pneumonia',
     'LLl',
     'left lower lobe pneumonia',
     'LLz',
     'right lower zone pneumonia',
     'lobar pneumonia right lower lobe',
     'right basal pneumonia',
     'RLz',
     'RLl',
     'right lower lobe pneumonia',
     'secondary hemorrhage postprocedure',
     'postmastectomy lymphedema syndrome',
     'post-mastectomy secondary lymphoedema',
     'post-mastectomy secondary lymphedema',
     'elephantiasis due to mastectomy',
     'obliteration of lymphatic vessel due to mastectomy',
     'enthesopathy of wrist and/or carpus',
     'disorganized schizophrenia',
     'hebephrenic schizophrenia',
     'recurrent herpes simplex',
     'pulmonary mycobacterium avium complex infection',
     'pulmonary mycobacterium avium-intracellulare infection',
     'battey disease',
     'pulmonary mycobacterium intracellulare infection',
     'postoperative complication',
     'disorder of function of stomach',
     'functional gastric disturbance',
     'functional gastric disorder',
     'disorder of stomach function',
     'disorder of gastric function',
     'intravenous therapy',
     'intravenous (iv) therapy',
     'intravenous care',
     'removal of ovarian cyst',
     'excision of cyst of ovary',
     'ovarian cystectomy',
     'oophorocystectomy',
     'abnormal blood pressure',
     'osteoporosis due to corticosteroids',
     'fh: alzheimers disease',
     'family history: alzheimers disease',
     'family history: alzheimer disease',
     'family history of alzheimers',
     'death of relative',
     'relatives died',
     'death of family member',
     'relatives dead',
     'family members deceased',
     'relatives deceased',
     'family members dead',
     'relative died',
     'relative dying',
     'chronic anterior uveitis',
     'chronic iritis',
     'sprain of ligament',
     'ligament sprain',
     'basal cell carcinoma of ear',
     'cancer of skin of ear, basal cell',
     'recurrent brief depressive disorder',
     'body mass index 40+ - severely obese',
     'body mass index 40+ - morbidly obese',
     'obese class iii',
     'vision screening',
     'pathological fracture of femur',
     'intermittent explosive disorder',
     'fracture of lower leg',
     'fracture of lower limb',
     'fracture of bone of lower limb',
     'leg fracture',
     'fracture of leg',
     'hernia of anterior abdominal wall without obstruction and without gangrene',
     'ventral hernia without obstruction and without gangrene',
     'restrictive cardiomyopathy',
     'constrictive cardiomyopathy',
     'primary restrictive cardiomyopathy',
     'accidental poisoning by carbon monoxide',
     'mass lesion of brain',
     'space-occupying lesion of brain',
     'brain mass',
     'unexplained weight loss',
     'squamous cell carcinoma of skin of neck',
     'cancer of skin of neck, squamous cell',
     'allergic fungal sinusitis',
     'thygeson superficial punctate keratitis',
     'punctate keratitis',
     'punctate epithelial keratoconjunctivitis',
     'thygesons superficial punctate keratitis',
     'superficial punctate keratitis',
     'chronic progressive renal failure',
     'chronic progressive renal insufficiency',
     'active living will',
     'diabetic autonomic neuropathy associated with type 1 diabetes mellitus',
     'type 1 diabetic autonomic neuropathy',
     'closed posterior dislocation of elbow',
     'complicated grieving',
     'history of non-hodgkins lymphoma',
     'recurrent bacterial infection',
     'dependence on haemodialysis due to end stage renal disease',
     'sympathicoblastoma',
     'sympathicogonioma',
     'sympathogonioma',
     'neuroblastoma',
     'Nb',
     'central neuroblastoma',
     'strain of trapezius muscle',
     'atypical squamous cells on cervical papanicolaou smear cannot exclude high grade squamous intraepithelial lesion',
     'emotional stress',
     'atherosclerosis of autologous vein bypass graft of limb',
     'oxyphilic adenocarcinoma',
     'follicular carcinoma, oxyphilic cell',
     'oncocytic carcinoma',
     'oncocytic adenocarcinoma',
     'hurthle cell carcinoma',
     'hurthle cell adenocarcinoma',
     'incision of gallbladder',
     'cholecystotomy',
     'enchondroma of bone',
     'ependymoma',
     'clear cell ependymoma',
     'cellular ependymoma',
     'ependymoma, no icd-o subtype',
     'ependymoma, no international classification of diseases for oncology subtype',
     'epithelial ependymoma',
     'exposure to hepatitis c virus',
     'cocaine-induced organic mental disorder',
     'cocaine induced mental disorder',
     'cocaine-related disorder',
     'epidermal nevus syndrome',
     'schimmelpeming feuerstein mims syndrome',
     'linear sebaceous nevus sequence',
     'linear sebaceous naevus',
     'linear sebaceous nevus',
     'klippel-trenaunay-weber syndrome',
     'haemangiectatic hypertrophy',
     'hemangiectatic hypertrophy',
     'cerebrofacial angiomatosis',
     'severe depressed bipolar i disorder without psychotic features',
     'severe bipolar i disorder, most recent episode depressed without psychotic features',
     'influenza with non-respiratory manifestation',
     'embryonic fimbrial cyst',
     'parovarian cyst',
     'fimbrial cyst',
     'parainfluenza virus pneumonia',
     'pneumonia due to parainfluenza virus',
     'parainfluenza pneumonia',
     'parainfluenzal pneumonia',
     'sensorineural hearing loss of combined sites',
     'bacterial overgrowth syndrome',
     'blind loop syndrome',
     'contaminated small bowel syndrome',
     'stagnant loop syndrome',
     'catheterisation of left heart',
     'cardiac catheterization, left heart',
     'cardiac catheterisation, left heart',
     'pruritus of vulva',
     'itching of vulva',
     'pruritus vulvae',
     'vulval itching',
     'menopausal problem',
     'contusion of buttock',
     'fear of open spaces',
     'agoraphobia',
     'fear of open places',
     'phobia of going out',
     'mild manic bipolar i disorder',
     'mild bipolar i disorder, most recent episode manic',
     'food poisoning',
     'samonella',
     'closed fracture of three ribs',
     'acute suppuration of nasal sinus',
     'acute suppurative inflammation of nasal sinus',
     'acute suppurative sinusitis',
     'skin sensation disturbance',
     'functional disorder of intestine',
     'open angle with borderline findings',
     'corrected transposition of great vessels',
     'transposition of great vessels with ventricular inversion',
     'congenitally corrected transposition of great arteries',
     'discordant ventriculoarterial connection with discordant atrioventricular connection',
     'haemorrhoidectomy by simple ligature',
     'ligation of hemorrhoids',
     'simple ligature of haemorrhoid',
     'simple ligature of hemorrhoid',
     'ligation of haemorrhoids',
     'hemorrhoidectomy, internal, by rubber band ligation(s)',
     'bipolar disorder in remission',
     'bipolar disorder, in remission',
     'dacryocystitis',
     'unstable lie',
     'foetus - unstable lie',
     'baby keeps changing position',
     'fetus - unstable lie',
     'prinzmetal angina',
     'variant angina',
     'coronary artery spasm angina',
     'prinzmetals angina',
     'variant angina pectoris',
     'hilar lymphadenopathy',
     'chronic tubotympanic suppurative otitis media',
     'chronic suppurative otitis media - tubotympanic',
     'lower gastrointestinal haemorrhage',
     'lower git - gastrointestinal haemorrhage',
     'lower git - gastrointestinal hemorrhage',
     'lower gastrointestinal bleeding',
     'lower gi hemorrhage',
     'lower gi bleeding',
     'lower gi haemorrhage',
     'tetraparesis',
     'quadriparesis',
     'benign neoplasm of cranial nerve',
     'benign neoplasm of skin of scalp',
     'benign neoplasm of scalp',
     'secondary malignant neoplasm of adrenal gland',
     'suprarenal metastasis',
     'metastasis to adrenal gland',
     'metastatic malignant neoplasm to adrenal gland',
     'cancer metastatic to adrenal gland',
     'neoplasm of uncertain behaviour of bladder',
     'ruptured cyst of ovary',
     'ruptured ovarian cyst',
     'rupture of ovarian cyst',
     'cellulitis of external ear',
     'staring',
     'gazing fixedly',
     'salpingo-oophorectomy',
     'tubo-oophorectomy',
     'So',
     'scarred macula',
     'macular scar',
     'cyst and pseudocyst of pancreas',
     'gastrostomy feeding',
     'hypertrophic condition of skin',
     'hypertrophic skin',
     'h/o: cardiovascular disease',
     'history of circulatory system disease',
     'history of cardiovascular disease',
     'open wound of finger with complication',
     'malignant tumour of peritoneum',
     'peritoneal cancer',
     'cancer of peritoneum',
     'hyperlipoproteinemia',
     'onychia of toe',
     'exercises education, guidance, and counselling',
     'exercises teaching, guidance, and counselling',
     'exercises teaching, guidance, and counseling',
     'posterior capsule opacification',
     'posterior capsule fibrosis',
     'posterior capsular opacification',
     'posterior capsular fibrosis',
     'PCf',
     'cloudy posterior capsule',
     'insertion of stent into ureter',
     'asymmetrical sensorineural hearing loss',
     'asymmetric sensorineural hearing loss',
     'pressure ulcer of ankle',
     'osteogenesis imperfecta',
     'osteopsathyrosis',
     'fragilitas ossium',
     'brittle bone syndrome',
     'brittle bone disease',
     'Oi',
     'narcissistic personality disorder',
     'sprain of hand',
     'sprain of joint of hand',
     'sprain of ligament of hand',
     'hand sprain',
     'sprain of hand joint',
     'malignant neoplasm of liver',
     'ca - liver cancer',
     'liver cancer',
     'malignant tumor of liver',
     'malignant tumour of liver',
     'severe mixed bipolar i disorder with psychotic features',
     'severe bipolar i disorder, most recent episode mixed, with psychotic features',
     'mechanical complication of internal orthopaedic device',
     'combined form of senile cataract',
     'torus mandibularis',
     'lingual torus',
     'hairy cell leukaemia (clinical)',
     'LRe',
     'leukaemic reticuloendotheliosis',
     'HCl',
     'hairy cell leukaemia',
     'leukemic reticuloendotheliosis',
     'hairy cell leukemia',
     'leukemic reticuloendotheliosis (clinical) [obs]',
     'leukaemic reticuloendotheliosis (clinical) [obs]',
     'hairy cell leukemia variant',
     'hairy cell leukaemia variant',
     'retroperitoneal lymphadenopathy',
     'vascular headache',
     'disorder of lumbar spine',
     'noncompliance with dietary regimen',
     'noncompliance: dietary regimen',
     'noncompliance: recommended nutrition plan',
     'noncompliance with recommended nutrition plan',
     'delayed union of fracture',
     'fracture, delayed union',
     'multi-infarct dementia with depression',
     'vascular dementia, with depressive mood',
     'ruptured abdominal aortic aneurysm',
     'abdominal aortic aneurysm which has ruptured',
     'ruptured aneurysm of abdominal aorta',
     'tilt table test',
     'tilt test',
     'esophageal bleeding',
     'hemorrhage of esophagus',
     'esophageal hemorrhage',
     'haemorrhage of oesophagus',
     'oesophageal haemorrhage',
     'learning difficulties',
     'h/o: gout',
     'history of gout',
     'h/o: ileostomy',
     'history of ileostomy',
     'abnormal weight gain',
     'abnormal increase in weight',
     'serum cholesterol raised',
     'renal function tests abnormal',
     'abnormal results of kidney function studies',
     'renal osteodystrophy',
     'renal rickets',
     'renal bone disease',
     'ROd',
     'arthritis associated with another disorder',
     'mediastinal emphysema',
     'pneumomediastinum',
     'a/n care: h/o stillbirth',
     'antenatal care: history of stillbirth',
     'loss of hypoglycaemic warning',
     'hypoglycemia unawareness',
     'hypoglycaemia unawareness',
     'renal tubular acidosis',
     'RTa',
     'renotubular acidaemia',
     'renotubular acidemia',
     'block dissection of cervical lymph nodes',
     'neck dissection',
     'enteric campylobacteriosis',
     'campylobacter diarrhoea',
     'campylobacter diarrhea',
     'campylobacter intestinal infection',
     'unilateral recurrent inguinal hernia with obstruction but no gangrene',
     'unilateral obstructed recurrent inguinal hernia',
     'chronic viral hepatitis b without delta-agent',
     'malignant tumour of glottis',
     'ca larynx - glottis',
     'malignant neoplasm of glottis',
     'malignant tumor of supraglottis',
     'ca larynx - supraglottis',
     'malignant neoplasm of supraglottis',
     'malignant neoplasm of connective and soft tissue of hip and lower limb',
     'mood swings',
     'emotional lability',
     'emotional instability',
     'emotionally labile',
     'labile in mood',
     'mood swing',
     'variable mood',
     'changeable mood',
     'unstable mood',
     'labile mood',
     'mixed bipolar affective disorder',
     'nondependent cocaine abuse, continuous',
     'benign secondary hypertension',
     'secondary benign hypertension',
     'current tear of semilunar cartilage',
     'current tear of meniscus of knee',
     'tear of meniscus of knee joint',
     'pulmonary stenosis, non-rheumatic',
     'pulmonary valve stenosis, nonrheumatic',
     'torsion of appendix of testis',
     'torsion of hydatid of morgagni - male',
     'infection of obstetric surgical wound',
     'localized scleroderma',
     'localized dermatosclerosis',
     'morphea scleroderma',
     'circumscribed scleroderma',
     'addisons keloid',
     'localized morphea',
     'morphoea scleroderma',
     'localised dermatosclerosis',
     'localised morphoea',
     'haemarthrosis of knee',
     'tibialis posterior tendinitis',
     'calculus in urethra',
     'urethral stone',
     'urethral calculus',
     'kyphoscoliosis and scoliosis',
     'closed fracture finger proximal phalanx, shaft',
     'osler haemorrhagic telangiectasia syndrome',
     'osler-weber-rendu disease',
     'hereditary hemorrhagic telangiectasia',
     'oslers disease',
     'osler-rendu-weber syndrome',
     'osler-rendu-weber disease',
     'hereditary haemorrhagic telangiectasia',
     'HHt',
     'neurosarcoidosis',
     'discitis',
     'intervertebral discitis',
     'ocular myasthenia',
     'delayed and/or secondary postpartum haemorrhage',
     'delayed postpartum haemorrhage',
     'delayed postpartum hemorrhage',
     'hemorrhage after first 24 hours following delivery of placenta',
     'postpartum hemorrhage specified as delayed or secondary',
     'postpartum hemorrhage, delayed and/or secondary',
     'haemorrhage after first 24 hours following delivery of placenta',
     'postpartum haemorrhage specified as delayed or secondary',
     'postpartum haemorrhage, delayed and/or secondary',
     'rupture of globe',
     'basal cell carcinoma of eyelid',
     'rodent ulcer of eyelid',
     'stenosis of nasolacrimal duct',
     'nasolacrimal duct obstruction',
     'obstruction of nasolacrimal duct',
     'nasolacrimal duct obstructed',
     'vascular cannula removal',
     'removal of drip needle',
     'decannulation',
     'vascular catheter removal',
     'old anterior myocardial infarction',
     'thoracoabdominal aortic aneurysm',
     'cerebral arteriovenous malformation',
     'congenital cerebral arteriovenous malformation',
     'avm - cerebral arteriovenous malformation',
     'fit partial denture',
     'insertion of partial denture',
     'insertion of part denture',
     'bolus obstruction of intestine',
     'liver transplant rejection',
     'calculus in pelviureteric junction',
     'stone in puj - pelviureteric junction',
     'stone in pelviureteric junction',
     'coronary artery spasm',
     'angina pectoris with documented spasm',
     'coronary spasm',
     'plane wart',
     'verruca plana',
     'flat wart',
     'poor short-term memory',
     'short-term memory loss',
     'short term memory loss',
     'perinatal jaundice from excessive haemolysis',
     'nasal deviation',
     'crooked nose',
     'dysfunctional voiding of urine',
     'voiding dysfunction',
     'dysfunctional elimination of urine',
     'high anal fistula',
     'ligament rupture',
     'complete tear of ligament',
     'ligament tear',
     'endometriosis of ovary',
     'history of gastrointestinal tract bypass',
     'history of gastrointestinal tract anastomosis',
     'genitourinary symptoms',
     'gu symptoms',
     'gut symptoms',
     'symbolic dysfunction',
     'chest swelling',
     'epicranial subaponeurotic haemorrhage',
     'malignant glioma of brain',
     'tooth absent',
     'tooth missing',
     'absence of teeth',
     'edentulous',
     'no natural teeth',
     'non-traumatic intracranial subdural hematoma',
     'strain of abdominal muscle',
     'atrophy of vagina',
     'lower limb nerve lesion',
     'wound pain',
     'removal of acoustic neuroma',
     'removal of vestibular schwannoma',
     'anxiety attack',
     'multiple endocrine neoplasia, type 1',
     'men 1 - multiple endocrine neoplasia syndrome type 1',
     'men 1 syndrome',
     'multiple endocrine neoplasia syndrome type 1',
     'men, type 1',
     'wermer syndrome',
     'mea, type 1',
     'multiple endocrine adenomatosis, type 1',
     'urinary tract infection in pregnancy',
     'urinary tract infection complicating pregnancy',
     'uti - urinary tract infection in pregnancy',
     'osteoporosis localized to spine',
     'infection of intravenous catheter',
     'colonoscopic polypectomy',
     'severe nonproliferative diabetic retinopathy',
     'severe nonproliferative retinopathy',
     'severe npdr',
     'laser assisted in situ keratomileusis',
     'LASIk',
     'chronic gastric ulcer with perforation',
     'perforated chronic gastric ulcer',
     'papilloma of eyelid',
     'type 2 diabetes mellitus with neuropathic arthropathy',
     'type ii diabetes mellitus with neuropathic arthropathy',
     'postoperative pneumonia',
     'anergy',
     'clonal anergy',
     'nephrosclerosis',
     'renal sclerosis',
     'interstitial arteriosclerotic nephritis',
     'chronic arteriosclerotic nephritis',
     'arteriosclerosis of kidney',
     'renal arteriosclerosis',
     'open wound of leg with complication',
     'neonatal hypotonia',
     'floppy infant',
     'floppy baby',
     'floppy infant syndrome',
     'congenital hypotonia',
     'bullous myringitis',
     'myringitis bullosa',
     'myringitis bullosa haemorrhagica',
     'myringitis bullosa hemorrhagica',
     'polyotia',
     'supernumerary ear',
     'supernumerary external ear',
     'accessory auricle',
     'accessory auricle of ear',
     'contusion of eye',
     'black eye',
     'bruise of eye',
     'iron deficiency anemia secondary to inadequate dietary iron intake',
     'vaginoscopy with biopsy of cervix',
     'colposcopy',
     'systemic mast cell disease',
     'systemic mastocytosis',
     'smcd - systemic mast cell disease',
     'malignant mastocytosis',
     'systemic tissue mast cell disease',
     'tympanotomy',
     'myringotomy',
     'tympanostomy',
     'myringostomy',
     'myringotomies',
     'disorder characterized by back pain',
     'disorder of pituitary gland',
     'pituitary disease',
     'disorder of pituitary',
     'dyspituitarism',
     'type 1 diabetes mellitus with persistent microalbuminuria',
     'repair of aneurysm of abdominal aorta',
     'herpes simplex type 2 infection',
     'herpes simplex 2 infection',
     'herpes simplex 2',
     'lipodermatosclerosis',
     'mental health problem',
     'internal hordeolum',
     'hordeolum internum',
     'infected chalazion',
     'infected cyst of meibomian gland',
     'infected meibomian cyst',
     'infection of meibomian gland',
     'meibomian adenitis',
     'open fracture of tibia and fibula',
     'closure of patent foramen ovale',
     'pfo - closure of patent foramen ovale',
     'repair of patent foramen ovale',
     'amputee',
     'amputated',
     'mononeuropathy associated with type 2 diabetes mellitus',
     'mononeuropathy associated with type ii diabetes mellitus',
     'adenolymphoma',
     'warthins tumour',
     'papillary cystadenoma lymphomatosum',
     'warthins tumor',
     'squamous cell carcinoma of chin',
     'squamous cell carcinoma of auricle of ear',
     'squamous cell carcinoma of pinna',
     'squamous cell carcinoma of auricle',
     'uninsured medical expenses',
     'fibroma',
     'fibroma, no icd-o subtype',
     'fibroma, no international classification of diseases for oncology subtype',
     'fibromas',
     'unbalanced diet',
     'bilateral sensory hearing loss',
     'embolism from thrombosis of vein of distal lower extremity',
     'embolism from venous thrombosis of lower leg',
     'cardiac pacemaker in situ',
     'iatrogenic pneumothorax',
     'idiopathic hypersomnia without long sleep time',
     'seminoma of testis',
     'seminoma',
     'seminoma, no icd-o subtype',
     'seminoma, no international classification of diseases for oncology subtype',
     'willing to be donor of liver',
     'pulmonary arterial hypertension associated with portal hypertension',
     'portopulmonary hypertension',
     'intracranial cavernous hemangioma',
     'glossitis',
     'inflammation of tongue',
     'severe mixed bipolar i disorder without psychotic features',
     'severe bipolar i disorder, most recent episode mixed, without psychotic features',
     'brain injury without open intracranial wound and with moderate loss of consciousness (1-24 hours)',
     'pyonephrosis',
     'closed fracture of distal end of ulna',
     'closed fracture of lower end of ulna',
     'closed fracture of lower epiphysis',
     'acidosis',
     'acidoses',
     'homocystinemia',
     'ischemia',
     'parasomnia',
     'parasomnia (sleep disorder)',
     'acquired pancytopenia',
     'pancytopenia - acquired',
     'unilateral femoral hernia without obstruction and without gangrene',
     'unilateral simple femoral hernia',
     'complete external haemorrhoidectomy',
     'cradle cap',
     'prurigo nodularis',
     'hydes disease',
     'abscess of buttock',
     'abscess of gluteal region',
     'gluteal abscess',
     'interstitial pneumonia',
     'interstitial pneumonitis',
     'radical retropubic prostatectomy',
     'beta thalassaemia',
     'beta thalassemia',
     'beta thalassemia syndrome',
     'beta thalassaemia syndrome',
     'beta thalassaemia',
     'beta^+^ thalassaemia',
     'beta plus thalassemia',
     'beta plus thalassaemia',
     'beta + thalassemia',
     'beta + thalassaemia',
     'smells of urine',
     'urinary body odor',
     'urinary body odour',
     'peritoneal adhesion',
     'adhesive peritoneal band',
     'peritoneal band',
     'acute subendocardial infarction',
     'acute nontransmural infarction',
     'pyoderma',
     'pyodermia',
     'purulent dermatitis',
     'suppurative dermatitis',
     'septic dermatitis',
     'pyogenic dermatitis',
     'pustuloderma',
     'pyogenic infection of skin and subcutis',
     'struck by falling object',
     'tuberous sclerosis syndrome',
     'Ts',
     'tuberous sclerosis',
     'bournevilles disease',
     'adenoma sebaceum syndrome',
     'epiloia',
     'tuborous sclerosis',
     'tuberous sclerosus',
     'tubero',
     'unilateral modified radical mastectomy',
     'modified radical mastectomy, unilateral',
     'chronic gingivitis',
     'heavy-for-dates at birth regardless of gestation period',
     'large-for-dates infant regardless of gestation period',
     'heavy for dates',
     'heavy for gestation age infant',
     'empyema of gallbladder',
     'stevens-johnson syndrome',
     'erythema multiforme exudativum',
     'ectodermosis erosiva pluriorificialis',
     'erythema multiforme major',
     'erythema multiforme bullosum',
     'stevens johnson syndrome',
     'bullous erythema multiforme',
     'chronic tophaceous gout',
     'open fracture of phalanx of foot',
     'open fracture of phalanx of toe',
     'open fracture of toe',
     'renal disease in pregnancy and/or puerperium without hypertension',
     'nephropathy in pregnancy and/or puerperium without hypertension',
     'thrombosed haemorrhoids',
     'derangement of posterior horn of lateral meniscus',
     'lateral meniscus, posterior horn derangement',
     'anaemia due to unknown mechanism',
     'alcoholic polyneuropathy',
     'alcoholic peripheral neuropathy',
     'alcohol-induced polyneuropathy',
     'collagen disease',
     'diffuse disease of connective tissue',
     'collagen disorder',
     'chews tobacco',
     'congenital insufficiency of tricuspid valve',
     'tr - congenital tricuspid regurgitation',
     'congenital tricuspid regurgitation',
     'biopsy - action',
     'Bx',
     'biopsy',
     'biopsies',
     'old bucket handle tear of medial meniscus',
     'fall from ladder',
     'fall on and from ladder',
     'visual field study',
     'examination of visual fields',
     'visual field testing',
     'visual field',
     'pyometra',
     'pyometrium',
     'contracture of joint of shoulder region',
     'joint contracture of shoulder',
     'contracture of shoulder joint',
     'polyp of vocal cord',
     'vocal fold polyp',
     'benign neoplasm of pituitary gland',
     'benign tumour of pituitary gland',
     'benign tumor of pituitary gland',
     'benign neoplasm of skin of neck',
     'biventricular congestive heart failure',
     'biventricular failure',
     'primary malignant neoplasm of thyroid gland',
     'secondary malignant neoplasm of oesophagus',
     'metastatic malignant neoplasm to esophagus',
     'cancer metastatic to esophagus',
     'metastatic malignant neoplasm to oesophagus',
     'secondary malignant neoplasm of large intestine',
     'metastasis to large intestine',
     'metastatic malignant neoplasm to large intestine',
     'cancer metastatic to large intestine',
     'duodenal haemorrhage',
     'gallstone pancreatitis',
     'corneal graft rejection',
     'dyskinesia',
     'skin striae',
     'striae atrophicae',
     'physiological striae',
     'striae distensae',
     'stretch marks',
     'striae gravidarum',
     'linear atrophy',
     'striae',
     'lineae albicantes',
     'lineae atrophicae',
     'open fracture of phalanx of finger',
     'open fracture of finger',
     'ulnar nerve entrapment',
     'non-tuberculous mycobacterial pneumonia',
     'malignant tumour of hypopharynx',
     'malignant tumour of laryngopharynx',
     'malignant tumor of laryngopharynx',
     'malignant neoplasm of laryngopharynx',
     'malignant neoplasm of male breast',
     'ca breast - male',
     'carcinoma of male breast',
     'serous detachment of retinal pigment epithelium',
     'retinal pigment epithelium serous detachment',
     'juvenile absence epilepsy',
     'childhood absence epilepsy',
     'pykno-epilepsy',
     'petit-mal epilepsy',
     'pyknolepsy',
     'childhood - juvenile - absence epilepsy',
     'absence seizure',
     'petit mal',
     'petit-mal seizure',
     'respiratory obstruction',
     'embarrassed airway',
     'nocturnal muscle spasm',
     'history and physical examination, immigration',
     'homocysteine measurement',
     'homocysteine',
     'lobular carcinoma in situ of breast',
     'decreased oestrogen level',
     'congenital or acquired abnormality of cervix affecting pregnancy',
     'ovarian failure',
     'repair of complete shoulder cuff avulsion, chronic',
     'reconstruction of complete shoulder (rotator) cuff avulsion, chronic (includes acromioplasty)',
     'neoplasm of oropharynx',
     'tumor of oropharynx',
     'tumour of oropharynx',
     'neoplasm of rectum',
     'tumor of rectum',
     'tumour of rectum',
     'bronchiolitis obliterans organising pneumonia',
     'cryptogenic organizing pneumonia',
     'bronchiolitis obliterans with organizing pneumonia',
     'BOOp',
     'cryptogenic organising pneumonia',
     'COp',
     'cryptogenic organising pneumonitis',
     'bronchiolitis obliterans and organising pneumonitis',
     'bronchiolitis obliterans and organizing pneumonitis',
     'cryptogenic organizing pneumonitis',
     'bronchiolitis obliterans with organising pneumonia',
     'ovarian hyperstimulation syndrome',
     'secondary meigs syndrome',
     'total urinary incontinence',
     'urinary incontinence with continuous leakage',
     'double incontinence',
     'double incontinence - urine and stool',
     'total incontinence',
     'abscess of axilla',
     'hypopituitarism due to radiotherapy',
     'post-radiotherapy hypopituitarism',
     'radiotherapy-induced hypopituitarism',
     'h/o: injury',
     'history of injury',
     'rheumatoid factor positive',
     'a/n care: h/o perinatal death',
     'antenatal care: history of perinatal death',
     'postnatal visit',
     'abdominoplasty',
     'repair of abdominal wall',
     'reconstruction of abdominal wall',
     'incision and drainage of wound',
     'repeat prescription card duplicate issue',
     'malignant tumour of middle third of oesophagus',
     'ca middle third oesophagus',
     'ca middle third esophagus',
     'malignant neoplasm of middle third of esophagus',
     'malignant neoplasm of middle third of oesophagus',
     'pituitary dependent hypercortisolism',
     'pituitary-dependent cushings disease',
     'cushing disease',
     'pituitary cushing syndrome',
     'pituitary dependent cushing disease',
     'cushing basophilism',
     'pituitary hyperadrenal corticism',
     'cushings disease',
     'recurrent depression',
     'nonorganic insomnia',
     'psychogenic feeding disorder of infancy and childhood',
     'late effects of intracranial abscess or pyogenic infection',
     'cauda equina syndrome',
     'anal skin tag',
     'anal tag',
     'fibrous polyp of anus',
     'sigmoid volvulus',
     'volvulus of sigmoid colon',
     'hypotonic bladder',
     'hypotonicity of bladder',
     'stenosis of urinary meatus',
     'meatal stenosis',
     'pinhole meatus',
     'meatal stricture',
     'stricture of urethral meatus',
     'hypertrophic scar',
     'hypertrophic cicatrix',
     'deliveries by cesarean',
     'cesarean delivery',
     'caesarean delivery',
     'diabetic neuropathic arthropathy',
     'diabetic charcots arthropathy',
     'sacroiliac disorder',
     'arthropathy of sacroiliac joint',
     'capsular cataract',
     'pain in scrotum',
     'pain of scrotum',
     'scrotal pain',
     'fetal or neonatal effect of vacuum extraction delivery',
     'fetal or neonatal effect of delivery by vacuum extractor',
     'foetal or neonatal effect of vacuum extraction delivery',
     'foetal or neonatal effect of delivery by vacuum extractor',
     'neonatal necrotising enterocolitis',
     'NNe',
     'NNEc',
     'neonatal necrotizing enterocolitis',
     'open wound of hip and/or thigh',
     'open wound of hip and thigh',
     'injury of nerves at shoulder and upper arm level',
     'postoperative pain',
     'closed fracture of medial condyle of humerus',
     'accidents caused by hot substance or object, caustic or corrosive material and steam',
     'inattention',
     'general inattentiveness',
     'platelet disorder',
     'thrombocytopathy',
     'frontotemporal dementia',
     'brainstem death',
     'motility disorder of intestine',
     'bowel dyskinesia',
     'acute-on-chronic renal failure',
     'acute on chronic renal failure',
     'pituitary mass',
     'mass of pituitary',
     'premature ovarian failure',
     'POf',
     'psoriasis-eczema overlap condition',
     'deficiency of anterior cruciate ligament',
     'anterior cruciate ligament deficiency',
     'seronegative rheumatoid arthritis',
     'microscopic polyarteritis nodosa',
     'microscopic polyangiitis',
     'bursitis of foot region',
     'bone necrosis',
     'osteonecrosis',
     'placenta praevia partialis',
     'partial previa',
     'partial placenta previa',
     'partial placenta praevia',
     'retinal drusen',
     'colloid bodies in retina',
     'sciatic nerve palsy',
     'self-harm',
     'self-damage',
     'bleeding gastric varices',
     'toe-walking gait',
     'toe walking',
     'merkel cell carcinoma',
     'trabecular cell carcinoma of skin',
     'merkel cell tumour',
     'merkel cell tumor',
     'apudoma of skin',
     'primary cutaneous neuroendocrine carcinoma',
     'macroprolactinoma',
     'extramammary pagets disease of skin',
     'extramammary paget disease of skin',
     'extramammary pagets disease of skin',
     'b-k mole (nevus) syndrome',
     'FAMMm',
     'familial atypical mole malignant melanoma syndrome',
     'atypical mole syndrome',
     'functionless pituitary adenoma',
     'non-functioning pituitary adenoma',
     'null cell pituitary adenoma',
     'follicular thyroid carcinoma',
     'FTc',
     'follicular adenocarcinoma, well differentiated',
     'follicular adenocarcinoma - well differentiated',
     'follicular carcinoma - well differentiated',
     'follicular carcinoma, well differentiated',
     'follicular adenocarcinoma',
     'follicular carcinoma',
     'radical prostatectomy',
     'total prostatectomy',
     'traumatic intracranial subarachnoid haemorrhage',
     'traumatic cranial subarachnoid hemorrhage',
     'traumatic cranial subarachnoid haemorrhage',
     'joint capsule sprain',
     'iliopsoas abscess',
     'psoas muscle abscess',
     'psoas abscess',
     'uric acid urolithiasis',
     'hypo-osmolality and or hyponatraemia',
     'neonatal hypocalcemia',
     'benign neoplasm of skin of ear and external auditory meatus',
     'breasts asymmetrical',
     'burping',
     'eructation',
     'belching',
     'belching symptom',
     'eructation, function',
     'eructations',
     'disorder of cardiac pacemaker system',
     'fracture of phalanx of thumb',
     'fracture of thumb',
     'open insertion of hickman central venous catheter',
     'disorder of smell',
     'extreme immaturity',
     'very premature baby',
     'intraduct papilloma of pancreas',
     'carcinoma of fallopian tube',
     'lumbar puncture',
     'rachicentesis',
     'spinal puncture',
     'spinal tap',
     'injury of scrotum',
     'family disruption',
     'acute infective exacerbation of chronic obstructive airways disease',
     'removal of plate from bone',
     'immune thrombocytopenia',
     'thrombocytopenia due to immune destruction',
     'closed fracture of scapula',
     'closed fracture of shoulder blade',
     'mass of skin',
     'lump of skin',
     'foreign body of skin of finger',
     'swelling of finger',
     'swollen finger',
     'herpes simplex dendritic keratitis',
     'hsv dendritic keratitis',
     'lump on face',
     'mass of face',
     'pain in thumb',
     'lesion of lip',
     'neck swelling',
     'mononeuritis multiplex',
     'skin grafting',
     'skin graft operation',
     'dermatoplasty',
     'repair of skin',
     'decreased range of cervical spine movement',
     'pseudomyxoma peritonei',
     'amputated big toe',
     'acute gingivitis',
     'acute gingival inflammation',
     'cyanosis of skin',
     'cyanosed',
     'cyanosis',
     'tuberculosis of vertebral column',
     'tuberculosis of spine (potts)',
     'tuberculosis of vertebral column - potts',
     'potts disease',
     'tuberculous spondylitis',
     'malignant tumor of anal canal',
     'cancer of anal canal',
     'malignant tumour of nasal cavity',
     'malignant neoplasm of nasal cavities',
     'skin irritation',
     'screening for substance abuse',
     'screens for substance abuse',
     'drug of abuse screen',
     'drugs of abuse screen',
     'drugs of abuse screening',
     'amputation of toe',
     'amputation, toe',
     'dysplasia of vagina',
     'vaginal pain',
     'pain in vagina',
     'serum ferritin high',
     'hypovolemic shock',
     'low volume shock',
     'thyroglossal duct cyst',
     'thyroglossal cyst',
     'bronchopneumonia',
     'bronchial pneumonia',
     'percutaneous transluminal coronary angioplasty with rotoablation, single vessel',
     'diverticulosis of colon without diverticulitis',
     'adenoma of rectum',
     'disorder of taste',
     'not yet walking',
     'basal cell carcinoma of neck',
     'cancer of skin of neck, basal cell',
     'decorative tattoo',
     'decorative tattoo of skin',
     'squamous cell carcinoma of scalp',
     'cancer of the scalp, squamous cell',
     'squamous cell carcinoma of hand',
     'domestic violence',
     'drug-induced hyperpyrexia',
     'drug fever',
     'acute esophagitis',
     'ocular histoplasmosis syndrome',
     'ohs-ocular histoplasmosis syndrome',
     'olfactory neuroblastoma',
     'aesthesioneuroblastoma',
     'esthesioneuroblastoma',
     'aesthesioneuroepithelioma',
     'olfactory neuroepithelioma',
     'asthesioneuroblastoma',
     'tendinitis of finger',
     'tendonitis of finger',
     'migraine variants, not intractable',
     'relapsing remitting multiple sclerosis',
     'multiple sclerosis (ms) relapsing remitting',
     'flaccid neurogenic bladder',
     'flaccid neuropathic bladder',
     'calcium renal calculus',
     'diverticulitis of sigmoid colon',
     'diverticulitis of sigmoid',
     'history of transurethral prostatectomy',
     'history of transurethral resection of prostate',
     'history of amputation of hallux',
     'history of amputation of great toe',
     'history of adenomatous polyp of colon',
     'patellar instability',
     'instability of femoropatellar joint',
     'repetitive strain injury',
     'RSIs',
     'repetitive strain injury syndrome',
     'RSi',
     'family history of malignant neoplasm of genital structure',
     'family history of cancer of genital system',
     'pain in male perineum',
     'male perineal pain',
     'chronic systolic heart failure',
     'lytic lesion of bone on x-ray',
     'complete tetraplegia due to lesion at c1-c4 level',
     'complete quadriplegia due to lesion at c1-c4 level',
     'complete quadriplegia due to spinal cord lesion between first and fourth cervical vertebra',
     'cardiac defibrillator in situ',
     'imaging of lung abnormal',
     'chondroma of bone',
     'osteochondroma',
     'cartilaginous exostosis',
     'osteocartilaginous exostosis',
     'ecchondroma',
     'papule',
     'papula',
     'papulae',
     'papules',
     'disorder of joint of shoulder region',
     'exposure to viral hepatitis',
     'raised antinuclear antibody',
     'antinuclear antibody above reference range',
     'spondylolysis of cervical spine',
     'low lying placenta',
     'left sided ulcerative colitis',
     'cytomegalovirus serostatus positive',
     'cytomegalovirus antibody in serum positive',
     'aphonia',
     'loss of voice',
     'does not vocalise',
     'does not vocalize',
     'does not produce voice',
     'does not phonate',
     'absence of voice',
     'foreign body in lower limb',
     'laceration of lower lip',
     'ureteric stone of lower third of ureter',
     'ureterolithiasis of lower third of ureter',
     'calculus of lower third of ureter',
     'sepsis due to streptococcus',
     'von hippel-lindau syndrome',
     'von hippel-lindau syndrome',
     'vhl - von hippel-lindau syndrome',
     'lindaus disease',
     'familial cerebello-retinal angiomatosis',
     'lindau disease',
     'cerebroretinal angiomatosis',
     'torsion of spermatic cord',
     'torsion of testis',
     'torsion of testicle',
     'testicular torsion',
     'acute cor pulmonale',
     'fracture of patella',
     'fracture of knee-cap',
     'sciatic neuropathy',
     'nasal septoplasty with graft',
     'alcohol-induced mood disorder',
     'alcohol induced mood disorder',
     'arthrodesis by anterior interbody technique of cervical region below c2 with bone graft',
     'cervical tuberculous lymphadenitis',
     'scrofula',
     'tuberculosis of cervical lymph nodes',
     'tuberculous cervical lymphadenitis',
     'deficient knowledge',
     'knowledge deficit',
     'brain injury without open intracranial wound and with prolonged loss of consciousness (more than 24 hours) with return to pre-existing conscious level',
     'retinal vascular disorder',
     'changes in retinal vascular appearance',
     'failure of exfoliation of primary tooth',
     'delayed shedding of deciduous tooth',
     'delayed exfoliation of deciduous tooth',
     'prolonged retention of deciduous tooth',
     'retained deciduous tooth',
     'delayed exfoliation of primary teeth',
     'failure of resorption of root of tooth',
     'persistent primary tooth',
     'persistent deciduous tooth',
     'primary teeth retained',
     'deciduous teeth retained',
     'tuberculosis of meninges',
     'tuberculous meningitis',
     'TBm',
     'lagophthalmos',
     'defective lid closure',
     'poor closure eyelids',
     'infection by coccidioides immitis',
     'coccidioidomycosis',
     'posadas-wernicke disease',
     'coccidioidomycosis infection',
     'operative procedure on foot',
     'operative procedure on foot or toes',
     'unlisted procedure, foot or toes',
     'bitemporal hemianopia',
     'lumbosacral neuritis',
     'bacterial infection due to pseudomonas',
     'pseudomonas infection',
     'amygdalolith',
     'calculus of tonsil',
     'tonsillolith',
     'tonsil stone',
     'tonsillar calculus',
     'tonsillith',
     'tonsillolithiasis',
     'repair of ruptured supraspinatus tendon, chronic',
     'cheilopalatoschisis',
     'cleft palate with cleft lip',
     'cleft palate and lip',
     'suture granuloma',
     'lumbar spondylosis with myelopathy',
     'spondylogenic compression of lumbar spinal cord',
     'closed fracture of coronoid process of ulna',
     'closed fracture of ulna, coronoid',
     'mucocele of salivary gland',
     'retention cyst of salivary gland',
     'sialocele',
     'salivary cyst',
     'ptyalocele',
     'mucous retention cyst of salivary gland',
     'chronic ethmoidal sinusitis',
     'chronic ethmoiditis',
     'ill-defined disease',
     'ill-defined condition',
     'ill defined condition',
     'closed fracture of neck of metacarpal bone',
     'activity intolerance',
     'malnutrition of mild degree (gomez: 75% to less than 90% of standard weight)',
     'iridocyclitis',
     'chest pain on exertion',
     'arthroscopy of knee with meniscus repair',
     'arthroscopic repair of meniscus',
     'arthroscopy, knee, surgical; with meniscus repair (medial or lateral)',
     'ineffective thermoregulation',
     'thermoregulation impairment',
     'heavy drinker',
     'drinks heavily',
     'convulsions in the newborn',
     'neonatal seizures',
     'seizures in newborn',
     'fits in newborn',
     'seizures in the newborn',
     'fits in the newborn',
     'neonatal convulsions',
     'haematoma of pinna',
     'hematoma of auricle',
     'haematoma of auricle',
     'alteration in nutrition: less than body requirements',
     'body nutrition deficit',
     'imbalanced nutrition: less than body requirements',
     'hypoproteinaemia',
     'spinal cord injury',
     'SCi',
     'victim of physical assault',
     'mural thrombus of heart',
     'mural thrombosis',
     'endocardial thrombosis',
     'allergy to peanuts',
     'back pain complicating pregnancy',
     'cancer in situ of urinary bladder',
     'CIs',
     'carcinoma in situ of bladder',
     'secondary malignant neoplasm of vertebral column',
     'secondary malignant neoplasm of spine',
     'metastatic malignant neoplasm to spine',
     'metastatic malignant neoplasm to vertebral column',
     'secondary tumor of spine',
     'metastasis to vertebral column',
     'metastasis to spine',
     'secondary cancer of spine',
     'ca - secondary cancer of spine',
     'spinal metastases',
     'secondary tumour of spine',
     'muscular ventricular septum defect',
     'muscular ventricular septal defect',
     'neonatal cyanosis',
     'ulnar neuritis',
     'generalised retinal degeneration',
     'recurrent cholangitis',
     'third degree perineal laceration',
     'obstetrical laceration, third degree',
     'laceration of tissues between vaginal and/or perineal muscular layers and rectal mucosa',
     'coronary artery bypass with autogenous graft, three grafts',
     'sleep apnoea assessment',
     'refusal of treatment by patient',
     'electrolyte imbalance',
     'acute promyelocytic leukaemia, fab m3',
     'acute promyelocytic leukaemia',
     'm3 - acute promyelocytic leukaemia',
     'APMl',
     'acute promyelocytic leukemia',
     'm3 - acute promyelocytic leukemia',
     'acute promyelocytic leukaemia (clinical)',
     'acute promyelocytic leukemia (clinical)',
     'acute promyelocytic leukemia, t(15;17)(q22;q11-12)',
     'acute promyelocytic leukemia, pml/rar-alpha',
     'acute myeloid leukemia, t(15;17)(q22;q11-12)',
     'acute myeloid leukemia, pml/rar-alpha',
     'acute myeloid leukaemia, pml/rar-alpha',
     'acute promyelocytic leukaemia, t(15;17)(q22;q11-12)',
     'acute promyelocytic leukaemia, pml/rar-alpha',
     'acute myeloid leukaemia, t(15;17)(q22;q11-12)',
     'fab m3',
     'ankylosis of joint',
     'ankylosis',
     'frozen joint',
     'fusion of joint',
     'operative procedure on hand',
     'operation on accessory sinus',
     'operation on nasal sinus',
     'nasal sinus procedure',
     'surgical procedures on the accessory sinuses',
     'valvuloplasty of aortic valve',
     'aortic valvuloplasty',
     'repair of aortic valve',
     'aortic valvoplasty',
     'AVp',
     'plastic repair of aortic valve',
     'valvuloplasty, aortic valve',
     'neoplasm of pelvis',
     'tumour of pelvis',
     'tumor of pelvis',
     'neoplasm of larynx',
     'tumor of larynx',
     'tumour of larynx',
     'neoplasm of small intestine',
     'tumour of small intestine',
     'tumor of small intestine',
     'neoplasm of retroperitoneum',
     'tumour of retroperitoneum',
     'tumor of retroperitoneum',
     'neoplasm of the retroperitoneum',
     'separation anxiety disorder of childhood',
     'separation anxiety disorder',
     'separation anxiety',
     'disorder of elbow',
     'complex regional pain syndrome',
     'complex regional pain syndrome (crps)',
     'complex regional pain syndromes',
     'disorder of sacrum',
     'right heart failure',
     'autonomic dysreflexia',
     'dysreflexia',
     'ineffective infant feeding pattern',
     'congenital spondylolisthesis',
     'cellulitis of knee',
     'hypothermia of newborn',
     'combined anteroposterior colporrhaphy',
     'anterior and posterior repair of vagina',
     'colporrhaphy for repair of cystocele and rectocele',
     'repair of cystocele and rectocele',
     'anterior and posterior colporrhaphy',
     'anterior colporrhaphy and posterior colpoperineorrhaphy',
     'ACPCp',
     'anterior and posterior repair',
     'ACPc',
     'fh: blood disorder',
     'family history of blood disorder',
     'family history: blood disorder',
     'fh: glaucoma',
     'family history of glaucoma',
     'family history: glaucoma',
     'h/o: perinatal problem',
     'history of perinatal problem',
     'h/o: radiation exposure',
     'history of radiation exposure',
     'history of irradiation',
     'h/o: heart recipient',
     'history of heart recipient',
     'h/o: renal dialysis',
     'h/o: kidney dialysis',
     'history of renal dialysis',
     'pain in female genitalia',
     'female genital pain',
     'ear pressure sensation',
     'open wound of abdominal wall with complication',
     'echocardiogram abnormal',
     'antenatal ultrasound scan abnormal',
     'abnormal ultrasonic finding on antenatal screening of mother',
     'a/n u/s scan abnormal',
     'respiratory disease screening',
     'percutaneous transluminal ablation of atrioventricular node',
     'dental calculus',
     'tartar',
     'odontolith',
     'bilateral inguinal hernia repair',
     'fluid volume disorder',
     'disorder of fluid balance',
     'fluid imbalance',
     'alcohol intoxication delirium',
     'malignant melanoma of scalp and/or neck',
     'lymphosarcoma of lymph nodes of multiple sites',
     'hemangioma of intra-abdominal structure',
     'hemangioma of intra-abdominal structures',
     'haemangioma of intra-abdominal structures',
     'uterovaginal prolapse',
     'vaginal and cervical prolapse',
     'iodine hypothyroidism',
     'simple schizophrenia',
     'schizophrenia simplex',
     'acute exacerbation of chronic schizoaffective schizophrenia',
     'schizoaffective schizophrenia in remission',
     'schizoaffective schizophrenia, in remission',
     'schizophrenia, schizoaffective, in remission',
     'atypical depressive disorder',
     'chronic anxiety',
     'nondependent opioid abuse in remission',
     'prolonged depressive adjustment reaction',
     'acute exudative otitis media',
     'benign hypertensive heart disease without congestive cardiac failure',
     'benign hypertensive heart disease without congestive heart failure',
     'spider naevus',
     'nevus araneus of skin',
     'spider nevus of skin',
     'telangiectatic nevus',
     'stellar nevus of skin',
     'naevus araneus of skin',
     'stellar naevus of skin',
     'telangiectatic naevus',
     'spider naevus of skin',
     'arterial spider',
     'arterial spider of skin',
     'capillary angioma',
     'spider angioma of skin',
     'spider telangiectasis of skin',
     'spider telangiectasia',
     'spider angioma',
     'chronic adenotonsillitis',
     'bilateral total vocal cord paralysis',
     'complete bilateral paralysis of vocal cords',
     'complete paralysis of both vocal cords',
     'endometriosis of pelvic peritoneum',
     'diabetes mellitus during pregnancy - baby not yet delivered',
     'pityriasis simplex',
     'furfuracea',
     'dandruff',
     'scurfiness of skin',
     'localised, secondary osteoarthritis of the shoulder region',
     'secondary localized osteoarthrosis of shoulder region',
     'secondary localised osteoarthrosis of shoulder region',
     'clicking hip',
     'transient synovitis',
     'toxic synovitis',
     'closed fracture of proximal humerus, anatomical neck',
     'closed fracture of anatomical neck of humerus',
     'closed fracture multiple phalanges, toe',
     'complete tear, knee, anterior cruciate ligament',
     'open wounds involving multiple regions of lower limb(s)',
     'drowning and non-fatal immersion',
     'drowning and non-fatal submersion',
     'postoperative deep vein thrombosis',
     'mechanical lagophthalmos',
     'overexertion and strenuous movements',
     'intellectual functioning disability',
     'intellectual disability',
     'intellectual limitation',
     'long-term drug misuser',
     'vascular dementia of acute onset',
     'meige syndrome',
     'blepharospasm - oromandibular dystonia',
     'brueghel syndrome',
     'episodic tension-type headache',
     'motor neuropathy with multiple conduction block',
     'multifocal motor neuropathy',
     'MMn',
     'excision of tumor of brain meninges',
     'removal of meningioma',
     'language development disorder',
     'developmental language disorder',
     'menstrual migraine',
     'posterior proliferative vitreoretinopathy',
     'neovascular glaucoma',
     'secondary angle closure glaucoma with rubeosis',
     'rubeotic glaucoma',
     'implantation of automatic cardiac defibrillator',
     'fracture of upper limb',
     'fracture of bone of upper limb',
     'arm fracture',
     'fracture of arm',
     'metatarsus adductus',
     'congenital metatarsus adductus',
     'postoperative intestinal obstruction',
     'postprocedural intestinal obstruction',
     'pseudo-obstruction of intestine',
     'obstructed internal hernia',
     'medullary sponge kidney',
     'obstruction of peritoneal dialysis catheter',
     'blockage of peritoneal dialysis catheter',
     'chlamydial cervicitis',
     'vaginal vault haematoma',
     'polymorphous light eruption',
     'polymorphic light eruption',
     'PLe',
     'polymorphic photodermatitis',
     'rupture of posterior cruciate ligament',
     'PCl',
     'posterior cruciate ligament rupture',
     'tear of posterior cruciate ligament',
     'accident due to contact with hot or corrosive substance',
     'excess skin of eyelid',
     'dermatochalasis',
     'intercostal neuralgia',
     'labial adhesions',
     'tetrachlorodibenzodioxin poisoning',
     'dioxin poisoning',
     'tcdd poisoning',
     'carcinoma of pylorus',
     'compound naevus of skin',
     'compound nevus',
     'dermal and epidermal naevus',
     'dermal and epidermal nevus',
     'angiomyolipoma of kidney',
     'hamartoma of kidney',
     'neurosyphilis',
     'syphilis of central nervous system',
     'total great vessel transposition',
     'complete transposition of great vessels',
     'complete tga',
     'transposition of the great arteries',
     'classical transposition of great vessels',
     'discordant ventriculoarterial connection with concordant atrioventricular connection',
     'sprain of ligament of elbow',
     'sprain of elbow joint',
     'sprain of elbow',
     'elbow sprain',
     'total colectomy',
     'total removal of colon',
     'total excision of colon',
     'complete resection of colon',
     'thyroid dysfunction',
     'insertion of pleural tube drain',
     'insertion of chest tube',
     'tube thoracostomy',
     'insertion of chest drain',
     'insertion of intercostal drain',
     'ICc',
     'insertion of intercostal catheter',
     'steroid-induced myopathy',
     'steroid myopathy',
     'pruritus of genital organs',
     'genital pruritus',
     'congenital unilateral subluxation of hip',
     'congenital subluxation of hip, unilateral',
     'sprain of elbow and forearm',
     'sprain elbow and forearm',
     'benign neoplasm of nose, middle ear and accessory sinuses',
     'flatulence, eructation and gas pain',
     'thoracic back sprain',
     'superficial injury of wrist',
     'fall on same level due to impact against another person',
     'fh: ovarian carcinoma',
     'family history: ovarian carcinoma',
     'fh: arthritis',
     'family history: arthritis',
     'metastasis to digestive organs',
     'cancer metastatic to digestive organs',
     'non-suppurative otitis media',
     'peripheral arteriovenous malformation',
     'screening for malignant neoplasm of rectum',
     'rectal neoplasm screening',
     't-cell chronic lymphocytic leukemia',
     'TCLl',
     't-cell chronic lymphocytic leukaemia',
     'facial swelling',
     'swollen face',
     'hepatitis a immune',
     'cervicogenic headache',
     'chronic alcoholic brain syndrome',
     'dementia associated with alcoholism',
     'alcoholic dementia',
     'alcohol-induced persisting dementia',
     'incision and drainage of abscess',
     'Id',
     'incision and drainage abscess',
     'left sided abdominal pain',
     'severe manic bipolar i disorder with psychotic features',
     'severe bipolar i disorder, most recent episode manic, with psychotic features',
     'incisional biopsy of breast mass',
     'mass of vulva',
     'lump of vulva',
     'progressive supranuclear ophthalmoplegia',
     'PSp',
     'progressive supranuclear palsy',
     'nuchal dystonia-dementia syndrome',
     'steele-richardson-olszewski syndrome',
     'contact hypersensitivity',
     'chronic undifferentiated schizophrenia',
     'motorcycle accident',
     'motorbike accident',
     'temporomandibular joint tender',
     'deformity of knee joint',
     'loin abscess',
     'abscess of groin',
     'groin abscess',
     'gastrostomy present',
     'neoplasm of paraganglion',
     'gangliocytic paraganglioma',
     'paraganglioma',
     'implantation of umbrella into vena cava',
     'vulvar vestibulitis',
     'vulval vestibulitis',
     'vestibulodynia',
     'caffeine-related disorder',
     'intracapsular cataract extraction and insertion of intraocular lens',
     'ICCe',
     'villous adenoma of colon',
     'cryoglobulinemia',
     'cryoimmunoglobulinaemia',
     'cryoimmunoglobulinemia',
     'varicella-zoster virus infection',
     'subtotal thyroidectomy',
     'partial thyroidectomy',
     'pseudostrabismus',
     'localized, primary osteoarthritis of elbow',
     'lymphocytic-plasmacytic colitis',
     'lymphocytic colitis',
     'peripheral visual field defect',
     'idiopathic progressive polyneuropathy',
     'foreign body in digestive tract',
     'foreign body of digestive structure',
     'foreign body in alimentary tract',
     'drusen of optic disc',
     'hyaline bodies of optic disc',
     'optic nerve head drusen',
     'optic disc drusen',
     'postoperative infection',
     'traumatic cataract',
     'anovulation',
     'duodenal ulcer without hemorrhage, without perforation and without obstruction',
     'pityriasis',
     'primary chronic pseudo-obstruction of colon',
     'acute pseudo-obstruction of colon',
     'ogilvies syndrome',
     'strabismic amblyopia',
     'suppression amblyopia',
     'fibromuscular dysplasia',
     'fibromuscular hyperplasia of artery',
     'FMd',
     'arterial fibromuscular dysplasia',
     'repair of paraoesophageal diaphragmatic hernia',
     'repair of parahiatal diaphragmatic hernia',
     'oesophageal hiatus hernia repair',
     'hiatal herniorrhaphy',
     'hiatus hernia repair',
     'repair of diaphragmatic hiatal hernia',
     'carcinoid heart disease',
     'poisoning by local anti-inflammatory drug',
     'malignant tumor of palate',
     'malignant tumour of roof of mouth',
     'malignant tumor of roof of mouth',
     'malignant neoplasm of roof of mouth',
     'cancer of palate',
     'malignant tumor of neck',
     'fracture of distal phalanx of finger',
     'lumbosacral stenosis',
     'hyperproteinemia',
     'carcinoma of endocervix',
     'malignant neoplasm of skin of trunk',
     'ca skin - trunk',
     'macular retinal edema',
     'macular oedema',
     'macular edema',
     'contusion of multiple sites',
     'multiple bruising',
     'choanal polyp',
     'nasopharyngeal polyp',
     'nasal polyp - posterior',
     'polyp in nasopharynx',
     'premature menopause',
     'laryngomalacia',
     'endometrial biopsy',
     'Eb',
     'endometrial sampling',
     'endometrial sampling (biopsy) performed in conjunction with colposcopy (list separately in addition to code for primary procedure)',
     'cholesteatoma of attic',
     'traumatic pneumothorax without open wound into thorax',
     'closed traumatic pneumothorax',
     'breast screening declined',
     'follow-up encounter',
     'coronary artery bypass with autogenous graft, four grafts',
     'phacomorphic glaucoma',
     'lens swelling glaucoma',
     'lens induced angle closure glaucoma',
     'telogen effluvium',
     'suppurative arthritis',
     'pyoarthrosis',
     'pyogenic arthritis',
     'infective arthritis',
     'infection-associated arthritis',
     'acute and chronic cholecystitis',
     'acute on chronic cholecystitis',
     'amputation of lower limb',
     'amputation of leg',
     'hind limb amputation',
     'lower limb amputation',
     'amputation of the lower limb',
     'ehlers-danlos syndrome',
     'cutis hyperelastica dermatorrhexis',
     'dystrophia mesodermalis congenita',
     'india rubber skin',
     'dermatorrhexis with dermatochalasis and arthrochalasis',
     'hereditary collagen dysplasia',
     'meekeren-ehlers-danlos syndrome',
     'cutis elastica',
     'cutis hyperelastica',
     'danlos disease',
     'fibrodysplasia elastica generalisata',
     'disorder of artery',
     'arterial disease',
     'generalized atherosclerosis',
     'arteriopathy',
     'arteriopath',
     'mycobacterium chelonae infection of skin',
     'squamous cell carcinoma of skin of lower extremity',
     'squamous cell cancer of skin of lower limb',
     'peripheral degeneration of retina',
     'peripheral retinal degeneration',
     'acute osteomyelitis',
     'juvenile rheumatoid arthritis',
     'JRa',
     'acquired kyphosis',
     'acquired hunchback',
     'fall from playground equipment',
     'fall involving playground equipment',
     'myelocystocele',
     'meningomyelocele',
     'myelomeningocele',
     'disorder of pinna',
     'st. louis encephalitis virus infection',
     'st. louis viral disease',
     'neuroinvasive st. louis encephalitis virus infection',
     'st. louis encephalitis virus neuroinvasive disease',
     'saint louis encephalitis',
     'st. louis encephalitis',
     'type c lethargic encephalitis',
     'neuroinvasive saint louis encephalitis virus infection',
     'encephalitis, st-louis',
     'deformity',
     'deformities',
     'insomnia with sleep apnea',
     'peripheral circulatory disorder associated with diabetes mellitus',
     'diabetic peripheral circulatory disorder',
     'diabetic cataract associated with type 1 diabetes mellitus',
     'diabetic cataract associated with type i diabetes mellitus',
     'diabetes type 1 with cataract',
     'thoracic radiculopathy',
     'thoracic root lesion',
     'dementia associated with parkinsons disease',
     'dementia associated with parkinson disease',
     'dementia in parkinsons disease',
     'bilateral pleural effusion',
     'chronic organic mental disorder',
     'chronic organic brain syndrome',
     'cerebral vasculitis',
     'transformed migraine',
     'chronic migraine',
     'infection of amputation stump',
     'history of augmentation of breast',
     'history of breast augmentation',
     'history of repair of musculotendinous cuff of shoulder',
     'history of repair of rotator cuff',
     'tear of skin',
     'skin tear',
     'traumatic tear of skin',
     'history of bilateral mastectomy',
     'closed fracture of tibial plateau',
     'history of amputation of finger',
     'complex cyst of uterine adnexa',
     'complex uterine adnexal cyst',
     'history of radiation therapy',
     'fetal hydronephrosis',
     'foetal hydronephrosis',
     'congenital anomaly of peripheral blood vessel',
     'peripheral vascular congenital anomaly',
     'secondary adhesive capsulitis',
     'acquired adhesive capsulitis',
     'intentional poisoning by drug',
     'chronic pain due to injury',
     'increased cancer antigen 125',
     'raised cancer antigen 125',
     'elevated cancer antigen 125',
     'acute injury of anterior cruciate ligament',
     'hereditary thrombophilia',
     'hereditary hypercoagulable disorder',
     'primary thrombophilia',
     'stimulant abuse',
     'insertion of tunneled venous catheter',
     'dental caries extending into dentin',
     'dental caries extending into dentine',
     'dentine caries',
     'dentin caries',
     'suicide',
     'suicides',
     'adenoma',
     'adenoma, no subtype',
     'benign adenomatous neoplasm',
     'benign adenoma',
     'adenomas',
     'adenomata',
     'carcinoid tumor - morphology',
     'carcinoid tumor',
     'carcinoid',
     'carcinoid tumour',
     'giant cell tumour',
     'latent syphilis',
     'thymoma',
     'thymoma, no icd-o subtype',
     'thymoma, no international classification of diseases for oncology subtype',
     'exposure to bordetella pertussis',
     'exposure to whooping cough',
     'tubular adenoma, no icd-o subtype',
     'tubular adenoma, no international classification of diseases for oncology subtype',
     'tubular adenoma',
     'glioma of brainstem',
     'irregular bowel habits',
     'prophylactic immunotherapy',
     'clotting time above reference range',
     'weakness of vocal cord',
     'raised low density lipoprotein cholesterol',
     'low density lipoprotein cholesterol above reference range',
     'orthostatic headache',
     'scar of face',
     'facial scar',
     'sepsis due to staphylococcus aureus',
     'nonspecific mesenteric adenitis',
     'nonspecific mesenteric lymphadenitis',
     'mesenteric lymphadenitis',
     'cyst of eyelid',
     'closed fracture of shaft of metacarpal bone',
     'replacement of intrauterine contraceptive device',
     'change of intrauterine contraceptive device',
     'iucd - change of intrauterine contraceptive device',
     'iud - change of intrauterine device',
     'change of coil',
     'replacement of contraceptive intrauterine device',
     'replacement of iud',
     'miliary tuberculosis',
     'MTb',
     'cortex contusion without open intracranial wound and with moderate loss of consciousness (1-24 hours)',
     'first degree uterine prolapse',
     'typhoid fever',
     'infection by salmonella typhi',
     'acute anterior uveitis',
     'aau - acute anterior uveitis',
     'acute iridocyclitis',
     'intussusception',
     'introsusception',
     'intussusception of intestine',
     'ISn',
     'intussusception of the intestine',
     'intestinal intussusception',
     'invagination of intestine',
     'femoral-popliteal artery bypass graft with vein',
     'femoropopliteal reversed vein bypass graft',
     'femoral-popliteal in-situ vein bypass',
     'bypass graft, with vein; femoral-popliteal',
     'arterial retinal branch occlusion',
     'branch retinal artery occlusion',
     'BRAo',
     'myocarditis',
     'myocardial inflammation',
     'chronic contact dermatitis',
     'disorder of anterior pituitary',
     'anterior pituitary disease',
     'disorder of adenohypophysis',
     'arteritis',
     'inflammation of artery',
     'ileitis',
     'necrotizing fasciitis',
     'necrotising cellulitis',
     'necrotizing cellulitis',
     'necrotising myositis',
     'necrotizing myositis',
     'foreign body in conjunctival sac',
     'bilateral breast implants',
     'insertion of bilateral breast prostheses',
     'bilateral insertion of prosthesis into breast',
     'insertion of prosthetic device into breast, bilateral',
     'derangement of posterior horn of medial meniscus',
     'medial meniscus, posterior horn derangement',
     'megaloblastic anemia',
     'peroneal tendinitis',
     'peroneal tendonitis',
     'abnormal male sexual function',
     'male sexual dysfunction',
     'simple phobia',
     'isolated phobia',
     'specific phobia',
     'insertion of permanent pacemaker with transvenous electrodes, ventricular',
     'initial insertion of transvenous pacemaker electrode leads into ventricle',
     'initial insertion of electrode into ventricle of heart',
     'insertion of new or replacement of permanent pacemaker with transvenous electrode(s); ventricular',
     'tooth extraction',
     'extract tooth',
     'xtn - extraction of tooth',
     'extn - extraction of tooth',
     'removal of tooth',
     'exodontia',
     'x - extraction of tooth',
     'exodontia procedure',
     'arthroscopically aided anterior cruciate ligament repair',
     'staphylococcal infectious disease',
     'staphylococcosis',
     'staphylococcal infection',
     'creation of conduit of right atrium and pulmonary artery',
     'fontan type operation',
     'fontan operation with direct anastomosis of right atrium to pulmonary artery',
     'formation of conduit of right atrium and pulmonary artery',
     'creation of shunt of right atrium and pulmonary artery',
     'fontan operation',
     'poisoning by cardiotonic glycoside',
     'poisoning by cardiac glycoside',
     'closed fracture of vault of skull with subarachnoid, subdural and/or extradural hemorrhage',
     'viral meningitis',
     'meningitis, viral',
     'aseptic meningitis, viral',
     'abacterial meningitis',
     'dribbling of urine',
     'dribbling',
     'repair of ruptured supraspinatus tendon, acute',
     'herpes simplex disciform keratitis',
     'hsv disciform keratitis',
     'severe depressed bipolar i disorder with psychotic features',
     'severe bipolar i disorder, most recent episode depressed with psychotic features',
     'disorder of magnesium metabolism',
     'drug withdrawal syndrome in neonate of dependent mother',
     'drug withdrawal syndrome in infant of dependent mother',
     'orthopnoea',
     'breathlessness lying flat',
     'tachyarrhythmia',
     'nonobliterative otosclerosis involving oval window',
     'otosclerosis involving oval window - non-obliterative',
     'non-obliterative otosclerosis',
     'altered body image',
     'distorted body image',
     'body image disturbance',
     'disturbed body image',
     'pruritic rash',
     'prurigo',
     'itchy skin eruption',
     'enteritis of small intestine',
     'enteritis',
     'enteritis, inflammatory disorder of small intestine',
     'inflammation of small intestine',
     'arterial thrombosis',
     'acquired cavovarus deformity of foot',
     'acquired cavovarus foot deformity',
     'langerhans cell histiocytosis',
     'histiocytosis x [obs]',
     'langerhans cell granulomatosis',
     'langerhans cell histiocytosis, no icd-o subtype',
     'langerhans cell histiocytosis, no international classification of diseases for oncology subtype',
     'langerhans cell histiocytosis, multifocal',
     'langerhans cell histiocytosis, poly-ostotic',
     'hand-schuller-christian disease [obs]',
     'langerhans cell histiocytosis, polyostotic',
     'histiocytosis x syndrome',
     'hand-schüller-christian disease',
     'schüller-christian syndrome',
     'multifocal and unisystemic langerhans cell histiocytosis',
     'schuller-christian syndrome',
     'generalised histiocytosis of bones',
     'hand-schuller-christian disease',
     'chronic differentiated progressive histiocytosis',
     'chronic disseminated histiocytosis x',
     'chronic histiocytosis x',
     'generalized histiocytosis of bones',
     'chronic idiopathic xanthomatosis',
     'histiocytosis x',
     'langerhans cell disease',
     'differentiated progressive histiocytosis',
     'LCh',
     'langerhans cell histiocytosis',
     'closed fracture of sternum',
     'closed fracture sternum',
     'chronic steatorrhoea',
     'fatty stool',
     'steatorrhea',
     'steatorrhoea',
     'latent syphilis with positive serology',
     'latent syphilis (+ sero.)',
     'disturbance of temperature regulation of newborn',
     'perinatal disorders of temperature regulation',
     'intolerant of heat',
     'sensitive to heat',
     'gets overheated',
     'retroperitoneal mass',
     'mass of retroperitoneal structure',
     'acquired anal stenosis',
     'stricture of anus',
     'anal stricture',
     'stenosis of anus',
     'anal stenosis',
     'cholelithiasis without obstruction',
     'cholelithiasis, non obstructive',
     'agenesis of eye',
     'anophthalmos',
     'agenesis of eyes',
     'congenital absence of eyes',
     'anophthalmia',
     'clinical anophthalmos',
     'congenital absence of eye',
     'inclusion body myositis',
     'IBm',
     'nodular degeneration of cornea',
     'supraventricular arrhythmia',
     'prickly heat',
     'heat rash',
     'miliaria rubra',
     'miliaria tropicalis',
     'prickley heat',
     'stenosis of lacrimal punctum',
     'punctal stenosis',
     'closed traumatic dislocation of joint of finger',
     'malnutrition of moderate degree (gomez: 60% to less than 75% of standard weight)',
     'opioid intoxication',
     'placenta praevia without haemorrhage',
     'placenta previa found during pregnancy without hemorrhage',
     'low implantation of placenta without hemorrhage',
     'low implantation of placenta without haemorrhage',
     'placenta praevia found during pregnancy without haemorrhage',
     'thigh pain',
     'late effect of radiation',
     'delayed effect of radiation',
     'congenital cataract',
     'implantation of tricuspid valve prosthesis or synthetic device',
     'haemarthrosis',
     'bleeding into joint',
     'congestive rheumatic heart failure',
     'familial hypokalaemic periodic paralysis',
     'hypokalemic periodic paralysis',
     'hypokalaemic periodic paralysis',
     'periodic paralysis i',
     'agitated depression',
     'slowing of urinary stream',
     'parapharyngeal abscess',
     'abscess of parapharyngeal space',
     'abscess of lateral pharyngeal space',
     'peripharyngeal abscess',
     'sensory hearing loss',
     'cochlear hearing loss',
     'inner ear hearing loss',
     'acute suppurative otitis media with spontaneous rupture of ear drum',
     'acute suppurative otitis media - tympanic membrane ruptured',
     'acute suppurative otitis media with discharge',
     'open reduction of fracture',
     'open # reduction',
     'radical perineal prostatectomy',
     'prostatectomy, perineal radical',
     'disorder of skeletal system',
     'disease of bone and/or joint',
     'osteoarthropathy',
     'iatrogenic hypothyroidism',
     'subarachnoid haemorrhage following injury without open intracranial wound and with prolonged loss of consciousness (more than 24 hours) and return to pre-existing conscious level',
     'microbial identification kit, rapid strep method',
     'streptococcus screening test, direct',
     'congenital genu valgum',
     'congenital knock-knee',
     'brain injury without open intracranial wound',
     'closed traumatic brain injury',
     'placenta marginalis',
     'marginal placenta',
     'placenta marginata',
     'placenta circummarginata',
     'thoracentesis',
     'pleurocentesis',
     'pleural tap',
     'thoracic paracentesis',
     'pleural cavity tap',
     'pleural aspiration',
     'thoracocentesis',
     'chest aspiration',
     'thoracenteses',
     'thoracocenteses',
     'benign neoplasm of choroid',
     'benign tumor of choroid',
     'benign tumour of choroid',
     'benign neoplasm of orbit',
     'benign tumour of orbit',
     'benign orbital tumour',
     'benign tumor of orbit',
     'benign orbital tumor',
     'neurofibromatosis type 2',
     'BANf',
     'bilateral acoustic neurofibromatosis',
     'familial vestibular schwannoma',
     'nf2',
     'neurofibromatosis, central type',
     'familial acoustic neuroma',
     'chronic leukaemia (category)',
     'chronic leukemia - category',
     'chronic leukaemia - category',
     'chronic leukaemia',
     'chronic leukaemia, disease',
     'chronic leukemia, disease',
     'dermatitis of eyelid',
     'ophthalmic herpes simplex',
     'herpes simplex ophthalmicus',
     'herpes simplex keratitis',
     'herpetic keratitis',
     'hsv keratitis',
     'secondary malignant neoplasm of bladder',
     'metastatic malignant neoplasm to bladder',
     'cancer metastatic to urinary bladder',
     'metastasis to bladder',
     'metastatic tumour to bladder',
     'metastatic tumor to bladder',
     'secondary malignant neoplasm of lymph nodes of face',
     'metastatic malignant neoplasm to lymph nodes of face',
     'cancer metastatic to lymph nodes of face',
     'secondary malignant neoplasm of testis',
     'metastatic tumour to testis',
     'metastatic tumor to testis',
     'metastasis to testis',
     'metastatic malignant neoplasm to testis',
     'neoplasm of uncertain behaviour of kidney',
     'renal neoplasm of uncertain behaviour',
     'renal neoplasm of uncertain behavior',
     'neoplasm of uncertain behaviour of nervous system',
     'neoplasm of uncertain behaviour of parotid gland',
     'victim of terrorism',
     'skin necrosis',
     'cutaneous necrosis',
     'sloughing of skin',
     'polymyalgia',
     'haemorrhagic duodenitis',
     'acute duodenitis',
     'erosive duodenitis',
     'multiple duodenal erosions',
     'obstruction of duodenum',
     'cerebellar degeneration',
     'peripheral motor neuropathy',
     'motor peripheral neuropathy',
     'motor neuritis',
     'sensation of burning of skin',
     'holiday relief care of patient',
     'dieulafoys vascular malformation',
     'dieulafoy disease',
     'dieulafoy vascular malformation',
     'calibre persistent artery',
     'caliber persistent artery',
     'refractory anemia (clinical)',
     'myelodysplastic syndrome: refractory anaemia, without ringed sideroblasts, without excess blasts',
     'myelodysplastic syndrome: refractory anemia, without ringed sideroblasts, without excess blasts',
     'refractory anemia',
     'erythrodysplasia',
     'refractory anaemia',
     'injury of penis',
     'injuries of penis',
     'dermatitis herpetiformis',
     'duhrings disease',
     'dermatosis herpetiformis',
     'duhring-brocq disease',
     'Dh',
     'diastasis of muscle',
     'ureteritis',
     'inflammation of ureter',
     'haemolytic uraemic syndrome',
     'HUs',
     'hemolytic uremic syndrome',
     'gassers syndrome',
     'prolonged qt interval',
     'increased qt interval',
     'femoral-popliteal artery bypass graft',
     'FPb',
     'femoropopliteal bypass',
     'above knee femoral popliteal bypass graft',
     'femoropopliteal bypass graft',
     'femoral-popliteal artery vascular bypass',
     'creation of femoropopliteal shunt',
     'bypass graft, with other than vein; femoral-popliteal',
     'idiopathic granulomatous interstitial nephropathy',
     'paralytic stroke',
     'breast finding',
     'breast observations',
     'lymphocyte-rich classical hodgkin lymphoma',
     'hodgkins disease, lymphocytic predominance',
     'hodgkins disease, lymphocytic-histiocytic predominance (clinical)',
     'hodgkin lymphoma, lymphocyte-rich (clinical)',
     'hodgkin lymphoma, lymphocyte-rich',
     'hodgkins disease, lymphocytic-histiocytic predominance [obs]',
     'hodgkins disease, lymphocyte predominance, diffuse [obs]',
     'hodgkins disease, lymphocyte predominance [obs]',
     'classical hodgkin lymphoma, lymphocyte-rich',
     'open wound of toe',
     'intertrochanteric fracture',
     'extracapsular fracture of neck of femur',
     'pertrochanteric fracture of neck of femur',
     'trochanteric fracture of neck of femur',
     'intertrochanteric fracture of femur',
     'pertrochanteric fracture',
     'hammer toe operation',
     'repair of hammer toe',
     'correction of hammer toe',
     'correction, hammertoe (eg, interphalangeal fusion, partial or total phalangectomy)',
     'disorder of cervical spine',
     'noncompliance with therapeutic regimen',
     'noncompliance: therapeutic regimen',
     'reduced visual acuity',
     'gastric motor function disorder',
     'idiopathic gastric motility disorder',
     'gastric motor dysfunction',
     'vocal cord dysfunction',
     'overanxious disorder of childhood',
     'monofilament foot sensation test',
     'hemifacial spasm',
     'facial hemispasm',
     'lymphangitis',
     'inflammation of lymphatics',
     'measles',
     'rubeola',
     'morbilli',
     'measels',
     'failed vacuum extractor',
     'failed vacuum extraction delivery',
     'urethral fistula',
     'osteotomy',
     'osteotomy - action',
     'division of bone',
     'osteotomies',
     'thromboendarterectomy with graft of carotid artery by neck incision',
     'disorder of cornea',
     'corneal disorder',
     'restrictive cardiomyopathy secondary to infiltrations',
     'polyarteritis nodosa',
     'periarteritis nodosa',
     'PAn',
     'acute pericarditis',
     'protein s deficiency disease',
     'protein s deficiency',
     'h/o: hypothyroidism',
     'history of hypothyroidism',
     'h/o: diabetes mellitus',
     'pre-existing diabetes mellitus',
     'history of diabetes mellitus',
     'h/o: artificial eyeglobe',
     'history of artificial eyeglobe',
     'h/o: miscarriage',
     'history of miscarriage',
     'stiff neck',
     'crick in neck',
     'neck stiffness',
     'hartmann operation, rectal resection',
     'hartmanns procedure',
     'rectosigmoidectomy, closure of rectal stump and exteriorization of bowel',
     'rectosigmoidectomy, closure of rectal stump and exteriorisation of bowel',
     'autoantibody titer positive',
     'pneumococcal infectious disease',
     'pneumococcal infection',
     'mitochondrial myopathy',
     'ragged red myopathy',
     'barium enema abnormal',
     'hereditary coagulation factor deficiency',
     'spontaneous rupture of membranes',
     'SRm',
     'SROm',
     'ruptured membranes',
     'rupture of foetal membranes',
     'spontaneous rupture of foetal membranes',
     'ROm',
     'waters broken',
     'spontaneous rupture of fetal membranes',
     'rupture of fetal membranes',
     'rupture of membranes',
     'requires a tetanus booster',
     'medical counselling',
     'trichotillomania',
     'hair plucking',
     'trichologia',
     'mechanical prosthetic aortic valve replacement',
     'replacement of aortic valve with prosthesis',
     'hypochondriasis',
     'hypochondriacal neurosis',
     'hypochondria',
     'inflammatory disorder of the eye',
     'ophthalmitis',
     'ophthalmia',
     'endophthalmitis',
     'congenital stenosis of aortic valve',
     'congenital aortic stenosis',
     'congenital stenosis of aorta',
     'congenital stricture of aorta',
     'congenital narrowed aorta',
     'late latent syphilis',
     'histoplasmosis with retinitis',
     'cyclical vomiting syndrome',
     'periodic vomiting',
     'cyclical vomiting',
     'malignant tumor of tail of pancreas',
     'ca tail of pancreas',
     'malignant neoplasm of tail of pancreas',
     'malignant melanoma of shoulder',
     'malignant melanoma of skin of shoulder',
     'malignant melanoma of upper arm',
     'malignant melanoma of skin of upper arm',
     'senile dementia with delirium',
     'senile delirium',
     'paranoid disorder',
     'paranoia',
     'paranoid psychosis',
     'dissociative convulsions',
     'factitious seizures',
     'sham seizures',
     'hysterical seizures',
     'hysterical fit',
     'dissociative seizures',
     'pseudoseizures',
     'psychogenic seizures',
     'continuous opioid dependence',
     'cerebral venous sinus thrombosis',
     'thrombosis of intracranial venous sinus',
     'drug-induced immune thrombocytopenia',
     'drug-induced thrombocytopenic purpura',
     'drug-induced itp',
     'myopathy due to sjögrens disease',
     'myopathy due to sjögren disease',
     'myopathy due to sjogrens disease',
     'myopathy due to sjogrens disease',
     'mitral and aortic incompetence',
     'mitral and aortic insufficiency',
     'mitral and aortic regurgitation',
     'atrial thrombosis',
     'cerebral infarction due to thrombosis of cerebral arteries',
     'unilateral partial vocal cord paralysis',
     'partial unilateral paralysis of vocal cords',
     'partial paralysis of one vocal cord',
     'recurrent bronchiectasis',
     'spontaneous tension pneumothorax',
     'paroxysmal nocturnal haemoglobinuria',
     'PNh',
     'paroxysmal nocturnal hemoglobinuria',
     'marchiafava-micheli syndrome',
     'monocytosis',
     'submandibular sialolithiasis',
     'submandibular calculus',
     'atrophy of kidney',
     'renal atrophy',
     'postoperative ureteric constriction',
     'postprocedural ureteric obstruction',
     'recurrent cystitis',
     'rhinophyma',
     'hypertrophic rosacea',
     'history of recurrent miscarriage - not delivered',
     'habitual aborter - not delivered',
     'pre-existing type 2 diabetes mellitus',
     'rhesus isoimmunisation with antenatal problem',
     'abnormal biochemical finding on antenatal screening of mother',
     'congenital anomaly of skin',
     'congenital cutaneous anomaly',
     'congenital malformation of the skin',
     'congenital skin anomalies',
     'carbuncle of back',
     'abscess of face',
     'localised, primary osteoarthritis of the hand',
     'primary localized osteoarthrosis of hand',
     'primary localised osteoarthrosis of hand',
     'acromioclavicular joint pain',
     'arthralgia of acromioclavicular joint',
     'cervical post-laminectomy syndrome',
     'rotator cuff shoulder syndrome and allied disorders',
     'full thickness rotator cuff tear',
     'complete rupture of rotator cuff',
     'flexor tenosynovitis of finger',
     'osteomyelitis of vertebra',
     'aneurysmal bone cyst',
     'ABc',
     'preauricular sinus and fistula',
     'alcoholic gastritis',
     'congenital atresia of pulmonary valve',
     'congenital atresia of the pulmonary valve',
     'pulmonary atresia',
     'unilateral complete cleft palate with cleft lip',
     'hypospadias, penile',
     'urethral meatus underneath penis',
     'hypospadias, balanic',
     'hypospadias, glandular',
     'gingivostomatitis',
     'fetal or neonatal effect of prolapsed cord',
     'foetal or neonatal effect of prolapsed cord',
     'vertical heterophoria',
     'delusions',
     'delusional thoughts',
     'delusional ideas',
     'delusion',
     'psychologic conversion disorder',
     'conversion disorder',
     'psychogenic fugue',
     'hysterical fugue',
     'dissociative fugue',
     'dissociative disorder',
     'hysterical neurosis, conversion type',
     'hysterical disorder',
     'hysteria',
     'hysterical neurosis, dissociative type',
     'fugue',
     'scapulalgia',
     'scapulodynia',
     'pain in scapula',
     'shoulder blade pain',
     'closed fracture thoracic vertebra',
     'closed fracture of tibia and fibula, shaft',
     'acute meniscal tear, lateral',
     'closed traumatic dislocation ankle joint',
     'open wound of hand with complication',
     'abrasion of face',
     'graze of face',
     'facial graze',
     'abrasion, face',
     'osteomyelitis of lower leg',
     'crush injury, toe(s)',
     'crushing injury of toe',
     'crush injury of toe',
     'chronic meningitis',
     'fall from stairs',
     'fall from steps',
     'bite of nonvenomous snakes and lizards',
     'accident caused by electric current',
     'accidents caused by electric current',
     'pericoronitis',
     'tired',
     'perineal pain',
     'autonomic facial cephalgia',
     'carotodynia',
     'carotidynia',
     'brain ventricular shunt obstruction',
     'speech and language disorder',
     'conjunctival foreign body',
     'anterior proliferative vitreoretinopathy',
     'chronic non-suppurative otitis media',
     'disorder of nasal cavity',
     'attention to tracheostomy',
     'permanent cardiac pacemaker procedure',
     'transjugular intrahepatic portosystemic shunt',
     'TIPs',
     'stripping of vein',
     'asbestos-induced pleural plaque',
     'postoperative cardiac complication',
     'accelerated coronary artery disease in transplanted heart',
     'carotid artery aneurysm',
     'aneurysm of carotid artery',
     'lymphocyst',
     'lymphocele',
     'lymph cyst',
     'microscopic colitis',
     'small intestinal gangrene',
     'angiodysplasia of intestine',
     'intestinal vascular dysplasia',
     'fecal peritonitis',
     'malignant ascites',
     'renal artery occlusion',
     'renal transplant rejection',
     'calyceal renal calculus',
     'calyceal stone',
     'acne keloidalis',
     'acne cheloidalis',
     'acne keloid',
     'hypertrophic acne scar',
     'keloidal acne',
     'hypertrophic acne scars',
     'livedo reticularis',
     'livedo racemosa',
     'contracture of joint of finger',
     'knee pyogenic arthritis',
     'seropositive rheumatoid arthritis',
     'ischial bursitis',
     'enthesopathy of foot region',
     'tenosynovitis of fingers',
     'osteoporotic vertebral collapse',
     'collapse of vertebra due to osteoporosis',
     'erythroderma neonatorum',
     'toxic erythema of newborn',
     'neonatal erythema toxicum',
     'erythema toxicum neonatorum',
     'ETn',
     'toxic erythema of the newborn',
     'erythema neonatorum',
     'epstein-barr virus disease',
     'cyst of vagina',
     'vaginal cyst',
     'loss of part of visual field',
     'comedone',
     'blackhead',
     'open comedone',
     'comedo',
     'open follicle comedo',
     'black head',
     'comedones',
     'rectal mass',
     'rectal lump',
     'atrophic vulva',
     'atrophy of vulva',
     'atrophic vulvitis',
     'adhesions of foreskin',
     'vocal cord cyst',
     'hand joint stiff',
     'aortic root dilatation',
     'av nodal re-entry tachycardia',
     'atrioventricular nodal re-entry tachycardia',
     'alveolar periostitis',
     'alveolitis of jaw',
     'alveolar osteitis',
     'dry socket',
     'alveolitis sicca dolorosa',
     'alveoalgia',
     'alveolalgia',
     'dry tooth socket',
     'chiari malformation',
     'arnold-chiari syndrome',
     'generalized intestinal dysmotility',
     'carcinoma of parotid gland',
     'tumour of submandibular gland',
     'carcinoma of duodenum',
     'polyp of intestine',
     'intestinal polyp',
     'malignant tumour of unknown origin',
     'malignant tumor - unknown primary',
     'ukp - malignant tumor - unknown primary',
     'malignant tumour - unknown primary',
     'ukp - malignant tumour - unknown primary',
     'cancer of unknown origin',
     'metastasis - unknown primary',
     'cancer - unknown origin',
     'pleomorphic adenoma of parotid gland',
     'adenoma of parotid gland',
     'PSa',
     'traumatic injury of common peroneal nerve',
     'traumatic subdural haematoma',
     'traumatic cranial subdural haematoma',
     'traumatic intracranial subdural haematoma',
     'traumatic intracranial subdural hematoma',
     'traumatic cranial subdural hematoma',
     'rupture of muscle',
     'muscle tear',
     'torn muscle',
     'primary open reduction and internal fixation of proximal femoral fracture with screw/nail and plate device',
     'cerclage of uterine cervix',
     'banding of uterine cervix',
     'cerclage of cervix',
     'cerclage of uterine cervix, nonobstetrical',
     'inadequate housing',
     'child health care',
     'child care',
     'arm sprain - upper',
     'malignant tumour of greater curve of stomach',
     'ca greater curvature - stomach',
     'benign tumor of breast',
     'benign neoplasm of breast',
     'malignant melanoma of head and neck',
     'benign neoplasm of pituitary gland and craniopharyngeal duct',
     'anasarca',
     'generalized edema',
     'generalised oedema',
     'oedema, generalised',
     'anasarcas',
     'leucoplakia',
     'leukoplakia',
     'leucoplacia',
     'open wound of abdominal wall',
     'surgical biopsy of breast',
     'perianal dermatitis',
     'superficial vein thrombosis',
     'lithium monitoring',
     'immunisation refused',
     'dysmorphism',
     'dysmorphia',
     'birth defect',
     'squamous cell carcinoma of floor of mouth',
     'large cell anaplastic lymphoma',
     '(ki-1+) lymphoma',
     'anaplastic large cell lymphoma, t cell and null cell type',
     'large cell (ki-1+) lymphoma [obs]',
     'anaplastic large cell lymphoma, cd30+',
     'anaplastic large cell lymphoma',
     'anaplastic large cell lymphoma (alcl) cd30+',
     'toe swelling',
     'swollen toe',
     'pain in cervical spine',
     'recurrent mouth ulcers',
     'recurrent oral ulceration',
     'ROu',
     'clavicle injury',
     'collar bone injury',
     'laceration of forehead',
     'localised retinal oedema',
     'cellulitis of leg, excluding foot',
     'cellulitis of leg, except foot',
     'objective tinnitus',
     'moderate bipolar i disorder, single manic episode',
     'lesion of cervix',
     'bloody nipple discharge',
     'sanguinous nipple discharge',
     'immunoglobulin a deficiency',
     'iga deficiency',
     'finger joint inflamed',
     'finger joint active',
     'amputated above knee',
     'amputated - transfemoral',
     'lesion of external ear',
     'ear problem',
     'secondary female infertility',
     'nephroblastoma',
     'wilms, tumour',
     'wilms, tumor',
     'embryonal nephroma',
     'embryonal adenosarcoma',
     'renal adenosarcoma',
     'nephroma',
     'perlman syndrome',
     'wilms tumor',
     'wilms tumour',
     'wilms tumor',
     'synovial sarcoma',
     'malignant synovioma',
     'synovioma, malignant',
     'synovioma',
     'keratoconjunctivitis sicca',
     'KCs',
     'atherosclerotic renal artery stenosis',
     'ARAs',
     'tendinitis and/or tenosynovitis of the ankle region',
     'tendonitis and/or tenosynovitis of the ankle region',
     'neurogenic claudication',
     'glossodynia',
     'painful tongue',
     'glossalgia',
     'soreness of tongue',
     'h/o: treatment for ischemic heart disease',
     'history of treatment for ischemic heart disease',
     'history of treatment for ischaemic heart disease',
     'repair of pulmonary valve with tissue graft',
     'biological pulmonary valve replacement',
     'bioprosthetic pulmonary valve replacement',
     'replacement of pulmonary valve with tissue graft',
     'tissue pulmonary valve replacement',
     'unsatisfactory living conditions',
     'unfavourable living conditions',
     'unfavorable living conditions',
     'abscess of back',
     'numbness of face',
     'profound mental retardation (i.q. below 20)',
     'profound mental handicap',
     'profound mental retardation (intelligence quotient below 20)',
     'substance abuse counselling',
     'counseling for substance abuse',
     'counselling for substance abuse',
     'inr raised',
     'international normalized ratio (inr) raised',
     'international normalised ratio (inr) raised',
     'international normalized ratio raised',
     'international normalised ratio raised',
     'dislocation of hip joint prosthesis',
     'divergence insufficiency',
     'disseminated idiopathic skeletal hyperostosis',
     'ankylosing vertebral hyperostosis',
     'diffuse idiopathic skeletal hyperostosis',
     'DISh',
     'forestiers disease',
     'lynch syndrome',
     'hnpcc - hereditary nonpolyposis colorectal cancer',
     'hereditary nonpolyposis colon cancer',
     'hnpcc - hereditary nonpolyposis colon cancer',
     'multiple personality disorder',
     'dissociative identity disorder',
     'fracture of scaphoid bone of wrist',
     'fracture of scaphoid bone',
     'fracture of navicular bone of wrist',
     'facial spasm',
     'discolouration of skin',
     'dyschromia',
     'fractures of multiple bones of lower limb',
     'compensatory emphysema',
     'idiopathic osteoporosis',
     'unilateral congenital dislocation of hip',
     'benign mucous membrane pemphigoid',
     'ocular pemphigoid',
     'scarring pemphigoid',
     'mucous membrane pemphigoid',
     'benign mucosal pemphigoid',
     'cicatricial pemphigoid',
     'mucosynechia atrophic bullous dermatitis',
     'benign mucous membrane pemphigoid with ocular involvement',
     'cicatricial pemphigoid with ocular involvement',
     'rabies vaccination',
     'rabies immunisation',
     'rabies immunization',
     'hydrophobia vaccination',
     'band-shaped keratopathy',
     'band keratopathy',
     'band shaped keratopathy',
     'frostbite of foot',
     'congenital syphilis',
     'umbilical discharge',
     'chronic arthritis',
     'neuroplasty and transposition of median nerve at carpal tunnel',
     'transposition of median nerve at carpal tunnel',
     'ischiorectal abscess',
     'r.i.n.d. syndrome',
     'RINd',
     'reversible ischaemic neurological defect',
     'reversible ischemic neurological defect',
     'reversible ischemic neurologic deficit (r.i.n.d.) syndrome',
     'reversible ischaemic neurologic deficit (r.i.n.d.) syndrome',
     'reversible ischaemic neurologic deficit syndrome',
     'reversible ischemic neurologic deficit syndrome',
     'malignant tumour of anterior two-thirds of tongue',
     'malignant tumour of appendix',
     'malignant tumor of vagina',
     'cancer of vagina',
     'malignant tumor of renal pelvis',
     'malignant tumor of urethra',
     'malignant urethral tumour',
     'malignant urethral tumor',
     'urethral ca',
     'cancer of urethra',
     'malignant tumor of penis',
     'cancer of penis',
     'penile ca',
     'ca penis',
     'myelogram',
     'myelography',
     'myelograms',
     'pituitary dwarfism',
     'lorain - levi dwarfism',
     'hypopituitary dwarfism',
     'pituitary nanism',
     'hyposomatotropic dwarfism',
     'prepubertal dwarfism',
     'isolated deficiency of growth hormone in children',
     'prepuberal dwarfism',
     'asexual dwarfism',
     'lorain-levi dwarfism',
     'hypophyseal infantilism',
     'frohlich dwarfism',
     'brissaud dwarfism',
     'brissaud-meige syndrome',
     'burnier syndrome',
     'hypopituitary dwarfism with failure of development of sexual characteristics',
     'nebecourt syndrome',
     'late effect of fracture of lower extremities',
     'superficial laceration of foot',
     'motion sickness',
     'travel sickness',
     'riders, vertigo',
     'cellulitis of external nose',
     'occupational injury',
     'infection due to mycobacterium avium-intracellulare group',
     'carcinoma of glottis',
     'dermabrasion',
     'dermabrasion of skin',
     'removal of superficial layer of skin by dermabrasion',
     'abrasion of skin',
     'sandpapering of skin',
     'planing of skin',
     'dermabrasion; superficial, any site (eg, tattoo removal)',
     'implantation of posterior chamber intraocular lens',
     'posterior chamber intraocular lens implantation',
     'infection due to mycobacterium intracellulare',
     'meckels diverticulum',
     'meckel diverticulum',
     'persistent omphalomesenteric duct',
     'persistent vitelline duct',
     'persistent intestinal end of vitelline duct',
     'extrinsic allergic alveolitis',
     'extrinsic allergic bronchiolo-alveolitis',
     'hypersensitivity pneumonia',
     'allergic alveolitis',
     'allergic pneumonitis',
     'allergic interstitial pneumonitis',
     'hypersensitivity pneumonitis',
     'EAa',
     'acute hepatitis',
     'contusion of abdominal wall',
     'superficial bruising of abdominal wall',
     'rumination disorder',
     'cardiac care',
     'cardiac care treatments and procedures',
     'hepatic vein thrombosis',
     'pigment dispersion syndrome',
     'closed fracture of four ribs',
     'redundant prepuce',
     'long foreskin',
     'redundant foreskin',
     'primary insomnia',
     'idiopathic insomnia',
     'pyosalpinx',
     'pyosalpingitis',
     'growth hormone deficiency',
     'GHd',
     'growth hormone insufficiency',
     'alternating esotropia',
     'thiamin deficiency',
     'aneurin deficiency',
     'thiamine deficiency',
     'vitamin b1 deficiency',
     'acquired plantar keratoderma',
     'delay in sexual development and/or puberty',
     'delayed sexual development',
     'delayed puberty',
     'creation of aortofemoral shunt',
     'creation of aortofemoral vascular bypass',
     'bypass graft, with other than vein; aortofemoral',
     'dermatitis of external ear',
     'venous lake',
     'acquired venous haemangioma',
     'acquired venous hemangioma',
     'recurrent genital herpes simplex',
     'recurrent herpes genitalis',
     'second degree burn',
     'second degree burn injury',
     'burn blister with epidermal loss',
     'basal cell carcinoma of scalp',
     'cancer of scalp, basal cell',
     'bleeding from urethra',
     'urethrorrhagia',
     'urethral bleeding',
     'hemorrhage of urethra',
     'squamous cell carcinoma of larynx',
     'cancer of the larynx, squamous cell',
     'primary squamous cell carcinoma of larynx',
     'obstruction of colon',
     'occlusion of colon',
     'complex regional pain syndrome, type ii, upper limb',
     'ostomy care education, guidance, and counselling',
     'ostomy care teaching, guidance and counselling',
     'ostomy care teaching, guidance and counseling',
     'adenoviral pneumonia',
     'acute follicular conjunctivitis',
     'conjunctival folliculosis',
     'anaemia due to blood loss',
     'neonatal candidiasis',
     'neonatal candida infection',
     'neonatal candidosis',
     'disorder of right cardiac ventricle',
     'cervicovaginal cytology: high grade squamous intraepithelial lesion or carcinoma',
     'cervicovaginal cytology: hgsil or carcinoma',
     'cervicovaginal cytology: hsil or carcinoma',
     'family history of renal stone',
     'family history of nephrolithiasis',
     'family history of kidney stone',
     'systolic heart failure',
     'hypertrophy of lingual tonsil',
     'enlarged lingual tonsil',
     'benign neoplasm of oral cavity',
     'benign tumor of oral cavity',
     'oral benign tumor',
     'benign tumour of oral cavity',
     'oral benign tumour',
     'malignant lymphoma of the eye region',
     'ischaemic stroke',
     'idiopathic hypereosinophilic syndrome',
     'hypereosinophilic syndrome',
     'idiopathic hypereosinophilic syndrome (hes)',
     'tendinitis of hand',
     'tendonitis of hand',
     'alcohol-induced psychosis',
     'alcohol induced psychosis',
     'alcoholic psychosis',
     'delayed hypersensitivity skin test for tuberculin ppd',
     'delayed hypersensitivity skin test for tuberculin purified protein derivative',
     'skin test; tuberculosis, intradermal',
     'closed fracture of base of fifth metatarsal bone',
     'foreign body of neck',
     'chemical burn',
     'central pain syndrome',
     'chronic hypercapnic respiratory failure',
     'chronic type ii respiratory failure',
     'chronic type 2 respiratory failure',
     'female infertility due to diminished ovarian reserve',
     'mass of shoulder region',
     'history of operative procedure on knee',
     'history of knee surgery',
     'ulcer of genital labium',
     'genital labial ulcer',
     'labial ulceration',
     'history of repair of coarctation of aorta',
     'history of coarctation of aorta repair',
     'history of repair of inguinal hernia',
     'history of inguinal hernia surgery',
     'history of placement of stent for coronary artery disease',
     'history of coronary artery disease with stent placement',
     'localized superficial swelling of skin',
     'history of thyroidectomy',
     'history of placement of stent in anterior descending branch of left coronary artery',
     'history of stent placement in anterior descending branch of left coronary artery',
     'history of medullary carcinoma of thyroid',
     'history of medullary thyroid carcinoma',
     'history of malignant neoplasm of endometrium',
     'history of excision of lamina of lumbar vertebra for decompression of spinal cord',
     'history of decompressive lumbar laminectomy',
     'disorder of tendon of biceps',
     'disorder of biceps tendon',
     'biceps tendon disorder',
     'accidental poisoning by aromatic analgesic',
     'cholestasis in newborn',
     'neonatal cholestasis',
     'crushing injury of foot',
     'crush injury of foot',
     'crush injury to foot',
     'cardiovascular stress test abnormal',
     'atypical squamous cells of undetermined significance on vaginal papanicolaou smear',
     'strain of muscle and/or tendon of wrist',
     'strain of wrist',
     'nonspecific tuberculin test reaction',
     'flaccid hemiplegia of nondominant side',
     'mass of foot',
     'procedure on wound',
     'neoplasm of lymphoid system structure',
     'splinter foreign body',
     'splinter',
     'splintering',
     'splinters',
     'autoantibody screening for coeliac disease positive',
     'celiac disease autoantibody profile positive',
     'coeliac disease autoantibody profile positive',
     'sleep hypoventilation',
     'sleep related hypoventilation',
     'multiple system atrophy, parkinsons variant',
     'multiple system atrophy, parkinson variant',
     'hypersomnia disorder related to a known organic factor',
     'mucosa-associated lymphoid tissue (malt) lymphoma of stomach',
     'extranodal marginal zone lymphoma of mucosa-associated lymphoid tissue of stomach',
     'dissecting aneurysm of thoracoabdominal aorta',
     'fussy toddler',
     'viral sinusitis',
     'dependence on ventilator',
     'oedema of face',
     'high carcinoembryonic antigen level',
     'carcinoembryonic antigen above reference range',
     'cervical laminectomy',
     'excision of lamina of cervical vertebra',
     'epstein-barr virus serostatus positive',
     'epstein-barr virus antibody in serum positive',
     'degenerative joint disease of pelvis',
     'osteoarthritis of pelvis',
     'hyperactive behavior',
     'hyperactivity',
     'hyperkinesia',
     'hyperkinesis',
     'increased purposeful goal-directed activity',
     'hyperkinesias',
     'hyperactive',
     'hyperactivity',
     'erysipelas',
     'patch of erysipelas',
     'laceration of nail bed of toe',
     'paralysis',
     'palsy',
     'paralyses',
     'palsies',
     'neurohypophyseal diabetes insipidus',
     'diabetes insipidus - pituitary',
     'vasopressin deficiency',
     'central diabetes insipidus',
     'diabetes insipidus secondary to vasopressin deficiency',
     'vasopressin deficiency syndrome',
     'pituitary diabetes insipidus',
     'primary central diabetes insipidus',
     'neurogenic diabetes insipidus',
     'cranial diabetes insipidus',
     'contusion of lower leg',
     'superficial bruising of lower leg',
     'contusion, lower leg',
     'superficial bruising of lower limb',
     'leg bruise',
     'superficial bruising of leg',
     'retrosternal pain',
     'retrosternal chest pain',
     'posterior cyclitis',
     'pars planitis',
     'anemia in mother complicating pregnancy, childbirth and/or puerperium',
     'microalbuminuria measurement',
     'urine microalbumin level',
     'urine microalbumin',
     'actinic cheilitis',
     'solar keratosis of lip',
     'tarsal tunnel syndrome',
     'TTs',
     'tarsal tunnel syndrome (ankle condition)',
     'abnormal deglutition',
     'abnormal swallowing',
     'chronic nasopharyngitis',
     'aneurysm of popliteal artery',
     'popliteal artery aneurysm',
     'respiratory crackles',
     'rales',
     'respiratory crepitations',
     'crackles',
     'crepitations',
     'crepitation',
     'keratosis follicularis',
     'darier disease',
     'psorospermosis follicularis vegetans',
     'psorospermosis',
     'hereditary follicular keratosis',
     'dariers disease',
     'dyskeratosis follicularis',
     'darier-white disease',
     'lymphocytopenia',
     'lymphopenia',
     'alymphocytosis',
     'disorder of external ear',
     'subtotal gastrectomy',
     'partial excision of stomach',
     'partial gastrectomy',
     'periorbital edema',
     'oedema around eye',
     'edema around eye',
     'pure red cell aplasia',
     'red cell hypoplasia',
     'pure red cell anaemia',
     'primary red cell aplasia',
     'pure red cell anemia',
     'corneal erosion',
     'superficial ulcer of cornea',
     'crushing injury of hand',
     'crush injury of hand',
     'crush injury to hand',
     'primary female infertility',
     'chorioretinal scar',
     'chronic post-traumatic headache',
     'post-traumatic headache',
     'posttraumatic headache',
     'closed fracture of base of metacarpal bone other than first metacarpal',
     'arterial embolism',
     'embolic arterial occlusion',
     'arterial embolus',
     'contusion of ankle',
     'contusion, ankle',
     'superficial bruising of ankle',
     'organic delusional disorder',
     'paranoid organic state',
     'photopsia',
     'flashes',
     'light flashes',
     'craniosynostosis syndrome',
     'CSo',
     'craniosynostosis',
     'premature cranial suture closure',
     'craniostosis',
     'premature closure of cranial sutures',
     'congenital ossification of cranial sutures',
     'congenital ossification of sutures of skull',
     'craniostenosis',
     'alternating exotropia with v pattern',
     'disorder of spleen',
     'splenic disease',
     'spleen disease',
     'splenic disorder',
     'hypersplenism',
     'dyssplenism',
     'big spleen syndrome',
     'hypersplenia',
     'aspiration of food',
     'disorder of conjunctiva',
     'disease of conjunctiva',
     'cervical arthrodesis by anterior technique',
     'cervical spinal fusion by anterior technique',
     'paralytic lagophthalmos',
     'unable to concentrate',
     'lack of concentration',
     'concentration impairment',
     'bowing deformity of lower limb',
     'pre-existing type 1 diabetes mellitus in pregnancy',
     'thyroid stimulating hormone measurement',
     'tsh measurement',
     'thyrotropin measurement',
     'thyrotropin stimulating hormone measurement',
     'thyroid stimulating hormone level',
     'tsh level',
     'thyroid stimulating hormone (tsh)',
     'aggressive behaviour',
     'aggression',
     'aggressive',
     'simple chronic bronchitis',
     'catarrhal bronchitis',
     'chronic catarrhal bronchitis',
     'ill-defined disorder of eye',
     'paranoid schizophrenia in remission',
     'paranoid schizophrenia, in remission',
     'bundle branch block',
     'BBb',
     'fascicular block',
     'congenital nystagmus',
     'primary degenerative dementia of the alzheimer type, presenile onset, uncomplicated',
     'dementia of the alzheimers type, with early onset, uncomplicated',
     'rectovaginal fistula',
     'RVf',
     'subluxation of lens',
     'partial dislocation of lens',
     'hiccoughs',
     'singultus',
     'hiccough',
     'hiccups',
     'finding of hiccoughs',
     'observation of hiccoughs',
     'hiccoughing',
     'closed fracture of femoral condyle of femur',
     'laparoscopic cholecystectomy with exploration of common duct',
     'acute-on-chronic respiratory failure',
     'acute on chronic respiratory failure',
     'tracheostomy complication',
     'disorder of tracheostomy',
     'closed fracture of multiple sites of phalanges of hand',
     'chronic gouty arthritis',
     'urethral caruncle',
     'fracture of shaft of tibia',
     'monoarthritis',
     'mastitis associated with lactation',
     'postpartum mastitis',
     'mastitis associated with breastfeeding',
     'puerperal mastitis',
     'internal carotid artery dissection',
     'occult spinal dysraphism sequence',
     'tethered cord malformation sequence',
     'tethered cord syndrome',
     'congenital tethering of spinal cord',
     'precordial pain',
     'disorder of refraction and/or accommodation',
     'disorder of refraction and accommodation',
     'relapsing polychondritis',
     'meyenburg-altherr-uehlinger syndrome',
     'meyenburgs disease',
     'chronic polychondritis',
     'systemic chondromalacia',
     'bullous dermatosis',
     'bullous dermatitis',
     'bullous dermatoses',
     'neuralgic amyotrophy',
     'parsonage-turner syndrome',
     'parsonage-aldren-turner syndrome',
     'shoulder girdle syndrome',
     'brachial neuritis',
     'recurrent pterygium',
     'benign secondary renovascular hypertension',
     'secondary benign renovascular hypertension',
     'enteritis due to radiation',
     'radiation enteritis',
     'radiation induced enteritis',
     'mucopurulent chronic bronchitis',
     'derangement of temporomandibular joint',
     'temporomandibular joint (tmj) derangement',
     'phenylketonuria',
     'classical phenylketonuria',
     'follings syndrome',
     'phenylalanine hydroxylase deficiency',
     'pah deficiency',
     'imbecilitus phenylpyruvica',
     'PKu',
     'hyperphenylalaninemia, type i',
     'oligophrenia phenylpyruvica',
     'phenylpyruvic oligophrenia',
     'severe phenylalanine hydroxylase deficiency',
     'pku1 - phenylketonuria',
     'PAh',
     'hyperphenylalaninaemia, type i',
     'infection due to clostridium tetani',
     'tetanus',
     'lock jaw',
     'tetnus',
     'lockjaw',
     'amputation of finger, except thumb',
     'amputation of finger',
     'adhesive middle ear disease',
     'chronic adhesive otitis media',
     'fibrotic adhesive otitis media',
     'adhesive otitis media',
     'spina bifida of lumbar region',
     'lumbar spina bifida',
     'thrombotic thrombocytopenic purpura',
     'moschowitzs syndrome',
     'TTp',
     'moschcowitz syndrome',
     'erb-duchenne paralysis',
     'erbs paralysis',
     'erb-duchenne brachial plexus injury',
     'erbs palsy',
     'adhesive capsulitis of shoulder',
     'duplays periarthritis syndrome',
     'pericapsulitis of shoulder',
     'frozen shoulder',
     'periarthritis of shoulder',
     'subdeltoid bursitis',
     'congenital genu varum',
     'congenital varus deformity of knee',
     'congenital bow leg',
     'congenital bowleg',
     'congenital bowed limb',
     'enophthalmos',
     'enophthalmia',
     'sleep-wake schedule disorder, delayed phase type',
     'delayed sleep phase syndrome',
     'circadian rhythm sleep disorder, delayed sleep phase type',
     'circadian rhythm sleep disorder, delayed sleep phase',
     'allergic granulomatosis angiitis',
     'allergic granulomatous angiitis',
     'CSs',
     'churg-strauss syndrome',
     'transient visual loss',
     'transient blindness',
     'atrophic gastritis',
     'chronic atrophic gastritis',
     'gastric atrophy',
     'Ag',
     'autoimmune gastritis',
     'schizoaffective disorder, depressive type',
     'schizophreniform psychosis, depressive type',
     'epiphora due to insufficient drainage',
     'testosterone measurement, total',
     'testosterone; total',
     'primary ulcer of intestine',
     'ulceration of intestine',
     'centesis',
     'paracentesis',
     'puncture and aspiration',
     'puncture and drainage',
     'needling and drainage',
     'tap',
     'paracenteses',
     'tapped',
     'tapping',
     'taps',
     'hypertensive heart and renal disease',
     'cardiorenal disease',
     'traumatic urethral stricture',
     'post-traumatic urethral stricture',
     'total elbow replacement',
     'arthroplasty of elbow with synthetic joint prosthesis',
     'prosthetic total arthroplasty of the elbow',
     'TEr',
     'cholesteatoma of middle ear',
     'epidermosis of middle ear',
     'haemorrhagic cystitis',
     'acquired deformity of ankle and/or foot',
     'acquired ankle and/or foot deformity',
     'chronic congestive heart failure',
     'oedema of eyelid',
     'hydroblepharon',
     'vesicovaginal fistula',
     'VVf',
     'hyperplastic polyp of intestine',
     'metaplastic polyp of intestine',
     'somatoform pain disorder',
     'psychalgia',
     'psychogenic pain',
     'algopsychalia',
     'haematoma of scrotum',
     'scrotal hematoma',
     'scrotal haematoma',
     'traumatic pneumothorax',
     'benign neoplasm of skin of lip',
     'benign neoplasm of soft tissues of lower limb',
     'benign tumour of soft tissue of leg',
     'benign tumour of soft tissue of lower limb',
     'benign tumor of soft tissue of leg',
     'benign tumor of soft tissue of lower limb',
     'lipoma of spermatic cord',
     'malignant lymphoma of lymph nodes of head, face and/or neck',
     'primary malignant neoplasm of prostate',
     'secondary malignant neoplasm of central nervous system',
     'metastatic malignant neoplasm to central nervous system',
     'cancer metastatic to cns (central nervous system)',
     'secondary malignant neoplasm of intrathoracic lymph nodes',
     'metastatic malignant neoplasm to intrathoracic lymph nodes',
     'cancer metastatic to intrathoracic lymph nodes',
     'secondary malignant neoplasm of mediastinum',
     'secondary malignant tumour of mediastinum',
     'secondary malignant tumor of mediastinum',
     'metastasis to mediastinum',
     'mediastinal metastasis',
     'metastatic malignant neoplasm to mediastinum',
     'cancer metastatic to mediastinum',
     'secondary malignant neoplasm of retroperitoneum',
     'metastasis to retroperitoneum',
     'retroperitoneal metastasis',
     'secondary malignant neoplasm of the retroperitoneum',
     'metastatic malignant neoplasm to the retroperitoneum',
     'muscle spasms of head and/or neck',
     'pulmonary artery stenosis',
     'asteroid hyalosis',
     'bensons disease',
     'asteroid hyalitis',
     'congenital hearing disorder',
     'fracture of scapula',
     'fracture of shoulder blade',
     'acute alcoholic liver disease',
     'acute alcoholic hepatitis',
     'urethral discharge',
     'observation of urethral discharge',
     'Ud',
     'urethrorrhea',
     'urethrorrhoea',
     'tardive dyskinesia',
     'drug-induced tardive dyskinesia',
     'Td',
     'kaposis sarcoma (clinical)',
     'epidemic kaposis sarcoma',
     'Ks',
     'kaposis sarcoma',
     'kaposi sarcoma',
     'kaposis sarcoma associated with aids',
     'kaposi sarcoma associated with aids',
     'kaposis sarcoma associated with acquired immunodeficiency syndrome',
     'kaposis sarcoma, morphology',
     'kaposi sarcoma, morphology',
     'multiple haemorrhagic sarcoma',
     'multiple hemorrhagic sarcoma',
     'hepatoblastoma (clinical)',
     'hepatoblastoma',
     'hepatoblastoma of liver',
     'HBl',
     'embryonal hepatoma',
     'exostosis of jaw',
     'organic mental disorder of unknown aetiology',
     'organic mental disorder',
     'localised pain',
     'fuchs, heterochromic cyclitis',
     'fuchs, heterochromic uveitis',
     'fuchs, heterochromic iridocyclitis',
     'fuchs uveitis syndrome',
     'necrotising vasculitis',
     'mycosis fungoides (clinical)',
     'mycosis fungoides',
     'pagetoid reticulosis',
     'disorder of neck',
     'reconstruction procedure',
     'reconstruction',
     'reconstruction - action',
     'reconstructions',
     'crowding of teeth',
     'imbrication of teeth',
     'cholangiectasis',
     'dilation of biliary tract',
     'injury of forearm',
     'forearm injury',
     'crushing injury',
     'crushing injury - disorder',
     'crushing injury (morphology)',
     'compression injury',
     'crush injury',
     'pressure injury',
     'neoplasm of stomach',
     'tumour of stomach',
     'tumor of stomach',
     'neoplasm of thyroid gland',
     'thyroid tumour',
     'tumour of thyroid gland',
     'thyroid tumor',
     'tumor of thyroid gland',
     'eye infection',
     'periocular infection',
     'infectious disorder of kidney',
     'renal infection',
     'kidney infection',
     'polypharmacy',
     'old disruption of ligament of knee',
     'bladder pain',
     'vesical pain',
     'urinary bladder pain',
     'fh: gastrointestinal disease',
     'family history of gastrointestinal disease',
     'family history: gastrointestinal disease',
     'fh: congenital anomaly',
     'family history: congenital anomaly',
     'light cigarette smoker (1-9 cigs/day)',
     'school problem',
     'school difficulties',
     'h/o: bells palsy',
     'history of bells palsy',
     'history of bell palsy',
     'h/o: perinatal fetal loss',
     'h/o: perinatal death',
     'history of perinatal fetal loss',
     'h/o: perinatal foetal loss',
     'history of - perinatal foetal loss',
     'chronic disease of tonsils and/or adenoids',
     'mammography normal',
     'insertion of intra-aortic balloon counterpulsation',
     'insertion of intra-aortic balloon pump',
     'insertion of pulsation-type balloon into aorta',
     'skin disease screening',
     'abnormal urinalysis',
     'urinalysis = abnormal',
     'urine screening abnormal',
     'laser trabeculoplasty',
     'ltp - laser trabeculoplasty',
     'yag trabeculopuncture',
     'LTp',
     'open ablation of atrioventricular node',
     'accident while engaged in work-related activity',
     'work accident',
     'accident at work',
     'abnormal defecation',
     'gender identity disorder of adulthood',
     'acquired deformity of nose',
     'acquired nasal deformity',
     'acquired anomaly of nose',
     'disorder of appendix',
     'ethmoidal sinusitis',
     'malignant tumour of cardia',
     'ca cardia - stomach',
     'malignant neoplasm of cardia of stomach',
     'malignant tumour of pancreatic duct',
     'malignant neoplasm of pancreatic duct',
     'malignant melanoma of ear and/or external auditory canal',
     'malignant melanoma of ear and/or external auricular canal',
     'malignant melanoma of foot',
     'malignant melanoma of skin of foot',
     'malignant tumor of vault of bladder',
     'malignant tumour of bladder dome',
     'malignant neoplasm of vault of bladder',
     'malignant neoplasm of dome of urinary bladder',
     'malignant tumor of bladder dome',
     'malignant neoplasm of head, neck and face',
     'corticobasal degeneration',
     'cortical basal ganglionic degeneration',
     'corticodentatonigral degeneration with neuronal achromasia',
     'type 1 diabetes mellitus with ulcer',
     'type i diabetes mellitus with ulcer',
     'insulin-dependent diabetes mellitus with ulcer',
     'type 2 diabetes mellitus with gangrene',
     'type ii diabetes mellitus with gangrene',
     'gangrene associated with type 2 diabetes mellitus',
     'gangrene associated with type ii diabetes mellitus',
     'diabetes type 2 with gangrene',
     'chronic gouty nephropathy',
     'gouty nephropathy',
     'chronic urate nephropathy',
     'psychogenic hyperventilation',
     'HVs',
     'hyperventilation syndrome',
     'psychogenic overbreathing',
     'lesion of radial nerve',
     'radial nerve lesion',
     'swelling of eyelid',
     'gangrene of foot',
     'recurrent acute tonsillitis',
     'RAt',
     'pansinusitis',
     'chronic bullous emphysema',
     'emphysematous bulla',
     'bullous emphysema',
     'superior mesenteric vein thrombosis',
     'deep venous thrombosis of the superior mesenteric vein',
     'constipation - functional',
     'functional constipation',
     'constipation-functional',
     'rectal abscess',
     'rectal boil',
     'anal and rectal polyp',
     'biliary dyskinesia',
     'intestinal malabsorption of fat',
     'chronic focal glomerulonephritis',
     'threatened premature labour - not delivered',
     'false premature labor',
     'false premature labour',
     'post-term pregnancy - not delivered',
     'infantile seborrheic dermatitis',
     'complement 5 dysfunction',
     'generalised seborrhoeic dermatitis of infants',
     'leiners disease',
     'erythroderma desquamativum',
     'ocular rosacea',
     'café au lait spots',
     'café au lait spot',
     'cafe au lait spots',
     'cafe-au-lait spots',
     'systemic onset juvenile chronic arthritis',
     'systemic onset juvenile rheumatoid arthritis',
     'systemic juvenile idiopathic arthritis',
     'juvenile onset stills disease',
     'juvenile arthritis with systemic onset',
     'old medial collateral ligament disruption',
     'old disruption of medial collateral ligament',
     'recurrent subluxation of the patella',
     'lumbar disc prolapse with radiculopathy',
     'lumbar disc prolapse with root compression',
     'contracture of tendo achilles',
     'achilles tendon contracture',
     'contracture of achilles tendon',
     'spasm of back muscles',
     'congenital pigmentary skin anomalies',
     'congenital pigmentary anomaly of skin',
     'open reduction with internal fixation',
     'open reduction of fracture with internal fixation',
     'open reduction and internal fixation of fracture',
     'ORIf',
     'tricuspid valve disorder',
     'tricuspid valve disease',
     'fracture of orbital floor',
     'orbital floor fracture',
     'closed fracture finger proximal phalanx, multiple',
     'infestation by pediculus',
     'pediculosis',
     'louse infestation',
     'infestation by anoplura',
     'anoplurosis',
     'infestation by sucking lice',
     'lice infestation',
     'chronic glomerulonephritis',
     'CGn',
     'sexual abuse',
     'Sa',
     'retractile testis',
     'testicular retraction',
     'retractible testis',
     'fine motor impairment',
     'malignant meningitis',
     'neoplastic meningitis',
     'carcinomatous meningitis',
     'meningeal carcinomatosis',
     'lymphomatous meningitis',
     'spastic cerebral palsy',
     'pyramidal cerebral palsy',
     'congenital spastic paralysis',
     'spastic infantile paralysis',
     'littles disease',
     'infantile spastic cerebral palsy',
     'congenital spastic cerebral palsy',
     'unprotected sexual intercourse',
     'unprotected intercourse',
     'unprotected coitus',
     'seborrhoeic blepharitis',
     'blepharitis in seborrhoeic eczema',
     'blepharitis in seborrheic eczema',
     'giant papillary conjunctivitis',
     'GPc',
     'congenital hypertrophy of retinal pigment epithelium',
     'CHRPe',
     'congenital hyperplasia of retinal pigment epithelium',
     'high frequency deafness',
     'rupture of eye with partial loss of intraocular tissue',
     'reprogramming of cardiac pacemaker',
     'pulmonary emphysema in alpha-1 pi deficiency',
     'pulmonary emphysema in alpha-1 primary immunodeficiency deficiency',
     'cardiac syndrome x',
     'chest pain with normal coronary angiography',
     'disorder of implantable defibrillator',
     'icd - disorder of implantable cardiac defibrillator',
     'disorder of implantable cardiovertor',
     'erosion of oral mucosa',
     'dilatation of bile duct',
     'dilatation of bile duct stricture',
     'colonic diverticular abscess',
     'diverticular abscess',
     'obstruction of biliary tree',
     'retraction of stoma',
     'recession of stoma',
     'chronic pericarditis',
     'post-micturition incontinence',
     'post-micturition dribbling',
     'nasal vestibulitis',
     'teenage pregnancy',
     'familial lipoprotein lipase deficiency',
     'fredrickson type i hyperlipoproteinemia',
     'fredrickson type i hyperlipoproteinaemia',
     'familial hyperlipoproteinemia, type i',
     'familial hyperlipoproteinaemia, type i',
     'familial hyperchylomicronaemia',
     'hyperchylomicronemia',
     'primary hyperchylomicronemia',
     'primary hyperchylomicronaemia',
     'burger-grutz syndrome',
     'hyperchylomicronaemia',
     'hyperlipoproteinaemia, type i',
     'hepatosplenomegalic lipoidosis',
     'endogenous hypertriglyceridaemia',
     'familial fat-induced hypertriglyceridaemia',
     'hypercholesterinaemic xanthomatosis',
     'endogenous hypertriglyceridemia',
     'hypercholesterinemic xanthomatosis',
     'familial fat-induced hypertriglyceridemia',
     'familial lipoprotein lipase deficiency with type i phenotype',
     'familial type i hyperlipoproteinaemia',
     'familial type i hyperlipoproteinemia',
     'psoriasiform eczema',
     'psoriasiform dermatitis',
     'vulvodynia',
     'rupture of medial collateral ligament of knee',
     'MCl',
     'medial collateral ligament rupture of the knee',
     'tear of medial collateral ligament of knee joint',
     'contracture of wrist joint',
     'wrist joint contracture',
     'persistent prosthetic joint pain',
     'attacks of weakness',
     'lump on finger',
     'mass of finger',
     'severe adrenal insufficiency',
     'adrenal crisis',
     'addisonian crisis',
     'acute adrenal insufficiency',
     'adrenocortical crisis',
     'choking',
     'knee stiff',
     'hip stiff',
     'resting tremor',
     'benign phaeochromocytoma',
     'atrioventricular septal defect: atrial and ventricular components',
     'complete atrioventricular septal defect',
     'peripheral neuralgia',
     'angiosarcoma of skin',
     'thyroid adenoma',
     'thyroid follicular adenoma',
     'squamous cell carcinoma of lip',
     'laceration of liver',
     'hepatic laceration',
     'sprain of toe joint',
     'sprained toe',
     'toe sprain',
     'fracture of shaft of radius and/or ulna',
     'fracture of shaft of radius and ulna',
     'slipped upper femoral epiphysis',
     'non-traumatic slipped upper femoral epiphysis',
     'SUFe',
     'nontraumatic slipped upper femoral epiphysis',
     'slipped femoral epiphysis',
     'ventricular hypertrophy',
     'dilatation of aorta',
     'h/o: gastrointestinal disease',
     'history of gastrointestinal disease',
     'h/o: intestinal disease',
     'h/o: abortion',
     'h/o: termination',
     'history of fetal loss',
     'history of abortion',
     'history of foetal loss',
     'excessive eating - polyphagia',
     'hyperphagia',
     'gluttony',
     'polyphagia',
     'excessive eating',
     'overeating',
     'overeats',
     'thoracic spondylosis without myelopathy',
     'dorsal spondylosis without myelopathy',
     'congenital torticollis',
     'contracture of sternocleidomastoid muscle',
     'congenital sternomastoid torticollis',
     'congenital wry neck',
     'congenital wryneck',
     'hepatitis b immune',
     'chorea',
     'choreiform movement',
     'choreic movement',
     'subungual hematoma, foot',
     'electroencephalogram abnormal',
     'eeg abnormal',
     'orchitis',
     'inflammation of testis',
     'non-specific orchitis',
     'injury of nail',
     'h/o: heart disorder',
     'history of heart disorder',
     'h/o: intestinal by-pass',
     'history of intestinal by-pass',
     'squamous cell carcinoma of palate',
     'berry aneurysm',
     'berry aneurysm - disorder',
     'follicular non-hodgkins lymphoma, large cell (clinical)',
     'follicular non-hodgkins lymphoma, large cell',
     'follicular non-hodgkin lymphoma, large cell',
     'follicular lymphoma grade 3',
     'follicular malignant lymphoma - large cell',
     'malignant lymphoma - histiocytic, nodular',
     'malignant lymphoma - non-cleaved, follicular',
     'follicular non-hodgkins large cell lymphoma',
     'malignant lymphoma, centroblastic type, follicular',
     'germinoblastic sarcoma, follicular',
     'follicular lymphoma, grade 3',
     'malignant lymphoma, lymphocytic, well differentiated, nodular [obs]',
     'malignant lymphoma, centroblastic, follicular',
     'malignant lymphoma, large cell, follicular',
     'malignant lymphoma, large cell, noncleaved, follicular',
     'malignant lymphoma, histiocytic, nodular',
     'malignant lymphoma, large cleaved cell, follicular',
     'malignant lymphoma, noncleaved, follicular',
     'scalp itchy',
     'pruritus of scalp',
     'scalp pruritus',
     'pertussis',
     'infection due to bordetella pertussis',
     'whooping cough',
     'Wc',
     'afibrinogenemia',
     'fibrinogen deficiency',
     'arteriovenous graft thrombosis',
     'noninflammatory disorder of vulva',
     'vesicocolic fistula',
     'colovesical fistula',
     'CVf',
     'interstitial nephritis',
     'In',
     'tubulointerstitial nephropathy',
     't.i.n.',
     'renal tubulo-interstitial disease',
     'synovitis/tenosynovitis - knee',
     'anterior spinal artery occlusion syndrome',
     'occlusion of anterior spinal artery',
     'becks syndrome',
     'ventral medullary syndrome',
     'foot joint inflamed',
     'foot joint active',
     'range of joint movement increased',
     'hypermobility of joint',
     'joint laxity',
     'pain of sternum',
     'elbow joint unstable',
     'elbow joint instability',
     'deformity of hip joint',
     'lesion of testis',
     'pain in axilla',
     'axillary pain',
     'controlled atrial fibrillation',
     'nasal sinus problem',
     'lesion of vocal cord',
     'anal inflammation',
     'inflammation of anus',
     'congenital deformity of foot',
     'infective discitis',
     'enthesopathy of wrist and/or hand',
     'enthesopathy of wrist and hand',
     'decreased range of knee movement',
     'degenerative drusen',
     'old healed fracture of bone',
     'intention tremor',
     'cerebellar tremor',
     'volitional tremor',
     'action tremor',
     'kinetic tremor',
     'behcets syndrome',
     'adamantiades-behcet disease',
     'behcet syndrome',
     'behcets disease',
     'family counselling',
     'percutaneous insertion of nephrostomy tube',
     'percutaneous nephrostomy',
     'laparoscopic nissen fundoplication using abdominal approach',
     'metastasis from malignant tumor of bone',
     'polyp of nasal sinus',
     'nasal sinus polyp',
     'polyp of accessory sinus',
     'microscopic cystic corneal dystrophy',
     'microcystoid epithelial dystrophy',
     'cogan microcystic dystrophy',
     'MDf',
     'map-dot-fingerprint corneal dystrophy',
     'ebmd - corneal epithelial basement membrane dystrophy',
     'map dot fingerprint corneal dystrophy',
     'corneal epithelial basement membrane dystrophy',
     'cogans corneal dystrophy',
     'cogan-guerry syndrome',
     'occipital headache',
     'pancreatectomy',
     'pancreas excision',
     'excision of pancreas',
     'excision procedures on the pancreas',
     'teleradiotherapy procedure',
     'external beam radiotherapy',
     'teletherapy',
     'teletherapy procedure',
     'foreign body in vagina',
     'mixed bipolar i disorder in remission',
     'bipolar i disorder, most recent episode mixed, in remission',
     'angioplasty of vein',
     'venous angioplasty',
     'aneurysm of renal artery',
     'malignant tumour of floor of mouth',
     'fom - malignant tumour of floor of mouth',
     'fom - malignant tumor of floor of mouth',
     'fom - cancer of floor of mouth',
     'cancer of floor of mouth',
     'malignant neoplasm of floor of mouth',
     'malignant tumor of spinal cord',
     'malignant tumour of mediastinum',
     'malignant neoplasm of mediastinum',
     'drainage of external ear',
     'drainage of pinna',
     'guttate psoriasis',
     'psoriasis guttata',
     'head tilt',
     'adverse reaction to food',
     'disorder of endocrine ovary',
     'ovarian dysfunction',
     'cholestasis of parenteral nutrition',
     'release for de quervains tenosynovitis of hand',
     'release for de quervain tenosynovitis of hand',
     'laser iridotomy',
     'laser peripheral iridotomy',
     'LPi',
     'congenital pelviureteric junction obstruction',
     'avoidant personality disorder',
     'osteoarthrosis of the carpometacarpal joint of the thumb',
     'osteoarthritis basilar joint of thumb',
     'osteoarthritis of first carpometacarpal joint',
     'plastic repair of tricuspid valve',
     'repair of tricuspid valve',
     'TVr',
     'tricuspid valve repair',
     'TVp',
     'tricuspid valve plasty',
     'tricuspid valvoplasty',
     'tricuspid valvuloplasty',
     'valvuloplasty, tricuspid valve',
     'h/o: bilateral oophorectomy',
     'history of bilateral oophorectomy',
     'foley catheter long term use',
     'insertion of foley catheter for long term use',
     'abnormal renal function',
     'pityriasis rubra (hebra)',
     'generalized erythroderma',
     'generalized exfoliative dermatitis',
     'pityriasis rubra of hebra',
     'erythroderma',
     'exfoliative dermatitis',
     'erythrodermatitis',
     'erythrodermic eczema',
     'plastic repair of tendon',
     'repair of tendon',
     'tenoplasty',
     'tenomyoplasty',
     'plastic operation on tendon',
     'contusion of forearm',
     'contusion, forearm area',
     'superficial bruising of forearm',
     'headache following lumbar puncture',
     'lumbar puncture headache',
     'postspinal headache',
     'post-lumbar puncture headache',
     'spinal headache',
     'glossopyrosis',
     'burning tongue',
     'male sterilization',
     'pseudofolliculitis barbae',
     'congenital malformation syndrome',
     'multiple congenital malformations',
     'multiple congenital anomalies',
     'basal cell carcinoma of lower extremity',
     'cancer of skin of lower limb, basal cell',
     'germ cell neoplasm',
     'germinoma',
     'germ cell tumour',
     'germ cell tumor',
     'condylomata lata of perianal skin',
     'squamous cell carcinoma of skin of trunk',
     'digital mucous cyst',
     'digital synovial cyst',
     'digital myxoid cyst',
     'cholesterol retinal embolus',
     'hollenhorst plaque',
     'widespread metastatic malignant neoplastic disease',
     'generalised cancer',
     'generalised malignancy',
     'generalized malignancy',
     'generalized cancer',
     'disseminated malignancy',
     'disseminated cancer',
     'malignant neoplasm, disseminated',
     'ca - disseminated cancer',
     'alteration in parenting',
     'parenting alteration',
     'skew deviation',
     'complex regional pain syndrome, type ii, lower limb',
     'partial thromboplastin time increased',
     'viral labyrinthitis',
     'viral otitis interna',
     'mild pre-eclampsia',
     'pet - mild pre-eclamptic toxaemia',
     'pet - mild pre-eclamptic toxemia',
     'mild proteinuric hypertension of pregnancy',
     'mild pre-eclamptic toxaemia',
     'mild pre-eclamptic toxemia',
     'chronic lung disease',
     'cervicovaginal cytology normal or benign',
     'salmon patch nevus',
     'unnas naevus',
     'stork bite',
     'unnas nevus',
     'port-wine stain of skin',
     'PWs',
     'port-wine stain',
     'port-wine birthmark',
     'nevus flammeus',
     'port-wine nevus',
     'portwine nevus',
     'port-wine naevus',
     'portwine naevus',
     'naevus flammeus',
     'port wine stain of skin',
     'acquired obstruction of pylorus',
     'acquired pyloric obstruction',
     'papillary conjunctivitis',
     'narrow angle',
     'angle narrow',
     'narrow angle anterior chamber',
     'unresponsive',
     'sarcoma of head and neck',
     'carcinoma of uterine cervix, invasive',
     'cancer of uterine cervix, invasive',
     'invasive cervical cancer',
     'herpes zoster keratoconjunctivitis',
     'herpes zoster with keratoconjunctivitis',
     'generalised dystonia',
     'fistula of hard palate',
     'paroxysmal atrial flutter',
     'incompetence of nasal valve',
     'nasal valve incompetence',
     'chronic ulcer of ankle',
     'history of tympanostomy',
     'anemia due to medication',
     'drug induced anaemia',
     'drug induced anemia',
     'history of mastectomy',
     'thrombophlebitis of iliac vein',
     'history of decompression of median nerve',
     'history of carpal tunnel decompression',
     'history of radical hysterectomy',
     'history of malignant neoplasm of ureter',
     'family history of hereditary disease',
     'family history of genetic disease',
     'family history of heritable disorder',
     'acute lower urinary tract infection',
     'lower urinary tract infection of sudden onset and/or short duration',
     'posterior uveitis',
     'acute epididymitis',
     'history of tonsillectomy',
     'vascular ectasia of gastric antrum',
     'GAVe',
     'gastric antral vascular ectasia',
     'watermelon stomach',
     'diabetic cataract',
     'arthritis of elbow',
     'premature ejaculation',
     'premature orgasm - male',
     'ejaculates too soon',
     'ejaculatio praecox',
     'thallium stress test abnormal',
     'refractory generalized nonconvulsive epilepsy',
     'stress fracture of tibia',
     'granulomatosis',
     'sarcoma, metastatic',
     'metastatic sarcoma',
     'hypertensive urgency',
     'cellulitis of buttock',
     'cellulitis of gluteal region',
     'anovulatory amenorrhoea',
     'cytomegalovirus serostatus negative',
     'cytomegalovirus antibody in serum negative',
     'excessive selfcriticism',
     'acute exacerbation of bronchiectasis',
     'sensory ataxia',
     'microcytic hypochromic anemia',
     'hypochromic microcytic anaemia',
     'hypochromic microcytic anemia',
     'common ventricle',
     'single ventricle',
     'cor triloculare biatriatum',
     'absence of interventricular septum',
     'nondiabetic proliferative retinopathy',
     'cellulitis of trunk',
     'congenital hydrocephalus',
     'hydrocephalus in newborn',
     'congenital hydrocephaly',
     'primary hydrocephaly',
     'primary hydrocephalus',
     'congenital hydrencephalus',
     'congenital hydrencephaly',
     'congenital dilatation of cerebral ventricles',
     'diastolic hypertension',
     'disorder of diaphragm',
     'diaphragmatic disease',
     'diaphragmatic disorder',
     'radiation-induced dermatosis',
     'radiation dermatitis',
     'radiodermatitis',
     'total pneumonectomy',
     'pneumonectomy',
     'mastoidectomy',
     'excision of mastoid',
     'mastoidectomies',
     'after-cataract not obscuring vision',
     'after cataract not obscuring vision',
     'skull congenital deformities',
     'congenital anomaly of skull',
     'congenital skull malformation',
     'dystrophy of vulva',
     'rubeosis iridis',
     'iris neovascularisation',
     'iris neovascularization',
     'new vessels in iris',
     'NVi',
     'new vessels iris',
     'rubeotic iris',
     'acquired genu valgum',
     'acquired deformity of the knee',
     'acquired valgus deformity of knee',
     'knock-knees - acquired',
     'acquired knock-knee',
     'acquired deformity of knee',
     'stiff-man syndrome',
     'moersch-woltman syndrome',
     'stiff man syndrome',
     'moersch-woltmann syndrome',
     'gamma neuron overactivity syndrome',
     'chronic ulcerative rectosigmoiditis',
     'ulcerative rectosigmoiditis',
     'ulcerative colitis confined to rectum and sigmoid colon',
     'ulcerative proctosigmoiditis',
     'chronic ulcerative proctosigmoiditis',
     'excessive sweating',
     'sweating profusely',
     'profuse sweating',
     'articular cartilage disorder',
     'disorder of articular cartilage',
     'articular cartilage disease',
     'injury to brachial plexus as birth trauma',
     'laryngeal spasm',
     'laryngospasm',
     'laryngismus stridulus',
     'laryngeal stridor',
     'laryngismus',
     'second degree burn of hand',
     'partial thickness burn of hand',
     'replacement of tricuspid valve with tissue graft',
     'biological tricuspid valve replacement',
     'tissue tricuspid valve replacement',
     'bioprosthetic tricuspid valve replacement',
     'hebras prurigo',
     'hebra prurigo',
     'prurigo simplex',
     'urticaria papulosa of hebra',
     'lichen urticatus',
     'papular urticaria',
     'strophulus',
     'unilateral agenesis of kidney',
     'unilateral congenital absence of kidney',
     'congenital absence of one kidney',
     'disorder of pericardium',
     'pericardial disorder',
     'cubital tunnel syndrome',
     'stenosis of intestine',
     'enterostenosis',
     'stricture of intestine',
     'localized amyloidosis',
     'victim of sexual aggression',
     'sexual assault victim',
     'traumatic dislocation of knee joint',
     'huntingtons chorea',
     'Hc',
     'huntington chorea',
     'chronic progressive chorea',
     'chronic progressive hereditary chorea',
     'pseudohypoparathyroidism',
     'constitutional chronic hypocalcaemia',
     'constitutional chronic hypocalcemia',
     'familial pseudohypoparathyroidism',
     'parathyroid hormone resistant hypoparathyroidism',
     'reduction mammoplasty',
     'breast reduction',
     'reduction mammaplasty',
     'mckissock operation, breast reduction',
     'herpetic ulceration of vulva',
     'herpes simplex vulvitis',
     'contusion of nose',
     'nose - bruise',
     'contusion, nose',
     'acute anteroseptal myocardial infarction',
     'abscess of neck',
     'cervical abscess',
     'chronic vomiting',
     'radionuclide therapy for hyperthyroidism',
     'glaucoma associated with ocular trauma',
     'traumatic glaucoma',
     'glaucoma due to ocular trauma',
     'capsulitis',
     'contact dermatitis of eyelid',
     'allergic dermatitis of eyelid',
     'allergic blepharitis',
     'contact allergy eyelid',
     'contact eczema eyelid',
     'venous catheterisation for renal dialysis',
     'congenital anomaly of oesophagus',
     'congenital abnormality of esophagus',
     'congenital malformation of the esophagus',
     'congenital abnormality of oesophagus',
     'congenital malformation of the oesophagus',
     'decrease in height',
     'urgent desire for stool',
     'defecation urgency',
     'fecal urgency',
     'defaecation urgency',
     'precipitancy of defaecation',
     'precipitancy of defecation',
     'laryngectomy',
     'larynx excision',
     'excision of larynx',
     'congenital subaortic stenosis',
     'congenital subvalvular aortic stenosis',
     'thickening of pleura',
     'pleural thickening',
     'pleural cuirasse',
     'central corneal ulcer',
     'chromosomal disorder',
     'congenital chromosomal disease',
     'chromosomopathy',
     'chromosomal abnormality syndrome',
     'chromosomal hereditary disorder',
     'chromosomal imbalance syndrome',
     'anomaly of chromosome',
     'congenital disorder due to abnormality of chromosome number or structure',
     'pauciarticular juvenile rheumatoid arthritis',
     'jca - pauciarticular onset juvenile chronic arthritis',
     'pauciarticular onset juvenile arthritis',
     'pauciarticular onset juvenile chronic arthritis',
     'tachycardia-bradycardia',
     'brady-tachy syndrome',
     'bradycardia-tachycardia syndrome',
     'tachycardia bradycardia syndrome',
     'double outlet right ventricle',
     'origin of both great vessels from right ventricle',
     'taussig-bing syndrome',
     'taussig-bing defect',
     'transposition of great vessels, interventricular septal defect and overriding aorta',
     'DORv',
     'congenital deformity of spine',
     'congenital anomaly of spine',
     'congenital anomaly of vertebral column',
     'congenital spine malformation',
     'congenital malformation of spine',
     'feeling angry',
     'anger',
     'angry',
     'extrapyramidal disease',
     'extrapyramidal disorder',
     'neurotrophic keratoconjunctivitis',
     'disorder of optic nerve',
     'nutritional optic neuropathy',
     'optic neuropathy',
     'congenital biliary atresia',
     'biliary atresia',
     'atresia of bile ducts',
     'Ba',
     'congenital atresia of extrahepatic bile duct',
     'extrahepatic biliary atresia',
     'EHBa',
     'multiple cranial nerve palsy',
     'bipolar i disorder, single manic episode, in partial remission',
     'bipolar 1 disorder, single manic episode, in partial remission',
     'chronic peritoneal effusion',
     'contracture of joint',
     'joint contraction',
     'tailors bunion',
     'tailors bunion',
     'bunionette',
     'inferior vena cava syndrome',
     'implantation of pulmonary valve prosthesis or synthetic device',
     'mesangiocapillary glomerulonephritis',
     'membranoproliferative glomerulonephritis',
     'lobular glomerulonephritis',
     'MCGn',
     'MPGn',
     'prolapsed internal haemorrhoids',
     'internal prolapsed haemorrhoids',
     'internal prolapsed hemorrhoids',
     'sleep walking disorder',
     'somnambulism',
     'sleep walking',
     'sleepwalking disorder',
     'sleepwalking',
     'cardiovascular stress test using treadmill',
     'stress test ecg - treadmill',
     'unilateral radical mastectomy',
     'parotidectomy',
     'excision of parotid gland',
     'excision of parotid tumor or parotid gland; lateral lobe, without nerve dissection',
     'parotidectomies',
     'aggressive type unsocialised behaviour disorder',
     'aggressive type unsocialized conduct disorder',
     'aggressive type unsocialised conduct disorder',
     'arachnoiditis',
     'cyst of nasal sinus',
     'mucocele of nasal sinus',
     'mucous cyst of nasal sinus',
     'nasal sinus cyst',
     'hyperreflexia',
     'exaggeration of the deep reflexes',
     'increased tendon reflexes',
     'bacterial infection by site',
     'bacterial infectious disease',
     'disease caused by bacteria',
     'bacterial infection',
     'bacterial disease',
     'congenital anomaly of nervous system',
     'congenital disease of nervous system',
     'congenital lesion of nervous system',
     'congenital deformity of nervous system',
     'congenital malformation of the nervous system',
     'sleep terror disorder',
     'sleep terrors',
     'vertebral artery stenosis',
     'vertebral artery narrowing',
     'acute frontal sinusitis',
     'benign neoplasm of pancreas',
     'benign tumour of pancreas',
     'benign tumor of pancreas',
     'lipoma of skin and subcutaneous tissue of neck',
     'lipoma of neck',
     'primary malignant neoplasm of skin of scalp',
     'primary malignant neoplasm of soft tissues of lower limb',
     'secondary malignant neoplasm of choroid',
     'secondary choroidal tumour',
     'secondary choroidal tumor',
     'metastasis to choroid',
     'metastatic malignant neoplasm to choroid',
     'cancer metastatic to choroid',
     'secondary malignant neoplasm of endometrium',
     'metastatic malignant neoplasm to endometrium',
     'neoplasm of uncertain behaviour of neck',
     'radiation retinopathy',
     'rhinitis medicamentosa',
     'tracheomalacia',
     'thrombosis of mesenteric vein',
     'thrombophlebitis of superficial veins of upper extremities',
     'superficial thrombophlebitis of arm',
     'hypoplasia of the optic nerve',
     'optic nerve hypoplasia',
     'ONh',
     'hypoplasia of optic nerve',
     'naevus of iris',
     'benign melanoma of iris',
     'iris naevus',
     'iris nevus',
     'subluxation of radial head',
     'pulled elbow',
     'nursemaids, elbow',
     'malgaignes luxation',
     'goyrands injury',
     'soft tissue infection',
     'labyrinthine hydrops',
     'cochlear hydrops',
     'endolymphatic hydrops',
     'donor for liver transplant',
     'liver donor',
     'spastic diplegia',
     'xerophthalmia',
     'tear film insufficiency',
     'dry eye syndrome',
     'refractory anemia without sideroblasts',
     'refractory anaemia without sideroblasts',
     'amputation of the foot',
     'amputation, foot',
     'unclassifiable eczema',
     'eczema',
     'eczma',
     'dermatitus',
     'ecsema',
     'exzema',
     'ecxema',
     'hypoalphalipoproteinemia',
     'condyloma',
     'condylomata',
     'systemic sclerosis',
     'scleroderma',
     'systemic scleroderma',
     'thibierge-weissenbach syndrome',
     'scleroderma syndrome',
     'chronic kidney disease',
     'ckd - chronic kidney disease',
     'chronic renal impairment',
     'chronic renal disease',
     'cellulitis of lower limb',
     'knee joint - varus deformity',
     'bow legs',
     'bandy legged',
     'bow legged',
     'genu varus',
     'repair of coarctation of aorta',
     'diplegic cerebral palsy',
     'spastic diplegic cerebral palsy',
     'congenital diplegia',
     'cellulitis of upper limb',
     'carcinoma of central portion of breast',
     'swelling of upper limb',
     'steatosis of liver',
     'fatty liver',
     'hepatic lipidosis',
     'steatohepatitis',
     'sprain',
     'sprained',
     'sprains',
     'atherosclerosis',
     'atheromatosis',
     'atherosclerotic',
     'atypical squamous cells of undetermined significance',
     'atypical squamous cells of uncertain significance',
     'ASCUs',
     'lymphoid neoplasm',
     'stem cell donor',
     'osteoporotic fracture',
     'digestive system reflux',
     'refluxed',
     'refluxes',
     'refluxing',
     'malignant fibromatous neoplasm',
     'fibrosarcoma',
     'prerenal azotemia',
     'azotaemia due to inadequate renal perfusion',
     'azotemia due to inadequate renal perfusion',
     'seroma',
     'neonatal abstinence syndrome',
     'newborn drug withdrawal syndrome',
     'drug withdrawal syndrome in newborn',
     'neonatal withdrawal syndrome',
     'neonatal drug withdrawal syndrome',
     'neonatal abstinence syndrome',
     'glioblastoma multiforme',
     'glioblastoma',
     'spongioblastoma multiforme',
     'GLm',
     'GBm',
     'glioblastoma, no icd-o subtype',
     'glioblastoma, no international classification of diseases for oncology subtype',
     'impending infarction',
     'preinfarction syndrome',
     'preinfarction angina',
     'worsening angina',
     'pre-infarction syndrome',
     'unstable angina',
     'intermediate coronary syndrome',
     'crescendo angina',
     'angina decubitus',
     'angina at rest',
     'anginal chest pain at rest',
     'hyperemia of surface of eye',
     'bloodshot eye',
     'injection of surface of eye',
     'cholesterol, serum or whole blood, total'
]
     